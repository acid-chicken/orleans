...
namespace Orleans.GrainDirectory {
  ...
  public interface IGrainDirectory {
    ...
    /// Only one <see cref="GrainAddress"/> per <see
    /// cref="GrainAddress.GrainId"/> can be registered. If there is already an
    /// existing entry, the directory will not override it.
    /// </summary>
    /// <param name="address">The <see cref="GrainAddress"/> to register</param>
    /// <returns>The <see cref="GrainAddress"/> that is effectively registered
    /// in the directory.</returns>
    Task<GrainAddress>Register(GrainAddress address);

    /// <summary>
    /// Unregister a <see cref="GrainAddress"/> entry in the directory.
    /// </summary>
    /// <param name="address">The <see cref="GrainAddress"/> to
    /// unregister</param>
    Task Unregister(GrainAddress address);

    /// <summary>
    /// Unregister a batch of <see cref="GrainAddress"/> entries in the
    /// directory.
    /// </summary>
    ...
    /// <returns>The <see cref="GrainAddress"/> entry found in the directory, if
    /// any</returns>
    Task<GrainAddress>Lookup(string grainId);

    /// <summary>
    /// Unregister from the directory all entries that point to one of the silo
    /// in argument. Can be a NO-OP depending on the implementation.
    /// </summary>
    /// <param name="siloAddresses">The silos to be removed from the
    /// directory</param>
  ...
  public class GrainAddress {
    ...
    public override int GetHashCode() {
      ...
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<string>.Default.GetHashCode(this.SiloAddress);
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<string>.Default.GetHashCode(this.GrainId);
      hashCode =
          hashCode * -1521134295 +
          EqualityComparer<string>.Default.GetHashCode(this.ActivationId);
      return hashCode;
    }
  }
}
