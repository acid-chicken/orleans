...
namespace Orleans.Serialization {
  ...
  public readonly struct GrainReferenceKeyInfo {
    ...
    /// <summary>
    /// Generic argument. Only applicable if <see cref="HasGenericArgument"/> is
    /// true.
    /// </summary>
    ...
    /// <summary>
    /// Grain key. For more specialized views, see <c>KeyAs*</c> methods and
    /// <c>KeyFrom*</c>.
    /// </summary>
    ...
    public static(ulong, ulong, ulong, string)
        KeyFromGuid(Guid key, ulong typeCode) {
    ...
    public static(ulong, ulong, ulong, string)
        KeyFromGuidWithExt(Guid key, string ext, ulong typeCode) {
      var (n0, n1) = FromGuid(key);
    ...
    public static(ulong, ulong, ulong, string)
        KeyFromLong(long key, ulong typeCode) {
    ...
    public static(ulong, ulong, ulong, string)
        KeyFromLongWithExt(long key, string ext, ulong typeCode) {
      var n1 = unchecked((ulong) key);
    ...
    public static(ulong, ulong, ulong, string)
        KeyFromString(string key, ulong typeCode) {
    ...
    public GrainReferenceKeyInfo((ulong, ulong, ulong, string) key,
                                 (IPEndPoint endpoint, int generation)
                                     targetSilo) {
      Key = key;
      ObserverId = Guid.Empty;
    ...
    private static Guid ToGuid((ulong n0, ulong n1, ulong, string) key) =>
        new Guid((uint)(key.n0 & 0xffffffff), (ushort)(key.n0 >> 32),
                 (ushort)(key.n0 >> 48), (byte) key.n1, (byte)(key.n1 >> 8),
                 (byte)(key.n1 >> 16), (byte)(key.n1 >> 24),
                 (byte)(key.n1 >> 32), (byte)(key.n1 >> 40),
                 (byte)(key.n1 >> 48), (byte)(key.n1 >> 56));
