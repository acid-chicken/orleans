...
namespace Orleans.Runtime {
  ...
  /// </para>
  /// <para>
  /// There are some constraints on the types of deadlock that can currently be
  /// detected by Orleans silos.
  /// </para>
  /// </remarks>
  [Serializable]
  public class DeadlockException : OrleansException {
    internal IEnumerable<GrainId>CallChain {
      get;
      private set;
    }
    ...
    protected DeadlockException(SerializationInfo info,
                                StreamingContext context)
        : base(info, context) {
      if (info != null) {
        CallChain = (IEnumerable<GrainId>) info.GetValue(
            "CallChain", typeof (IEnumerable<GrainId>));
      }
    }
    ...
    public override void GetObjectData(SerializationInfo info,
                                       StreamingContext context) {
      if (info != null) {
        info.AddValue("CallChain", this.CallChain,
                      typeof (IEnumerable<GrainId>));
      }
