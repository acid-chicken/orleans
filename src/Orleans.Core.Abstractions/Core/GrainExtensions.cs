...
public static class GrainExtensions {
  private const string WRONG_GRAIN_ERROR_MSG =
  ...
  AsWeaklyTypedReference(this IAddressable grain) {
    ...
    if (systemTarget != null)
      return GrainReference.FromGrainId(
          systemTarget.GrainId, systemTarget.GrainReferenceRuntime, null);
  ...
  /// </summary>
  /// <typeparam name="TGrainInterface">The type of the grain
  /// interface.</typeparam> <param name="grain">The grain to convert.</param>
  /// <returns>A strongly typed <c>GrainReference</c> of grain interface type
  /// TGrainInterface.</returns>
  public static TGrainInterface
  AsReference<TGrainInterface>(this IAddressable grain) {
    ThrowIfNullGrain(grain);
  ...
  /// </summary>
  /// <typeparam name="TGrainInterface">The type of the grain
  /// interface.</typeparam> <param name="grain">The grain to cast.</param>
  ...
  public static void BindGrainReference(this IAddressable grain,
  ...
  internal static GrainId GetGrainId(IAddressable grain) {
    ...
    default:
      throw new ArgumentException(
          String.Format(
              "GetGrainId has been called on an unexpected type: {0}.",
  ...
  public static GrainId GetGrainIdentity(this IGrain grain) {
    ...
    default:
      throw new ArgumentException(
          String.Format(
              "GetGrainIdentity has been called on an unexpected type: {0}.",
              grain.GetType().FullName),
          "grain");
    }
  }
  ...
  /// <param name="keyExt">The output parameter to return the extended key part
  /// of the grain primary key, if extended primary key was provided for that
  /// grain.</param> <returns>A long representing the primary key for this
  /// grain.</returns>
  ...
  /// <param name="keyExt">The output parameter to return the extended key part
  /// of the grain primary key, if extended primary key was provided for that
  /// grain.</param> <returns>A Guid representing the primary key for this
  /// grain.</returns>
  ...
  public static long GetPrimaryKeyLong(this IGrain grain, out string keyExt) {
    return ((LegacyGrainId) GetGrainIdentity(grain))
        .GetPrimaryKeyLong(out keyExt);
  }
  ...
  /// <summary>
  /// Invokes a method of a grain interface is one-way fashion so that no
  /// response message will be sent to the caller.
  /// </summary>
  /// <typeparam name="T">Grain interface</typeparam>
  /// <param name="grainReference">Grain reference which will be copied and then
  /// a call executed on it</param> <param name="grainMethodInvocation">Function
  /// that should invoke grain method and return resulting task</param>
  public static void InvokeOneWay<T>(this T grainReference,
                                     Func<T, Task>grainMethodInvocation) where T
      : class
  , IAddressable {
    var oneWayGrainReferenceCopy =
        new GrainReference(grainReference.AsWeaklyTypedReference(),
                           InvokeMethodOptions.OneWay)
            .Cast<T>();

    // Task is always completed at this point. Should also help to catch
    // situations of mistakenly calling the method on original grain reference
    var invokationResult = grainMethodInvocation(oneWayGrainReferenceCopy);
    if (!invokationResult.IsCompleted) {
      throw new InvalidOperationException(
          "Invoking of methods with one way flag must result in completed task");
    }
  }
  ...
  /// <param name="grainReference">Grain reference which will be copied and then
  /// a call executed on it</param> <param name="grainMethodInvocation">Function
  /// that should invoke grain method and return resulting ValueTask</param>
  public static void
  InvokeOneWay<T>(this T grainReference,
                  Func<T, ValueTask>grainMethodInvocation) where T : class
  , IAddressable {
    var oneWayGrainReferenceCopy =
        new GrainReference(grainReference.AsWeaklyTypedReference(),
                           InvokeMethodOptions.OneWay)
