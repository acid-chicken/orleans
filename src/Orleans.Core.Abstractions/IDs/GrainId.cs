...
namespace Orleans.Runtime {
  ...
  public readonly struct GrainId : IEquatable<GrainId>,
                                   IComparable<GrainId>,
                                   ISerializable {
    /// <summary>
    ...
    public GrainId(byte[] type, byte[] key)
        : this(new GrainType(type), new IdSpan(key)) {}
    ...
    private GrainId(SerializationInfo info, StreamingContext context) {
      Type = new GrainType(IdSpan.UnsafeCreate(
          (byte[]) info.GetValue("tv", typeof (byte[])), info.GetInt32("th")));
      Key = IdSpan.UnsafeCreate((byte[]) info.GetValue("kv", typeof (byte[])),
                                info.GetInt32("kh"));
    }
    ...
    // TODO: remove implicit conversion (potentially make explicit to start
    // with)
    public static implicit
    operator LegacyGrainId(GrainId id) => LegacyGrainId.FromGrainId(id);

    /// <summary>
    /// Creates a new <see cref="GrainType"/> instance.
    /// </summary>
    public static GrainId
    Create(string type, string key) => Create(GrainType.Create(type), key);

    /// <summary>
    /// Creates a new <see cref="GrainType"/> instance.
    /// </summary>
    public static GrainId Create(GrainType type, string key) =>
        new GrainId(type, Encoding.UTF8.GetBytes(key));

    /// <summary>
    /// Creates a new <see cref="GrainType"/> instance.
    /// </summary>
    public static GrainId Create(GrainType type,
                                 IdSpan key) => new GrainId(type, key);
    ...
    /// <inheritdoc/>
    public override bool Equals(object obj) => obj is GrainId id &&
                                               this.Equals(id);

    /// <inheritdoc/>
    public bool Equals(GrainId other) => this.Type.Equals(other.Type) &&
    ...
    public override string ToString() =>
        $"{Type.ToStringUtf8()}/{Key.ToStringUtf8()}";

    /// <summary>
    /// An <see cref="IEqualityComparer{T}"/> and <see cref="IComparer{T}"/>
    /// implementation for <see cref="GrainId"/>.
    /// </summary>
    public sealed class Comparer : IEqualityComparer<GrainId>,
                                   IComparer<GrainId> {
      /// <summary>
