...
namespace Orleans.Runtime {
  [Serializable]
  public class LegacyGrainId : IEquatable<LegacyGrainId>,
                               IComparable<LegacyGrainId>,
                               IGrainIdentity {
    private static readonly object lockable = new object();
    private const int INTERN_CACHE_INITIAL_SIZE = InternerConstants.SIZE_LARGE;
    private static readonly TimeSpan internCacheCleanupInterval =
        InternerConstants.DefaultCacheCleanupFreq;
    ...
    public bool IsGrain => Category == UniqueKey.Category.Grain ||
                           Category == UniqueKey.Category.KeyExtGrain;
    ...
    operator GrainId(LegacyGrainId legacy) => legacy.ToGrainId();

    public static LegacyGrainId NewId() {
      return FindOrCreateGrainId(
          UniqueKey.NewKey(Guid.NewGuid(), UniqueKey.Category.Grain));
    }
    ...
    internal static LegacyGrainId NewClientId(Guid id) {
      return FindOrCreateGrainId(
          UniqueKey.NewKey(id, UniqueKey.Category.Client, 0));
    }
    ...
    internal static LegacyGrainId GetGrainIdForTesting(Guid guid) {
      return FindOrCreateGrainId(
          UniqueKey.NewKey(guid, UniqueKey.Category.None));
    }

    internal static LegacyGrainId GetSystemTargetGrainId(long typeData) {
      return FindOrCreateGrainId(
          UniqueKey.NewSystemTargetKey(Guid.Empty, typeData));
    }

    internal static LegacyGrainId GetGrainId(long typeCode, long primaryKey,
                                             string keyExt = null) {
      return FindOrCreateGrainId(
          UniqueKey.NewKey(primaryKey, keyExt == null ? UniqueKey.Category.Grain
                           : UniqueKey.Category.KeyExtGrain, typeCode, keyExt));
    }

    internal static LegacyGrainId GetGrainId(long typeCode, Guid primaryKey,
                                             string keyExt = null) {
      return FindOrCreateGrainId(
          UniqueKey.NewKey(primaryKey, keyExt == null ? UniqueKey.Category.Grain
                           : UniqueKey.Category.KeyExtGrain, typeCode, keyExt));
    ...
    internal static LegacyGrainId GetGrainId(long typeCode, string primaryKey) {
      return FindOrCreateGrainId(UniqueKey.NewKey(
          0L, UniqueKey.Category.KeyExtGrain, typeCode, primaryKey));
    }

    internal static LegacyGrainId GetGrainServiceGrainId(short id,
                                                         int typeData) {
      return FindOrCreateGrainId(UniqueKey.NewGrainServiceKey(id, typeData));
    }

    internal static LegacyGrainId GetGrainServiceGrainId(int typeData,
                                                         string systemGrainId) {
      return FindOrCreateGrainId(
          UniqueKey.NewGrainServiceKey(systemGrainId, typeData));
    }
    ...
    internal long GetPrimaryKeyLong() { return Key.PrimaryKeyToLong(); }

    public Guid GetPrimaryKey(out string keyExt) {
      return Key.PrimaryKeyToGuid(out keyExt);
    }

    internal Guid GetPrimaryKey() { return Key.PrimaryKeyToGuid(); }
    ...
    private GrainType GetGrainType() {
      ...
      if (this.IsSystemTarget) {
        return GrainType.Create(
            $"{GrainTypePrefix.SystemTargetPrefix}{key.TypeCodeData:X16}");
      } else if (this.IsClient) {
        return GrainType.Create(
            $"{GrainTypePrefix.ClientPrefix}.{key.TypeCodeData:X16}");
      } else {
        return GrainType.Create(
            $"{GrainTypePrefix.LegacyGrainPrefix}{key.TypeCodeData:X16}");
      }
    }

    public static GrainType CreateGrainTypeForGrain(int typeCode) {
      return GrainType.Create(
          $"{GrainTypePrefix.LegacyGrainPrefix}{typeCode:X16}");
    }

    public static GrainType CreateGrainTypeForSystemTarget(int typeCode) {
      return GrainType.Create(
          $"{GrainTypePrefix.SystemTargetPrefix}{typeCode:X16}");
    }

    private IdSpan GetGrainKey() {
      // TODO: intern
      var key = this.Key;
      return IdSpan.Create($"{key.N0:X16}{key.N1:X16}{(key.HasKeyExt ? (" +
                           " + key.KeyExt) : string.Empty)}");
    }
    ...
    public static bool TryConvertFromGrainId(GrainId id,
                                             out LegacyGrainId legacyId) {
    ...
    private static unsafe LegacyGrainId FromGrainIdInternal(GrainId id) {
      ...
      } else if (typeSpan.StartsWith(
                     GrainTypePrefix.SystemTargetPrefixBytes.Span)) {
      ...
      fixed(byte *typeCodeBytes = typeCodeSlice) {
        ...
        if (!ulong.TryParse(typeCodeString, NumberStyles.HexNumber,
                            CultureInfo.InvariantCulture, out typeCodeData)) {
          return null;
        }
      }
      ...
      fixed(byte *idBytes = keySpan) {
        ...
        if (!ulong.TryParse(n0String, NumberStyles.HexNumber,
                            CultureInfo.InvariantCulture, out n0)) {
          return null;
        }

        // TODO: noalloc
        var n1String =
            Encoding.UTF8.GetString(idBytes + fieldLength, fieldLength);
        if (!ulong.TryParse(n1String, NumberStyles.HexNumber,
                            CultureInfo.InvariantCulture, out n1)) {
          return null;
        }

        const int keySpanPrefixLength = fieldLength + fieldLength + 1;
        if (keySpan.Length > keySpanPrefixLength &&
            idBytes[keySpanPrefixLength - 1] == (byte) '+') {
          // Take every byte after the '+' and interpret it as UTF8
          keyExt =
              Encoding.UTF8.GetString(idBytes + keySpanPrefixLength,
                                      keySpan.Length - keySpanPrefixLength);
        } else {
          keyExt = default;
        }
      }
    ...
    ThrowNotLegacyGrainId(GrainId id) {
      throw new InvalidOperationException(
          $"Cannot convert non-legacy id {id} into legacy id");
    }

    private static LegacyGrainId FindOrCreateGrainId(UniqueKey key) {
      // Note: This is done here to avoid a wierd cyclic dependency / static
      // initialization ordering problem involving the GrainId, Constants &
      // Interner classes
      ...
      lock(lockable) {
        if (grainIdInternCache == null) {
          grainIdInternCache = new Interner<UniqueKey, LegacyGrainId>(
              INTERN_CACHE_INITIAL_SIZE, internCacheCleanupInterval);
        }
      }
      return grainIdInternCache.FindOrCreate(key, k => new LegacyGrainId(k));
    }
    ...
    // Keep compiler happy -- it does not like classes to have Equals(...)
    // without GetHashCode() methods
    public override int GetHashCode() { return Key.GetHashCode(); }

    /// <summary>
    /// Get a uniformly distributed hash code value for this grain, based on
    /// Jenkins Hash function. NOTE: Hash code value may be positive or
    /// NEGATIVE.
    /// </summary>
    /// <returns>Hash code for this LegacyGrainId</returns>
    public uint GetUniformHashCode() { return Key.GetUniformHashCode(); }

    public override string ToString() { return ToStringImpl(false); }

    // same as ToString, just full primary key and type code
    internal string ToDetailedString() { return ToStringImpl(true); }
    ...
    private string ToStringImpl(bool detailed) {
      ...
      // this should grab the least-significant half of n1, suffixing it with
      // the key extension.
      ...
      return detailed ? String.Format("{0}-0x{1, 8:X8}", fullString,
                                     GetUniformHashCode())
          : fullString;
    }
    ...
    internal string ToFullString() {
      string kx;
      string pks = Key.IsLongKey ? GetPrimaryKeyLong(out kx)
                       .ToString(CultureInfo.InvariantCulture)
          : GetPrimaryKey(out kx)
                       .ToString();
      string pksHex = Key.IsLongKey ? GetPrimaryKeyLong(out kx)
                          .ToString("X")
          : GetPrimaryKey(out kx)
                          .ToString("X");
      return String.Format(
          ...
          Key.HasKeyExt ? String.Format(", KeyExtension: {0}", kx)
          : ""); // 8
    }
    ...
    internal string ToStringWithHashCode() {
      return String.Format("{0}-0x{1, 8:X8}", this.ToString(),
                           this.GetUniformHashCode());
    }

    /// <summary>
    /// Return this LegacyGrainId in a standard string form, suitable for later
    /// use with the <c>FromParsableString</c> method.
    /// </summary>
    /// <returns>LegacyGrainId in a standard string format.</returns>
    internal string ToParsableString() {
      // NOTE: This function must be the "inverse" of FromParsableString, and
      // data must round-trip reliably.

      return Key.ToHexString();
    }

    /// <summary>
    /// Return this LegacyGrainId in a standard components form, suitable for
    /// later use with the <see cref="FromKeyInfo"/> method.
    /// </summary>
    ...
    /// <summary>
    /// Create a new LegacyGrainId object by parsing string in a standard form
    /// returned from <c>ToParsableString</c> method.
    /// </summary>
    /// <param name="grainId">String containing the LegacyGrainId info to be
    /// parsed.</param> <returns>New LegacyGrainId object created from the input
    /// data.</returns>
    ...
    /// parsed.</param> <returns>New LegacyGrainId object created from the input
    /// data.</returns>
    internal static LegacyGrainId
    FromParsableString(ReadOnlySpan<char>grainId) {
      // NOTE: This function must be the "inverse" of ToParsableString, and data
      // must round-trip reliably.
    ...
    /// <summary>
    /// Create a new LegacyGrainId object by parsing components returned form
    /// <see cref="ToKeyInfo"/>.
    /// </summary>
    /// <param name="grainId">Components containing the LegacyGrainId to be
    /// parsed.</param> <returns>New LegacyGrainId object created from the input
    /// data.</returns>
    internal static LegacyGrainId FromKeyInfo((ulong, ulong, ulong, string)
                                                  grainId) {
      // NOTE: This function must be the "inverse" of ToKeyInfo, and data must
      // round-trip reliably.
