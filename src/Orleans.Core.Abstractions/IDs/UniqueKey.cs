...
namespace Orleans.Runtime {
  [Serializable]
  internal class UniqueKey : IComparable<UniqueKey>,
    ...
    private static bool
    IsSystemTarget(Category category) => category == Category.SystemTarget
                                         || category
                                                == Category.KeyExtSystemTarget;

    public bool HasKeyExt => IsKeyExt(IdCategory);

    private static bool
    IsKeyExt(Category category) => category == Category.KeyExtGrain
                                   || category == Category.KeyExtSystemTarget;

    internal static readonly UniqueKey Empty =
        new UniqueKey{N0 = 0, N1 = 0, TypeCodeData = 0, KeyExt = null};

    internal static UniqueKey Parse(ReadOnlySpan<char>input) {
      var trimmed = input.Trim().ToString();

      // first, for convenience we attempt to parse the string using GUID
      // syntax. this is needed by unit tests but i don't know if it's needed
      // for production.
      Guid guid;
      if (Guid.TryParse(trimmed, out guid))
        ...
        var n0 =
            ulong.Parse(fields[0].Substring(0, 16), NumberStyles.HexNumber);
        var n1 =
            ulong.Parse(fields[0].Substring(16, 16), NumberStyles.HexNumber);
        var typeCodeData =
            ulong.Parse(fields[0].Substring(32, 16), NumberStyles.HexNumber);
        string keyExt = null;
        switch (fields.Length) {
        default:
          throw new InvalidDataException(
              "UniqueKey hex strings cannot contain more than one + separator.");
        case 1:
          break;
        case 2:
          if (fields[1] != "null") {
    ...
    private static UniqueKey NewKey(ulong n0, ulong n1, Category category,
                                    long typeData, string keyExt) {
      if (!IsKeyExt(category) && keyExt != null)
        throw new ArgumentException(
            "Only key extended grains can specify a non-null key extension.");

      var typeCodeData =
          ((ulong) category << 56) + ((ulong) typeData & 0x00FFFFFFFFFFFFFF);

      return NewKey(n0, n1, typeCodeData, keyExt);
    }

    internal static UniqueKey NewKey(long longKey,
                                     Category category = Category.None,
                                     long typeData = 0, string keyExt = null) {
    ...
    public static UniqueKey NewKey() { return NewKey(Guid.NewGuid()); }
    ...
    internal static UniqueKey NewKey(ulong n0, ulong n1, ulong typeCodeData,
                                     string keyExt) {
      ValidateKeyExt(keyExt, typeCodeData);
      return new UniqueKey{N0 = n0, N1 = n1, TypeCodeData = typeCodeData,
                           KeyExt = keyExt};
    }
    ...
    private void ThrowIfIsNotLong() {
      if (!IsLongKey)
        throw new InvalidOperationException(
            "this key cannot be interpreted as a long value");
    }
    ...
    // We really want Equals to be as fast as possible, as a minimum cost, as
    // close to native as possible. No function calls, no boxing, inline.
    ...
    // We really want CompareTo to be as fast as possible, as a minimum cost, as
    // close to native as possible. No function calls, no boxing, inline.
    public int CompareTo(UniqueKey other) {
      return TypeCodeData < other.TypeCodeData ? - 1 : TypeCodeData >
                 other.TypeCodeData ? 1 : N0 < other.N0 ? - 1 : N0 >
                 other.N0 ? 1 : N1 < other.N1 ? - 1 : N1 >
                 other.N1 ? 1 : !HasKeyExt ||
             KeyExt == null ? 0
    ...
    internal uint GetUniformHashCode() {
      // Disabling this ReSharper warning; hashCache is a logically read-only
      // variable, so accessing them in GetHashCode is safe. ReSharper disable
      // NonReadonlyFieldInGetHashCode
      if (uniformHashCache == 0) {
        ...
        // Unchecked is required because the Jenkins hash is an unsigned 32-bit
        // integer, which we need to convert to a signed 32-bit integer.
        uniformHashCache = n;
      }
    ...
    internal ReadOnlySpan<byte>ToByteArray() {
      ...
      var offset = 0;
      var ulongBytes = MemoryMarshal.Cast<byte, ulong>(
          spanBytes.Slice(offset, sizeof(ulong) * 3));
      ...
      // Copy KeyExt
      if (extBytes != null) {
        MemoryMarshal.Cast<byte, int>(spanBytes.Slice(offset, sizeof(int)))[0] =
            extBytesLength;
        offset += sizeof(int);
    ...
    private Guid ConvertToGuid() {
      return new Guid((UInt32)(N0 & 0xffffffff), (UInt16)(N0 >> 32),
                      (UInt16)(N0 >> 48), (byte) N1, (byte)(N1 >> 8),
                      (byte)(N1 >> 16), (byte)(N1 >> 24), (byte)(N1 >> 32),
                      (byte)(N1 >> 40), (byte)(N1 >> 48), (byte)(N1 >> 56));
    }
      ...
      if (category == Category.KeyExtGrain ||
        ...
        if (string.IsNullOrWhiteSpace(keyExt)) {
          if (null == keyExt) {
            ...
            throw new ArgumentException("Extended key is empty or white space.",
                                        "keyExt");
          }
        }
      }
