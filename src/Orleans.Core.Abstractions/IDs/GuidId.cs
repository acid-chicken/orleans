...
namespace Orleans.Runtime {
  ...
  /// Can be used in places where Guid is optional and in those cases it can be
  /// set to null and will not use the storage of an empty Guid struct.
  /// </summary>
  [Serializable]
  [Immutable]
  public sealed class GuidId : IEquatable<GuidId>,
                               IComparable<GuidId>,
                               ISerializable {
    private static readonly Lazy<Interner<Guid, GuidId>>guidIdInternCache =
        new Lazy<Interner<Guid, GuidId>>(
            () => new Interner<Guid, GuidId>(
                InternerConstants.SIZE_LARGE,
                InternerConstants.DefaultCacheCleanupFreq));

    public readonly Guid Guid;

    public static GuidId Default { get; }
    = new GuidId(Guid.Empty);

    // TODO: Need to integrate with Orleans serializer to really use Interner.
    private GuidId(Guid guid) { this.Guid = guid; }
    ...
    internal string ToDetailedString() { return this.Guid.ToString(); }

    public string ToParsableString() { return Guid.ToString(); }
    ...
    internal static GuidId
    DeserializeFromStream(IBinaryTokenStreamReader stream) {
    ...
    public static bool operator !=(GuidId a, GuidId b) { return !(a == b); }
