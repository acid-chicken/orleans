...
namespace Orleans.Runtime {
  ...
  public class ActivationId : IEquatable<ActivationId> {
    ...
    public bool IsSystem {
      get { return Key.IsSystemTargetKey; }
    }
    ...
    static ActivationId() {
      legacyKeyInterner = new Interner<UniqueKey, ActivationId>(
          InternerConstants.SIZE_LARGE,
          InternerConstants.DefaultCacheCleanupFreq);
      interner = new Interner<GrainId, ActivationId>(
          InternerConstants.SIZE_LARGE,
          InternerConstants.DefaultCacheCleanupFreq);
    ...
    // No need to encode SiloAddress in the activation address for system
    // target. System targets have unique grain ids and addressed to a concrete
    // silo, so in fact we don't need ActivationId at all for System targets.
    // Need to remove it all together. For now, just use grain id as activation
    // id.
    public static ActivationId GetDeterministic(GrainId grain) {
      ...
      static ActivationId CreateActivationId(GrainId grainId) {
        ...
        var key = UniqueKey.NewKey(a, b, UniqueKey.Category.KeyExtGrain,
                                   typeData : 0, keyExt
                                   : grainId.ToString());
