...
namespace Orleans.Runtime {
  ...
  public readonly struct IdSpan : IEquatable<IdSpan>,
                                  IComparable<IdSpan>,
                                  ISerializable {
    ...
    /// Creates a new <see cref="IdSpan"/> instance from the provided value.
    /// </summary>
    public static IdSpan
    Create(string id) => new IdSpan(Encoding.UTF8.GetBytes(id));
    ...
    /// </summary>
    /// <remarks>
    /// This method is intended for use by serializers and other low-level
    /// libraries.
    /// </remarks>
    public static IdSpan
    UnsafeCreate(byte[] value, int hashCode) => new IdSpan(value, hashCode);
    ...
    public int CompareTo(IdSpan other) =>
        _value.AsSpan().SequenceCompareTo(other._value.AsSpan());
    ...
    private static int
        GetHashCode(byte[] value) => (int) JenkinsHash.ComputeHash(value);

    /// <summary>
    /// An <see cref="IEqualityComparer{T}"/> and <see cref="IComparer{T}"/>
    /// implementation for <see cref="IdSpan"/>.
    /// </summary>
    public sealed class Comparer : IEqualityComparer<IdSpan>,
                                   IComparer<IdSpan> {
      /// <summary>
