...
namespace Orleans.Runtime {
  ...
  public class SiloAddress : IEquatable<SiloAddress>,
                             ...
                             IComparable {
    internal static readonly int SizeBytes =
        24; // 16 for the address, 4 for the port, 4 for the generation

    /// <summary> Special constant value to indicate an empty SiloAddress.
    /// </summary>
    public static SiloAddress Zero {
      get;
      private set;
    }
    ...
    private static readonly Interner<SiloAddress, SiloAddress>
        siloAddressInterningCache;

    static SiloAddress() {
      siloAddressInterningCache = new Interner<SiloAddress, SiloAddress>(
    ...
    /// <summary>
    /// Factory for creating new SiloAddresses with specified IP endpoint
    /// address and silo generation number.
    /// </summary>
    /// <param name="ep">IP endpoint address of the silo.</param>
    /// <param name="gen">Generation number of the silo.</param>
    /// <returns>SiloAddress object initialized with specified address and silo
    /// generation.</returns>
    ...
    public static int AllocateNewGeneration() {
      long elapsed =
          (DateTime.UtcNow.Ticks - epoch.Ticks) / TimeSpan.TicksPerSecond;
      return unchecked(
          (int) elapsed); // Unchecked to truncate any bits beyond the lower 32
    }

    /// <summary>
    /// Return this SiloAddress in a standard string form, suitable for later
    /// use with the <c>FromParsableString</c> method.
    /// </summary>
    /// <returns>SiloAddress in a standard string format.</returns>
    public string ToParsableString() {
      // This must be the "inverse" of FromParsableString, and must be the same
      // across all silos in a deployment. Basically, this should never change
      // unless the data content of SiloAddress changes

      return String.Format("{0}:{1}@{2}", Endpoint.Address, Endpoint.Port,
                           Generation);
    }

    /// <summary>
    /// Create a new SiloAddress object by parsing string in a standard form
    /// returned from <c>ToParsableString</c> method.
    /// </summary>
    /// <param name="addr">String containing the SiloAddress info to be
    /// parsed.</param> <returns>New SiloAddress object created from the input
    /// data.</returns>
    public static SiloAddress FromParsableString(string addr) {
      // This must be the "inverse" of ToParsableString, and must be the same
      // across all silos in a deployment. Basically, this should never change
      // unless the data content of SiloAddress changes
    ...
    public static unsafe SiloAddress FromUtf8String(ReadOnlySpan<byte>addr) {
      ...
      fixed(byte *hostBytes = hostSlice) {
        host = IPAddress.Parse(
            Encoding.UTF8.GetString(hostBytes, hostSlice.Length));
      }
      ...
      fixed(byte *genBytes = genSlice) {
        generation =
            int.Parse(Encoding.UTF8.GetString(genBytes, genSlice.Length));
      }
    ...
    ThrowInvalidUtf8SiloAddress(ReadOnlySpan<byte>addr) {
      string addrString;
    ...
    /// </summary>
    /// <remarks>
    /// Note: This string value is not comparable with the
    /// <c>FromParsableString</c> method -- use the <c>ToParsableString</c>
    ...
    /// <summary>
    /// Return a long string representation of this SiloAddress, including it's
    ...
    public string ToStringWithHashCode() {
      return String.Format("{0}/x{1, 8:X8}", ToString(),
    ...
    public override int GetHashCode() {
      // Note that Port cannot be used because Port==0 matches any non-zero Port
      // value for .Equals
    ...
    public int GetConsistentHashCode() {
      ...
      // Note that Port cannot be used because Port==0 matches any non-zero Port
      // value for .Equals
      string siloAddressInfoToHash =
          Endpoint + Generation.ToString(CultureInfo.InvariantCulture);
    ...
    private static int CalculateIdHash(string text) {
      SHA256 sha = SHA256.Create(); // This is one implementation of the
                                    // abstract class SHA1.
      int hash = 0;
      try {
        ...
        for (int i = 0; i < result.Length; i += 4) {
          int tmp = (result[i] << 24) | (result[i + 1] << 16) |
    ...
    private List<uint>GetUniformHashCodesImpl(int numHashes) {
      ...
      var bytes = new byte[16 + sizeof(int) + sizeof(int) +
                           sizeof(int)]; // ip + port + generation + extraBit
      ...
      if (this.Endpoint.AddressFamily == AddressFamily.InterNetwork) // IPv4
      {
        ...
        Buffer.BlockCopy(this.Endpoint.Address.GetAddressBytes(), 0, bytes, 12,
                         4);
      } else // IPv6
      {
        Buffer.BlockCopy(this.Endpoint.Address.GetAddressBytes(), 0, bytes, 0,
                         16);
      }
      var offset = 16;
      // Port
    ...
    /// <summary>
    /// Two silo addresses match if they are equal or if one generation or the
    /// other is 0
    /// </summary>
    /// <param name="other"> The other SiloAddress to compare this one with.
    /// </param> <returns> Returns <c>true</c> if the two SiloAddresses are
    /// considered to match -- if they are equal or if one generation or the
    /// other is 0. </returns>
    internal bool Matches(SiloAddress other) {
      return other != null && Endpoint.Address.Equals(other.Endpoint.Address) &&
             ...
             ((Generation == other.Generation) || (Generation == 0) ||
              (other.Generation == 0));
    }
    ...
    internal bool IsSameLogicalSilo(SiloAddress other) {
      return other != null &&
             this.Endpoint.Address.Equals(other.Endpoint.Address) &&
             this.Endpoint.Port == other.Endpoint.Port;
    }

    public bool IsSuccessorOf(SiloAddress other) {
      return IsSameLogicalSilo(other) && this.Generation != 0 &&
             other.Generation != 0 && this.Generation > other.Generation;
    }
    ...
    // non-generic version of CompareTo is needed by some contexts. Just calls
    // generic version.
    public int CompareTo(object obj) { return CompareTo((SiloAddress) obj); }

    public int CompareTo(SiloAddress other) {
      if (other == null)
        return 1;
      // Compare Generation first. It gives a cheap and fast way to compare,
      // avoiding allocations and is also semantically meaningfull - older silos
      // (with smaller Generation) will appear first in the comparison order.
      // Only if Generations are the same, go on to compare Ports and IPAddress
      // (which is more expansive to compare). Alternatively, we could compare
      // ConsistentHashCode or UniformHashCode.
    ...
    // The comparions code is taken from:
    // http://www.codeproject.com/Articles/26550/Extending-the-IPAddress-object-to-allow-relative-c
    // Also note that this comparison does not handle semantic equivalence  of
    // IPv4 and IPv6 addresses. In particular, 127.0.0.1 and::1 are semanticaly
    // the same, but not syntacticaly. For more information refer to:
    // http://stackoverflow.com/questions/16618810/compare-ipv4-addresses-in-ipv6-notation
    // and
    // http://stackoverflow.com/questions/22187690/ip-address-class-getaddressbytes-method-putting-octets-in-odd-indices-of-the-byt
    // and dual stack sockets, described at
