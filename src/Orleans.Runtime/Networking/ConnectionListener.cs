...
namespace Orleans.Runtime.Messaging {
  internal abstract class ConnectionListener {
    ...
    private readonly ConcurrentDictionary<Connection, Task>connections =
        new ConcurrentDictionary<Connection, Task>(
    ...
    protected ConnectionListener(IConnectionListenerFactory listenerFactory,
    ...
    protected IServiceProvider ServiceProvider =>
        this.connectionShared.ServiceProvider;

    protected NetworkingTrace NetworkingTrace =>
        this.connectionShared.NetworkingTrace;
    ...
    ConfigureConnectionBuilder(IConnectionBuilder connectionBuilder) {}
    ...
    public void Start() {
      if (this.listener is null)
        throw new InvalidOperationException("Listener is not bound");
      this.acceptLoopTcs = new TaskCompletionSource<object>(
          TaskCreationOptions.RunContinuationsAsynchronously);
      ThreadPool.UnsafeQueueUserWorkItem(this.StartAcceptingConnections,
                                         this.acceptLoopTcs);
    }
    ...
    private void StartAcceptingConnections(object completionObj) {
      ...
      async Task RunAcceptLoop(TaskCompletionSource<object>completion) {
        try {
          ...
          this.NetworkingTrace.LogCritical(
              "Exception in AcceptAsync: {Exception}", exception);
        } finally {
      ...
      try {
        ...
        while (this.ConnectionCount > 0) {
          ...
          if (++cycles > 100 && cycles % 500 == 0 && this.ConnectionCount > 0) {
            this.NetworkingTrace.LogWarning(
        ...
        this.NetworkingTrace.LogWarning(
            "Exception during shutdown: {Exception}", exception);
      }
    }
    ...
    private async Task RunConnectionAsync(Connection connection) {
      ...
      using(this.BeginConnectionScope(connection)) {
        try {
          ...
          this.NetworkingTrace.LogInformation(
              "Connection {@Connection} terminated", connection);
        } catch (Exception exception) {
          this.NetworkingTrace.LogInformation(
              exception,
              "Connection {@Connection} terminated with an exception",
              connection);
        } finally {
    ...
    private IDisposable BeginConnectionScope(Connection connection) {
      if (this.NetworkingTrace.IsEnabled(LogLevel.Critical)) {
        return this.NetworkingTrace.BeginScope(
            new ConnectionLogScope(connection));
      }
