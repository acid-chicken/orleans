...
namespace Orleans.Runtime.Messaging {
  internal sealed class SiloConnection : Connection {
    ...
    public SiloConnection(
        SiloAddress remoteSiloAddress, ConnectionContext connection,
        ConnectionDelegate middleware, MessageCenter messageCenter,
        ILocalSiloDetails localSiloDetails, ConnectionManager connectionManager,
        ConnectionOptions connectionOptions, ConnectionCommon connectionShared)
    ...
    public SiloAddress RemoteSiloAddress {
      get;
      private set;
    }
    ...
    protected override ConnectionDirection ConnectionDirection =>
        ConnectionDirection.SiloToSilo;

    protected override IMessageCenter MessageCenter => this.messageCenter;

    public NetworkProtocolVersion RemoteProtocolVersion {
      get;
      private set;
    }
    ...
    protected override void OnReceivedMessage(Message msg) {
      ...
      if (msg.IsExpired) {
        this.MessagingTrace.OnDropExpiredMessage(
            msg, MessagingStatisticsGroup.Phase.Receive);
        return;
      }

      // If we've stopped application message processing, then filter those out
      // now Note that if we identify or add other grains that are required for
      // proper stopping, we will need to treat them as we do the membership
      // table grain here.
      if (messageCenter.IsBlockingApplicationMessages &&
          (msg.Category == Message.Categories.Application) &&
          !Constants.SystemMembershipTableType.Equals(msg.SendingGrain)) {
        ...
        var rejection = this.MessageFactory.CreateRejectionResponse(
            msg, Message.RejectionTypes.Unrecoverable, "Silo stopping");
        this.Send(rejection);
        return;
      }

      // Make sure the message is for us. Note that some control messages may
      // have no target information, so a null target silo is OK.
      if ((msg.TargetSilo == null) ||
          msg.TargetSilo.Matches(this.LocalSiloAddress)) {
      ...
      if (!msg.TargetSilo.Endpoint.Equals(this.LocalSiloAddress.Endpoint)) {
        // If the message is for some other silo altogether, then we need to
        // forward it.
        if (this.Log.IsEnabled(LogLevel.Trace))
          this.Log.Trace("Forwarding message {0} from {1} to silo {2}", msg.Id,
                         msg.SendingSilo, msg.TargetSilo);
        messageCenter.OutboundQueue.SendMessage(msg);
        return;
      }

      // If the message was for this endpoint but an older epoch, then reject
      // the message (if it was a request), or drop it on the floor if it was a
      // response or one-way.
      if (msg.Direction == Message.Directions.Request) {
        MessagingStatisticsGroup.OnRejectedMessage(msg);
        var rejection = this.MessageFactory.CreateRejectionResponse(
        ...
        if (this.Log.IsEnabled(LogLevel.Debug)) {
          this.Log.Debug(
              ...
              this.LocalSiloAddress.ToLongString(), msg);
        }
      }
    }
    ...
    private void HandlePingMessage(Message msg) {
      ...
      if (this.Log.IsEnabled(LogLevel.Trace)) {
        ...
        this.Log.LogTrace(
            "Responding to Ping from {Silo} with object id {ObjectId}. Message {Message}",
            msg.SendingSilo, objectId, msg);
      }

      if (!msg.TargetSilo.Equals(this.LocalSiloAddress)) {
        // Got ping that is not destined to me. For example, got a ping to my
        // older incarnation.
        MessagingStatisticsGroup.OnRejectedMessage(msg);
    ...
    protected override void OnSendMessageFailure(Message message,
                                                 string error) {
    ...
    protected override async Task RunInternal() {
      ...
      try {
        if (this.connectionOptions.ProtocolVersion ==
            NetworkProtocolVersion.Version1) {
          // This version of the protocol does not support symmetric preamble,
          // so either send or receive preamble depending on Whether or not this
          // is an inbound or outbound connection.
          if (this.RemoteSiloAddress is null) {
            // Inbound connection
            var protocolVersion = await ReadPreamble();

            // To support graceful transition to higher protocol versions, send
            // a preamble if the remote endpoint supports it.
          ...
          // Later versions of the protocol send and receive preamble at both
          // ends of the connection.
          if (this.RemoteSiloAddress is null) {
            // Inbound connection
            var protocolVersion = await ReadPreamble();

            // To support graceful transition from lower protocol versions, only
            // send a preamble if the remote endpoint supports it.
            if (protocolVersion >= NetworkProtocolVersion.Version2) {
              await WritePreamble();
            }
          } else {
        ...
        this.MessageReceivedCounter =
            MessagingStatisticsGroup.GetMessageReceivedCounter(
                this.RemoteSiloAddress);
        this.MessageSentCounter =
            MessagingStatisticsGroup.GetMessageSendCounter(
                this.RemoteSiloAddress);
        ...
        if (!(this.RemoteSiloAddress is null)) {
          this.connectionManager.OnConnectionTerminated(this.RemoteSiloAddress,
                                                        this, error);
        }
      }
      ...
      async Task WritePreamble() {
        await ConnectionPreamble.Write(
            this.Context, Constants.SiloDirectConnectionId,
            this.connectionOptions.ProtocolVersion, this.LocalSiloAddress);
      }

      async ValueTask<NetworkProtocolVersion>ReadPreamble() {
        var (grainId, protocolVersion, siloAddress) =
            await ConnectionPreamble.Read(this.Context);

        if (!grainId.Equals(Constants.SiloDirectConnectionId)) {
          throw new InvalidOperationException(
              "Unexpected non-proxied connection on silo endpoint.");
        }
    ...
    protected override bool PrepareMessageForSend(Message msg) {
      ...
      if (msg.IsExpired) {
        this.MessagingTrace.OnDropExpiredMessage(
            msg, MessagingStatisticsGroup.Phase.Send);
      ...
      // Fill in the outbound message with our silo address, if it's not already
      // set
      ...
      if (this.RemoteSiloAddress is object && msg.TargetSilo is object &&
          !this.RemoteSiloAddress.Matches(msg.TargetSilo)) {
        this.Log.LogWarning(
            "Attempting to send message addressed to {TargetSilo} to connection with {RemoteSiloAddress}. Message {Message}",
            msg.TargetSilo, this.RemoteSiloAddress, msg);
      }
      ...
      if (msg.Direction == Message.Directions.Request) {
        if (this.Log.IsEnabled(LogLevel.Debug))
          this.Log.LogDebug(
              ...
              this.LocalSiloAddress, msg, reason);
        ...
        this.MessagingTrace.OnSiloDropSendingMessage(this.LocalSiloAddress, msg,
                                                     reason);
      }
    }

    public override void Send(Message message) {
      if (this.RemoteProtocolVersion == NetworkProtocolVersion.Version1 &&
          this.RemoteSiloAddress is null) {
