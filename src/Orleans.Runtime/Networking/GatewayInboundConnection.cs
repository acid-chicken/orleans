...
namespace Orleans.Runtime.Messaging {
  internal sealed class GatewayInboundConnection : Connection {
    ...
    public GatewayInboundConnection(
        ...
        ILocalSiloDetails siloDetails, ConnectionOptions connectionOptions,
        MessageCenter messageCenter, ConnectionCommon connectionShared)
    ...
    protected override void OnReceivedMessage(Message msg) {
      ...
      if (targetAddress is null) {
        ...
        if (SystemTargetGrainId.TryParse(msg.TargetGrain,
                                         out var systemTargetId)) {
          msg.TargetSilo = this.myAddress;
          msg.TargetGrain =
              systemTargetId.WithSiloAddress(this.myAddress).GrainId;
        ...
        if (SystemTargetGrainId.TryParse(msg.TargetGrain,
                                         out var systemTargetId)) {
          msg.TargetGrain =
              systemTargetId.WithSiloAddress(targetAddress).GrainId;
          msg.TargetActivation = ActivationId.GetDeterministic(msg.TargetGrain);
        }
    ...
    protected override async Task RunInternal() {
      ...
      if (!ClientGrainId.TryParse(grainId, out var clientId)) {
        throw new InvalidOperationException(
            $"Unexpected connection id {grainId} on proxy endpoint from {siloAddress?.ToString() ?? " unknown
                silo "}");
      }
