...
namespace Orleans.Runtime.Messaging {
  internal sealed class GatewayInboundConnection : Connection {
    ...
    public GatewayInboundConnection(
        ConnectionContext connection, ConnectionDelegate middleware,
        Gateway gateway, OverloadDetector overloadDetector,
        ILocalSiloDetails siloDetails,
        IOptions<MultiClusterOptions>multiClusterOptions,
        ConnectionOptions connectionOptions, MessageCenter messageCenter,
      ...
      this.loadSheddingCounter =
          CounterStatistic.FindOrCreate(StatisticNames.GATEWAY_LOAD_SHEDDING);
      this.myAddress = siloDetails.SiloAddress;
      this.MessageReceivedCounter =
          CounterStatistic.FindOrCreate(StatisticNames.GATEWAY_RECEIVED);
    ...
    protected override ConnectionDirection ConnectionDirection =>
        ConnectionDirection.GatewayToClient;
    ...
    protected override void OnReceivedMessage(Message msg) {
      ...
      // return address translation for geo clients (replace sending address
      // cli/* with gcl/*)
      if (this.multiClusterOptions.HasMultiClusterNetwork &&
          msg.SendingAddress.Grain.Category != UniqueKey.Category.GeoClient) {
        msg.SendingGrain = GrainId.NewClientId(
            msg.SendingAddress.Grain.PrimaryKey, this.siloDetails.ClusterId);
      }
      ...
      if (this.overloadDetector.Overloaded) {
        ...
        Message rejection = this.MessageFactory.CreateRejectionResponse(
            msg, Message.RejectionTypes.GatewayTooBusy, "Shedding load");
        this.messageCenter.TryDeliverToProxy(rejection);
        if (this.Log.IsEnabled(LogLevel.Debug))
          this.Log.Debug("Rejecting a request due to overloading: {0}",
                         msg.ToString());
      ...
      if (targetAddress is null) {
        ...
        if (msg.TargetGrain.IsSystemTarget) {
          ...
          msg.TargetActivation =
              ActivationId.GetSystemActivation(msg.TargetGrain, this.myAddress);
        }
    ...
    protected override async Task RunInternal() {
      ...
      if (protocolVersion >= NetworkProtocolVersion.Version2) {
        await ConnectionPreamble.Write(
            ...
            this.connectionOptions.ProtocolVersion, this.myAddress);
      }

      if (grainId.Equals(Constants.SiloDirectConnectionId)) {
        throw new InvalidOperationException(
            $"Unexpected direct silo connection on proxy endpoint from {siloAddress?.ToString() ?? " unknown
                silo "}");
      }
      ...
      if (grainId.Category == UniqueKey.Category.GeoClient) {
        ...
        if (this.multiClusterOptions.HasMultiClusterNetwork) {
          grainId = GrainId.NewClientId(grainId.PrimaryKey,
