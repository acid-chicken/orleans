...
namespace Orleans.Runtime.Placement {
  internal class PlacementDirectorsManager {
    private readonly ConcurrentDictionary<Type, IPlacementDirector>directors =
        new ConcurrentDictionary<Type, IPlacementDirector>();
    private readonly ConcurrentDictionary<Type, IActivationSelector>selectors =
        new ConcurrentDictionary<Type, IActivationSelector>();
    private readonly PlacementStrategy defaultPlacementStrategy;
    private readonly ClientObserversPlacementDirector
        clientObserversPlacementDirector;
    private readonly IActivationSelector defaultActivationSelector;

    private readonly IServiceProvider serviceProvider;

    public PlacementDirectorsManager(
        IServiceProvider services, PlacementStrategy defaultPlacementStrategy,
      ...
      this.defaultActivationSelector =
          ResolveSelector(RandomPlacement.Singleton, true);
    }
    ...
    private IPlacementDirector ResolveDirector(PlacementStrategy strategy) {
      ...
      if (!this.directors.TryGetValue(strategyType, out result)) {
        result = this.serviceProvider
                     .GetRequiredServiceByKey<Type, IPlacementDirector>(
                         strategyType);
    ...
    private IActivationSelector
    ResolveSelector(PlacementStrategy strategy,
                    bool addIfDoesNotExist = false) {
      IActivationSelector result;
      var strategyType = strategy.GetType();
      if (!this.selectors.TryGetValue(strategyType, out result) &&
          addIfDoesNotExist) {
        result = this.serviceProvider
                     .GetRequiredServiceByKey<Type, IActivationSelector>(
                         strategyType);
    ...
    public bool TrySelectActivationSynchronously(
        ActivationAddress sendingAddress, PlacementTarget targetGrain,
        IPlacementRuntime context, PlacementStrategy strategy,
      ...
      if (targetGrain.IsClient) {
        return clientObserversPlacementDirector
            .TrySelectActivationSynchronously(
                strategy, (GrainId) targetGrain.GrainIdentity, context,
                out placementResult);
      }

      var actualStrategy = strategy ?? defaultPlacementStrategy;
      var director = ResolveSelector(actualStrategy);
      return director.TrySelectActivationSynchronously(
          strategy, (GrainId) targetGrain.GrainIdentity, context,
          out placementResult);
    }

    public async Task<PlacementResult>
    SelectOrAddActivation(PlacementTarget targetGrain,
                          IPlacementRuntime context,
                          PlacementStrategy strategy) {
      if (targetGrain.IsClient) {
        var res = await clientObserversPlacementDirector.OnSelectActivation(
            strategy, (GrainId) targetGrain.GrainIdentity, context);
        if (res == null) {
      ...
      var actualStrategy = strategy ?? defaultPlacementStrategy;
      var director = ResolveSelector(actualStrategy);
      var result = await director.OnSelectActivation(
          strategy, targetGrain.GrainIdentity, context);
      if (result != null)
        return result;
    ...
    AddActivation(PlacementTarget target, IPlacementRuntime context,
                  PlacementStrategy strategy) {
      if (target.IsClient)
      ...
      var grainTypeName = context.GetGrainTypeName(
          ((LegacyGrainId) target.GrainIdentity).TypeCode);
      ...
      return PlacementResult.SpecifyCreation(siloAddress, activationId,
                                             strategy, grainTypeName);
    }
    ...
    private void ResolveBuiltInStrategies() {
      ...
      var placementStrategies = new PlacementStrategy[]{
          RandomPlacement.Singleton, ActivationCountBasedPlacement.Singleton,
          statelessWorker, PreferLocalPlacement.Singleton};

      foreach(var strategy in placementStrategies) this.ResolveDirector(
          strategy);
      ...
      foreach(var strategy in selectorStrategies) this.ResolveSelector(strategy,
                                                                       true);
    }
  }
}
