...
namespace Orleans.Runtime {
  /// <summary>
  /// This class collects runtime statistics for all silos in the current
  /// deployment for use by placement.
  /// </summary>
  internal class DeploymentLoadPublisher : SystemTarget,
    ...
    private readonly ConcurrentDictionary<SiloAddress, SiloRuntimeStatistics>
        periodicStats;
    private readonly TimeSpan statisticsRefreshTime;
    private readonly IList<ISiloStatisticsChangeListener>
        siloStatisticsChangeListeners;
    private readonly ILogger logger;
    private IDisposable publishTimer;

    public ConcurrentDictionary<SiloAddress, SiloRuntimeStatistics>
        PeriodicStatistics {
      get { return periodicStats; }
    }

    public DeploymentLoadPublisher(
        ILocalSiloDetails siloDetails, ISiloStatusOracle siloStatusOracle,
        IOptions<DeploymentLoadPublisherOptions>options,
        IInternalGrainFactory grainFactory, OrleansTaskScheduler scheduler,
        ILoggerFactory loggerFactory, IMessageCenter messageCenter,
        ActivationDirectory activationDirectory,
        ActivationCollector activationCollector,
        IAppEnvironmentStatistics appEnvironmentStatistics,
        IHostEnvironmentStatistics hostEnvironmentStatistics,
        IOptions<LoadSheddingOptions>loadSheddingOptions)
        : base(Constants.DeploymentLoadPublisherSystemTargetType,
      ...
      periodicStats =
          new ConcurrentDictionary<SiloAddress, SiloRuntimeStatistics>();
    ...
    public async Task Start() {
      ...
      if (statisticsRefreshTime > TimeSpan.Zero) {
        ...
        // but also upon start publish my stats to everyone and take everyone's
        // stats for me to start with something.
        var randomTimerOffset = random.NextTimeSpan(statisticsRefreshTime);
        this.publishTimer = this.RegisterTimer(
            PublishStatistics, null, randomTimerOffset, statisticsRefreshTime,
    ...
    private async Task PublishStatistics(object _) {
      try {
        ...
        var members =
            this.siloStatusOracle.GetApproximateSiloStatuses(true).Keys;
        var tasks = new List<Task>();
        var activationCount = this.activationDirectory.Count;
        var recentlyUsedActivationCount =
            this.activationCollector.GetNumRecentlyUsed(
                TimeSpan.FromMinutes(10));
        var myStats = new SiloRuntimeStatistics(
            this.messageCenter, activationCount, recentlyUsedActivationCount,
            this.appEnvironmentStatistics, this.hostEnvironmentStatistics,
            this.loadSheddingOptions, DateTime.UtcNow);
        foreach(var siloAddress in members) {
          try {
            tasks.Add(this.grainFactory
                          .GetSystemTarget<IDeploymentLoadPublisher>(
                              Constants.DeploymentLoadPublisherSystemTargetType,
                              siloAddress)
                          .UpdateRuntimeStatistics(this.siloDetails.SiloAddress,
                                                   myStats));
          } catch (Exception) {
            logger.Warn(
                ErrorCode.Placement_RuntimeStatisticsUpdateFailure_1,
                String.Format(
        ...
        logger.Warn(
            ErrorCode.Placement_RuntimeStatisticsUpdateFailure_2,
            String.Format(
    ...
    public Task UpdateRuntimeStatistics(SiloAddress siloAddress,
      ...
      if (logger.IsEnabled(LogLevel.Debug))
        logger.Debug("UpdateRuntimeStatistics from {0}", siloAddress);
    ...
    internal
        async Task<ConcurrentDictionary<SiloAddress, SiloRuntimeStatistics>>
        RefreshStatistics() {
      if (logger.IsEnabled(LogLevel.Debug))
        logger.Debug("RefreshStatistics.");
      await this.scheduler.RunOrQueueTask(() => {
        ...
        foreach(var siloAddress in members) {
          ...
          Task task =
              this.grainFactory
                  .GetSystemTarget<ISiloControl>(Constants.SiloControlType,
                                                 capture)
                  ...
                  .ContinueWith((Task<SiloRuntimeStatistics>statsTask) => {
                    if (statsTask.Status == TaskStatus.RanToCompletion) {
                      ...
                      logger.Warn(
                          ...
                          String.Format(
                              "An unexpected exception was thrown from RefreshStatistics by ISiloControl.GetRuntimeStatistics({0}). Will keep using stale statistics.",
                              capture),
                          statsTask.Exception);
                    }
                  });
    ...
    public bool
    SubscribeToStatisticsChangeEvents(ISiloStatisticsChangeListener observer) {
    ...
    public bool
    UnsubscribeStatisticsChangeEvents(ISiloStatisticsChangeListener observer) {
      lock(siloStatisticsChangeListeners) {
    ...
    private void
    NotifyAllStatisticsChangeEventsSubscribers(SiloAddress silo,
    ...
    public void SiloStatusChangeNotification(SiloAddress updatedSilo,
      ...
      this.ScheduleTask(() => {
            Utils.SafeExecute(() =>
                                  this.OnSiloStatusChange(updatedSilo, status),
                              this.logger);
          })
          .Ignore();
    }
