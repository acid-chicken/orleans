...
namespace Orleans.Runtime.Placement {
  internal class StatelessWorkerDirector : IPlacementDirector,
    ...
    public Task<PlacementResult>OnSelectActivation(PlacementStrategy strategy,
                                                   GrainId target,
    ...
    public bool
    TrySelectActivationSynchronously(PlacementStrategy strategy, GrainId target,
                                     IPlacementRuntime context,
    ...
    public Task<SiloAddress>OnAddActivation(PlacementStrategy strategy,
      ...
      // If the current silo is not shutting down, place locally if we are
      // compatible
      ...
      return Task.FromResult(
          compatibleSilos[random.Next(compatibleSilos.Count)]);
    }

    private PlacementResult SelectActivationCore(PlacementStrategy strategy,
                                                 GrainId target,
                                                 IPlacementRuntime context) {
      if (target.IsClient())
        throw new InvalidOperationException(
            "Cannot use StatelessWorkerStrategy to route messages to client grains.");

      // If there are available (not busy with a request) activations, it
      // returns the first one. If all are busy and the number of local
      // activations reached or exceeded MaxLocal, it randomly returns one of
      // them. Otherwise, it requests creation of a new activation.
      ...
      foreach(var activation in local) {
        ...
        return PlacementResult.IdentifySelection(ActivationAddress.GetAddress(
            context.LocalSilo, target, activation.ActivationId));
      }

      if (local.Count >= placement.MaxLocal) {
        var id =
            local[local.Count == 1 ? 0 : random.Next(local.Count)].ActivationId;
        return PlacementResult.IdentifySelection(
            ActivationAddress.GetAddress(context.LocalSilo, target, id));
      }
