...
namespace Orleans.Core {
  public class StateStorageBridge<TState>: IStorage<TState> {
    ...
    public StateStorageBridge(string name, GrainReference grainRef,
                              IGrainStorage store,
      ...
      this.store = store;
      this.grainState =
          new GrainState<TState>(Activator.CreateInstance<TState>());
    }

    /// <summary>
    /// Async method to cause refresh of the current grain state data from
    /// backing store. Any previous contents of the grain state data will be
    /// overwritten.
    /// </summary>
    public async Task ReadStateAsync() {
      ...
      try {
        ...
        this.logger.Error((int) ErrorCode.StorageProvider_ReadFailed, errMsg,
                          exc);
    ...
    public async Task WriteStateAsync() {
      ...
      try {
        ...
        this.logger.Error((int) ErrorCode.StorageProvider_WriteFailed,
                          errMsgToLog, exc);
    ...
    public async Task ClearStateAsync() {
      ...
      try {
        ...
        string errMsg = MakeErrorMsg(what, exc);
        this.logger.Error((int) ErrorCode.StorageProvider_DeleteFailed, errMsg,
                          exc);
        if (!(exc is OrleansException)) {
          throw new OrleansException(errMsg, exc);
        }
    ...
    private string MakeErrorMsg(string what, Exception exc) {
      ...
      return string.Format(
          "Error from storage provider {0} during {1} for grain Type={2} Pk={3} Id={4} Error={5}" +
              Environment.NewLine + " {6}",
          $"{this.store.GetType().Name}.{this.name}", what, name,
          grainRef.GrainId.ToDetailedString(), grainRef, errorCode,
