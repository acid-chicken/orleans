...
namespace Orleans.Runtime {
  ...
  /// </summary>
  internal sealed class HostedClient : IDisposable,
                                       ILifecycleParticipant<ISiloLifecycle> {
    private readonly Channel<Message>incomingMessages;
    private readonly Dictionary<Type, Tuple<IGrainExtension, IAddressable>>
        extensionsTable =
            new Dictionary<Type, Tuple<IGrainExtension, IAddressable>>();
    ...
    public HostedClient(IRuntimeClient runtimeClient,
      ...
      this.ClientId = ClientGrainId.Create(
          $"hosted-{messageCenter.MyAddress.ToParsableString()}");
      this.ClientAddress = ActivationAddress.NewActivationAddress(
          siloDetails.SiloAddress, this.ClientId.GrainId);
    }
    ...
    /// <inheritdoc />
    public StreamDirectory StreamDirectory { get; }
    = new StreamDirectory();

    /// <inheritdoc />
    public override string ToString() =>
        $"{nameof(HostedClient)}_{this.ClientAddress}";
    ...
    public GrainReference CreateObjectReference(IAddressable obj,
      ...
      if (obj is GrainReference)
        throw new ArgumentException(
            "Argument obj is already a grain reference.");

      var observerId = ObserverGrainId.Create(this.ClientId);
      var grainReference = GrainReference.NewObserverGrainReference(
          observerId, this.grainReferenceRuntime);
      if (!this.invokableObjects.TryRegister(obj, observerId, invoker)) {
        throw new ArgumentException(
            string.Format(
                "Failed to add new observer {0} to localObjects collection.",
                grainReference),
    ...
    /// <inheritdoc />
    public async Task<Tuple<TExtension, TExtensionInterface>>
    BindExtension<TExtension, TExtensionInterface>(
      ...
      using(await this.lockable.LockAsync()) {
        if (this.extensionsTable.TryGetValue(typeof (TExtensionInterface),
                                             out var entry)) {
          ...
          var obj =
              this.grainFactory.CreateObjectReference<TExtensionInterface>(
                  extension);
      ...
      // we have to return the extension as well as the IAddressable because the
      // caller needs to root the extension to prevent it from being collected
      // (the IAddressable uses a weak reference).
      return Tuple.Create(extension, typedAddressable);
    }

    /// <inheritdoc />
    public bool TryDispatchToClient(Message message) {
      if (!ClientGrainId.TryParse(message.TargetGrain, out var targetClient) ||
          !this.ClientId.Equals(targetClient)) {
        return false;
      }

      if (message.IsExpired) {
        this.messagingTrace.OnDropExpiredMessage(
            message, MessagingStatisticsGroup.Phase.Receive);
        return true;
      }

      if (message.Direction == Message.Directions.Response) {
        // Requests are made through the runtime client, so deliver responses to
        // the rutnime client so that the request callback can be executed.
        this.runtimeClient.ReceiveResponse(message);
      } else {
        // Requests agrainst client objects are scheduled for execution on the
        // client.
    ...
    void IDisposable.Dispose() {
      ...
      this.disposing = true;
      Utils.SafeExecute(
          () => this.clientObserverRegistrar.ClientDropped(this.ClientId));
      Utils.SafeExecute(() =>
                            this.clientObserverRegistrar.SetHostedClient(null));
        ...
        try {
          ...
          if (!more) {
            this.logger.LogInformation(
                $"{nameof(HostedClient)} completed processing all messages. Shutting down.");
            break;
          }
          ...
          while (reader.TryRead(out var message)) {
            ...
            case Message.Directions.Request:
              ...
              break;
            default:
              this.logger.LogError((int) ErrorCode.Runtime_Error_100327,
                                   "Message not supported: {Message}", message);
              break;
            }
          }
        } catch (Exception exception) {
          this.logger.LogError(
              (int) ErrorCode.Runtime_Error_100326,
              "RunClientMessagePump has thrown an exception: {Exception}. Continuing.",
              exception);
        }
      }
    }
    ...
    void ILifecycleParticipant<ISiloLifecycle>.Participate(
      ...
      lifecycle.Subscribe("HostedClient",
                          ServiceLifecycleStage.RuntimeGrainServices, OnStart,
                          OnStop);
      ...
      Task OnStart(CancellationToken cancellation) {
        ...
        // Register with the directory and message center so that we can receive
        // messages.
        ...
        var clusterClient = this.runtimeClient.ServiceProvider
