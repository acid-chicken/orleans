...
namespace Orleans.Runtime {
  ...
  internal class GrainMethodInvoker : IIncomingGrainCallContext,
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GrainMethodInvoker"/>
    /// class.
    /// </summary>
    ...
    /// <param name="interfaceToImplementationMapping">The implementation
    /// map.</param>
    public GrainMethodInvoker(IAddressable grain, InvokeMethodRequest request,
    ...
    public MethodInfo ImplementationMethod =>
        GetMethodEntry().ImplementationMethod;
    ...
    public async Task Invoke() {
      try {
        // Execute each stage in the pipeline. Each successive call to this
        // method will invoke the next stage. Stages which are not implemented
        // (eg, because the user has not specified an interceptor) are skipped.
      ...
      // If this method has been called more than the expected number of times,
      // that is invalid.
      ThrowInvalidCall();
    }
    ...
    ushort IGrainMethodInvoker.InterfaceVersion =>
        this.rootInvoker.InterfaceVersion;

    async Task<object>
        IGrainMethodInvoker.Invoke(IAddressable grain,
                                   InvokeMethodRequest invokeMethodRequest) {
    ...
    private InterfaceToImplementationMappingCache.Entry GetMethodEntry() {
      ...
      if (this.rootInvoker is IGrainExtensionMap extensionMap &&
          extensionMap.TryGetExtension(request.InterfaceId,
                                       out var extension)) {
    ...
    private void ValidateArguments(IAddressable grain,
                                   InvokeMethodRequest invokeMethodRequest) {
      if (!Equals(this.Grain, grain))
        throw new ArgumentException(
            $"Provided {nameof(IAddressable)} differs from expected value",
            nameof(grain));
      if (!Equals(this.request, invokeMethodRequest))
        throw new ArgumentException(
            $"Provided {nameof(InvokeMethodRequest)} differs from expected value",
