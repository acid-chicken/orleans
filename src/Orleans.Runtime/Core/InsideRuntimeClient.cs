...
namespace Orleans.Runtime {
  ...
  internal class InsideRuntimeClient : ISiloRuntimeClient,
    ...
    private IHostedClient HostedClient => this.hostedClient ??(
        this.hostedClient =
            this.ServiceProvider.GetRequiredService<IHostedClient>());
    private readonly InterfaceToImplementationMappingCache
    ...
    public InsideRuntimeClient(
        ILocalSiloDetails siloDetails, GrainTypeManager typeManager,
        TypeMetadataCache typeMetadataCache, OrleansTaskScheduler scheduler,
        IServiceProvider serviceProvider, MessageFactory messageFactory,
        ITransactionAgent transactionAgent, ILoggerFactory loggerFactory,
      ...
      this.invokeExceptionLogger = loggerFactory.CreateLogger(
          $"{typeof(Grain).FullName}.InvokeException");
      ...
      this.sharedCallbackData = new SharedCallbackData(
          ...
          this.messagingOptions, this.appRequestStatistics,
    ...
    private Catalog Catalog => this.catalog ??(
        this.catalog = this.ServiceProvider.GetRequiredService<Catalog>());

    private ILocalGrainDirectory Directory => this.directory ??(
        this.directory =
            this.ServiceProvider.GetRequiredService<ILocalGrainDirectory>());

    private List<IIncomingGrainCallFilter>GrainCallFilters =>
        this.grainCallFilters ??(
            this.grainCallFilters = new List<IIncomingGrainCallFilter>(
                this.ServiceProvider.GetServices<IIncomingGrainCallFilter>()));

    private Dispatcher Dispatcher => this.dispatcher ??(
        this.dispatcher =
            this.ServiceProvider.GetRequiredService<Dispatcher>());

    public IGrainReferenceRuntime GrainReferenceRuntime =>
        this.grainReferenceRuntime ??(
            this.grainReferenceRuntime =
                this.ServiceProvider
                    .GetRequiredService<IGrainReferenceRuntime>());

    public void SendRequest(GrainReference target, InvokeMethodRequest request,
                            TaskCompletionSource<object>context,
                            string debugContext, InvokeMethodOptions options,
                            string genericArguments = null) {
      var message = this.messageFactory.CreateMessage(request, options);
      EventSourceUtils.EmitEvent(
          message, OrleansInsideRuntimeClientEvent.SendRequestAction);
      SendRequestMessage(target, message, context, debugContext, options,
                         genericArguments);
    }

    private void SendRequestMessage(GrainReference target, Message message,
                                    TaskCompletionSource<object>context,
                                    string debugContext,
      ...
      SchedulingContext schedulingContext =
          RuntimeContext.CurrentActivationContext as SchedulingContext;
      ...
      if (schedulingContext == null) {
        ...
        case SchedulingContextType.SystemTarget:
          message.SendingActivation =
              schedulingContext.SystemTarget.ActivationId;
          message.SendingGrain =
              ((ISystemTargetBase) schedulingContext.SystemTarget).GrainId;
          break;
        }
      }
      ...
      if (targetGrainId.IsSystemTarget) {
        ...
        message.TargetActivation =
            ActivationId.GetSystemActivation(targetGrainId, targetSilo);
      ...
      if (context == null && !oneWay) {
        logger.Warn(ErrorCode.IGC_SendRequest_NullContext,
                    "Null context {0}: {1}", message, Utils.GetStackTrace());
      }

      if (message.IsExpirableMessage(
              this.messagingOptions.DropExpiredMessages)) {
      ...
      if (targetGrainId.IsSystemTarget) {
        // Messages to system targets bypass the task system and get sent
        // "in-line"
    ...
    public void SendResponse(Message request, Response response) {
      EventSourceUtils.EmitEvent(
          request, OrleansInsideRuntimeClientEvent.SendResponseAction);
    ...
    public void SniffIncomingMessage(Message message) {
      try {
        if (message.CacheInvalidationHeader != null) {
          foreach(
              ActivationAddress address in message.CacheInvalidationHeader) {
            this.Directory.InvalidateCacheEntry(
                address, message.IsReturnedFromRemoteCluster);
          }
        }
        ...
        logger.Warn(ErrorCode.IGC_SniffIncomingMessage_Exc,
                    "SniffIncomingMessage has thrown exception. Ignoring.",
                    exc);
      }
    }

    public async Task Invoke(IAddressable target, IInvokable invokable,
                             Message message) {
      try {
        // Don't process messages that have already timed out
        ...
        if (schedulingOptions.PerformDeadlockDetection &&
          ...
          UpdateDeadlockInfoInRequestContext(new RequestInvocationHistory(
              message.TargetGrain, message.TargetActivation,
              message.DebugContext));
          // RequestContext is automatically saved in the msg upon send and
          // propagated to the next hop in RuntimeClient.CreateMessage ->
          // RequestContextExtensions.ExportToMessage(message);
        }
        ...
        if (message.IsTransactionRequired && transactionInfo == null) {
          ...
          var transactionTimeout =
              Debugger.IsAttached ? TimeSpan.FromMinutes(30)
              : TimeSpan.FromSeconds(10);

          // Start a new transaction
          transactionInfo = await this.transactionAgent.StartTransaction(
              message.IsReadOnly, transactionTimeout);
        ...
        try {
          ...
          if (request.Arguments != null) {
            CancellationSourcesExtension.RegisterCancellationTokens(
                target, request, this.loggerFactory, logger, this,
                this.cancellationTokenRuntime);
          }
          ...
          if (invoker is IGrainExtensionMethodInvoker &&
              ...
              !TryInstallExtension(request.InterfaceId, invokable,
                                   message.GenericGrainType, ref invoker)) {
            // We are trying the invoke a grain extension method on a grain
            // -- most likely reason is that the dynamic extension is not
            // installed for this grain So throw a specific exception here
            // rather than a general InvalidCastException
            var error = String.Format(
                "Extension not installed on grain {0} attempting to invoke type {1} from invokable {2}",
                target.GetType().FullName, invoker.GetType().FullName,
                invokable.GetType().FullName);
            ...
            logger.Warn(ErrorCode.Stream_ExtensionNotInstalled,
                        string.Format("{0} for message {1} {2}", error, message,
                                      extraDebugInfo),
                        exc);
          ...
          // If a grain allowed an inconsistent state exception to escape and
          // the exception originated from this activation, then deactivate it.
          var ise = exc1 as InconsistentStateException;
          if (ise != null && ise.IsSourceActivation) {
            // Mark the exception so that it doesn't deactivate any other
            // activations.
            ise.IsSourceActivation = false;

            var activation = (target as Grain) ?.Data;
            if (activation != null) {
              invokeExceptionLogger.Info(
                  $"Deactivating {activation} due to inconsistent state.");
        ...
        if (transactionInfo != null) {
          try {
            ...
            transactionException =
                transactionInfo.MustAbort(serializationManager);

            // This request started the transaction, so we try to commit before
            // returning, or if it must abort, tell participants that it aborted
            if (startNewTransaction) {
              try {
                if (transactionException == null) {
                  var status =
                      await this.transactionAgent.Resolve(transactionInfo);
                  if (status != TransactionalStatus.Ok) {
                    transactionException =
                        status.ConvertToUserException(transactionInfo.Id);
                  }
                } else {
                  await this.transactionAgent.Abort(transactionInfo);
                }
            ...
            // we should never hit this, but if we do, the following message
            // will help us diagnose
            this.logger.LogError(
                e, "Error in transaction post-grain-method-invocation code");
            throw;
          }
        }
        ...
        logger.Warn(ErrorCode.Runtime_Error_100329,
                    "Exception during Invoke of message: " + message, exc2);
    ...
    private bool TryInstallExtension(int interfaceId, IInvokable invokable,
                                     string genericGrainType,
                                     ref IGrainMethodInvoker invoker) {
      IGrainExtension extension =
          TryGetCurrentActivationData(out ActivationData activationData)
              ? activationData.ActivationServices
                    .GetServiceByKey<int, IGrainExtension>(interfaceId)
              : this.ServiceProvider.GetServiceByKey<int, IGrainExtension>(
                    interfaceId);
      ...
      invoker =
          invokable.GetInvoker(typeManager, interfaceId, genericGrainType);
      return true;
    }

    private void SafeSendResponse(Message message, object resultObject) {
      try {
        SendResponse(message, new Response(this.serializationManager.DeepCopy(
                                  resultObject)));
      } catch (Exception exc) {
    ...
    private static readonly Lazy<Func<Exception, Exception>>
        prepForRemotingLazy = new Lazy<Func<Exception, Exception>>(
            CreateExceptionPrepForRemotingMethod);

    private static Func<Exception, Exception>
    CreateExceptionPrepForRemotingMethod() {
      var methodInfo = typeof (Exception).GetMethod(
          "PrepForRemoting", BindingFlags.Instance | BindingFlags.NonPublic);

      // This was added to avoid failure on .Net Core since Remoting APIs aren't
      // available there.
      if (methodInfo == null)
        return exc => exc;

      var method = new DynamicMethod(
          "PrepForRemoting", typeof (Exception), new[]{typeof (Exception)},
      ...
      return (Func<Exception, Exception>)
          method.CreateDelegate(typeof (Func<Exception, Exception>));
    }

    private static Exception PrepareForRemoting(Exception exception) {
      // Call the Exception.PrepForRemoting internal method, which preserves the
      // original stack when the exception is rethrown at the remote site (and
      // appends the call site stacktrace). If this is not done, then when the
      // exception is rethrown the original stacktrace is entire replaced.
      // Note: another commonly used approach since .NET 4.5 is to use
      // ExceptionDispatchInfo.Capture(ex).Throw() but that involves rethrowing
      // the exception in-place, which is not what we want here, but could in
      // theory be done at the receiving end with some rework (could be tackled
      // when we reopen #875 Avoid unnecessary use of TCS).
    ...
    private void SafeSendExceptionResponse(Message message, Exception ex) {
      try {
        var copiedException = PrepareForRemoting(
            (Exception) this.serializationManager.DeepCopy(ex));
        ...
        try {
          ...
          logger.Warn(
              ErrorCode.IGC_UnhandledExceptionInInvoke,
              "Exception trying to send an exception. Ignoring and not trying to send again. Exc: " +
                  exc2.Message,
              exc2);
        }
      }
    ...
    // assumes deadlock information was already loaded into RequestContext from
    // the message
    private static void UpdateDeadlockInfoInRequestContext(
        RequestInvocationHistory thisInvocation) {
      IList prevChain;
      object obj =
          RequestContext.Get(RequestContext.CALL_CHAIN_REQUEST_CONTEXT_HEADER);
      if (obj != null) {
        prevChain = ((IList) obj);
      } else {
        prevChain = new List<RequestInvocationHistory>();
        RequestContext.Set(RequestContext.CALL_CHAIN_REQUEST_CONTEXT_HEADER,
                           prevChain);
      }
    ...
    public void ReceiveResponse(Message message) {
      EventSourceUtils.EmitEvent(
          message, OrleansInsideRuntimeClientEvent.ReceiveResponseAction);
      if (message.Result == Message.ResponseTypes.Rejection) {
        if (!message.TargetSilo.Matches(this.MySilo)) {
          ...
          if (logger.IsEnabled(LogLevel.Trace))
            logger.Trace(ErrorCode.Dispatcher_NoCallbackForRejectionResp,
                         "No callback for rejection response message: {0}",
                         message);
        ...
        if (logger.IsEnabled(LogLevel.Debug))
          logger.Debug(ErrorCode.Dispatcher_HandleMsg, "HandleMessage {0}",
                       message);
        ...
        case Message.RejectionTypes.Transient:
          if (message.CacheInvalidationHeader == null) {
            // Remove from local directory cache. Note that SendingGrain is the
            // original target, since message is the rejection response. If
            // CacheMgmtHeader is present, we already did this. Otherwise, we
            // left this code for backward compatability. It should be retired
            // as we move to use CacheMgmtHeader in all relevant places.
            this.Directory.InvalidateCacheEntry(message.SendingAddress);
          }
          break;

        case Message.RejectionTypes.CacheInvalidation when
            message.HasCacheInvalidationHeader:
          // The message targeted an invalid (eg, defunct) activation and this
          // response serves only to invalidate this silo's activation cache.
          return;
        default:
      ...
      if (found) {
        ...
        // IMPORTANT: we do not schedule the response callback via the
        // scheduler, since the only thing it does is to resolve/break the
        // resolver. The continuations/waits that are based on this resolution
        // will be scheduled as work items.
    ...
    public string CurrentActivationIdentity {
      get {
        if (RuntimeContext.Current == null)
          return this.HostedClient.ToString();
    ...
    /// <inheritdoc />
    public TimeSpan
    GetResponseTimeout() => this.sharedCallbackData.ResponseTimeout;

    /// <inheritdoc />
    public void SetResponseTimeout(TimeSpan timeout) =>
        this.sharedCallbackData.ResponseTimeout = timeout;

    public GrainReference CreateObjectReference(IAddressable obj,
                                                IGrainMethodInvoker invoker) {
      if (RuntimeContext.Current == null)
        return this.HostedClient.CreateObjectReference(obj, invoker);
      throw new InvalidOperationException(
          "Cannot create a local object reference from a grain.");
    }
    ...
    public void DeleteObjectReference(IAddressable obj) {
      if (RuntimeContext.Current == null) {
        ...
        throw new InvalidOperationException(
            "Cannot delete a local object reference from a grain.");
      }
    }
    ...
    public void DeactivateOnIdle(ActivationId id) {
      ...
      data.ResetKeepAliveRequest(); // DeactivateOnIdle method would undo /
                                    // override any current “keep alive”
                                    // setting, making this grain immideately
                                    // avaliable for deactivation.
      ...
      lock(disposables) {
        foreach(var disposable in disposables) {
          try {
            ...
            logger.Warn(ErrorCode.IGC_DisposeError,
                        "Exception while disposing: " + e.Message, e);
          }
        }
      }
    ...
    private Task OnRuntimeInitializeStart(CancellationToken tc) {
      ...
      this.serializationManager =
          this.ServiceProvider.GetRequiredService<SerializationManager>();
      var timerLogger = this.loggerFactory.CreateLogger<SafeTimer>();
      var minTicks = Math.Min(this.messagingOptions.ResponseTimeout.Ticks,
                              TimeSpan.FromSeconds(1).Ticks);
      var period = TimeSpan.FromTicks(minTicks);
      this.callbackTimer = new SafeTimer(timerLogger, this.OnCallbackExpiryTick,
                                         null, period, period);
      ...
      this.logger.Info(
          ...
          $"Start InsideRuntimeClient took {stopWatch.ElapsedMilliseconds} Milliseconds");
      return Task.CompletedTask;
    }

    public IGrainTypeResolver GrainTypeResolver =>
        typeManager.GrainTypeResolver;
    ...
    public Task<Tuple<TExtension, TExtensionInterface>>
    BindExtension<TExtension, TExtensionInterface>(
        Func<TExtension>newExtensionFunc) where TExtension
        : IGrainExtension where TExtensionInterface : IGrainExtension {
      if (RuntimeContext.Current == null) {
        return this.HostedClient.BindExtension<TExtension, TExtensionInterface>(
            newExtensionFunc);
      }
      ...
      if (!TryGetExtensionHandler(out TExtension extension)) {
        ...
        if (!TryAddExtension(extension))
          throw new OrleansException("Failed to register " +
    ...
    public bool TryAddExtension(IGrainExtension handler) {
      ...
      var methodInvoker =
          TryGetExtensionMethodInvoker(this.typeManager, handler.GetType());
    ...
    public bool
    TryGetExtensionHandler<TExtension>(out TExtension result) where TExtension
        : IGrainExtension {
      ExtensionInvoker invoker = GetCurrentExtensionInvoker();
      IGrainExtension untypedResult;
      if (invoker.TryGetExtensionHandler(typeof (TExtension),
                                         out untypedResult)) {
    ...
    private ExtensionInvoker GetCurrentExtensionInvoker() {
      ...
      return (context.ContextType == SchedulingContextType.SystemTarget)
          ? (context as SchedulingContext)
                ?.SystemTarget.ExtensionInvoker
                : GetCurrentActivationData(context).ExtensionInvoker;
    }

    private ActivationData
    GetCurrentActivationData(ISchedulingContext context = null) {
      context = context ?? RuntimeContext.CurrentActivationContext;
      if (TryGetCurrentActivationData(context,
                                      out ActivationData activationData))
        return activationData;
      return ThrowInvalidOperationException();
      ActivationData
      ThrowInvalidOperationException() => throw new InvalidOperationException(
          "Attempting to GetCurrentActivationData when not in an activation scope");
    }

    private bool
    TryGetCurrentActivationData(out ActivationData activationData) {
      return TryGetCurrentActivationData(
          RuntimeContext.CurrentActivationContext, out activationData);
    }

    private bool
    TryGetCurrentActivationData(ISchedulingContext context,
                                out ActivationData activationData) {
      activationData = (context as SchedulingContext) ?.Activation;
      return (activationData != null);
    }

    internal static IGrainExtensionMethodInvoker
    TryGetExtensionMethodInvoker(GrainTypeManager typeManager,
                                 Type handlerType) {
      var interfaces =
          GrainInterfaceUtils.GetRemoteInterfaces(handlerType).Values;
      if (interfaces.Count != 1)
        throw new InvalidOperationException(
            $"Extension type {handlerType.FullName} implements more than one grain interface.");

      var interfaceId =
          GrainInterfaceUtils.GetGrainInterfaceId(interfaces.First());
    ...
    public void Participate(ISiloLifecycle lifecycle) {
      lifecycle.Subscribe<InsideRuntimeClient>(
          ServiceLifecycleStage.RuntimeInitialize, OnRuntimeInitializeStart,
    ...
    private void OnCallbackExpiryTick(object state) {
      ...
      foreach(var pair in callbacks) {
        ...
        if (callback.IsExpired(currentStopwatchTicks))
          callback.OnTimeout(responseTimeout);
      }
    }
  }
}
