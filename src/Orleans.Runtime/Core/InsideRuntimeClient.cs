  ...
  internal class InsideRuntimeClient : ISiloRuntimeClient,
    ...
    public void SendRequest(GrainReference target, InvokeMethodRequest request,
      ...
      if (context is null && !oneWay) {
        this.logger.Warn(ErrorCode.IGC_SendRequest_NullContext,
                         "Null context {0}: {1}", message,
                         Utils.GetStackTrace());
      }
    ...
    public void SendResponse(Message request, Response response) {
      ...
      if (request.IsExpired) {
        this.messagingTrace.OnDropExpiredMessage(
            request, MessagingStatisticsGroup.Phase.Respond);
        return;
      }
    ...
    public void SniffIncomingMessage(Message message) {
      try {
        ...
        this.logger.Warn(ErrorCode.IGC_SniffIncomingMessage_Exc,
    ...
    public async Task Invoke(IAddressable target, IInvokable invokable,
      ...
      try {
        ...
        if (message.IsExpired) {
          this.messagingTrace.OnDropExpiredMessage(
              message, MessagingStatisticsGroup.Phase.Invoke);
          return;
        }
        ...
        try {
          ...
          if (invoker is IGrainExtensionMethodInvoker &&
            ...
            this.logger.Warn(ErrorCode.Stream_ExtensionNotInstalled,
                             string.Format("{0} for message {1} {2}", error,
                                           message, extraDebugInfo),
                             exc);
          ...
          if (message.Direction == Message.Directions.OneWay) {
            this.invokeExceptionLogger.Warn(
            ...
            this.invokeExceptionLogger.Debug(
                ErrorCode.GrainInvokeException,
                "Exception during Grain method call of message: " + message +
                    ": " + LogFormatter.PrintException(exc1),
                exc1);
          ...
          if (ise != null && ise.IsSourceActivation) {
            ...
            if (activation != null) {
              this.invokeExceptionLogger.Info(
        ...
        this.logger.Warn(ErrorCode.Runtime_Error_100329,
                         "Exception during Invoke of message: " + message,
                         exc2);
    ...
    private void SafeSendResponse(Message message, object resultObject) {
      try {
        ...
        this.logger.Warn(ErrorCode.IGC_SendResponseFailed,
                         "Exception trying to send a response: " + exc.Message,
                         exc);
    ...
    public void ReceiveResponse(Message message) {
      ...
      if (message.Result == Message.ResponseTypes.Rejection) {
        if (!message.TargetSilo.Matches(this.MySilo)) {
          ...
          if (logger.IsEnabled(LogLevel.Trace))
            this.logger.Trace(ErrorCode.Dispatcher_NoCallbackForRejectionResp,
        ...
        if (logger.IsEnabled(LogLevel.Debug))
          this.logger.Debug(ErrorCode.Dispatcher_HandleMsg, "HandleMessage {0}",
                            message);
      ...
      if (found) {
        ...
        if (logger.IsEnabled(LogLevel.Debug))
          this.logger.Debug(ErrorCode.Dispatcher_NoCallbackForResp,
      ...
      lock(disposables) {
        foreach(var disposable in disposables) {
          try {
            ...
            this.logger.Warn(ErrorCode.IGC_DisposeError,
