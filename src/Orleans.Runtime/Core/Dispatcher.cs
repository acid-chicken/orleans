...
namespace Orleans.Runtime {
  internal class Dispatcher {
    ...
    internal Dispatcher(
        OrleansTaskScheduler scheduler, MessageCenter transport,
        Catalog catalog, IOptions<SiloMessagingOptions>messagingOptions,
        PlacementDirectorsManager placementDirectorsManager,
        ILocalGrainDirectory localGrainDirectory, IGrainLocator grainLocator,
        ActivationCollector activationCollector, MessageFactory messageFactory,
                ...
                async () => {
                  try {
                    if (this.logger.IsEnabled(LogLevel.Trace))
                      logger.Trace(
                          "UnregisterAfterNonexistingActivation addr={ActivationAddress} origin={SiloAddress}",
                          nonExistentActivation, origin);

                    await this.grainLocator.Unregister(
                        nonExistentActivation,
                        UnregistrationCause.NonexistentActivation);
                  } catch (Exception exc) {
                    logger.Warn(
                        ErrorCode.Dispatcher_FailedToUnregisterNonExistingAct,
    ...
    /// </summary>
    private void
    MarkSameCallChainMessageAsInterleaving(IGrainContext sendingActivation,
                                           Message outgoing) {
    ...
    public void HandleIncomingRequest(Message message,
      ...
      lock(targetActivation) {
        ...
        if (targetActivation.Grain.IsLegacyGrain() &&
          ...
          var currentVersion = this.grainTypeManager.GetLocalSupportedVersion(
              request.InterfaceId);
        ...
        scheduler.QueueWorkItem(new InvokeWorkItem(
            targetActivation, message, this, this.invokeWorkItemLogger));
      }
    }
    ...
    internal void ProcessRequestToInvalidActivation(
      ...
      if (rejectMessages) {
        ...
        scheduler.QueueAction(() => TryForwardRequest(message, oldAddress,
                                                      forwardingAddress,
                                                      failedOperation, exc),
                              catalog);
      }
...
internal void TryForwardRequest(Message message, ActivationAddress oldAddress,
                                ActivationAddress forwardingAddress,
                                string failedOperation, Exception exc = null) {
  bool forwardingSucceded = true;
  try {
    this.messagingTrace.OnDispatcherForwarding(
        message, oldAddress, forwardingAddress, failedOperation, exc);

    // if this message is from a different cluster and hit a non-existing
    // activation in this cluster (which can happen due to stale cache or
    // directory states) we forward it back to the original silo it came from in
    // the original cluster, and target it to a fictional activation that is
    // guaranteed to not exist. This ensures that the GSI protocol creates a new
    ...
    if (forwardingAddress == null &&
      ...
      message.IsReturnedFromRemoteCluster =
          true; // marks message to force invalidation of stale directory entry
      forwardingAddress = ActivationAddress.NewActivationAddress(
          message.SendingSilo, message.TargetGrain);
      logger.Info(
          ErrorCode.Messaging_Dispatcher_ReturnToOriginCluster,
    ...
    // If the message was a one-way message, send a cache invalidation response
    // even if the message was successfully forwarded.
    if (message.Direction == Message.Directions.OneWay) {
      this.RejectMessage(message, Message.RejectionTypes.CacheInvalidation, exc,
                         "OneWay message sent to invalid activation");
      sentRejection = true;
    }

    if (!forwardingSucceded) {
      this.messagingTrace.OnDispatcherForwardingFailed(
          message, oldAddress, forwardingAddress, failedOperation, exc);
      if (!sentRejection) {
        var str =
...
private void ResendMessageImpl(Message message,
                               ActivationAddress forwardingAddress = null) {
  if (logger.IsEnabled(LogLevel.Debug))
...
// Forwarding is used by the receiver, usually when it cannot process the
// message and forwards it to another silo to perform the processing (got here
// due to duplicate activation, outdated cache, silo is shutting
// down/overloaded, ...).
private static bool MayForward(Message message,
...
public Task AsyncSendMessage(Message message,
                             IGrainContext sendingActivation = null) {
  try {
    ...
    if (messageAddressingTask.Status == TaskStatus.RanToCompletion) {
      ...
      return TransportMessageAferSending(messageAddressingTask, message,
                                         sendingActivation);
    }
  } catch (Exception ex) {
  ...
  async Task TransportMessageAferSending(Task addressMessageTask, Message m,
...
// async/await yet, which is almost everything. there's no liability to
// discarding the Task returned by AsyncSendMessage()
...
/// </summary>
/// <param name="message"></param>
/// <returns>Resolve when message is addressed (modifies message
/// fields)</returns>
private Task AddressMessage(Message message) {
  ...
  // placement strategy is determined by searching for a specification. first,
  // we check for a strategy associated with the grain reference, second, we
  // check for a strategy associated with the target's interface. third, we
  // check for a strategy associated with the activation sending the message.
  var strategy = targetAddress.Grain.IsLegacyGrain()
                     ? catalog.GetGrainPlacementStrategy(targetAddress.Grain)
                     : null;

  var request = message.IsUsingInterfaceVersions
                      ? message
                    .BodyObject as InvokeMethodRequest : null;
  var target = new PlacementTarget(
      message.TargetGrain, message.RequestContextData, request?.InterfaceId ?? 0,
...
private async Task AddressMessageAsync(Message message, PlacementTarget target,
                                       PlacementStrategy strategy,
                                       ActivationAddress targetAddress) {
  var placementResult = await placementDirectorsManager.SelectOrAddActivation(
      target, (IPlacementRuntime) this.catalog, strategy);
  SetMessageTargetPlacement(message, placementResult, targetAddress);
}

private void SetMessageTargetPlacement(Message message,
                                       PlacementResult placementResult,
                                       ActivationAddress targetAddress) {
  if (placementResult.IsNewPlacement && targetAddress.Grain.IsClient()) {
    logger.Error(
        ErrorCode.Dispatcher_AddressMsg_UnregisteredClient,
  ...
  if (placementResult.IsNewPlacement) {
    CounterStatistic.FindOrCreate(StatisticNames.DISPATCHER_NEW_PLACEMENT)
        .Increment();
  }
  if (logger.IsEnabled(LogLevel.Trace))
    logger.Trace(ErrorCode.Dispatcher_AddressMsg_SelectTarget,
                 "AddressMessage Placement SelectTarget {0}", message);
}
...
internal void SendSystemTargetMessage(Message message) {
  ...
  if (message.TargetActivation is null) {
    message.TargetActivation =
        ActivationId.GetDeterministic(message.TargetGrain);
  }
...
public void TransportMessage(Message message,
  ...
  if (logger.IsEnabled(LogLevel.Trace))
    logger.Trace(ErrorCode.Dispatcher_Send_AddressedMessage,
                 "Addressed message {0}", message);
  Transport.SendMessage(message);
}

/// <summary>
/// Invoked when an activation has finished a transaction and may be ready for
/// additional transactions
/// </summary>
/// <param name="activation">The activation that has just completed processing
/// this message</param> <param name="message">The message that has just
/// completed processing. This will be <c>null</c> for the case of completion of
/// Activate/Deactivate calls.</param>
internal void OnActivationCompletedRequest(ActivationData activation,
                                           Message message) {
  lock(activation) {
#if DEBUG
    // This is a hot code path, so using #if to remove diags from Release
    ...
    if (logger.IsEnabled(LogLevel.Trace)) {
      logger.Trace(ErrorCode.Dispatcher_OnActivationCompletedRequest_Waiting,
                   ...
                   activation.ActivationId, activation.DumpStatus());
    }
#endif
...
internal void RunMessagePump(ActivationData activation) {
  ...
  if (logger.IsEnabled(LogLevel.Trace)) {
    logger.Trace(ErrorCode.Dispatcher_ActivationEndedTurn_Waiting,
                 "RunMessagePump {0}: Activation={1}", activation.ActivationId,
