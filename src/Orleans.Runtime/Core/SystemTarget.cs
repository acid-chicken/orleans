...
namespace Orleans.Runtime {
  /// <summary>
  /// Base class for various system services, such as grain directory, reminder
  /// service, etc. Made public for GrainSerive to inherit from it. Can be
  /// turned to internal after a refactoring that would remove the inheritance
  /// relation.
  /// </summary>
  public abstract class SystemTarget : ISystemTarget,
                                       ISystemTargetBase,
                                       IInvokable,
                                       IGrainContext {
    ...
    internal ActivationId ActivationId {
      get;
      set;
    }
    ...
    IGrainReferenceRuntime ISystemTargetBase.GrainReferenceRuntime =>
        this.RuntimeClient.GrainReferenceRuntime;

    GrainReference IGrainContext.GrainReference => selfReference ?? =
        GrainReference.FromGrainId(this.id.GrainId,
                                   this.RuntimeClient.GrainReferenceRuntime);
    ...
    internal SystemTarget(GrainType grainType, SiloAddress silo,
                          ILoggerFactory loggerFactory)
        : this(SystemTargetGrainId.Create(grainType, silo), silo, false,
               loggerFactory) {}

    internal SystemTarget(GrainType grainType, SiloAddress silo,
                          bool lowPriority, ILoggerFactory loggerFactory)
        : this(SystemTargetGrainId.Create(grainType, silo), silo, lowPriority,
               loggerFactory) {}

    internal SystemTarget(SystemTargetGrainId grainId, SiloAddress silo,
                          bool lowPriority, ILoggerFactory loggerFactory) {
      this.id = grainId;
      this.Silo = silo;
      this.ActivationAddress = ActivationAddress.GetAddress(
          this.Silo, this.id.GrainId, this.ActivationId);
    ...
    IGrainMethodInvoker IInvokable.GetInvoker(GrainTypeManager typeManager,
                                              int interfaceId,
    ...
    /// <returns></returns>
    public IDisposable RegisterTimer(Func<object, Task>asyncCallback,
                                     object state, TimeSpan dueTime,
                                     TimeSpan period, string name = null) {
      ...
      var timer = GrainTimer.FromTaskCallback(this.RuntimeClient.Scheduler,
                                              this.timerLogger, asyncCallback,
                                              state, dueTime, period, name);
      timer.Start();
      return timer;
    }
    ...
    public override string ToString() {
      return String.Format("[{0}SystemTarget: {1}{2}{3}]",
                           ...
                           : string.Empty, Silo, this.id, this.ActivationId);
    }

    /// <summary>Adds details about message currently being processed</summary>
    internal string ToDetailedString() {
      return String.Format("{0} CurrentlyExecuting={1}", ToString(),
