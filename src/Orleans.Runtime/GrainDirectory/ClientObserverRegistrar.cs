...
namespace Orleans.Runtime {
  internal class ClientObserverRegistrar
      : SystemTarget,
        IClientObserverRegistrar,
        ILifecycleParticipant<ISiloLifecycle> {
    private static readonly TimeSpan EXP_BACKOFF_ERROR_MIN =
        TimeSpan.FromSeconds(1);
    private static readonly TimeSpan EXP_BACKOFF_ERROR_MAX =
    ...
    public ClientObserverRegistrar(
        ILocalSiloDetails siloDetails, ILocalGrainDirectory grainDirectory,
        OrleansTaskScheduler scheduler,
        IOptions<SiloMessagingOptions>messagingOptions,
        ILoggerFactory loggerFactory,
        IClusterMembershipService clusterMembershipService,
        IAsyncTimerFactory timerFactory)
        : base(Constants.ClientObserverRegistrarType, siloDetails.SiloAddress,
               loggerFactory) {
      ...
      this.refreshTimer =
          timerFactory.Create(this.messagingOptions.ClientRegistrationRefresh,
    ...
    private async Task RunClientRefreshLoop() {
      var membershipUpdates =
          this.clusterMembershipService.MembershipUpdates.GetAsyncEnumerator(
              this.shutdownCancellation.Token);

      Task<bool>membershipTask = null;
      Task<bool>timerTask =
          this.refreshTimer.NextTick(new SafeRandom().NextTimeSpan(
    ...
    internal void ClientAdded(ClientGrainId clientId) {
      // Use a ActivationId that is hashed from clientId, and not random
      // ActivationId. That way, when we refresh it in the directiry, it's the
      // same one.
      var addr = GetClientActivationAddress(clientId);
      scheduler
          .QueueTask(
              () => ExecuteWithRetries(
                  () => grainDirectory.RegisterAsync(addr, singleActivation
                                                     : false),
                  ErrorCode.ClientRegistrarFailedToRegister,
                  String.Format("Directory.RegisterAsync {0} failed.", addr)),
              this)
          .Ignore();
    }
    ...
    internal void ClientDropped(ClientGrainId clientId) {
      ...
      scheduler
          .QueueTask(
              () => ExecuteWithRetries(
                  () => grainDirectory.UnregisterAsync(
                      addr, Orleans.GrainDirectory.UnregistrationCause.Force),
                  ErrorCode.ClientRegistrarFailedToUnregister,
                  String.Format("Directory.UnRegisterAsync {0} failed.", addr)),
              this)
          .Ignore();
    }

    private async Task ExecuteWithRetries(Func<Task>functionToExecute,
                                          ErrorCode errorCode,
                                          string errorStr) {
      try {
        // Try to register/unregister the client in the directory.
        // If failed, keep retrying with exponentially increasing time intervals
        // in between, until: either succeeds or max time of
        // orleansConfig.Globals.ClientRegistrationRefresh has reached. If
        // failed to register after that time, it will be retried further on by
        // clientRefreshTimer. In the unregsiter case just drop it. At the
        // worst, where will be garbage in the directory.
        await AsyncExecutorWithRetries.ExecuteWithRetries(
            _ => { return functionToExecute(); },
            AsyncExecutorWithRetries
                .INFINITE_RETRIES, // Do not limit the number of on-error
                                   // retries, control it via "maxExecutionTime"
            (exc, i) => true,      // Retry on all errors.
            this.messagingOptions
                .ClientRegistrationRefresh, // "maxExecutionTime"
            new ExponentialBackoff(
                EXP_BACKOFF_ERROR_MIN, EXP_BACKOFF_ERROR_MAX,
                EXP_BACKOFF_STEP)); // how long to wait between error retries
      } catch (Exception exc) {
    ...
    private async Task OnRefreshClients() {
      try {
        ...
        foreach(ClientGrainId clientId in clients) {
          ...
          Task task =
              grainDirectory.RegisterAsync(addr, singleActivation
                                           : false)
                  .LogException(
                      logger, ErrorCode.ClientRegistrarFailedToRegister_2,
                      String.Format("Directory.RegisterAsync {0} failed.",
                                    addr));
          tasks.Add(task);
        }
        ...
        logger.Error(
            ErrorCode.ClientRegistrarTimerFailed,
            String.Format("OnClientRefreshTimer has thrown an exceptions."),
            exc);
      }
    }

    private ActivationAddress
    GetClientActivationAddress(ClientGrainId clientId) {
      // Need to pick a unique deterministic ActivationId for this client.
      // We store it in the grain directory and there for every GrainId we use
      // ActivationId as a key so every GW needs to behave as a different
      // "activation" with a different ActivationId (its not enough that they
      // have different SiloAddress)
      string stringToHash =
          clientId.ToString() + myAddress.Endpoint +
          myAddress.Generation.ToString(
      ...
      return ActivationAddress.GetAddress(myAddress, clientId.GrainId,
                                          activationId);
    }

    public void Participate(ISiloLifecycle lifecycle) {
      lifecycle.Subscribe(nameof(ClientObserverRegistrar),
                          ServiceLifecycleStage.RuntimeServices,
                          _ => Task.CompletedTask, async ct => {
