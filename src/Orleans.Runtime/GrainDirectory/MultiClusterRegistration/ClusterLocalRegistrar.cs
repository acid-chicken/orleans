...
namespace Orleans.Runtime.GrainDirectory {
  ...
  internal class ClusterLocalRegistrar {
    ...
    public AddressAndTag Register(ActivationAddress address,
                                  bool singleActivation) {
      if (singleActivation) {
        var result = directoryPartition.AddSingleActivation(
            address.Grain, address.Activation, address.Silo,
            GrainDirectoryEntryStatus.ClusterLocal);
        return result;
      } else {
        var tag = directoryPartition.AddActivation(
            address.Grain, address.Activation, address.Silo);
        return new AddressAndTag(){Address = address, VersionTag = tag};
      }
    }

    public void Unregister(ActivationAddress address,
                           UnregistrationCause cause) {
      directoryPartition.RemoveActivation(address.Grain, address.Activation,
                                          cause);
    }

    public void Delete(GrainId gid) { directoryPartition.RemoveGrain(gid); }
  }
}
