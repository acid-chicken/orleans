...
namespace Orleans.Runtime.GrainDirectory {
  ...
  internal class ActivationInfo
      : IActivationInfo{public SiloAddress SiloAddress{get;
  private set;
}
...
public GrainDirectoryEntryStatus RegistrationStatus {
  get;
  set;
}
...
internal class GrainInfo : IGrainInfo {
  public Dictionary<ActivationId, IActivationInfo>Instances {
    get;
    private set;
  }
  ...
  static GrainInfo() { rand = new SafeRandom(); }
  ...
  public bool AddActivation(ActivationId act, SiloAddress silo) {
    ...
    Instances[act] =
        new ActivationInfo(silo, GrainDirectoryEntryStatus.ClusterLocal);
    VersionTag = rand.Next();
    return true;
  }

  public ActivationAddress
  AddSingleActivation(GrainId grain, ActivationId act, SiloAddress silo,
                      GrainDirectoryEntryStatus registrationStatus) {
    SingleInstance = true;
    if (Instances.Count > 0) {
      var item = Instances.First();
      return ActivationAddress.GetAddress(item.Value.SiloAddress, grain,
                                          item.Key);
    } else {
  ...
  public bool RemoveActivation(ActivationId act, UnregistrationCause cause,
                               TimeSpan lazyDeregistrationDelay,
                               out IActivationInfo info, out bool wasRemoved) {
    info = null;
    wasRemoved = false;
    if (Instances.TryGetValue(act, out info) &&
        info.OkToRemove(cause, lazyDeregistrationDelay)) {
  ...
  public Dictionary<SiloAddress, List<ActivationAddress>>
  Merge(GrainId grain, IGrainInfo other) {
    bool modified = false;
    ...
    if (SingleInstance && (Instances.Count > 0)) {
      // Grain is supposed to be in single activation mode, but we have two
      // activations!! Eventually we should somehow delegate handling this to
      // the silo, but for now, we'll arbitrarily pick one value.
      ...
      foreach(var activationPair in activationsToDrop) {
        var activation = ActivationAddress.GetAddress(
            activationPair.Value.SiloAddress, grain, activationPair.Key);

        List<ActivationAddress>activationsToRemoveOnSilo;
        if (!mapping.TryGetValue(activation.Silo,
                                 out activationsToRemoveOnSilo)) {
          activationsToRemoveOnSilo = mapping[activation.Silo] =
  ...
  public void CacheOrUpdateRemoteClusterRegistration(GrainId grain,
    ...
    Instances.Add(activation,
                  new ActivationInfo(silo, GrainDirectoryEntryStatus.Cached));
  }

  public bool UpdateClusterRegistrationStatus(
      ActivationId activationId, GrainDirectoryEntryStatus status,
      GrainDirectoryEntryStatus? compareWith = null) {
    IActivationInfo activationInfo;
    if (!Instances.TryGetValue(activationId, out activationInfo))
      return false;
    if (compareWith.HasValue &&
        compareWith.Value != activationInfo.RegistrationStatus)
      return false;
...
internal class GrainDirectoryPartition {
  // Should we change this to SortedList<> or SortedDictionary so we can extract
  // chunks better for shipping the full parition to a follower, or should we
  // leave it as a Dictionary to get O(1) lookups instead of O(log n), figuring
  // we do a lot more lookups and so can sort periodically?
  /// <summary>
  /// contains a map from grain to its list of activations along with the
  /// version (etag) counter for the list
  /// </summary>
  ...
  internal int Count {
    get { return partitionData.Count; }
  }

  public GrainDirectoryPartition(
      ISiloStatusOracle siloStatusOracle,
      IOptions<GrainDirectoryOptions>grainDirectoryOptions,
      IInternalGrainFactory grainFactory, ILoggerFactory loggerFactory) {
  ...
  internal void Clear() {
    lock(lockable) { partitionData.Clear(); }
  }
  ...
  public List<KeyValuePair<GrainId, IGrainInfo>>GetItems() {
    lock(lockable) { return partitionData.ToList(); }
  }
  ...
  internal virtual int AddActivation(GrainId grain, ActivationId activation,
                                     SiloAddress silo) {
    if (!IsValidSilo(silo)) {
  ...
  /// <returns>The registered ActivationAddress and version associated with this
  /// directory mapping</returns>
  internal virtual AddressAndTag
  AddSingleActivation(GrainId grain, ActivationId activation, SiloAddress silo,
                      GrainDirectoryEntryStatus registrationStatus) {
    if (log.IsEnabled(LogLevel.Trace))
      log.Trace("Adding single activation for grain {0}{1}{2}", silo, grain,
                activation);
  ...
  internal void
  RemoveActivation(GrainId grain, ActivationId activation,
                   UnregistrationCause cause = UnregistrationCause.Force) {
  ...
  /// <param name="wasRemoved">returns whether the entry was actually
  /// removed</param>
  internal void RemoveActivation(GrainId grain, ActivationId activation,
                                 UnregistrationCause cause,
                                 out IActivationInfo entry,
                                 out bool wasRemoved) {
    wasRemoved = false;
    entry = null;
    lock(lockable) {
      if (partitionData.ContainsKey(grain) &&
          partitionData[grain].RemoveActivation(
              activation, cause,
              this.grainDirectoryOptions.Value.LazyDeregistrationDelay,
              out entry, out wasRemoved))
        // if the last activation for the grain was removed, we remove the
        // entire grain info
        partitionData.Remove(grain);
    }
    if (log.IsEnabled(LogLevel.Trace))
      log.Trace("Removing activation for grain {0} cause={1} was_removed={2}",
                grain.ToString(), cause, wasRemoved);
  }

  /// <summary>
  /// Removes the grain (and, effectively, all its activations) from the
  /// directory
  /// </summary>
  /// <param name="grain"></param>
  internal void RemoveGrain(GrainId grain) {
    lock(lockable) { partitionData.Remove(grain); }
    if (log.IsEnabled(LogLevel.Trace))
      log.Trace("Removing grain {0}", grain.ToString());
  }

  /// <summary>
  /// Returns a list of activations (along with the version number of the list)
  /// for the given grain. If the grain is not found, null is returned.
  /// </summary>
  /// <param name="grain"></param>
  /// <returns></returns>
  internal AddressesAndTag LookUpActivations(GrainId grain) {
    ...
    lock(lockable) {
      ...
      if (grainInfoInstancesCount < arrayReusingThreshold) {
        if ((activationIds = activationIdsHolder) == null) {
          activationIdsHolder = activationIds =
              new ActivationId[arrayReusingThreshold];
        }

        if ((activationInfos = activationInfosHolder) == null) {
          activationInfosHolder = activationInfos =
              new IActivationInfo[arrayReusingThreshold];
        }
      } else {
    ...
    for (var i = 0; i < grainInfoInstancesCount; i++) {
      ...
      if (IsValidSilo(activationInfo.SiloAddress)) {
        result.Addresses.Add(ActivationAddress.GetAddress(
            activationInfo.SiloAddress, grain, activationIds[i]));
      }
  ...
  /// </summary>
  internal GrainDirectoryEntryStatus TryGetActivation(
      GrainId grain, out ActivationAddress address, out int version) {
    IGrainInfo grainInfo;
    address = null;
    version = 0;
    lock(lockable) {
      ...
      if (first.Value != null) {
        address = ActivationAddress.GetAddress(first.Value.SiloAddress, grain,
                                               first.Key);
  ...
  /// This method is supposed to be used by handoff manager to update the
  /// partitions when the system view (set of live silos) changes.
  /// </summary>
  /// <param name="other"></param>
  /// <returns>Activations which must be deactivated.</returns>
  internal Dictionary<SiloAddress, List<ActivationAddress>>
  Merge(GrainDirectoryPartition other) {
      ...
      foreach(var pair in other.partitionData) {
        if (partitionData.ContainsKey(pair.Key)) {
          if (log.IsEnabled(LogLevel.Debug))
            log.Debug("While merging two disjoint partitions, same grain " +
                      pair.Key + " was found in both partitions");
          var activationsToDrop =
              partitionData[pair.Key].Merge(pair.Key, pair.Value);
          if (activationsToDrop == null)
            continue;

          if (activationsToRemove == null)
            activationsToRemove =
                new Dictionary<SiloAddress, List<ActivationAddress>>();
          foreach(var siloActivations in activationsToDrop) {
            if (activationsToRemove.TryGetValue(siloActivations.Key,
                                                out var activations)) {
  ...
  /// <summary>
  /// Runs through all entries in the partition and moves/copies (depending on
  /// the given flag) the entries satisfying the given predicate into a new
  /// partition. This method is supposed to be used by handoff manager to update
  /// the partitions when the system view (set of live silos) changes.
  /// </summary>
  /// <param name="predicate">filter predicate (usually if the given grain is
  /// owned by particular silo)</param> <param name="modifyOrigin">flag
  /// controlling whether the source partition should be modified (i.e., the
  /// entries should be moved or just copied) </param> <returns>new grain
  /// directory partition containing entries satisfying the given
  /// predicate</returns>
  internal GrainDirectoryPartition Split(Predicate<GrainId>predicate,
                                         bool modifyOrigin) {
    var newDirectory = new GrainDirectoryPartition(
        this.siloStatusOracle, this.grainDirectoryOptions, this.grainFactory,
        this.loggerFactory);

    if (modifyOrigin) {
      // SInce we use the "pairs" list to modify the underlying collection
      // below, we need to turn it into an actual list here
      ...
      lock(lockable) {
        foreach(var pair in pairs) { partitionData.Remove(pair.Key); }
      }
    } else {
      lock(lockable) {
        ...
        if (pair.Value.SingleInstance == singleActivation) {
          result.AddRange(
              pair.Value.Instances
                  .Select(activationPair => ActivationAddress.GetAddress(
                              activationPair.Value.SiloAddress, grain,
                              activationPair.Key))
  ...
  /// <summary>
  /// Sets the internal partition dictionary to the one given as input
  /// parameter. This method is supposed to be used by handoff manager to update
  /// the old partition with a new partition.
  /// </summary>
  ...
  /// This method is supposed to be used by handoff manager to update the
  /// partition with a set of delta changes.
  /// </summary>
  /// <param name="newPartitionDelta">dictionary holding a set of delta updates
  /// to this partition. If the value for a given key in the delta is valid,
  /// then existing entry in the partition is replaced. Otherwise, i.e., if the
  /// value is null, the corresponding entry is removed.
  /// </param>
    ...
    lock(lockable) {
      foreach(var grainEntry in partitionData) {
        foreach(var activationEntry in grainEntry.Value.Instances) {
          sb.Append("    ")
              .Append(grainEntry.Key.ToString())
              .Append("[" + grainEntry.Value.VersionTag + "]")
              .Append(" => ")
              .Append(activationEntry.Key.ToString())
              .Append(" @ ")
              .AppendLine(activationEntry.Value.ToString());
        }
      }
    }
  ...
  public void CacheOrUpdateRemoteClusterRegistration(
      GrainId grain, ActivationId oldActivation,
      ActivationAddress otherClusterAddress) {
    lock(lockable) {
      if (partitionData.TryGetValue(grain, out var graininfo)) {
        graininfo.CacheOrUpdateRemoteClusterRegistration(
            grain, oldActivation, otherClusterAddress.Activation,
            otherClusterAddress.Silo);

      } else {
        AddSingleActivation(grain, otherClusterAddress.Activation,
                            otherClusterAddress.Silo,
  ...
  public bool UpdateClusterRegistrationStatus(
    ...
    lock(lockable) {
      if (partitionData.TryGetValue(grain, out var graininfo)) {
        return graininfo.UpdateClusterRegistrationStatus(
