...
namespace Orleans.Runtime.GrainDirectory {
  /// <summary>
  /// Implementation of <see cref="IGrainLocator"/> that uses an <see
  /// cref="IGrainDirectory"/> store.
  /// </summary>
  internal class GrainLocator : IGrainLocator,
                                ILifecycleParticipant<ISiloLifecycle>,
                                GrainLocator.ITestAccessor {
    ...
    MembershipVersion ITestAccessor.LastMembershipVersion {
      get;
      set;
    }

    public GrainLocator(IGrainDirectory grainDirectory,
      ...
      this.cache = new LRUBasedGrainDirectoryCache(
          GrainDirectoryOptions.DEFAULT_CACHE_SIZE,
    ...
    public async Task<List<ActivationAddress>>Lookup(GrainId grainId) {
      ...
      if (this.knownDeadSilos.Contains(activationAddress.Silo)) {
        ...
        this.cache.AddOrUpdate(
            grainId,
            new List<Tuple<SiloAddress, ActivationId>>{Tuple.Create(
                activationAddress.Silo, activationAddress.Activation)},
            0);
      }
    ...
    public bool TryLocalLookup(GrainId grainId,
      ...
      if (grainId.IsClient)
        return this.inClusterGrainLocator.TryLocalLookup(grainId,
                                                         out addresses);
      ...
      if (this.cache.LookUp(grainId, out var results)) {
        ...
        if (this.knownDeadSilos.Contains(result.Item1)) {
          ...
          addresses =
              new List<ActivationAddress>(){ActivationAddress.GetAddress(
                  result.Item1, grainId, result.Item2)};
          return true;
        }
      }
    ...
    public async Task UnregisterMany(List<ActivationAddress>addresses,
      ...
      try {
        var grainAddresses =
            addresses.Select(addr => addr.ToGrainAddress()).ToList();
        await this.grainDirectory.UnregisterMany(grainAddresses);
      } finally {
        foreach(var address in addresses) this.cache.Remove(address.Grain);
      }
    }
    ...
    public void Participate(ISiloLifecycle lifecycle) {
      ...
      lifecycle.Subscribe(nameof(GrainLocator),
    ...
    private async Task ListenToClusterChange() {
      ...
      // Update the list of known dead silos for lazy filtering for the first
      // time
      this.knownDeadSilos =
          new HashSet<SiloAddress>(previousSnapshot.Members.Values
      ...
      var updates =
          this.clusterMembershipService.MembershipUpdates.WithCancellation(
              this.shutdownToken.Token);
      await foreach(var snapshot in updates) {
        // Update the list of known dead silos for lazy filtering
        this.knownDeadSilos = new HashSet<SiloAddress>(
            snapshot.Members.Values.Where(m => m.Status == SiloStatus.Dead)
                .Select(m => m.SiloAddress));

        // Active filtering: detect silos that went down and try to clean
        // proactively the directory
        var changes = snapshot.CreateUpdate(previousSnapshot).Changes;
        var deadSilos =
            changes.Where(member => member.Status == SiloStatus.Dead)
                .Select(member => member.SiloAddress.ToParsableString())
                .ToList();

        if (deadSilos.Count > 0) {
          await this.grainDirectory.UnregisterSilos(deadSilos).WithCancellation(
              this.shutdownToken.Token);
        }
  ...
  internal static class AddressHelpers {
    public static ActivationAddress
    ToActivationAddress(this GrainAddress addr) {
      return ActivationAddress.GetAddress(
          ...
          ActivationId.GetActivationId(
              UniqueKey.Parse(addr.ActivationId.AsSpan())));
    }

    public static GrainAddress ToGrainAddress(this ActivationAddress addr) {
      return new GrainAddress{SiloAddress = addr.Silo.ToParsableString(),
                              GrainId = addr.Grain.ToParsableString(),
                              ActivationId =
                                  (addr.Activation.Key.ToHexString())};
    }
  }
}
