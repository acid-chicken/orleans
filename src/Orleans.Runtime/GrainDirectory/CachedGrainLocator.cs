...
namespace Orleans.Runtime.GrainDirectory {
  /// <summary>
  /// Implementation of <see cref="IGrainLocator"/> that uses <see
  /// cref="IGrainDirectory"/> stores.
  /// </summary>
  internal class CachedGrainLocator : IGrainLocator,
                                      ILifecycleParticipant<ISiloLifecycle>,
                                      CachedGrainLocator.ITestAccessor {
    ...
    public async Task<List<ActivationAddress>>Lookup(GrainId grainId) {
      ...
      var entry = await GetGrainDirectory(grainId).Lookup(
    ...
    public async Task Unregister(ActivationAddress address,
      ...
      try {
        await GetGrainDirectory(address.Grain)
            .Unregister(address.ToGrainAddress());
      } finally {
    ...
    public void Participate(ISiloLifecycle lifecycle) {
      ...
      lifecycle.Subscribe(nameof(CachedGrainLocator),
                          ServiceLifecycleStage.RuntimeGrainServices, OnStart,
                          OnStop);
    }

    private IGrainDirectory GetGrainDirectory(GrainId grainId) =>
        this.grainDirectoryResolver.Resolve(grainId);
    ...
    private async Task ListenToClusterChange() {
      ...
      await foreach(var snapshot in updates) {
        ...
        this.knownDeadSilos = new HashSet<SiloAddress>(
            snapshot.Members.Values.Where(m => m.Status.IsTerminating())
                .Select(m => m.SiloAddress));
        ...
        var deadSilos =
            changes.Where(member => member.Status.IsTerminating())
