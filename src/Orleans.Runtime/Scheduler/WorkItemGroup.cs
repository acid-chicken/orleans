...
namespace Orleans.Runtime.Scheduler {
  ...
  internal class WorkItemGroup : IWorkItem {
    private static readonly WaitCallback ExecuteWorkItemCallback = obj =>
        ((WorkItemGroup) obj).Execute();
    private enum WorkGroupStatus { Waiting = 0, Runnable = 1, Running = 2 }
    ...
    public bool IsSystemPriority =>
        this.GrainContext is SystemTarget systemTarget &&
        !systemTarget.IsLowPriority;
    ...
    internal int ExternalWorkItemCount {
      get {
        lock(lockable) { return WorkItemCount; }
      }
    }
    ...
    // This is the maximum number of work items to be processed in an activation
    // turn. If this is set to zero or a negative number, then the full work
    // queue is drained (MaxTimePerTurn allowing).
    private const int MaxWorkItemsPerTurn = 0; // Unlimited

    // This is the maximum number of waiting threads (blocked in
    // WaitForResponse) allowed per ActivationWorker. An attempt to wait when
    // there are already too many threads waiting will result in a
    // TooManyWaitersException being thrown.
    // private static readonly int MaxWaitingThreads = 500;

    internal
    WorkItemGroup(OrleansTaskScheduler sched, IGrainContext grainContext,
      ...
      TaskScheduler =
          new ActivationTaskScheduler(this, activationTaskSchedulerLogger);
      log = logger;

      if (schedulerStatistics.CollectShedulerQueuesStats) {
        queueTracking = new QueueTrackingStatistic("Scheduler." + this.Name,
                                                   statisticsOptions);
      ...
      if (schedulerStatistics.CollectPerWorkItemStats) {
        workItemGroupStatisticsNumber =
            schedulerStatistics.RegisterWorkItemGroup(
                this.Name, this.GrainContext, () => {
                  ...
                  lock(lockable) {
                    ...
                    if (state == WorkGroupStatus.Runnable)
                      sb.Append(String.Format("; oldest item is {0} old",
                                              workItems.Count >= 0
                                                  ? workItems.Peek().ToString()
                                                  : "null"));
                  }
                  return sb.ToString();
                });
    ...
    /// If we're adding it to the run list and we used to be waiting, now we're
    /// runnable.
    /// </summary>
      ...
      lock(lockable) {
        ...
        if (maxPendingItemsLimit > 0 && count > maxPendingItemsLimit) {
          log.LogWarning(
              ...
              count, this.Name, maxPendingItemsLimit);
        }
        ...
        if (log.IsEnabled(LogLevel.Trace)) {
          log.LogTrace(
              ...
              task, thisSequenceNumber, GrainContext);
        }
#endif
    ...
    internal IEnumerable<Task>GetScheduledTasks(){
        foreach(var task in this.workItems){yield return task;
  }
}

[MethodImpl(MethodImplOptions.NoInlining)]
private void
LogEnqueueOnStoppedScheduler(Task task) {
  var now = ValueStopwatch.GetTimestamp();
  LogLevel logLevel;
  if (this.lastShutdownWarningTimestamp == 0) {
    logLevel = LogLevel.Debug;
  } else if (ValueStopwatch
                 .FromTimestamp(this.lastShutdownWarningTimestamp, now)
                 .Elapsed >
             this.masterScheduler.StoppedWorkItemGroupWarningInterval) {
    // Upgrade the warning to an error after 1 minute, include a stack trace,
    // and continue to log up to once per minute.
    logLevel = LogLevel.Error;
  } else
    return;

  this.log.Log(
      logLevel, (int) ErrorCode.SchedulerEnqueueWorkWhenShutdown,
      ...
      OrleansTaskExtentions.ToString(task), this.DumpStatus(),
      DumpAsyncState(task.AsyncState), Utils.GetStackTrace());
...
private static object DumpAsyncState(object o) {
  if (o is Delegate action)
    return action.Target is null ? action.Method.DeclaringType + "." +
           action.Method.Name : action.Method.DeclaringType.Name + "." +
           action.Method.Name + ": " + DumpAsyncState(action.Target);
...
/// <summary>
/// Shuts down this work item group so that it will not process any additional
/// work items, even if they have already been queued.
/// </summary>
internal void Stop() {
  lock(lockable) {
    if (this.HasWork) {
      log.LogWarning(
          ...
          WorkItemCount, DumpStatus());
    }

    if (this.IsShutdown) {
      log.LogWarning((int) ErrorCode.SchedulerWorkGroupShuttingDown,
    ...
    if (this.schedulerStatistics.CollectPerWorkItemStats)
      this.schedulerStatistics.UnRegisterWorkItemGroup(
          workItemGroupStatisticsNumber);
...
// This method is always called in a single-threaded environment -- that is, no
// more than one thread will be in this method at once -- but other asynch
// threads may still be queueing tasks, etc.
public void Execute() {
  try {
    RuntimeContext.SetExecutionContext(this.GrainContext);

    // Process multiple items -- drain the applicationMessageQueue (up to max
    // items) for this physical activation
    int count = 0;
    ...
    do {
      lock(lockable) {
        ...
        if (cancellationToken.IsCancellationRequested) {
          this.log.LogWarning(
              ...
              Thread.CurrentThread.ManagedThreadId.ToString(), this.ToString(),
              this.WorkItemCount);
      ...
      if (log.IsEnabled(LogLevel.Trace)) {
        log.LogTrace(
            ...
            OrleansTaskExtentions.ToString(task), this.GrainContext);
      }
#endif
      ...
      try {
        ...
        if (taskLength > OrleansTaskScheduler.TurnWarningLengthThreshold) {
          ...
          this.log.LogWarning(
              ...
              this.GrainContext.ToString(), taskLength.ToString("g"),
      ...
      count++;
             ...
             ((masterScheduler.SchedulingOptions.ActivationSchedulingQuantum <=
               TimeSpan.Zero) ||
              (stopwatch.Elapsed <
               masterScheduler.SchedulingOptions.ActivationSchedulingQuantum)));
  } catch (Exception ex) {
    this.log.LogError(
        (int) ErrorCode.Runtime_Error_100032, ex,
        "Worker thread {Thread} caught an exception thrown from IWorkItem.Execute: {Exception}",
        Thread.CurrentThread.ManagedThreadId, ex);
  } finally {
    // Now we're not Running anymore.
    // If we left work items on our run list, we're Runnable, and need to go
    // back on the silo run queue; If our run list is empty, then we're waiting.
    lock(lockable) {
...
public string DumpStatus(){lock(lockable){var sb = new StringBuilder();
sb.Append(this);
sb.AppendFormat(
    ". Currently QueuedWorkItems={0}; Total EnQueued={1}; Total processed={2}; Quantum expirations={3}; ",
    WorkItemCount, totalItemsEnQueued, totalItemsProcessed, quantumExpirations);
if(CurrentTask != null) {
  sb.AppendFormat(" Executing Task Id={0} Status={1} for {2}.", CurrentTask.Id,
                  CurrentTask.Status, Utils.Since(currentTaskStarted));
}
...
if(GrainContext != null) {
  var detailedStatus = this.GrainContext switch {
      ActivationData activationData =>
          activationData.ToDetailedString(includeExtraDetails
                                          : true),
      SystemTarget systemTarget => systemTarget.ToDetailedString(),
      object obj => obj.ToString(), _ => "None"};
...
public static void
ScheduleExecution(WorkItemGroup workItem) {
#if NETCOREAPP
