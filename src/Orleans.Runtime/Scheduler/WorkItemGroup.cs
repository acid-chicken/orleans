...
namespace Orleans.Runtime.Scheduler {
  ...
  internal class WorkItemGroup : IWorkItem {
    ...
    public ISchedulingContext SchedulingContext {
      get;
      set;
    }
    ...
    public string Name {
      get {
        return SchedulingContext == null ? "unknown" : SchedulingContext.Name;
      }
    }
    ...
    internal WorkItemGroup(OrleansTaskScheduler sched,
      ...
      if (schedulerStatistics.CollectShedulerQueuesStats) {
        queueTracking = new QueueTrackingStatistic(
            "Scheduler." + SchedulingContext.Name, statisticsOptions);
      ...
      if (schedulerStatistics.CollectPerWorkItemStats) {
        workItemGroupStatisticsNumber =
            schedulerStatistics.RegisterWorkItemGroup(
                SchedulingContext.Name, SchedulingContext, () => {
      ...
      lock(lockable) {
        ...
        if (log.IsEnabled(LogLevel.Trace)) {
          log.LogTrace(
              ...
              task, thisSequenceNumber, SchedulingContext);
        }
#endif
    ...
    LogEnqueueOnStoppedScheduler(Task task) {
      ...
      if (this.lastShutdownWarningTimestamp == 0) {
        ...
        // Upgrade the warning to an error after 1 minute, include a stack
        // trace, and continue to log up to once per minute.
        this.log.LogError(
            (int) ErrorCode.SchedulerEnqueueWorkWhenShutdown,
            "Enqueuing task {Task} to a work item group which should have terminated. " +
                "Likely reasons are that the task is not being 'awaited' properly or a TaskScheduler was captured and is being used to schedule tasks " +
    ...
    /// <summary>
    /// Shuts down this work item group so that it will not process any
    /// additional work items, even if they have already been queued.
    /// </summary>
    internal void Stop() {
      lock(lockable) {
        if (this.HasWork) {
          ReportWorkGroupProblem(
              String.Format(
                  "WorkItemGroup is being shutdown while still active. workItemCount = {0}." +
                      "The likely reason is that the task is not being 'awaited' properly.",
                  WorkItemCount),
    ...
    // This method is always called in a single-threaded environment -- that is,
    // no more than one thread will be in this method at once -- but other
    // asynch threads may still be queueing tasks, etc.
    public void Execute() {
      try {
        RuntimeContext.SetExecutionContext(this.SchedulingContext);

        // Process multiple items -- drain the applicationMessageQueue (up to
        // max items) for this physical activation
        int count = 0;
        ...
        do {
          lock(lockable) {
            ...
            if (cancellationToken.IsCancellationRequested) {
              this.log.LogWarning(
                  ...
                  Thread.CurrentThread.ToString(), this.ToString(),
                  this.WorkItemCount);
          ...
          if (log.IsEnabled(LogLevel.Trace)) {
            log.LogTrace(
                ...
                OrleansTaskExtentions.ToString(task), this.SchedulingContext);
          }
#endif
          ...
          try {
            ...
            if (taskLength > OrleansTaskScheduler.TurnWarningLengthThreshold) {
              ...
              this.log.LogWarning(
                  ...
                  this.SchedulingContext.ToString(), taskLength.ToString("g"),
          ...
          count++;
            ...
            ((MaxWorkItemsPerTurn <= 0) || (count <= MaxWorkItemsPerTurn)) &&
        ...
        // back on the silo run queue; If our run list is empty, then we're
        // waiting.
        lock(lockable) {
      ...
      lock(lockable) {
        ...
        if (CurrentTask != null) {
          sb.AppendFormat(" Executing Task Id={0} Status={1} for {2}.",
                          ...
                          Utils.Since(currentTaskStarted));
        }

        if (AverageQueueLength > 0) {
          sb.AppendFormat("average queue length at enqueue: {0}; ",
                          AverageQueueLength);
          if (!totalQueuingDelay.Equals(TimeSpan.Zero) &&
              totalItemsProcessed > 0) {
            sb.AppendFormat("average queue delay: {0}ms; ",
                            totalQueuingDelay.Divide(totalItemsProcessed)
                                .TotalMilliseconds);
          }
        }
        ...
        if (SchedulingContext != null) {
          sb.AppendFormat("Detailed SchedulingContext=<{0}>",
                          SchedulingContext.DetailedStatus());
        }
        return sb.ToString();
      }
    }
