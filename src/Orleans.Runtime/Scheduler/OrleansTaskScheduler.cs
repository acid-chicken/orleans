...
namespace Orleans.Runtime.Scheduler {
  internal class OrleansTaskScheduler : TaskScheduler {
    ...
    private readonly ConcurrentDictionary<ISchedulingContext, WorkItemGroup>
        workgroupDirectory;
    private bool applicationTurnsStopped;

    private readonly CancellationTokenSource cancellationTokenSource;

    internal static TimeSpan TurnWarningLengthThreshold {
      get;
      set;
    }

    // This is the maximum number of pending work items for a single activation
    // before we write a warning log.
    internal int MaxPendingItemsSoftLimit {
      get;
      private set;
    }

    public OrleansTaskScheduler(IOptions<SchedulingOptions>options,
      ...
      this.MaxPendingItemsSoftLimit =
          options.Value.MaxPendingWorkItemsSoftLimit;
      this.StoppedWorkItemGroupWarningInterval =
          options.Value.StoppedActivationWarningInterval;
      workgroupDirectory =
          new ConcurrentDictionary<ISchedulingContext, WorkItemGroup>();

      this.taskWorkItemLogger = loggerFactory.CreateLogger<TaskWorkItem>();
      IntValueStatistic.FindOrCreate(
          StatisticNames.SCHEDULER_WORKITEMGROUP_COUNT,
          () => WorkItemGroupCount);
      ...
      FloatValueStatistic.FindOrCreate(
          new StatisticName(StatisticNames.QUEUES_QUEUE_SIZE_AVERAGE_PER_QUEUE,
                            "Scheduler.LevelTwo.Average"),
          () => AverageRunQueueLengthLevelTwo);
      FloatValueStatistic.FindOrCreate(
          new StatisticName(StatisticNames.QUEUES_ENQUEUED_PER_QUEUE,
                            "Scheduler.LevelTwo.Average"),
          () => AverageEnqueuedLevelTwo);
      FloatValueStatistic.FindOrCreate(
          new StatisticName(
              StatisticNames.QUEUES_AVERAGE_ARRIVAL_RATE_PER_QUEUE,
              "Scheduler.LevelTwo.Average"),
          () => AverageArrivalRateLevelTwo);
      FloatValueStatistic.FindOrCreate(
          new StatisticName(StatisticNames.QUEUES_QUEUE_SIZE_AVERAGE_PER_QUEUE,
                            "Scheduler.LevelTwo.Sum"),
          () => SumRunQueueLengthLevelTwo);
      FloatValueStatistic.FindOrCreate(
          new StatisticName(StatisticNames.QUEUES_ENQUEUED_PER_QUEUE,
                            "Scheduler.LevelTwo.Sum"),
          () => SumEnqueuedLevelTwo);
    ...
    private float AverageRunQueueLengthLevelTwo {
      get {
        ...
        return (float) workgroupDirectory.Values.Sum(
                   workgroup => workgroup.AverageQueueLength) /
               (float) workgroupDirectory.Values.Count;
      }
    }
    ...
    private float AverageEnqueuedLevelTwo {
      get {
        ...
        return (float) workgroupDirectory.Values.Sum(
                   workgroup => workgroup.NumEnqueuedRequests) /
               (float) workgroupDirectory.Values.Count;
      }
    }
    ...
    private float AverageArrivalRateLevelTwo {
      get {
        ...
        return (float) workgroupDirectory.Values.Sum(
                   workgroup => workgroup.ArrivalRate) /
               (float) workgroupDirectory.Values.Count;
      }
    }
    ...
    private float SumArrivalRateLevelTwo {
      get {
        return (float)
            workgroupDirectory.Values.Sum(workgroup => workgroup.ArrivalRate);
      }
    }
...
#endif
      // Do not RunDown the application run queue, since it is still used by low
      // priority system targets.
    ...
    protected override void QueueTask(Task task) {
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace(
            "QueueTask: Id={0} with Status={1} AsyncState={2} when TaskScheduler.Current={3}",
            task.Id, task.Status, task.AsyncState, Current);
#endif
      var context = contextObj as ISchedulingContext;
      var workItemGroup = GetWorkItemGroup(context);
      if (applicationTurnsStopped && (workItemGroup != null) &&
          !workItemGroup.IsSystemGroup) {
        // Drop the task on the floor if it's an application work item and
        // application turns are stopped
        logger.Warn(
            ErrorCode.SchedulerAppTurnsStopped_2,
            string.Format(
                "Dropping Task {0} because application turns are stopped",
                task));
        return;
      }
      ...
      if (workItemGroup == null) {
        var todo =
            new TaskWorkItem(this, task, context, this.taskWorkItemLogger);
        ScheduleExecution(todo);
      } else {
        var error = String.Format(
            "QueueTask was called on OrleansTaskScheduler for task {0} on Context {1}." +
    ...
    private static readonly WaitCallback ExecuteWorkItemCallback = obj =>
        ((IWorkItem) obj).Execute();
    ...
    public void QueueWorkItem(IWorkItem workItem, ISchedulingContext context) {
#if DEBUG
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("QueueWorkItem " + context);
#endif
      if (workItem is TaskWorkItem) {
        var error = String.Format(
            "QueueWorkItem was called on OrleansTaskScheduler for TaskWorkItem {0} on Context {1}." +
      ...
      if (applicationTurnsStopped && (workItemGroup != null) &&
        ...
        var msg = string.Format(
            "Dropping work item {0} because application turns are stopped",
            workItem);
      ...
      // We must wrap any work item in Task and enqueue it as a task to the
      // right scheduler via Task.Start.
      Task t = TaskSchedulerUtils.WrapWorkItemAsTask(workItem);

      // This will make sure the TaskScheduler.Current is set correctly on any
      // task that is created implicitly in the execution of this workItem.
      if (workItemGroup == null) {
        t.Start(this);
      } else {
    ...
    // Only required if you have work groups flagged by a context that is not a
    // WorkGroupingContext
    public WorkItemGroup RegisterWorkContext(ISchedulingContext context) {
      ...
      var wg = new WorkItemGroup(
          this, context, this.loggerFactory, this.cancellationTokenSource.Token,
          this.schedulerStatistics, this.statisticsOptions);
    ...
    GetWorkItemGroup(ISchedulingContext context) {
      switch (context) {
      case null:
        return null;
      case SchedulingContext schedulingContext when
          schedulingContext.WorkItemGroup is WorkItemGroup wg:
        return wg;
      default: {
        if (this.workgroupDirectory.TryGetValue(context, out var workGroup))
          return workGroup;
    ...
    private void
    ThrowNoWorkItemGroup(ISchedulingContext context) {
      var error = string.Format(
          "QueueWorkItem was called on a non-null context {0} but there is no valid WorkItemGroup for it.",
          context);
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    internal void
    CheckSchedulingContextValidity(ISchedulingContext context) {
      if (context is null) {
    ...
    protected override bool TryExecuteTaskInline(Task task,
      ...
      if (logger.IsEnabled(LogLevel.Trace)) {
        logger.Trace(
            "TryExecuteTaskInline Id={0} with Status={1} PreviouslyQueued={2} CanExecute={3}",
      ...
      if (!canExecuteInline)
        return false; // We can't execute tasks in-line on non-worker pool
                      // threads
      ...
      if (!done) {
        logger.Warn(
            ErrorCode.SchedulerTaskExecuteIncomplete1,
            "TryExecuteTaskInline: Incomplete base.TryExecuteTask for Task Id={0} with Status={1}",
            task.Id, task.Status);
      }
      return done;
    }
    ...
    public void RunTask(Task task) {
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace(
            "RunTask: Id={0} with Status={1} AsyncState={2} when TaskScheduler.Current={3}",
            task.Id, task.Status, task.AsyncState, Current);
#endif
      ...
      if (workItemGroup == null) {
        ...
        if (!done)
          logger.Warn(
              ErrorCode.SchedulerTaskExecuteIncomplete2,
              "RunTask: Incomplete base.TryExecuteTask for Task Id={0} with Status={1}",
              task.Id, task.Status);
      } else {
        var error = String.Format(
            "RunTask was called on OrleansTaskScheduler for task {0} on Context {1}. Should only call OrleansTaskScheduler.RunTask on tasks queued on a null context.",
            task.Id, context);
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace(
            "RunTask: Completed Id={0} with Status={1} task.AsyncState={2} when TaskScheduler.Current={3}",
            task.Id, task.Status, task.AsyncState, Current);
#endif
    }
    ...
    internal void PrintStatistics() {
      ...
      var stats = Utils.EnumerableToString(
          workgroupDirectory.Values.OrderBy(wg => wg.Name),
          wg => string.Format("--{0}", wg.DumpStatus()), Environment.NewLine);
      if (stats.Length > 0)
        logger.Info(
            ErrorCode.SchedulerStatistics,
            "OrleansTaskScheduler.PrintStatistics(): WorkItems={0}, Directory:" +
                Environment.NewLine + "{1}",
            WorkItemGroupCount, stats);
    }

    internal void DumpSchedulerStatus(bool alwaysOutput = true) {
      if (!logger.IsEnabled(LogLevel.Debug) && !alwaysOutput)
        return;
      ...
      sb.AppendFormat("CPUs={0} WorkItems={1} {2}", Environment.ProcessorCount,
                      workgroupDirectory.Count,
                      applicationTurnsStopped ? "STOPPING"
                      : "")
          .AppendLine();

      // todo: either remove or support. At the time of writting is being used
      // only in tests sb.AppendLine("RunQueue:"); RunQueue.DumpStatus(sb); -
      // woun't work without additional costs Pool.DumpStatus(sb);
