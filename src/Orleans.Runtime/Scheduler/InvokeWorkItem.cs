...
namespace Orleans.Runtime.Scheduler {
  internal class InvokeWorkItem : WorkItemBase {
    ...
    public InvokeWorkItem(ActivationData activation, Message message,
                          Dispatcher dispatcher, ILogger logger) {
      this.logger = logger;
      if (activation?.GrainInstance == null) {
        var str = string.Format(
            "Creating InvokeWorkItem with bad activation: {0}. Message: {1}",
            activation, message);
    ...
    public override void Execute() {
      try {
        ...
        // Note: This runs for all outcomes of resultPromiseTask - both Success
        // or Fault
        if (task.IsCompleted) {
          OnComplete();
        } else {
        ...
        logger.Warn(
            ErrorCode.InvokeWorkItem_UnhandledExceptionInInvoke,
            String.Format(
                "Exception trying to invoke request {0} on activation {1}.",
                message, activation),
            exc);
        OnComplete();
      } finally {
    ...
    public override string ToString() {
      return String.Format("{0} for activation={1} Message={2}",
                           base.ToString(), activation, message);
    }
  }
}
