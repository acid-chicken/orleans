...
namespace Orleans.Runtime {
  internal class GrainTypeManager {
    ...
    private Dictionary<int, Dictionary<ushort, List<SiloAddress>>>
        supportedSilosByInterface;

    internal IReadOnlyDictionary<SiloAddress, GrainInterfaceMap>
        GrainInterfaceMapsBySilo => this.grainInterfaceMapsBySilo;

    public IEnumerable<KeyValuePair<string, GrainTypeData>>GrainClassTypeData =>
        this.grainTypes;

    public GrainInterfaceMap ClusterGrainInterfaceMap {
      get;
      private set;
    }
    ...
    public GrainTypeManager(ILocalSiloDetails siloDetails,
      ...
      var localTestMode =
          siloDetails.SiloAddress.Endpoint.Address.Equals(IPAddress.Loopback);
      this.logger = logger;
      this.defaultPlacementStrategy = defaultPlacementStrategy;
      this.serializationManager = serializationManager;
      grainInterfaceMap =
          new GrainInterfaceMap(localTestMode, this.defaultPlacementStrategy);
      ...
      var grainClassFeature =
          applicationPartManager.CreateAndPopulateFeature<GrainClassFeature>();
      this.grainTypes =
          CreateGrainTypeMap(grainClassFeature, grainClassOptions.Value);
    ...
    internal bool TryGetPrimaryImplementation(string grainInterface,
                                              out string grainClass) {
      return grainInterfaceMap.TryGetPrimaryImplementation(grainInterface,
                                                           out grainClass);
    }
    ...
    internal GrainTypeData this[string className] {
      get {
        ...
        lock(this) {
          ...
          if (grainInterfaceMap.TryGetPrimaryImplementation(className,
                                                            out grainType))
          ...
          if (TypeUtils.IsGenericClass(className)) {
            ...
            if (grainInterfaceMap.TryGetPrimaryImplementation(templateName,
                                                              out grainType))
            ...
            if (grainTypes.ContainsKey(templateName)) {
              ...
              try {
                ...
                var genericGrainTypeData =
                    (GenericGrainTypeData) grainTypes[templateName];
                Type[] typeArgs =
                    TypeUtils.GenericTypeArgsFromClassName(className);
                var concreteTypeData =
                    genericGrainTypeData.MakeGenericType(typeArgs);
    ...
    internal void GetTypeInfo(int typeCode, out string grainClass,
      ...
      if (!ClusterGrainInterfaceMap.TryGetTypeInfo(
              typeCode, out grainClass, out placement, genericArguments))
        throw new OrleansException(string.Format(
            "Unexpected: Cannot find an implementation class for grain interface {0}",
            typeCode));
    }

    internal void
    SetInterfaceMapsBySilo(Dictionary<SiloAddress, GrainInterfaceMap>value) {
    ...
    internal IReadOnlyDictionary<ushort, IReadOnlyList<SiloAddress>>
    GetSupportedSilos(int typeCode, int ifaceId,
      ...
      foreach(var version in versions) {
        ...
        var silosWithCorrectVersion = supportedSilosByInterface [ifaceId]
                                      [version]
    ...
    private static Dictionary<int, InvokerData>
    CreateInvokerMap(GrainInterfaceFeature grainInterfaceFeature) {
      ...
      foreach(var grainInterfaceMetadata in grainInterfaceFeature.Interfaces) {
        ...
        if (result.ContainsKey(ifaceId))
          throw new InvalidOperationException(
              $"Grain method invoker classes {result[ifaceId]} and {grainInterfaceMetadata.InvokerType.FullName} use the same interface id {ifaceId}");
    ...
    private void InitializeInterfaceMap() {
      foreach(GrainTypeData grainType in grainTypes.Values)
          AddToGrainInterfaceToClassMap(grainType.Type,
                                        grainType.RemoteInterfaceTypes,
                                        grainType.IsStatelessWorker);
    }

    private void AddToGrainInterfaceToClassMap(Type grainClass,
                                               IEnumerable<Type>grainInterfaces,
                                               bool isUnordered) {
      var placement = GrainTypeData.GetPlacementStrategy(
          grainClass, this.defaultPlacementStrategy);

      foreach(var iface in grainInterfaces) {
        var isPrimaryImplementor = IsPrimaryImplementor(grainClass, iface);
        grainInterfaceMap.AddEntry(iface, grainClass, placement,
                                   isPrimaryImplementor);
      }
    ...
    private static bool IsPrimaryImplementor(Type grainClass, Type iface) {
      // If the class name exactly matches the interface name, it is considered
      // the primary (default) implementation of the interface, e.g. IFooGrain
      // -> FooGrain
    ...
    /// </summary>
    /// <returns></returns>
    internal string[] GetGrainTypeList() { return grainTypes.Keys.ToArray(); }
    ...
    internal IGrainMethodInvoker GetInvoker(int interfaceId,
      ...
      try {
        ...
        throw new OrleansException(
            String.Format(
                "Error finding invoker for interface ID: {0} (0x{0, 8:X8}). {1}",
                interfaceId, ex),
            ex);
      }
      ...
      Type type;
      var interfaceName =
          grainInterfaceMap.TryGetServiceInterface(interfaceId, out type)
              ? type.FullName
              : "*unavailable*";

      throw new OrleansException(String.Format(
          "Cannot find an invoker for interface {0} (ID={1},0x{1, 8:X8}).",
          interfaceName, interfaceId));
    }

    private void RebuildFullGrainInterfaceMap() {
      var newClusterGrainInterfaceMap =
          new GrainInterfaceMap(false, this.defaultPlacementStrategy);
      var newSupportedSilosByTypeCode =
          new Dictionary<int, List<SiloAddress>>();
      var newSupportedSilosByInterface =
          new Dictionary<int, Dictionary<ushort, List<SiloAddress>>>();
      foreach(var kvp in grainInterfaceMapsBySilo) {
        ...
        foreach(var supportedInterface in kvp.Value.SupportedInterfaces) {
          ...
          var supportedSilosByVersion =
              newSupportedSilosByInterface.GetValueOrAddNew(ifaceId);
          var supportedSilosForVersion =
              supportedSilosByVersion.GetValueOrAddNew(version);
        ...
        foreach(var grainClassData in kvp.Value.SupportedGrainClassData) {
          ...
          var supportedSilos =
              newSupportedSilosByTypeCode.GetValueOrAddNew(grainType);
    ...
    private static Dictionary<string, GrainTypeData>
    CreateGrainTypeMap(GrainClassFeature grainClassFeature,
      ...
      foreach(var grainClassMetadata in grainClassFeature.Classes) {
        ...
        var typeData =
            grainType.IsGenericTypeDefinition ? new GenericGrainTypeData(
                grainType)
    ...
    LogGrainTypesFound(ILogger logger,
                       IDictionary<string, GrainTypeData>grainTypeData) {
      var sb = new StringBuilder();
      sb.AppendLine(String.Format("Loaded grain type summary for {0} types: ",
                                  grainTypeData.Count));

      foreach(var grainType in grainTypeData.Values.OrderBy(
          gtd => gtd.Type.FullName)) {
        ...
        if (!typeof (ISystemTarget).IsAssignableFrom(grainType.Type)) {
          int grainClassTypeCode =
              CodeGeneration.GrainInterfaceUtils.GetGrainClassTypeCode(
                  grainType.Type);
          sb.AppendFormat(
              "Grain class {0}.{1} [{2} (0x{3})] from {4}.dll implementing interfaces: ",
              grainType.Type.Namespace,
              TypeUtils.GetTemplatedName(grainType.Type), grainClassTypeCode,
              grainClassTypeCode.ToString("X"), assemblyName);
          var first = true;
          ...
          foreach(var iface in grainType.RemoteInterfaceTypes) {
            ...
            if (CodeGeneration.GrainInterfaceUtils.IsGrainType(iface)) {
              int ifaceTypeCode =
                  CodeGeneration.GrainInterfaceUtils.GetGrainInterfaceId(iface);
              sb.AppendFormat(" [{0} (0x{1})]", ifaceTypeCode,
                              ifaceTypeCode.ToString("X"));
            }
            first = false;
          }
          sb.AppendLine();
    ...
    private class InvokerData {
      ...
      private readonly CachedReadConcurrentDictionary<
          string, IGrainMethodInvoker>cachedGenericInvokers;
      ...
      public InvokerData(Type invokerType) {
        ...
        if (this.isGeneric) {
          cachedGenericInvokers =
              new CachedReadConcurrentDictionary<string, IGrainMethodInvoker>();
        }
      }
      ...
      public IGrainMethodInvoker GetInvoker(string genericGrainType = null) {
        ...
        if (!this.isGeneric) {
          return invoker ??(invoker =
                               (IGrainMethodInvoker)
                                   Activator.CreateInstance(baseInvokerType));
        }

        if (this.cachedGenericInvokers.TryGetValue(
                genericGrainType, out IGrainMethodInvoker result))
          return result;

        var typeArgs =
            TypeUtils.GenericTypeArgsFromArgsString(genericGrainType);
