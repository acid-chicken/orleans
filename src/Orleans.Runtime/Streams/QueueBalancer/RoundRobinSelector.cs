...
namespace Orleans.Streams {
  ...
  internal class RoundRobinSelector<T>: IResourceSelector<T> {
    ...
    public RoundRobinSelector(IEnumerable<T>resources, Random random) {
      ...
      this.resources =
          resources.Distinct().OrderBy(_ => random.Next()).ToList();
    ...
    /// <summary>
    /// Try to select certain count of resources from resource list, which
    /// doesn't overlap with existing resources
    /// </summary>
    /// <param name="newSelectionCount"></param>
    /// <param name="existingSelection"></param>
    /// <returns></returns>
    public List<T>NextSelection(int newSelectionCount,
                                List<T>existingSelection) {
      var selection =
          new List<T>(Math.Min(newSelectionCount, this.resources.Count));
      int tries = 0;
      while (selection.Count < newSelectionCount &&
             tries++ < this.resources.Count) {
