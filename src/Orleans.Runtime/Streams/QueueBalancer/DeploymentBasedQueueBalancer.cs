...
namespace Orleans.Streams {
  /// <summary>
  /// DeploymentBasedQueueBalancer is a stream queue balancer that uses
  /// deployment information to help balance queue distribution.
  /// DeploymentBasedQueueBalancer uses the deployment configuration to
  /// determine how many silos to expect and uses a silo status oracle to
  /// determine which of the silos are available.  With this information it
  /// tries to balance the queues using a best fit resource balancing algorithm.
  /// </summary>
  public class DeploymentBasedQueueBalancer : QueueBalancerBase,
    ...
    public DeploymentBasedQueueBalancer(
        ...
        DeploymentBasedQueueBalancerOptions options, IServiceProvider services,
        ILogger<DeploymentBasedQueueBalancer>logger)
        : base(services, logger) {
      this.siloStatusOracle =
          siloStatusOracle ?? throw new ArgumentNullException(
              nameof(siloStatusOracle));
      this.deploymentConfig =
          deploymentConfig ?? throw new ArgumentNullException(
              nameof(deploymentConfig));
      this.options = options;

      isStarting = true;

      // record all already active silos as already mature.
      // Even if they are not yet, they will be mature by the time I mature
      // myself (after I become !isStarting).
      immatureSilos = new ConcurrentDictionary<SiloAddress, bool>(
          from s in siloStatusOracle.GetApproximateSiloStatuses(true)
              .Keys where !s.Equals(siloStatusOracle.SiloAddress)
                  select new KeyValuePair<SiloAddress, bool>(s, false));
    }

    public static IStreamQueueBalancer
    Create(IServiceProvider services, string name,
           IDeploymentConfiguration deploymentConfiguration) {
      var options =
          services
              .GetRequiredService<
                  IOptionsMonitor<DeploymentBasedQueueBalancerOptions>>()
              .Get(name);
      return ActivatorUtilities.CreateInstance<DeploymentBasedQueueBalancer>(
          services, options, deploymentConfiguration);
    }
    ...
    public override IEnumerable<QueueId>GetMyQueues() {
      ...
      Dictionary<string, List<QueueId>>distribution =
          useIdealDistribution
                ? balancer.IdealDistribution
          : balancer.GetDistribution(
                GetActiveSilos(siloStatusOracle, immatureSilos));

      List<QueueId>myQueues;
      if (distribution.TryGetValue(siloStatusOracle.SiloName, out myQueues)) {
        if (!useIdealDistribution) {
          HashSet<QueueId>queuesOfImmatureSilos = GetQueuesOfImmatureSilos(
              siloStatusOracle, immatureSilos, balancer.IdealDistribution);
    ...
    GetActiveSilos(ISiloStatusOracle siloStatusOracle,
      ...
      foreach(var kvp in siloStatusOracle.GetApproximateSiloStatuses(true)) {
        bool immatureBit;
        if (!(immatureSilos.TryGetValue(kvp.Key, out immatureBit) &&
              immatureBit)) // if not immature now or any more
        {
          string siloName;
    ...
    /// <summary>
    /// Checks to see if deployment configuration has changed, by adding or
    /// removing silos. If so, it updates the list of all silo names and creates
    /// a new resource balancer. This should occur rarely.
    /// </summary>
    ...
    private static HashSet<QueueId>GetQueuesOfImmatureSilos(
        ISiloStatusOracle siloStatusOracle,
        ConcurrentDictionary<SiloAddress, bool>immatureSilos,
        Dictionary<string, List<QueueId>>idealDistribution) {
      HashSet<QueueId>queuesOfImmatureSilos = new HashSet<QueueId>();
      foreach(var silo in immatureSilos.Where(
          s => s.Value)) // take only those from immature set that have their
                         // immature status bit set
      {
        string siloName;
    ...
    private async Task SignalClusterChange(HashSet<SiloAddress>activeSilos) {
      ...
      foreach(var silo in activeSilos) {
        if (!silo.Equals(siloStatusOracle.SiloAddress) &&
            !immatureSilos.ContainsKey(silo)) {
      ...
      if (tasks.Count > 0) {
        ...
        await this.NotifyListeners(); // notify, uncoditionaly, and deal with
                                      // changes it in GetMyQueues()
      }
    }
  }
}
