...
namespace Orleans.Runtime {
  /// <summary>
  /// Maintains additional per-activation state that is required for Orleans
  /// internal operations. MUST lock this object for any concurrent access
  /// Consider: compartmentalize by usage, e.g., using separate interfaces for
  /// data for catalog, etc.
  /// </summary>
  internal class ActivationData : IGrainActivationContext,
                                  IActivationData,
                                  IInvokable,
                                  IDisposable {
    internal class GrainActivationContextFactory {
      public IGrainActivationContext Context {
        get;
        set;
      }
    }

    // This is the maximum amount of time we expect a request to continue
    // processing
    ...
    public ActivationData(ActivationAddress addr, string genericArguments,
                          PlacementStrategy placedUsing,
                          IActivationCollector collector, TimeSpan ageLimit,
      ...
      this.lifecycle =
          new GrainLifecycle(loggerFactory.CreateLogger<LifecycleSubject>());
      ...
      GrainReference = GrainReference.FromGrainId(
          addr.Grain, runtimeClient.GrainReferenceRuntime, genericArguments);
    }
    ...
    internal WorkItemGroup WorkItemGroup {
      get;
      set;
    }
    ...
    internal ExtensionInvoker ExtensionInvoker {
      get {
        ...
        return this.extensionInvoker ??(this.extensionInvoker =
                                           new ExtensionInvoker());
      }
    }

    public IGrainMethodInvoker GetInvoker(GrainTypeManager typeManager,
                                          int interfaceId,
                                          string genericGrainType = null) {
      // Return previous cached invoker, if applicable
      if (lastInvoker != null &&
          interfaceId ==
              lastInvoker
                  .InterfaceId) // extension invoker returns InterfaceId==0, so
                                // this condition will never be true if an
                                // extension is installed
        return lastInvoker;

      if (extensionInvoker != null &&
          extensionInvoker.IsExtensionInstalled(interfaceId)) {
    ...
    public HashSet<ActivationId>RunningRequestsSenders { get; }
    = new HashSet<ActivationId>();
    ...
    internal void SetupContext(GrainTypeData typeData,
                               IServiceProvider grainServices) {
      ...
      if (typeData != null) {
        ...
        // Don't ever collect system grains or reminder table grain or memory
        // store grains.
        bool doNotCollect =
            typeof (IReminderTableGrain).IsAssignableFrom(grainType) ||
            typeof (IMemoryStorageGrain).IsAssignableFrom(grainType);
        if (doNotCollect) {
          this.collector = null;
        }
      }
    ...
    private static void
    SetGrainActivationContextInScopedServices(IServiceProvider sp,
                                              IGrainActivationContext context) {
      var contextFactory =
          sp.GetRequiredService<GrainActivationContextFactory>();
    ...
    internal async Task DeactivateStreamResources() {
      if (streamDirectory == null)
        return; // No streams - Nothing to do.
      if (extensionInvoker == null)
        return; // No installed extensions - Nothing to do.

      if (StreamResourceTestControl.TestOnlySuppressStreamCleanupOnDeactivate) {
        logger.Warn(
            0, "Suppressing cleanup of stream resources during tests for {0}",
            this);
        return;
      }
    ...
    public GrainTypeData GrainTypeData {
      get;
      private set;
    }
    ...
    public SiloAddress Silo {
      get { return Address.Silo; }
    }

    public GrainId Grain {
      get { return Address.Grain; }
    }
    ...
    /// <summary>
    /// If State == Invalid, this may contain a forwarding address for incoming
    /// messages
    /// </summary>
    ...
    public DateTime CollectionTicket {
      get;
      private set;
    }
    ...
    public bool TrySetCollectionCancelledFlag() {
      lock(this) {
        if (default(DateTime) == CollectionTicket || collectionCancelledFlag)
          return false;
    ...
    public void ResetCollectionCancelledFlag() {
      lock(this) { collectionCancelledFlag = false; }
    }
    ...
    public void SetCollectionTicket(DateTime ticket) {
      if (ticket == default(DateTime))
        throw new ArgumentException("default(DateTime) is disallowed",
                                    "ticket");
      if (CollectionTicket != default(DateTime)) {
        throw new InvalidOperationException(
            "call ResetCollectionTicket before calling SetCollectionTicket.");
      }

      CollectionTicket = ticket;
    }

    public PlacementStrategy PlacedUsing {
      get;
      private set;
    }

    // Currently, the only supported multi-activation grain is one using the
    // StatelessWorkerPlacement strategy.
    internal bool IsStatelessWorker =>
        this.PlacedUsing is StatelessWorkerPlacement;

    /// <summary>
    /// Returns a value indicating whether or not this placement strategy
    /// requires activations to be registered in the grain directory.
    /// </summary>
    internal bool IsUsingGrainDirectory =>
        this.PlacedUsing.IsUsingGrainDirectory;

    public Message Blocking {
      get;
      private set;
    }
    ...
    public void RecordRunning(Message message, bool isInterleavable) {
      // Note: This method is always called while holding lock on this
      // activation, so no need for additional locks here
    ...
    /// <summary>
    /// Number of messages that are actively being processed [as opposed to
    /// being in the Waiting queue]. In most cases this will be 0 or 1, but for
    /// Reentrant grains can be >1.
    /// </summary>
    public long InFlightCount {
      get { return Interlocked.Read(ref inFlightCount); }
    }

    /// <summary>
    /// Number of messages that are being received [as opposed to being in the
    /// scheduler queue or actively processed].
    /// </summary>
    public long EnqueuedOnDispatcherCount {
      get { return Interlocked.Read(ref enqueuedOnDispatcherCount); }
    }

    /// <summary>Increment the number of in-flight messages currently being
    /// processed.</summary>
    public void IncrementInFlightCount() {
      Interlocked.Increment(ref inFlightCount);
    }

    /// <summary>Decrement the number of in-flight messages currently being
    /// processed.</summary>
    public void DecrementInFlightCount() {
      Interlocked.Decrement(ref inFlightCount);
    }

    /// <summary>Increment the number of messages currently in the process of
    /// being received.</summary>
    public void IncrementEnqueuedOnDispatcherCount() {
      Interlocked.Increment(ref enqueuedOnDispatcherCount);
    }

    /// <summary>Decrement the number of messages currently in the process of
    /// being received.</summary>
    public void DecrementEnqueuedOnDispatcherCount() {
      Interlocked.Decrement(ref enqueuedOnDispatcherCount);
    }
    ...
    public int WaitingCount {
      get { return waiting == null ? 0 : waiting.Count; }
    }
      ...
      lock(this) {
        if (State == ActivationState.Invalid) {
          logger.Warn(ErrorCode.Dispatcher_InvalidActivation,
                      "Cannot enqueue message to invalid activation {0} : {1}",
                      this.ToDetailedString(), message);
          return EnqueueMessageResult.ErrorInvalidActivation;
        }
        if (State == ActivationState.FailedToActivate) {
          logger.Warn(
              ErrorCode.Dispatcher_InvalidActivation,
              "Cannot enqueue message to activation that failed in OnActivate {0} : {1}",
              this.ToDetailedString(), message);
        ...
        if (State == ActivationState.Deactivating) {
          ...
          if (deactivatingTime > maxRequestProcessingTime) {
            logger.Error(
                ErrorCode.Dispatcher_StuckActivation,
        ...
        if (this.Blocking != null) {
          var currentRequestActiveTime =
              DateTime.UtcNow - currentRequestStartTime;
    ...
    /// </summary>
    /// <param name="log">Logger to use for reporting any overflow
    /// condition</param> <returns>Returns LimitExceededException if overloaded,
    /// otherwise <c>null</c>c></returns>
    public LimitExceededException CheckOverloaded(ILogger log) {
      string limitName = LimitNames.LIMIT_MAX_ENQUEUED_REQUESTS;
      int maxRequestsHardLimit =
          this.messagingOptions.MaxEnqueuedRequestsHardLimit;
      int maxRequestsSoftLimit =
          this.messagingOptions.MaxEnqueuedRequestsSoftLimit;
      if (IsStatelessWorker) {
        limitName = LimitNames.LIMIT_MAX_ENQUEUED_REQUESTS_STATELESS_WORKER;
        maxRequestsHardLimit =
            this.messagingOptions.MaxEnqueuedRequestsHardLimit_StatelessWorker;
        maxRequestsSoftLimit =
            this.messagingOptions.MaxEnqueuedRequestsSoftLimit_StatelessWorker;
      }
      ...
      if (maxRequestsHardLimit > 0 &&
          count > maxRequestsHardLimit) // Hard limit
      {
        log.Warn(
            ErrorCode.Catalog_Reject_ActivationTooManyRequests,
            String.Format(
                "Overload - {0} enqueued requests for activation {1}, exceeding hard limit rejection threshold of {2}",
                count, this, maxRequestsHardLimit));

        return new LimitExceededException(
            limitName, count, maxRequestsHardLimit, this.ToString());
      }
      if (maxRequestsSoftLimit > 0 &&
          count > maxRequestsSoftLimit) // Soft limit
      {
        log.Warn(
            ErrorCode.Catalog_Warn_ActivationTooManyRequests,
            String.Format(
                "Hot - {0} enqueued requests for activation {1}, exceeding soft limit warning threshold of {2}",
    ...
    public TimeSpan GetIdleness(DateTime now) {
      if (now == default(DateTime))
        throw new ArgumentException(
            "default(DateTime) is not allowed; Use DateTime.UtcNow instead.",
            "now");
    ...
    /// <summary>
    /// Returns whether this activation has been idle long enough to be
    /// collected.
    /// </summary>
    ...
    public bool ShouldBeKeptAlive {
      get { return keepAliveUntil >= DateTime.UtcNow; }
    }
    ...
    public void OnTimerDisposed(IGrainTimer orleansTimerInsideGrain) {
      lock(this) // need to lock since dispose can be called on finalizer
                 // thread, outside garin context (not single threaded).
      {
    ...
    internal Task WaitForAllTimersToFinish() {
      lock(this) {
        ...
        var timerCopy = timers.ToList(); // need to copy since OnTimerDisposed
                                         // will change the timers set.
    ...
    public string DumpStatus() {
      ...
      lock(this) {
        ...
        if (waiting != null && waiting.Count > 0) {
          sb.AppendFormat("   Messages queued within ActivationData: {0}",
                          PrintWaitingQueue());
        }
      }
      return sb.ToString();
    ...
    public override string ToString() {
      return String.Format("[Activation: {0}{1}{2}{3} State={4}]", Silo, Grain,
                           this.ActivationId, GetActivationInfoString(), State);
    }
    ...
    internal string ToDetailedString(bool includeExtraDetails = false) {
      return String.Format(
          ...
          Silo.ToLongString(), Grain.ToString(), this.ActivationId,
          ...
          (includeExtraDetails && this.Blocking != null)
              ? " CurrentlyExecuting=" + this.Blocking
              : ""); // 11: Running
    }

    public string Name {
      get {
        return String.Format("[Activation: {0}{1}{2}{3}]", Silo, Grain,
                             this.ActivationId, GetActivationInfoString());
      }
    }
    ...
    /// </summary>
    /// <returns></returns>
    /// <remarks>Note: Caller must be holding lock on this activation while
    ...
    private string GetActivationInfoString() {
      var placement =
          PlacedUsing != null ? PlacedUsing.GetType().Name : String.Empty;
      return GrainInstanceType == null ? placement
          : String.Format(" #GrainType={0} Placement={1}",
                          GrainInstanceType.FullName, placement);
    }
    ...
    bool IEquatable<IGrainContext>.Equals(IGrainContext other) =>
