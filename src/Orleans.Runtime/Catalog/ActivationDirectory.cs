...
namespace Orleans.Runtime {
  internal class ActivationDirectory
      : IEnumerable<KeyValuePair<ActivationId, ActivationData>> {
    private readonly ILogger logger;

    private readonly ConcurrentDictionary<ActivationId, ActivationData>
        activations; // Activation data (app grains) only.
    private readonly ConcurrentDictionary<ActivationId, SystemTarget>
        systemTargets; // SystemTarget only.
    private readonly ConcurrentDictionary<GrainId, List<ActivationData>>
        grainToActivationsMap; // Activation data (app grains) only.
    private readonly ConcurrentDictionary<string, CounterStatistic>
        grainCounts; // simple statistics type->count
    private readonly ConcurrentDictionary<string, CounterStatistic>
        systemTargetCounts; // simple statistics systemTargetTypeName->count
    ...
    public ActivationDirectory(ILogger<ActivationDirectory>logger) {
      ...
      grainToActivationsMap =
          new ConcurrentDictionary<GrainId, List<ActivationData>>();
    ...
    internal void IncrementGrainCounter(string grainTypeName) {
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Increment Grain Counter {0}", grainTypeName);
    ...
    internal void DecrementGrainCounter(string grainTypeName) {
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Decrement Grain Counter {0}", grainTypeName);
      CounterStatistic ctr = FindGrainCounter(grainTypeName);
      ctr.DecrementBy(1);
    }
    ...
    private CounterStatistic FindGrainCounter(string grainTypeName) {
      ...
      var counterName = new StatisticName(StatisticNames.GRAIN_COUNTS_PER_GRAIN,
                                          grainTypeName);
      ctr = grainCounts[grainTypeName] =
          CounterStatistic.FindOrCreate(counterName, false);
      return ctr;
    }

    private CounterStatistic
    FindSystemTargetCounter(string systemTargetTypeName) {
      CounterStatistic ctr;
      if (systemTargetCounts.TryGetValue(systemTargetTypeName, out ctr))
        return ctr;

      var counterName = new StatisticName(StatisticNames.SYSTEM_TARGET_COUNTS,
                                          systemTargetTypeName);
      ctr = systemTargetCounts[systemTargetTypeName] =
          CounterStatistic.FindOrCreate(counterName, false);
      return ctr;
    }
    ...
    public void RecordNewTarget(ActivationData target) {
      if (!activations.TryAdd(target.ActivationId, target))
        return;
      grainToActivationsMap.AddOrUpdate(
          target.Grain, g => new List<ActivationData>{target}, (g, list) => {
            lock(list) { list.Add(target); }
            return list;
          });
    }
    ...
    public void RecordNewSystemTarget(SystemTarget target) {
      ...
      if (!Constants.IsSingletonSystemTarget(systemTarget.GrainId.Type)) {
        FindSystemTargetCounter(
            Constants.SystemTargetName(systemTarget.GrainId.Type))
            .Increment();
      }
    }
        ...
        lock(list) {
          ...
          if (list.Count == 0) {
            ...
            if (grainToActivationsMap.TryRemove(target.Grain, out list2)) {
              lock(list2) {
                if (list2.Count > 0) {
                  grainToActivationsMap.AddOrUpdate(target.Grain, g => list2,
                                                    (g, list3) => {
                                                      lock(list3) {
    ...
    /// <summary>
    /// Returns null if no activations exist for this grain ID, rather than an
    /// empty list
    /// </summary>
    ...
    public IEnumerable<KeyValuePair<string, long>>GetSimpleGrainStatistics() {
      return grainCounts
          .Select(s => new KeyValuePair<string, long>(
                      s.Key, s.Value.GetCurrentValue()))
          .Where(p => p.Value > 0);
    }
    ...
    public void PrintActivationDirectory() {
      if (logger.IsEnabled(LogLevel.Information)) {
        string stats = Utils.EnumerableToString(
            activations.Values.OrderBy(act => act.Name),
            act => string.Format("++{0}", act.DumpStatus()),
            Environment.NewLine);
        if (stats.Length > 0) {
          logger.Info(
              ErrorCode.Catalog_ActivationDirectory_Statistics,
              $"ActivationDirectory.PrintActivationDirectory(): Size = { activations.Count}, Directory:{Environment.NewLine}{stats}");
        }
      }
    }

    public IEnumerator<KeyValuePair<ActivationId, ActivationData>>
    GetEnumerator() {
