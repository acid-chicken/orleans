...
namespace Orleans.Runtime.Utilities {
  ...
  public static class OrleansDebuggerHelper {
    /// <summary>
    /// Returns the grain instance corresponding to the provided <paramref
    /// name="grainReference"/> if it is activated on current silo.
    /// </summary>
    /// <param name="grainReference">The grain reference.</param>
    /// <returns>
    /// The grain instance corresponding to the provided <paramref
    /// name="grainReference"/> if it is activated on current silo, or <see
    ...
    public static object GetGrainInstance(object grainReference) {
      ...
      case GrainReference reference: {
        var runtime =
            (reference.Runtime as GrainReferenceRuntime) ?.RuntimeClient;
        if (runtime is null)
          return null;

        var activations =
            runtime.ServiceProvider.GetService<ActivationDirectory>();
