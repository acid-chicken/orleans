...
namespace Orleans.Runtime {
  ...
  public class Silo {
    ...
    public Silo(ILocalSiloDetails siloDetails, IServiceProvider services) {
      ...
      messageCenter.RegisterLocalMessageHandler(new IncomingMessageHandler(
          messageCenter, activationDirectory, scheduler, catalog.Dispatcher,
          messageFactory,
    ...
    private async Task InjectDependencies() {
      ...
      if (reminderTable != null) {
        ...
        var timerFactory =
            this.Services.GetRequiredService<IAsyncTimerFactory>();
        reminderService =
            new LocalReminderService(this, reminderTable, this.initTimeout,
                                     this.loggerFactory, timerFactory);
      ...
      try {
        ...
        async Task StartDeploymentLoadCollector() {
          ...
          await this.scheduler
              .QueueTask(deploymentLoadPublisher.Start, deploymentLoadPublisher)
              .WithTimeout(
                  this.initTimeout,
        ...
        this.platformWatchdog = new Watchdog(
            ...
            this.loggerFactory.CreateLogger<Watchdog>());
      ...
      if (this.reminderService != null) {
        ...
        async Task StartReminderService() {
          ...
          this.reminderServiceContext =
              (this.reminderService as IGrainContext) ?? this.fallbackScheduler;
          await this.scheduler
    ...
    private async Task RegisterGrainService(IGrainService service) {
      ...
      await this.scheduler
          .QueueTask(() => grainService.Init(Services), grainService)
          .WithTimeout(
              this.initTimeout,
    ...
    private async Task StartGrainService(IGrainService service) {
      ...
      await this.scheduler.QueueTask(grainService.Start, grainService)
          .WithTimeout(
              this.initTimeout,
        ...
        if (gracefully) {
          ...
          await scheduler
              .QueueTask(() => localGrainDirectory.Stop(true),
                         localGrainDirectory.CacheValidator)
              .WithCancellation(
                  ct,
    ...
    private async Task OnActiveStop(CancellationToken ct) {
      ...
      foreach(var grainService in grainServices) {
        await this.scheduler.QueueTask(grainService.Stop, grainService)
            .WithCancellation(
                ct,
  ...
  internal class FallbackSystemTarget : SystemTarget {
    public FallbackSystemTarget(ILocalSiloDetails localSiloDetails,
        ...
        : base(Constants.FallbackSystemTargetType, localSiloDetails.SiloAddress,
               loggerFactory) {}
  }
  ...
  internal class LifecycleSchedulingSystemTarget : SystemTarget {
    public LifecycleSchedulingSystemTarget(ILocalSiloDetails localSiloDetails,
                                           ILoggerFactory loggerFactory)
        : base(Constants.LifecycleSchedulingSystemTargetType,
