...
namespace Orleans.Runtime.MembershipService {
  internal class SiloStatusOracle : ISiloStatusOracle {
    ...
    public SiloStatusOracle(ILocalSiloDetails localSiloDetails,
    ...
    public SiloStatus CurrentStatus =>
        this.membershipTableManager.CurrentStatus;
    ...
    public SiloStatus GetApproximateSiloStatus(SiloAddress silo) {
      var status =
          this.membershipTableManager.MembershipTableSnapshot.GetSiloStatus(
              silo);
      ...
      if (status == SiloStatus.None) {
        if (this.CurrentStatus == SiloStatus.Active &&
          ...
          this.log.Debug(
              ...
              "-The given siloAddress {0} is not registered in this MembershipOracle.",
              silo);
        }
      }
    ...
    public Dictionary<SiloAddress, SiloStatus>
    GetApproximateSiloStatuses(bool onlyActive = false) {
      if (ReferenceEquals(
              this.cachedSnapshot,
              this.membershipTableManager.MembershipTableSnapshot)) {
        return onlyActive ? this.siloStatusCacheOnlyActive
      ...
      lock(this.cacheUpdateLock) {
        ...
        var newSiloStatusCacheOnlyActive =
            new Dictionary<SiloAddress, SiloStatus>();
    ...
    public bool TryGetSiloName(SiloAddress siloAddress, out string siloName) {
      var snapshot =
          this.membershipTableManager.MembershipTableSnapshot.Entries;
    ...
    public bool SubscribeToSiloStatusEvents(ISiloStatusListener listener) =>
        this.listenerManager.Subscribe(listener);

    public bool UnSubscribeFromSiloStatusEvents(ISiloStatusListener listener) =>
        this.listenerManager.Unsubscribe(listener);

    public IReadOnlyList<SiloAddress>
    GetApproximateMultiClusterGateways() => throw new NotSupportedException();
