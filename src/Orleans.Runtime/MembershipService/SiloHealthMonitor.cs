...
namespace Orleans.Runtime.MembershipService {
  ...
  internal class SiloHealthMonitor : ITestAccessor {
    ...
    private readonly CancellationTokenSource stoppingCancellation =
    ...
    public SiloHealthMonitor(SiloAddress siloAddress,
    ...
    /// </summary>
    /// <param name="diagnosticProbeNumber">The probe number, for diagnostic
    /// purposes.</param> <param name="cancellation">A token to cancel and fail
    /// the probe attempt.</param> <returns>The number of failed probes since
    /// the last successful probe.</returns>
    public async Task<int>Probe(int diagnosticProbeNumber,
                                CancellationToken cancellation) {
      var id = Interlocked.Increment(ref this.nextProbeId);

      if (this.log.IsEnabled(LogLevel.Trace)) {
        this.log.LogTrace(
            "Going to send Ping #{ProbeNumber}/{Id} to probe silo {Silo}",
            diagnosticProbeNumber, id, this.SiloAddress);
      }

      var probeTask =
          this.PerformProbe(id, diagnosticProbeNumber, cancellation);
      var resultTask = await Task.WhenAny(this.stopping, probeTask);

      // If the probe finished and the result was valid then return the number
      // of missed probes.
      if (ReferenceEquals(resultTask, probeTask) &&
    ...
    private async Task<bool>PerformProbe(long id, int diagnosticProbeNumber,
                                         CancellationToken cancellation) {
      try {
        var probeCancellation = cancellation.WhenCancelled();
        var task = await Task.WhenAny(
            probeCancellation,
            this.prober.Probe(this.SiloAddress, diagnosticProbeNumber));

        if (ReferenceEquals(task, probeCancellation)) {
          return this.RecordFailure(
              id, diagnosticProbeNumber,
              new OperationCanceledException(
                  $"The ping attempt was cancelled. Ping #{diagnosticProbeNumber}/{id}"));
        } else {
          await task;
    ...
    private bool RecordSuccess(long id, int diagnosticProbeNumber) {
      if (this.log.IsEnabled(LogLevel.Trace)) {
        this.log.LogTrace(
            "Got successful ping response for ping #{ProbeNumber}/{Id} from {Silo}",
            diagnosticProbeNumber, id, this.SiloAddress);
      }
    ...
    private bool RecordFailure(long id, int diagnosticProbeNumber,
                               Exception failureReason) {
      if (this.log.IsEnabled(LogLevel.Trace)) {
        this.log.LogTrace(
            "Got failed ping response for ping #{ProbeNumber}/{Id} from {Silo}: {Exception}",
            diagnosticProbeNumber, id, this.SiloAddress, failureReason);
      }
      ...
      lock(this.lockObj) {
        if (id <= this.highestCompletedProbeId) {
          ...
          if (this.log.IsEnabled(LogLevel.Trace)) {
            this.log.LogTrace(
                "Current number of failed probes for {Silo}: {MissedProbes}",
                this.SiloAddress, missed);
          }
