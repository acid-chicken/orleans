...
namespace Orleans.Runtime.MembershipService {
  /// <summary>
  /// Responsible for updating membership table with details about the local
  /// silo.
  /// </summary>
  internal class MembershipAgent : IHealthCheckParticipant,
                                   ILifecycleParticipant<ISiloLifecycle>,
                                   IDisposable,
                                   MembershipAgent.ITestAccessor {
    ...
    public MembershipAgent(MembershipTableManager tableManager,
    ...
    Action ITestAccessor.OnUpdateIAmAlive {
      get;
      set;
    }
    Func<DateTime>ITestAccessor.GetDateTime {
      get => this.getUtcDateTime;
      set => this.getUtcDateTime =
    ...
    private async Task UpdateIAmAlive() {
      if (this.log.IsEnabled(LogLevel.Debug))
        this.log.LogDebug(
            "Starting periodic membership liveness timestamp updates");
      try {
        TimeSpan? onceOffDelay = default;
        while (await this.iAmAliveTimer.NextTick(onceOffDelay) &&
               !this.tableManager.CurrentStatus.IsTerminating()) {
          ...
          try {
            ...
            if (this.log.IsEnabled(LogLevel.Trace))
              this.log.LogTrace("Updating IAmAlive took {Elapsed}",
                                stopwatch.Elapsed);
          ...
          when(this.fatalErrorHandler.IsUnexpected(exception)) {
        this.log.LogError("Error updating liveness timestamp: {Exception}",
                          exception);
        this.fatalErrorHandler.OnFatalException(this, nameof(UpdateIAmAlive),
                                                exception);
      } finally {
        if (this.log.IsEnabled(LogLevel.Debug))
          this.log.LogDebug(
              "Stopping periodic membership liveness timestamp updates");
      }
    }

    private async Task BecomeActive() {
      this.log.LogInformation((int) ErrorCode.MembershipBecomeActive,
                              "-BecomeActive");
      ...
      try {
        ...
        this.log.LogInformation((int) ErrorCode.MembershipFinishBecomeActive,
                                "-Finished BecomeActive.");
      } catch (Exception exception) {
        this.log.LogInformation((int) ErrorCode.MembershipFailedToBecomeActive,
                                "BecomeActive failed: {Exception}", exception);
        throw;
      }
    }

    private async Task ValidateInitialConnectivity() {
      // Continue attempting to validate connectivity until some reasonable
      // timeout.
      var maxAttemptTime = this.clusterMembershipOptions.ProbeTimeout.Multiply(
          5.0 * this.clusterMembershipOptions.NumMissedProbesLimit);
      var attemptNumber = 1;
      ...
      while (true) {
        try {
          ...
          foreach(
              var item in this.tableManager.MembershipTableSnapshot.Entries) {
            var entry = item.Value;
            if (entry.Status != SiloStatus.Active)
              continue;
            if (entry.SiloAddress.IsSameLogicalSilo(this.localSilo.SiloAddress))
              continue;
            if (entry.HasMissedIAmAlivesSince(this.clusterMembershipOptions,
                                              now) != default)
              continue;
          ...
          var failedSilos =
              await this.clusterHealthMonitor.CheckClusterConnectivity(
                  activeSilos.ToArray());
          var successfulSilos =
              activeSilos.Where(s => !failedSilos.Contains(s)).ToList();

          // If there were no failures, terminate the loop and return without
          // error.
          ...
          this.log.LogError(
              ...
              failedSilos.Count, activeSilos.Count,
              Utils.EnumerableToString(successfulSilos),
              Utils.EnumerableToString(failedSilos), attemptUntil,
              attemptNumber);

          if (now + TimeSpan.FromSeconds(5) > attemptUntil) {
            canContinue = false;
            var msg =
                $"Failed to get ping responses from {failedSilos.Count} of {activeSilos.Count} active silos. " +
                "Newly joining silos validate connectivity with all active silos that have recently updated their 'I Am Alive' value before joining the cluster. " +
          ...
          this.log.LogError(
              "Failed to validate initial cluster connectivity: {Exception}",
              exception);
    ...
    private async Task BecomeJoining() {
      ...
      try {
        ...
        this.log.Error(ErrorCode.MembershipFailedToJoin,
                       "Error updating status to Joining", exc);
        throw;
      }
    }
    ...
    private async Task BecomeShuttingDown() {
      ...
      try {
        ...
        this.log.Error(ErrorCode.MembershipFailedToShutdown,
                       "Error updating status to ShuttingDown", exc);
        throw;
      }
    }
    ...
    private async Task BecomeStopping() {
      ...
      try {
        ...
        log.Error(ErrorCode.MembershipFailedToStop,
                  "Error updating status to Stopping", exc);
        throw;
      }
    }

    private async Task BecomeDead() {
      this.log.LogInformation((int) ErrorCode.MembershipKillMyself,
                              "Updating status to Dead");

      try {
        await this.UpdateStatus(SiloStatus.Dead);
      } catch (Exception exception) {
        this.log.LogError((int) ErrorCode.MembershipFailedToKillMyself,
                          "Failure updating status to " +
                              nameof(SiloStatus.Dead) + ": {Exception}",
                          exception);
        throw;
      }
    }
    ...
    void ILifecycleParticipant<ISiloLifecycle>.Participate(
        ISiloLifecycle lifecycle) {
      {
        ...
        async Task OnRuntimeInitializeStop(CancellationToken ct) {
          ...
          await Task.WhenAny(Task.Run(() => this.BecomeDead()),
        ...
        lifecycle.Subscribe(
            ...
            ServiceLifecycleStage.RuntimeInitialize +
                1, // Gossip before the outbound queue gets closed
            OnRuntimeInitializeStart, OnRuntimeInitializeStop);
      }

      {
        ...
        Task AfterRuntimeGrainServicesStop(CancellationToken ct) =>
            Task.CompletedTask;

        lifecycle.Subscribe(nameof(MembershipAgent),
...

      {
        ...
        async Task OnBecomeActiveStop(CancellationToken ct) {
          ...
          if (ct.IsCancellationRequested) {
            ...
            var gracePeriod = Task.WhenAll(
                Task.Delay(
                    ClusterMembershipOptions.ClusteringShutdownGracePeriod),
                cancellationTask);
            var task =
                await Task.WhenAny(gracePeriod, this.BecomeShuttingDown());
            if (ReferenceEquals(task, gracePeriod)) {
              this.log.LogWarning(
                  "Graceful shutdown aborted: starting ungraceful shutdown");
              await Task.Run(() => this.BecomeStopping());
            } else {
        ...
        lifecycle.Subscribe(nameof(MembershipAgent),
                            ...
                            OnBecomeActiveStart, OnBecomeActiveStop);
      }
    }

    public void Dispose() { this.iAmAliveTimer.Dispose(); }
