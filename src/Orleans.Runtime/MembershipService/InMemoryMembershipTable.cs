...
namespace Orleans.Runtime.MembershipService {
  ...
  internal class InMemoryMembershipTable {
    ...
    private readonly Dictionary<SiloAddress, Tuple<MembershipEntry, string>>
        siloTable;
    ...
    public MembershipTableData Read(SiloAddress key) {
      return siloTable.TryGetValue(key, out var data)
                 ? new MembershipTableData(
                       (Tuple<MembershipEntry, string>) this
    ...
    public MembershipTableData ReadAll() {
      return new MembershipTableData(
          siloTable.Values
              .Select(tuple => new Tuple<MembershipEntry, string>(
                          (MembershipEntry) this.serializationManager.DeepCopy(
                              tuple.Item1),
                          tuple.Item2))
              .ToList(),
          tableVersion);
    ...
    public TableVersion ReadTableVersion() { return tableVersion; }
    ...
    public bool Update(MembershipEntry entry, string etag,
                       TableVersion version) {
      Tuple<MembershipEntry, string>data;
      siloTable.TryGetValue(entry.SiloAddress, out data);
      if (data == null)
        return false;
      if (!data.Item2.Equals(etag) ||
          !tableVersion.VersionEtag.Equals(version.VersionEtag))
        return false;
    ...
    public void UpdateIAmAlive(MembershipEntry entry) {
      ...
      siloTable[entry.SiloAddress] =
          new Tuple<MembershipEntry, string>(data.Item1, NewETag());
    }

    public override string ToString() {
      return String.Format("Table = {0}, ETagCounter={1}", ReadAll().ToString(),
                           lastETagCounter);
    }
