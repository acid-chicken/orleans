...
namespace Orleans.Runtime.MembershipService {
  ...
  /// </summary>
  internal class MembershipTableCleanupAgent
    ...
    public void Dispose() { this.cleanupDefunctSilosTimer?.Dispose(); }

    private async Task CleanupDefunctSilos() {
      if (!this.clusterMembershipOptions.DefunctSiloCleanupPeriod.HasValue) {
        if (this.log.IsEnabled(LogLevel.Debug)) {
          this.log.LogDebug(
              $"Membership table cleanup is disabled due to {nameof(ClusterMembershipOptions)}.{nameof(ClusterMembershipOptions.DefunctSiloCleanupPeriod)} not being specified");
        }
      ...
      try {
        ...
        var delay = random.NextTimeSpan(TimeSpan.FromMinutes(2),
                                        TimeSpan.FromMinutes(10));
        while (await this.cleanupDefunctSilosTimer.NextTick(delay)) {
          // Select a random time within the next window.
          // The purpose of this is to add jitter to a process which could be
          // affected by contention with other silos.
          delay = random.NextTimeSpan(period, period + TimeSpan.FromMinutes(5));
          try {
            var dateLimit = DateTime.UtcNow -
                            this.clusterMembershipOptions.DefunctSiloExpiration;
            await this.membershipTableProvider.CleanupDefunctSiloEntries(
                dateLimit);
          } catch (Exception exception)
              when(exception is NotImplementedException ||
            ...
            this.log.LogError(
                (int) ErrorCode.MembershipCleanDeadEntriesFailure,
                "Failed to clean up defunct membership table entries: {Exception}",
                exception);
          }
        }
      } finally {
    ...
    void ILifecycleParticipant<ISiloLifecycle>.Participate(
      ...
      lifecycle.Subscribe(nameof(MembershipTableCleanupAgent),
                          ServiceLifecycleStage.Active, OnStart, OnStop);
    ...
    bool IHealthCheckable.CheckHealth(DateTime lastCheckTime) {
      var ok =
          this.cleanupDefunctSilosTimer?.CheckHealth(lastCheckTime) ?? true;
      return ok;
    }
  }
}
