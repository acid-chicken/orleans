...
namespace Orleans.Runtime.Messaging {
  ...
  internal sealed class OutboundMessageQueue : IDisposable {
    ...
    public void SendMessage(Message msg) {
      ...
      if (stopped) {
        logger.Info(
            ErrorCode.Runtime_Error_100112,
            "Message was queued for sending after outbound queue was stopped: {0}",
            msg);
        return;
      }
      ...
      if (msg.TargetSilo == null) {
        logger.Error(ErrorCode.Runtime_Error_100113,
      ...
      if (!messageCenter.TrySendLocal(msg)) {
        if (stopped) {
          logger.Info(
              ErrorCode.Runtime_Error_100115,
              "Message was queued for sending after outbound queue was stopped: {0}",
              msg);
          return;
        }
        ...
        if (messageCenter.ShouldDrop?.Invoke(msg) == true) {
          logger.Info(ErrorCode.Messaging_SimulatedMessageLoss,
        ...
        if (this.siloStatusOracle.IsDeadSilo(msg.TargetSilo)) {
          ...
          var reason =
              $"Target {msg.TargetSilo.ToLongString()} silo is known to be dead";

          if (logger.IsEnabled(LogLevel.Debug)) {
            logger.LogDebug(
                (int) ErrorCode.MessagingSendingRejection,
                "Silo {siloAddress} is rejecting message: {message}. Reason = {reason}",
                messageCenter.MyAddress, msg, reason);
          }

          this.messageCenter.SendRejection(
              msg, Message.RejectionTypes.Transient, reason);
          return;
        }
