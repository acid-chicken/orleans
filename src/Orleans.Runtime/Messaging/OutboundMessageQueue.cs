...
namespace Orleans.Runtime.Messaging {
  ...
  internal sealed class OutboundMessageQueue : IDisposable {
    ...
    internal OutboundMessageQueue(MessageCenter mc,
    ...
    public void SendMessage(Message msg) {
      ...
      if (stopped) {
        logger.LogInformation(
            (int) ErrorCode.Runtime_Error_100115,
            "Message was queued for sending after outbound queue was stopped: {Message}",
            msg);
        messageCenter.SendRejection(
            msg, Message.RejectionTypes.Unrecoverable,
            "Message was queued for sending after outbound queue was stopped");
        return;
      }
      ...
      // First check to see if it's really destined for a proxied client,
      // instead of a local grain.
      ...
      if (msg.TargetSilo == null) {
        logger.LogError((int) ErrorCode.Runtime_Error_100113,
                        "Message does not have a target silo: " + msg +
                            " -- Call stack is: " + Utils.GetStackTrace());
        messageCenter.SendRejection(
            msg, Message.RejectionTypes.Unrecoverable,
            "Message to be sent does not have a target silo");
        return;
      }
      ...
      if (!messageCenter.TrySendLocal(msg)) {
        ...
        if (messageCenter.ShouldDrop?.Invoke(msg) == true) {
          logger.LogInformation((int) ErrorCode.Messaging_SimulatedMessageLoss,
                                "Message blocked by test");
          messageCenter.SendRejection(msg, Message.RejectionTypes.Unrecoverable,
                                      "Message blocked by test");
          return;
        }

        if (this.siloStatusOracle.IsDeadSilo(msg.TargetSilo)) {
          this.messagingTrace.OnRejectSendMessageToDeadSilo(
              this.messageCenter.MyAddress, msg);
          this.messageCenter.SendRejection(msg,
          ...
          async Task SendAsync(ValueTask<Connection>c, Message m) {
            try {
              ...
              this.messageCenter.SendRejection(
                  m, Message.RejectionTypes.Transient,
