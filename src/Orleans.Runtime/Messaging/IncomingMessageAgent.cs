...
namespace Orleans.Runtime.Messaging {
  internal class IncomingMessageAgent : TaskSchedulerAgent {
    ...
    internal
    IncomingMessageAgent(Message.Categories cat, IMessageCenter mc,
                         ActivationDirectory ad, OrleansTaskScheduler sched,
                         Dispatcher dispatcher, MessageFactory messageFactory,
    ...
    public override void Start() {
      ...
      if (Log.IsEnabled(LogLevel.Trace))
        Log.Trace(
            "Started incoming message agent for silo at {0} for {1} messages",
            messageCenter.MyAddress, category);
    }
    ...
    protected override async Task Run() {
      ...
      while (true) {
        ...
        var more =
            moreTask.IsCompletedSuccessfully ? moreTask.GetAwaiter().GetResult()
            : await moreTask;
        if (!more)
          return;
    ...
    private void ReceiveMessage(Message msg) {
      ...
      // Find the activation it targets; first check for a system activation,
      // then an app activation
      if (msg.TargetGrain.IsSystemTarget) {
        SystemTarget target = directory.FindSystemTarget(msg.TargetActivation);
        if (target == null) {
          MessagingStatisticsGroup.OnRejectedMessage(msg);
          Message response = this.messageFactory.CreateRejectionResponse(
              msg, Message.RejectionTypes.Unrecoverable,
              String.Format("SystemTarget {0} not active on this silo. Msg={1}",
                            msg.TargetGrain, msg));
          messageCenter.SendMessage(response);
          Log.Warn(ErrorCode.MessagingMessageFromUnknownActivation,
                   "Received a message {0} for an unknown SystemTarget: {1}",
        ...
        // Run this code on the target activation's context, if it already
        // exists
        ActivationData targetActivation =
            directory.FindTarget(msg.TargetActivation);
        if (targetActivation != null) {
          lock(targetActivation) {
            var target =
                targetActivation; // to avoid a warning about nulling
                                  // targetActivation under a lock on it
            if (target.State == ActivationState.Valid) {
              ...
              if (msg.Direction != Message.Directions.Response) {
                ...
                if (overloadException != null) {
                  // Send rejection as soon as we can, to avoid creating
                  // additional work for runtime
                  dispatcher.RejectMessage(
                      msg, Message.RejectionTypes.Overloaded, overloadException,
                      "Target activation is overloaded " + target);
                  return;
                }
              }
          ...
          // No usable target activation currently, so run ReceiveMessage in
          // system context
    ...
    private void EnqueueReceiveMessage(Message msg,
                                       ActivationData targetActivation,
                                       ISchedulingContext context) {
      this.messagingTrace.OnEnqueueMessageOnActivation(msg, context);
