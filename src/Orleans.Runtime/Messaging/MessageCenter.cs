...
namespace Orleans.Runtime.Messaging {
  internal class MessageCenter : ISiloMessageCenter, IDisposable {
    ...
    internal OutboundMessageQueue OutboundQueue {
      get;
      set;
    }
    ...
    internal bool IsBlockingApplicationMessages {
      get;
      private set;
    }

    public void
    SetHostedClient(IHostedClient client) => this.hostedClient = client;

    public bool IsProxying =>
        this.Gateway != null || this.hostedClient?.ClientId != null;

    public bool TryDeliverToProxy(Message msg) {
      if (msg.TargetGrain == null || !msg.TargetGrain.IsClient)
        return false;
      if (this.Gateway != null && this.Gateway.TryDeliverToProxy(msg))
        return true;
      return this.hostedClient?.TryDispatchToClient(msg) ?? false;
    }

    // This is determined by the IMA but needed by the OMS, and so is kept here
    // in the message center itself.
    public SiloAddress MyAddress {
      get;
      private set;
    }

    public MessageCenter(ILocalSiloDetails siloDetails,
      ...
      inboundQueue = new InboundMessageQueue(
          this.loggerFactory.CreateLogger<InboundMessageQueue>(),
          statisticsOptions, this.messagingTrace);
      OutboundQueue = new OutboundMessageQueue(
          this, this.loggerFactory.CreateLogger<OutboundMessageQueue>(),
          this.senderManager, siloStatusOracle, this.messagingTrace);
      ...
      messageHandlers = new Action<Message>[
    ...
    private void WaitToRerouteAllQueuedMessages() {
      DateTime maxWaitTime =
          DateTime.UtcNow + this.messagingOptions.ShutdownRerouteTimeout;
      while (DateTime.UtcNow < maxWaitTime) {
        var applicationMessageQueueLength =
    ...
    public void StopAcceptingClientMessages() {
      ...
      try {
        ...
        log.Error(ErrorCode.Runtime_Error_100109, "Stop failed.", exc);
      }
      Gateway = null;
    }
      ...
      set {
        if (rerouteHandler != null)
          throw new InvalidOperationException(
              "MessageCenter RerouteHandler already set");
    ...
    public Action<Message>SniffIncomingMessage {
      set {
        if (this.sniffIncomingMessageHandler != null)
          throw new InvalidOperationException(
              "IncomingMessageAcceptor SniffIncomingMessage already set");
    ...
    public void SendMessage(Message msg) {
      // Note that if we identify or add other grains that are required for
      ...
      if (IsBlockingApplicationMessages &&
          ...
          (msg.Result != Message.ResponseTypes.Rejection) &&
          !Constants.SystemMembershipTableId.Equals(msg.TargetGrain)) {
        // Drop the message on the floor if it's an application message that
        // isn't a rejection
    ...
    internal void SendRejection(Message msg,
                                Message.RejectionTypes rejectionType,
                                string reason) {
      MessagingStatisticsGroup.OnRejectedMessage(msg);
      if (string.IsNullOrEmpty(reason))
        reason = string.Format("Rejection from silo {0} - Unknown reason.",
                               MyAddress);
      Message error = this.messageFactory.CreateRejectionResponse(
          msg, rejectionType, reason);
      // rejection msgs are always originated in the local silo, they are never
      // remote.
      this.OnReceivedMessage(error);
    }

    public ChannelReader<Message>
    GetReader(Message.Categories type) => inboundQueue.GetReader(type);
    ...
    public int SendQueueLength {
      get { return OutboundQueue.GetCount(); }
    }

    public int ReceiveQueueLength {
      get { return inboundQueue.Count; }
    }

    /// <summary>
    /// Indicates that application messages should be blocked from being sent or
    /// received. This method is used by the "fast stop" process. <para>
    /// Specifically, all outbound application messages are dropped, except for
    /// rejections and messages to the membership table grain. Inbound
    /// application requests are rejected, and other inbound application
    /// messages are dropped.
    /// </para>
    /// </summary>
    public void BlockApplicationMessages() {
      if (log.IsEnabled(LogLevel.Debug))
        log.Debug("BlockApplicationMessages");
      IsBlockingApplicationMessages = true;
    }
  }
}
