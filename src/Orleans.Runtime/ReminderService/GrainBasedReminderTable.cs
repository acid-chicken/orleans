...
namespace Orleans.Runtime.ReminderService {
  [Reentrant]
  internal class GrainBasedReminderTable : Grain,
                                           IReminderTableGrain {
    ...
    public override Task OnActivateAsync() {
      ...
      logger = loggerFactory.CreateLogger(
          String.Format("{0}_{1}", typeof (GrainBasedReminderTable).FullName,
                        Data.Address.ToString()));
      logger.Info("GrainBasedReminderTable {0} Activated. Full identity: {1}",
                  Identity, Data.Address.ToFullString());
      remTable = new Table(loggerFactory);
      base.DelayDeactivation(TimeSpan.FromDays(
          10 * 365)); // Delay Deactivation for GrainBasedReminderTable
    ...
    public override Task OnDeactivateAsync() {
      logger.Info(
          "GrainBasedReminderTable {0} OnDeactivateAsync. Full identity: {1}",
          Identity, Data.Address.ToFullString());
      return Task.CompletedTask;
    }
    ...
    public Task<ReminderTableData>ReadRows(uint begin, uint end) {
      ...
      logger.Debug("Read {0} reminders from memory: {1}, {2}",
                   t.Reminders.Count, Environment.NewLine,
                   Utils.EnumerableToString(t.Reminders));
      return Task.FromResult(t);
    }

    public Task<ReminderEntry>ReadRow(GrainReference grainRef,
                                      string reminderName) {
    ...
    /// <returns>true if a row with <paramref name="grainRef"/> and <paramref
    /// name="reminderName"/> existed and was removed successfully, false
    /// otherwise</returns>
    public Task<bool>RemoveRow(GrainReference grainRef, string reminderName,
                               string eTag) {
      if (logger.IsEnabled(LogLevel.Debug))
        logger.Debug(
            "RemoveRow entry grainRef = {0}, reminderName = {1}, eTag = {2}",
            grainRef, reminderName, eTag);
      bool result = remTable.RemoveRow(grainRef, reminderName, eTag);
      if (result == false) {
        logger.Warn(
            ErrorCode.RS_Table_Remove,
            "RemoveRow failed for grainRef = {0}, ReminderName = {1}, eTag = {2}. Table now is: {3}",
            grainRef.ToDetailedString(), reminderName, eTag,
            remTable.ReadAll());
      }
    ...
    private class Table {
      ...
      private Dictionary<GrainReference, Dictionary<string, ReminderEntry>>
          reminderTable;
      ...
      public ReminderTableData ReadRows(uint begin, uint end) {
        ...
        IEnumerable<GrainReference>keys =
            reminderTable.Keys.Where(range.InRange);
        ...
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              "Selected {0} out of {1} reminders from memory for {2}. List is: {3}{4}",
              list.Count, reminderTable.Count, range.ToString(),
              Environment.NewLine,
      ...
      public ReminderEntry ReadRow(GrainReference grainRef,
                                   string reminderName) {
        ...
        if (logger.IsEnabled(LogLevel.Trace)) {
          if (result == null)
            logger.Trace("Reminder not found for grain {0} reminder {1} ",
                         grainRef, reminderName);
          else
            logger.Trace("Read for grain {0} reminder {1} row {2}", grainRef,
                         reminderName, result.ToString());
        }
        return result;
      }
      ...
      public string UpsertRow(ReminderEntry entry) {
        ...
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace("Upserted entry {0}, replaced {1}", entry, old);
        return entry.ETag;
      }

      public bool RemoveRow(GrainReference grainRef, string reminderName,
                            string eTag) {
        Dictionary<string, ReminderEntry>data = null;
        ReminderEntry e = null;

        // assuming the calling grain executes one call at a time, so no need to
        // lock
      ...
      // use only for internal printing during testing ... the reminder table
      // can be huge in a real deployment!
      ...
      public void Reset() {
        reminderTable =
            new Dictionary<GrainReference, Dictionary<string, ReminderEntry>>();
      }
    }
  }
}
