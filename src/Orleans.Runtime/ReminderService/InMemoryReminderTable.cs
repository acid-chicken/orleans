...
namespace Orleans.Runtime.ReminderService {
  internal class InMemoryReminderTable : IReminderTable,
    ...
    public InMemoryReminderTable(IGrainFactory grainFactory) {
      this.reminderTableGrain =
          grainFactory.GetGrain<IReminderTableGrain>(ReminderTableGrainId);
    }
    ...
    public Task<ReminderTableData>ReadRows(uint begin, uint end) {
      return this.isAvailable ? this.reminderTableGrain
          .ReadRows(begin, end)
    ...
    private void ThrowIfNotAvailable() {
      if (!this.isAvailable)
        throw new InvalidOperationException(
            "The reminder service is not currently available.");
    }
    ...
    void ILifecycleParticipant<ISiloLifecycle>.Participate(
      ...
      lifecycle.Subscribe(nameof(GrainBasedReminderTable),
