...
internal class LocalReminderService : GrainService, IReminderService
{
    ...
    public override async Task OnRangeChange(IRingRange oldRange, IRingRange newRange, bool increased)
    {
        ...
        else if (logger.IsEnabled(LogLevel.Debug)) logger.Debug("Ignoring range change until ReminderService is Started -- Current status = {0}", Status);
    }
    ...
    private class LocalReminderData
    {
        ...
        public ReminderIdentity Identity {
            get;
        }
    ...
    {
        public readonly GrainReference GrainRef {
            get;
        }
