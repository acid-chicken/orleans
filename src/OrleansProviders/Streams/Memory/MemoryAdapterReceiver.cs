...
namespace Orleans.Providers {
  internal class MemoryAdapterReceiver<TSerializer>
      : IQueueAdapterReceiver where TSerializer : class,
    ...
    public MemoryAdapterReceiver(IMemoryStreamQueueGrain queueGrain,
                                 ILogger logger, TSerializer serializer,
    ...
    GetQueueMessagesAsync(int maxCount) {
      ...
      try {
        ...
        batches = eventData
                      .Select(data => new MemoryBatchContainer<TSerializer>(
        ...
        if (eventData.Count > 0) {
          ...
          this.receiverMonitor?.TrackMessagesReceived(
              eventData.Count, oldestMessage.EnqueueTimeUtc,
              newestMessage.EnqueueTimeUtc);
        }
      } catch (Exception exc) {
        logger.LogError(
            (int) ProviderErrorCode
                .MemoryStreamProviderBase_GetQueueMessagesAsync,
            exc,
