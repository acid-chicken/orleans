...
namespace Orleans.Providers.Streams.Common {
  ...
  public class SimpleQueueCacheCursor : IQueueCacheCursor {
    ...
    private IBatchContainer
        current; // this is a pointer to the current element in the cache. It is
                 // what will be returned by GetCurrent().

    // This is also a pointer to the current element in the cache. It differs
    // from current, in that current is just the batch, and is null before the
    // first call to MoveNext after construction. (Or after refreshing if we had
    // previously run out of batches). Upon MoveNext being called in that
    // situation, current gets set to the batch included in Element. That is
    // needed to implement the Enumerator pattern properly, since in that
    // pattern MoveNext gets called before the first access of (Get)Current.

    internal LinkedListNode<SimpleQueueCacheItem>Element {
      get;
      private set;
    }
    ...
    public SimpleQueueCacheCursor(SimpleQueueCache cache,
      ...
      this.logger = logger;
      current = null;
      SimpleQueueCache.Log(
          logger, "SimpleQueueCacheCursor New Cursor for {Guid}, {NameSpace}",
    ...
    /// If there was a stream error, an error exception will be provided in the
    /// output.
    /// </returns>
    public virtual IBatchContainer GetCurrent(out Exception exception) {
      SimpleQueueCache.Log(
          logger, "SimpleQueueCacheCursor.GetCurrent: {Current}", current);
    ...
    /// If it returns false, there are no more messages.  The enumerator is
    /// still
    ///  valid however and can be called again when more data has come in on
    ///  this stream.
    /// </summary>
    /// <returns></returns>
    ...
    private bool IsInStream(IBatchContainer batchContainer) {
      return batchContainer != null &&
             ...
             string.Equals(batchContainer.StreamNamespace,
    ...
    public override string ToString() {
      return $"<SimpleQueueCacheCursor: Element={Element?.Value.Batch.ToString() ?? " null
             "}, SequenceToken={SequenceToken?.ToString() ?? " null "}>";
    }
  }
}
