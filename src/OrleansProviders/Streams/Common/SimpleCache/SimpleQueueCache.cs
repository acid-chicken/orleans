...
namespace Orleans.Providers.Streams.Common {
  internal class CacheBucket {
    ...
    // We update the NumCurrentItems when we add and remove cache item
    // (potentially opening or removing a bucket) We update NumCurrentCursors
    // every time we move a cursor If the first (most outdated bucket) has at
    // least one cursor pointing to it, we say we are under back pressure (in a
    // full cache).
    internal int NumCurrentItems {
      get;
      private set;
    }
    internal int NumCurrentCursors {
      get;
      private set;
    }
  ...
  public class SimpleQueueCache : IQueueCache {
    ...
    private readonly List<CacheBucket>
        cacheCursorHistogram; // for backpressure detection
    ...
    /// </summary>
    public int GetMaxAddCount() { return CACHE_HISTOGRAM_MAX_BUCKET_SIZE; }
    ...
    public SimpleQueueCache(int cacheSize, ILogger logger) {
      ...
      CACHE_HISTOGRAM_MAX_BUCKET_SIZE = Math.Max(
          cacheSize / NUM_CACHE_HISTOGRAM_BUCKETS, 1); // we have 10 buckets
    }
    ...
    public virtual bool
    TryPurgeFromCache(out IList<IBatchContainer>purgedItems) {
      purgedItems = null;
      if (cachedMessages.Count == 0)
        return false; // empty cache
      if (cacheCursorHistogram.Count == 0)
        return false; // no cursors yet - zero consumers basically yet.
      if (cacheCursorHistogram[0].NumCurrentCursors > 0)
        return false; // consumers are still active in the oldest bucket - fast
                      // path

      var allItems = new List<IBatchContainer>();
      while (cacheCursorHistogram.Count > 0 &&
             cacheCursorHistogram[0].NumCurrentCursors == 0) {
      ...
      Log(logger,
          "TryPurgeFromCache: purged {PurgedItemsCount} items from cache.",
          purgedItems.Count);
      return true;
    }
    ...
    private List<IBatchContainer>DrainBucket(CacheBucket bucket) {
      ...
      // and remove from the cache the oness that reside in the given bucket
      // until we jump to a next bucket
    ...
    public virtual void AddToCache(IList<IBatchContainer>msgs) {
      ...
      foreach(var message in msgs) { Add(message, message.SequenceToken); }
    }

    /// <summary>
    /// Acquire a stream message cursor.  This can be used to retrieve messages
    /// from the
    ...
    /// <returns></returns>
    public virtual IQueueCacheCursor
    GetCacheCursor(IStreamIdentity streamIdentity, StreamSequenceToken token) {
      var cursor = new SimpleQueueCacheCursor(this, streamIdentity, logger);
      InitializeCursor(cursor, token);
      return cursor;
    }

    internal void InitializeCursor(SimpleQueueCacheCursor cursor,
                                   StreamSequenceToken sequenceToken) {
      Log(logger, "InitializeCursor: {Cursor} to sequenceToken {SequenceToken}",
          cursor, sequenceToken);
      ...
      // If sequenceToken is too new to be in cache, unset token, and wait for
      // more data.
      ...
      if (sequenceToken.Older(lastMessage.Value.SequenceToken)) {
        throw new QueueCacheMissException(
            sequenceToken, cachedMessages.Last.Value.SequenceToken,
            cachedMessages.First.Value.SequenceToken);
      }

      // Now the requested sequenceToken is set and is also within the limits of
      // the cache.

      // Find first message at or below offset
      // Events are ordered from newest to oldest, so iterate from start of list
      // until we hit a node at a previous offset, or the end.
    ...
    internal void RefreshCursor(SimpleQueueCacheCursor cursor,
                                StreamSequenceToken sequenceToken) {
      Log(logger,
          "RefreshCursor: {RefreshCursor} to sequenceToken {SequenceToken}",
          cursor, sequenceToken);
    ...
    /// <returns></returns>
    internal bool TryGetNextMessage(SimpleQueueCacheCursor cursor,
                                    out IBatchContainer batch) {
      ...
      // If we are at the end of the cache unset cursor and move offset one
      // forward
    ...
    private void AdvanceCursor(SimpleQueueCacheCursor cursor,
                               LinkedListNode<SimpleQueueCacheItem>item) {
      Log(logger, "UpdateCursor: {Cursor} to item {Item}", cursor,
          item.Value.Batch);

      cursor.Element.Value.CacheBucket.UpdateNumCursors(
          -1); // remove from prev bucket
      cursor.Set(item);
      cursor.Element.Value.CacheBucket.UpdateNumCursors(
          1); // add to next bucket
    }

    internal void SetCursor(SimpleQueueCacheCursor cursor,
                            LinkedListNode<SimpleQueueCacheItem>item) {
      Log(logger, "SetCursor: {Cursor} to item {Item}", cursor,
          item.Value.Batch);
    ...
    internal void UnsetCursor(SimpleQueueCacheCursor cursor,
    ...
    private void Add(IBatchContainer batch, StreamSequenceToken sequenceToken) {
      ...
      if (cacheCursorHistogram.Count == 0) {
        ...
        cacheBucket =
            cacheCursorHistogram[cacheCursorHistogram.Count - 1]; // last one
      }

      if (cacheBucket.NumCurrentItems ==
          CACHE_HISTOGRAM_MAX_BUCKET_SIZE) // last bucket is full, open a new
                                           // one
      {
        cacheBucket = new CacheBucket();
        cacheCursorHistogram.Add(cacheBucket);
      ...
      var item =
          new SimpleQueueCacheItem{Batch = batch, SequenceToken = sequenceToken,
                                   CacheBucket = cacheBucket};
    ...
    internal static void Log(ILogger logger, string format,
                             params object[] args) {
