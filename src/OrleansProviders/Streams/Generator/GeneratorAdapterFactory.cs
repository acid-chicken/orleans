...
namespace Orleans.Providers.Streams.Generator {
  ...
  /// This factory acts as the adapter and the adapter factory.  It creates
  /// receivers that use configurable generator
  ///   to generate event streams, rather than reading them from storage.
  /// </summary>
  public class GeneratorAdapterFactory : IQueueAdapterFactory,
                                         IQueueAdapter,
                                         IQueueAdapterCache,
                                         IControllable {
    /// <summary>
    ...
    public StreamProviderDirection Direction =>
        StreamProviderDirection.ReadOnly;
    ...
    public GeneratorAdapterFactory(
      ...
      this.statisticOptions =
          statisticOptions ?? throw new ArgumentNullException(
              nameof(statisticOptions));
      this.serviceProvider = serviceProvider ?? throw new ArgumentNullException(
          nameof(serviceProvider));
      this.serializationManager =
          serializationManager ?? throw new ArgumentNullException(
              nameof(serializationManager));
      ...
      if (this.ReceiverMonitorFactory == null)
        this.ReceiverMonitorFactory = (dimensions, telemetryProducer) =>
            new DefaultQueueAdapterReceiverMonitor(dimensions,
                                                   telemetryProducer);
      generatorConfig =
          this.serviceProvider.GetServiceByName<IStreamGeneratorConfig>(
              this.Name);
    ...
    public IStreamQueueMapper GetStreamQueueMapper() {
      return streamQueueMapper ??(streamQueueMapper =
    ...
    /// <returns></returns>
    public Task
    QueueMessageBatchAsync<T>(Guid streamGuid, string streamNamespace,
                              IEnumerable<T>events, StreamSequenceToken token,
    ...
    public IQueueAdapterReceiver CreateReceiver(QueueId queueId) {
      ...
      Receiver receiver =
          receivers.GetOrAdd(queueId, qid => new Receiver(receiverMonitor));
    ...
    public Task<object>ExecuteCommand(int command, object arg) {
      ...
      if (generatorConfig == null) {
        throw new ArgumentOutOfRangeException(
            "arg", "Arg must by of type IStreamGeneratorConfig");
      }
    ...
    private class Receiver : IQueueAdapterReceiver {
      ...
      public IStreamGenerator QueueGenerator {
        private get;
        set;
      }
      ...
      public Task Initialize(TimeSpan timeout) {
        this.receiverMonitor?.TrackInitialization(true, TimeSpan.MinValue,
                                                  null);
        return Task.CompletedTask;
      }
      ...
      GetQueueMessagesAsync(int maxCount) {
        ...
        if (QueueGenerator == null ||
            !QueueGenerator.TryReadEvents(DateTime.UtcNow, maxCount,
                                          out batches)) {
        ...
        if (batches.Count > 0) {
          ...
          var newestMessage =
              batches[batches.Count - 1] as GeneratedBatchContainer;
          this.receiverMonitor?.TrackMessagesReceived(
              batches.Count, oldestMessage?.EnqueueTimeUtc,
      ...
      if (generatorConfig == null) {
        ...
        if (generator == null) {
          throw new OrleansException(
              $"StreamGenerator type not supported: {generatorConfig.StreamGeneratorType}");
        }
    ...
    public IQueueCache CreateQueueCache(QueueId queueId) {
      ...
      var dimensions = new CacheMonitorDimensions(
          queueId.ToString(), this.blockPoolMonitorDimensions.BlockPoolId);
      var cacheMonitor =
          this.CacheMonitorFactory(dimensions, this.telemetryProducer);
      return new GeneratorPooledCache(
          bufferPool,
          this.loggerFactory.CreateLogger(
              $"{typeof(GeneratorPooledCache).FullName}.{this.Name}.{queueId}"),
          serializationManager, cacheMonitor,
          this.statisticOptions.StatisticMonitorWriteInterval);
    }

    public static GeneratorAdapterFactory Create(IServiceProvider services,
                                                 string name) {
      ...
      var factory = ActivatorUtilities.CreateInstance<GeneratorAdapterFactory>(
          services, name, queueMapperOptions, statisticOptions);
      factory.Init();
      return factory;
    }
  }
