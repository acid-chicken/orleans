...
namespace Orleans.Storage {
  ...
  /// </summary>
  /// <remarks>
  /// This storage provider is ONLY intended for simple in-memory Development /
  /// Unit Test scenarios. This class should NOT be used in Production
  /// environment,
  ///  because [by-design] it does not provide any resilience
  ///  or long-term persistence capabilities.
  /// </remarks>
  /// <example>
  /// Example configuration for this storage provider in
  /// OrleansConfiguration.xml file: <code> &lt;OrleansConfiguration
  /// xmlns="urn:orleans">
  ///   &lt;Globals>
  ///     &lt;StorageProviders>
  ...
  public class MemoryGrainStorage : IGrainStorage,
    ...
    public MemoryGrainStorage(string name, MemoryGrainStorageOptions options,
      ...
      logger.LogInformation(
          "Init: Name={Name} NumStorageGrains={NumStorageGrains}", name,
          options.NumStorageGrains);
      ...
      for (int i = 0; i < storageGrains.Length; i++) {
        ...
        storageGrains[idx] = new Lazy<IMemoryStorageGrain>(
            () => grainFactory.GetGrain<IMemoryStorageGrain>(idx));
      }
    }

    /// <summary> Read state data function for this storage provider. </summary>
    /// <see cref="IGrainStorage.ReadStateAsync"/>
    public virtual async Task ReadStateAsync(string grainType,
                                             GrainReference grainReference,
                                             IGrainState grainState) {
      var keys = MakeKeys(grainType, grainReference);

      if (logger.IsEnabled(LogLevel.Trace))
        logger.LogTrace("Read Keys={Keys}",
    ...
    public virtual async Task WriteStateAsync(string grainType,
                                              GrainReference grainReference,
                                              IGrainState grainState) {
      var keys = MakeKeys(grainType, grainReference);
      string key = HierarchicalKeyStore.MakeStoreKey(keys);
      if (logger.IsEnabled(LogLevel.Trace))
        logger.LogTrace("Write {Write} ",
                        StorageProviderUtils.PrintOneWrite(
                            keys, grainState.State, grainState.ETag));
      IMemoryStorageGrain storageGrain = GetStorageGrain(key);
      try {
        grainState.ETag = await storageGrain.WriteStateAsync(STATE_STORE_NAME,
                                                             key, grainState);
    ...
    /// <summary> Delete / Clear state data function for this storage provider.
    /// </summary> <see cref="IGrainStorage.ClearStateAsync"/>
    public virtual async Task ClearStateAsync(string grainType,
                                              GrainReference grainReference,
                                              IGrainState grainState) {
      var keys = MakeKeys(grainType, grainReference);
      if (logger.IsEnabled(LogLevel.Trace))
        logger.LogTrace("Delete Keys={Keys} Etag={Etag}",
                        StorageProviderUtils.PrintKeys(keys), grainState.ETag);
      string key = HierarchicalKeyStore.MakeStoreKey(keys);
      IMemoryStorageGrain storageGrain = GetStorageGrain(key);
      try {
    ...
    private static Tuple<string, string>[] MakeKeys(string grainType,
    ...
    private IMemoryStorageGrain GetStorageGrain(string id) {
      int idx = StorageProviderUtils.PositiveHash(id.GetHashCode(),
                                                  this.storageGrains.Length);
  ...
  public class MemoryGrainStorageFactory {
    public static IGrainStorage Create(IServiceProvider services, string name) {
      return ActivatorUtilities.CreateInstance<MemoryGrainStorage>(
          services,
          services
