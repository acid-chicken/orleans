...
namespace Orleans.EventSourcing.Common {
  ...
  /// a variety of different log-consistency providers, all following the same
  /// basic pattern (read and write latest view from/to primary, and send
  /// notifications after writing).
  ///< para>
  /// Note that the log itself is transient, i.e. not actually saved to storage
  /// - only the latest view and some metadata (the log position, and write
  /// flags) is stored in the primary. It is safe to interleave calls to this
  /// adaptor (using grain scheduler only, of course).
  /// </para>
  ///< para>
  /// Subclasses override ReadAsync and WriteAsync to read from / write to
  /// primary. Calls to the primary are serialized, i.e. never interleave.
  /// </para>
  /// </summary>
  ...
  /// <typeparam name="TSubmissionEntry">The type of submission entries stored
  /// in pending queue</typeparam>
  public abstract class PrimaryBasedLogViewAdaptor<TLogView, TLogEntry,
                                                   TSubmissionEntry>
      : ILogViewAdaptor<TLogView, TLogEntry>where TLogView
      : class,
        new () where TLogEntry : class where TSubmissionEntry
    ...
    /// Should not throw exceptions, but record them in <see
    /// cref="LastPrimaryIssue"/>
    /// </summary>
    /// <returns></returns>
    ...
    /// Using a type parameter so we can add protocol-specific info to this
    /// class.
    /// </summary>
    /// <returns></returns>
    ...
    /// </summary>
    protected virtual bool SupportSubmissions {
      get { return true; }
    }
    ...
    /// </summary>
    protected virtual Task<ILogConsistencyProtocolMessage>
    OnMessageReceived(ILogConsistencyProtocolMessage payload) {
      // subclasses that define custom protocol messages must override this
      throw new NotImplementedException();
    }

    public virtual Task<IReadOnlyList<TLogEntry>>
    RetrieveLogSegment(int fromVersion, int length) {
      throw new NotSupportedException();
    }

    /// <summary>
    /// Handle notification messages. Override this to handle notification
    /// subtypes.
    /// </summary>
    protected virtual void
    OnNotificationReceived(INotificationMessage payload) {
      ...
      if (batchmsg != null) {
        foreach(var bm in batchmsg.Notifications) OnNotificationReceived(bm);
        return;
      }

      // subclass should have handled this in override
      throw new ProtocolTransportException(string.Format(
          "message type {0} not handled by OnNotificationReceived",
    ...
    /// <summary>
    /// Process stored notifications during worker cycle. Override to handle
    /// notification subtypes.
    /// </summary>
    protected virtual void ProcessNotifications() {
      if (lastVersionNotified > this.GetConfirmedVersion()) {
        Services.Log(LogLevel.Debug,
                     "force refresh because of version notification v{0}",
                     lastVersionNotified);
        needRefresh = true;
      }
    }
    ...
    /// <summary>
    /// Merge two notification messages, for batching. Override to handle
    /// notification subtypes.
    /// </summary>
    protected virtual INotificationMessage
    Merge(INotificationMessage earliermessage,
          INotificationMessage latermessage) {
      return new VersionNotificationMessage(){Version = latermessage.Version};
    }

    /// <summary>
    /// The grain that is using this adaptor.
    /// </summary>
    protected ILogViewAdaptorHost<TLogView, TLogEntry>Host {
      get;
      private set;
    }

    /// <summary>
    /// The runtime services required for implementing notifications between
    /// grain instances in different cluster.
    /// </summary>
    protected ILogConsistencyProtocolServices Services {
      get;
      private set;
    }
    ...
    protected PrimaryBasedLogViewAdaptor(
        ILogViewAdaptorHost<TLogView, TLogEntry>host, TLogView initialstate,
    ...
    public virtual Task PreOnActivate() {
      ...
      // we do not act on this yet, but wait until after user OnActivate has
      // run.
    ...
    /// called at beginning of WriteAsync to get current number of pending
    /// updates
    protected int GetNumberPendingUpdates() { return pending.Count; }
    ...
    /// <summary>
    /// A flag that indicates to the worker that the client wants to refresh the
    /// state
    /// </summary>
    private bool needRefresh;

    /// <summary>
    /// A flag that indicates that we have not read global state at all yet, and
    /// should do so
    /// </summary>
    ...
    /// For use by protocols. Determines if this cluster is part of the
    /// configured multicluster.
    protected bool IsMyClusterJoined() { return true; }
    ...
    public void Submit(TLogEntry logEntry) {
      if (!SupportSubmissions)
        throw new InvalidOperationException(
            "provider does not support submissions on cluster " +
            Services.MyClusterId);

      if (stats != null)
        stats.EventCounters["SubmitCalled"]++;
    ...
    public void SubmitRange(IEnumerable<TLogEntry>logEntries) {
      ...
      if (stats != null)
        stats.EventCounters["SubmitRangeCalled"]++;

      Services.Log(LogLevel.Trace, "SubmitRange");

      var time = DateTime.UtcNow;

      foreach(var e in logEntries) SubmitInternal(time, e);
    ...
    public Task<bool>TryAppend(TLogEntry logEntry) {
      ...
      if (stats != null)
        stats.EventCounters["TryAppendCalled"]++;
      ...
      SubmitInternal(DateTime.UtcNow, logEntry,
                     GetConfirmedVersion() + pending.Count, promise);
    ...
    public Task<bool>TryAppendRange(IEnumerable<TLogEntry>logEntries) {
      ...
      if (stats != null)
        stats.EventCounters["TryAppendRangeCalled"]++;
    ...
    private void
    SubmitInternal(DateTime time, TLogEntry logentry,
                   int conditionalPosition = unconditional,
                   TaskCompletionSource<bool>resultPromise = null) {
      ...
      if (this.tentativeStateInternal != null) {
        try {
          ...
          Services.CaughtUserCodeException("UpdateView", nameof(SubmitInternal),
                                           e);
        }
      }
      ...
      try {
        ...
        Services.CaughtUserCodeException("OnViewChanged",
                                         nameof(SubmitInternal), e);
      }
    }
    ...
    /// <returns></returns>
    public async Task<ILogConsistencyProtocolMessage>
    OnProtocolMessageReceived(ILogConsistencyProtocolMessage payLoad) {
      var notificationMessage = payLoad as INotificationMessage;

      if (notificationMessage != null) {
        Services.Log(LogLevel.Debug, "NotificationReceived v{0}",
                     notificationMessage.Version);
    ...
    /// </summary>
    /// <returns></returns>
    public LogConsistencyStatistics GetStats() { return stats; }

    private void CalculateTentativeState() {
      // copy the master
      this.tentativeStateInternal =
          (TLogView)
              Services.SerializationManager.DeepCopy(LastConfirmedView());

      // Now apply all operations in pending
      foreach(var u in this.pending) try {
        Host.UpdateView(this.tentativeStateInternal, u.Entry);
      } catch (Exception e) {
        Services.CaughtUserCodeException("UpdateView",
                                         nameof(CalculateTentativeState), e);
      }
    }
    ...
    internal async Task Work() {
      ...
      Services.Log(LogLevel.Debug, "<3 Storage htr={0} htw={1}", haveToRead,
                   haveToWrite);
      ...
      try {
        ...
        if (haveToWrite) {
          ...
          if (stats != null)
            stats.EventCounters["WritebackEvents"]++;
        }

      } catch (Exception e) {
        // this should never happen - we are supposed to catch and store
        // exceptions in the correct place (LastPrimaryException or notification
        // trackers)
    ...
    /// <summary>
    /// This function stores the operations in the pending queue as a batch to
    /// the primary. Retries until some batch commits or there are no updates
    /// left.
    /// </summary>
    internal async Task UpdatePrimary() {
      ...
      while (true) {
        try {
          ...
          // this should never happen - we are supposed to catch and store
          // exceptions in the correct place (LastPrimaryException or
          // notification trackers)
          Services.ProtocolError($"Exception in {nameof(UpdatePrimary)}: {e}",
                                 true);
        }
      }
    }
    ...
    public IEnumerable<TLogEntry>UnconfirmedSuffix {
      get { return pending.Select(te => te.Entry); }
    }
    ...
    /// <summary>
    /// go through updates and remove all the conditional updates that have
    /// already failed
    /// </summary>
    protected void RemoveStaleConditionalUpdates() {
      ...
      if (foundFailedConditionalUpdates) {
        ...
        try {
          ...
          Services.CaughtUserCodeException(
              "OnViewChanged", nameof(RemoveStaleConditionalUpdates), e);
        }
      }
    }
  }
  ...
  /// </summary>
  /// <typeparam name="TLogEntry">The type of entry for this
  /// submission</typeparam>
  public class SubmissionEntry<TLogEntry> {
    ...
    /// <summary> For conditional updates, a promise that resolves once it is
    /// known whether the update was successful or not.</summary>
    public TaskCompletionSource<bool>ResultPromise;

    /// <summary> For conditional updates, the log position at which this update
    /// is supposed to be applied. </summary>
