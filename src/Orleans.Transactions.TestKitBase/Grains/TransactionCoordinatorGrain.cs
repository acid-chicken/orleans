...
namespace Orleans.Transactions.TestKit {
  [StatelessWorker]
  public class TransactionCoordinatorGrain : Grain,
    ...
    public Task MultiGrainAdd(List<ITransactionTestGrain>grains,
                              int numberToAdd) {
    ...
    public async Task AddAndThrow(ITransactionTestGrain grain,
                                  int numberToAdd) {
    ...
    public async Task
    MultiGrainAddAndThrow(List<ITransactionTestGrain>throwGrains,
    ...
    public Task MultiGrainSetBit(List<ITransactionalBitArrayGrain>grains,
                                 int bitIndex) {
      return Task.WhenAll(grains.Select(g => g.SetBit(bitIndex)));
    }

    public Task
    MultiGrainAdd(ITransactionCommitterTestGrain committer,
    ...
    public async Task MultiGrainDoubleByRWRW(List<ITransactionTestGrain>grains,
                                             int numberToAdd) {
    ...
    public async Task MultiGrainDoubleByWRWR(List<ITransactionTestGrain>grains,
                                             int numberToAdd) {
      await Task.WhenAll(grains.Select(g => g.Add(numberToAdd)));
      await Task.WhenAll(grains.Select(g => g.Get()));
      await Task.WhenAll(grains.Select(g => g.Add(numberToAdd)));
