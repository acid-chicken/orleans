...
namespace Orleans.Transactions.TestKit {
  public interface INoAttributionGrain : IGrainWithGuidKey {
    Task<List<string>[]>GetNestedTransactionIds(
        int tier, List<ITransactionAttributionGrain>[] tiers);
  }

  public interface ISuppressAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOption.Suppress)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }

  public interface ICreateOrJoinAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOption.CreateOrJoin)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }

  public interface ICreateAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOption.Create)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }

  public interface IJoinAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOptionAlias.Mandatory)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }

  public interface ISupportedAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOption.Supported)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }

  public interface INotAllowedAttributionGrain : IGrainWithGuidKey {
    [Transaction(TransactionOption.NotAllowed)] Task<List<string>[]>
    GetNestedTransactionIds(int tier,
                            List<ITransactionAttributionGrain>[] tiers);
  }
...
#region wrappers
  ...
  public static class TransactionAttributionGrainExtensions {
    public static ITransactionAttributionGrain
    GetTransactionAttributionGrain(this IGrainFactory grainFactory, Guid id,
                                   TransactionOption? option = null) {
      if (!option.HasValue) {
        return new NoAttributionGrain(
            grainFactory.GetGrain<INoAttributionGrain>(id));
      }
      switch (option.Value) {
      case TransactionOption.Suppress:
        return new SuppressAttributionGrain(
            grainFactory.GetGrain<ISuppressAttributionGrain>(id));
      case TransactionOption.CreateOrJoin:
        return new CreateOrJoinAttributionGrain(
            grainFactory.GetGrain<ICreateOrJoinAttributionGrain>(id));
      case TransactionOption.Create:
        return new CreateAttributionGrain(
            grainFactory.GetGrain<ICreateAttributionGrain>(id));
      case TransactionOption.Join:
        return new JoinAttributionGrain(
            grainFactory.GetGrain<IJoinAttributionGrain>(id));
      case TransactionOption.Supported:
        return new SupportedAttributionGrain(
            grainFactory.GetGrain<ISupportedAttributionGrain>(id));
      case TransactionOption.NotAllowed:
        return new NotAllowedAttributionGrain(
            grainFactory.GetGrain<INotAllowedAttributionGrain>(id));
      default:
        throw new NotSupportedException(
            $"Transaction option {option.Value} is not supported.");
      }
    }
