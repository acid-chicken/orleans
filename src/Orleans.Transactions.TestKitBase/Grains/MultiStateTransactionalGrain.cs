...
namespace Orleans.Transactions.TestKit {
  ...
  public class MaxStateTransactionalGrain
      : MultiStateTransactionalGrainBaseClass {
    public MaxStateTransactionalGrain(ITransactionalStateFactory stateFactory,
        ...
        : base(
              Enumerable
                  ...
                  .Select(
                      i => stateFactory.Create<GrainData>(
  ...
  public class DoubleStateTransactionalGrain
    ...
    public DoubleStateTransactionalGrain(
        ...
        ILoggerFactory loggerFactory)
        : base(new ITransactionalState<GrainData>[ 2 ]{data1, data2},
               loggerFactory) {}
  }
  ...
  public class NoStateTransactionalGrain
      : MultiStateTransactionalGrainBaseClass {
    public NoStateTransactionalGrain(ILoggerFactory loggerFactory)
        : base(Array.Empty<ITransactionalState<GrainData>>(), loggerFactory) {}
  }

  public class MultiStateTransactionalGrainBaseClass : Grain,
      ...
      foreach(var data in this.dataArray) {
        await data.PerformUpdate(state => {
          this.logger.LogInformation(
              $"Setting from {state.Value} to {newValue}.");
    ...
    public async Task<int[]>Add(int numberToAdd) {
      ...
      for (int i = 0; i < dataArray.Length; i++) {
        result[i] = await dataArray[i].PerformUpdate(state => {
          ...
          this.logger.LogInformation(
              $"Value after Adding {numberToAdd} is {state.Value}.");
          return state.Value;
        });
      }
      return result;
  ...
  public class AddAndThrowException : Exception {
    ...
    protected AddAndThrowException(SerializationInfo info,
