...
namespace Orleans.Transactions.TestKit.Correctnesss {
  ...
  public class BitArrayState {
    ...
    public override string ToString() {
      ...
      foreach(var v in this.value) {
        builder.Append(
            Reverse(Convert.ToString(v, 2)).PadRight(BitsInInt, '0'));
    ...
    public static BitArrayState operator ^(BitArrayState left,
                                           BitArrayState right) {
    ...
    public static BitArrayState Apply(BitArrayState left, BitArrayState right,
                                      Func<int, int, int>op) {
      var result =
          new BitArrayState(left.value.Length > right.value.Length ? left
                            : right);
  ...
  public class MaxStateTransactionalGrain
    ...
    public MaxStateTransactionalGrain(ITransactionalStateFactory stateFactory,
        ...
        : base(
              Enumerable
                  .Range(0, TransactionTestConstants.MaxCoordinatedTransactions)
                  .Select(
                      i => stateFactory.Create<BitArrayState>(
                          new TransactionalStateConfiguration(
                              new TransactionalStateAttribute(
                                  $"data{i}",
  ...
  public class DoubleStateTransactionalGrain
    ...
    public DoubleStateTransactionalGrain(
        ...
        ILoggerFactory loggerFactory)
        : base(new ITransactionalState<BitArrayState>[ 2 ]{data1, data2},
               loggerFactory) {}
  }
  ...
  public class SingleStateTransactionalGrain
    ...
    public SingleStateTransactionalGrain(
        ...
        ILoggerFactory loggerFactory)
        : base(new ITransactionalState<BitArrayState>[ 1 ]{data},
               loggerFactory) {}
  }

  public class MultiStateTransactionalBitArrayGrain
      : Grain,
        ITransactionalBitArrayGrain {
    ...
    public override Task OnActivateAsync() {
      this.logger =
          this.loggerFactory.CreateLogger(this.GetGrainIdentity().ToString());
    ...
    public Task SetBit(int index) {
      return Task.WhenAll(this.dataArray.Select(data => data.PerformUpdate(state => {
        this.logger.LogTrace(
            $"Setting bit {index} in state {state}. Transaction {TransactionContext.CurrentTransactionId}");
    ...
    public async Task<List<BitArrayState>>Get() {
      return (await Task.WhenAll(
                  this.dataArray.Select(state => state.PerformRead(s => {
