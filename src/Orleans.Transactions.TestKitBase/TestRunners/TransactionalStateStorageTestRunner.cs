...
namespace Orleans.Transactions.TestKit {
  public abstract class TransactionalStateStorageTestRunner<TState>
      : TransactionTestRunnerBase where TState : class, new () {
    protected Func<Task<ITransactionalStateStorage<TState>>>stateStorageFactory;
    protected Func<int, TState>stateFactory;
    protected Func<EquivalencyAssertionOptions<TState>,
                   EquivalencyAssertionOptions<TState>>assertConfig;

    /// <summary>
    /// Constructor
    /// </summary>
    /// <param name="stateStorageFactory">factory to create
    /// ITransactionalStateStorage, the test runner are assuming the state in
    /// storage is empty when ITransactionalStateStorage was created </param>
    ...
    /// <param name="assertConfig">A reference to the
    /// FluentAssertions.Equivalency.EquivalencyAssertionOptions`1
    ///     configuration object that can be used to influence the way the
    ///     object graphs are compared</param>
    protected TransactionalStateStorageTestRunner(
        Func<Task<ITransactionalStateStorage<TState>>>stateStorageFactory,
        Func<int, TState>stateFactory, IGrainFactory grainFactory,
        Action<string>testOutput,
        Func<EquivalencyAssertionOptions<TState>,
             EquivalencyAssertionOptions<TState>>assertConfig = null)
    ...
    private static List<PendingTransactionState<TState>>emptyPendingStates =
        new List<PendingTransactionState<TState>>();
    ...
    public virtual async Task StoreWithoutChanges() {
      ...
      var etag1 =
          await stateStorage.Store(loadresponse.ETag, loadresponse.Metadata,
                                   emptyPendingStates, null, null);
      ...
      var metadata =
          new TransactionalStateMetaData(){TimeStamp = now, CommitRecords = cr};
      var etag2 = await stateStorage.Store(etag1, metadata, emptyPendingStates,
                                           null, null);
    ...
    public virtual async Task WrongEtags() {
      ...
      try {
        var etag1 =
            await stateStorage.Store("wrong-etag", loadresponse.Metadata,
                                     emptyPendingStates, null, null);
      ...
      var metadata =
          new TransactionalStateMetaData(){TimeStamp = now, CommitRecords = cr};
      var etag2 = await stateStorage.Store(null, metadata, emptyPendingStates,
                                           null, null);

      // update timestamp in metadata, then write back with wrong e-tag, must
      // fail
      try {
        var now2 = DateTime.UtcNow;
        var metadata2 = new TransactionalStateMetaData(){
            TimeStamp = now2, CommitRecords = MakeCommitRecords(3, 3)};
        await stateStorage.Store(null, metadata, emptyPendingStates, null,
                                 null);
        throw new Exception("storage did not catch e-tag mismatch");
      } catch (Exception) {
      }
    ...
    private PendingTransactionState<TState>
    MakePendingState(long seqno, TState val, bool tm) {
    ...
    private ParticipantId MakeParticipantId() {
      return new ParticipantId(
          "tm", null,
          // (GrainReference)
          // grainFactory.GetGrain<ITransactionTestGrain>(Guid.NewGuid(),
          // TransactionTestConstants.SingleStateTransactionalGrain),
          ParticipantId.Role.Resource | ParticipantId.Role.Manager);
    }

    private Dictionary<Guid, CommitRecord>MakeCommitRecords(int count,
                                                            int size) {
    ...
    private async Task PrepareOne() {
      ...
      etag = await stateStorage.Store(
          etag, metadata,
          new List<PendingTransactionState<TState>>(){pendingstate}, null,
          null);
      ...
      loadresponse.PendingStates[0].TimeStamp.Should().Be(
          pendingstate.TimeStamp);
      loadresponse.PendingStates[0].TransactionManager.Should().Be(
          pendingstate.TransactionManager);
      loadresponse.PendingStates[0].TransactionId.Should().Be(
    ...
    public virtual async Task ConfirmOne(bool useTwoSteps) {
      ...
      if (useTwoSteps) {
        etag = await stateStorage.Store(
            ...
            new List<PendingTransactionState<TState>>(){pendingstate}, null,
            null);
        etag = await stateStorage.Store(etag, metadata, emptyPendingStates, 1,
                                        null);
      } else {
        etag = await stateStorage.Store(
            etag, metadata,
            new List<PendingTransactionState<TState>>(){pendingstate}, 1, null);
      }
    ...
    public virtual async Task CancelOne() {
      ...
      etag = await stateStorage.Store(
          ...
          null);
      etag =
          await stateStorage.Store(etag, metadata, emptyPendingStates, null, 0);
    ...
    public virtual async Task ReplaceOne() {
      ...
      etag = await stateStorage.Store(
          etag, metadata,
          new List<PendingTransactionState<TState>>(){pendingstate1}, null,
          null);
      etag = await stateStorage.Store(
          etag, metadata,
          new List<PendingTransactionState<TState>>(){pendingstate2}, null,
          null);
      ...
      loadresponse.PendingStates[0].SequenceId.Should().Be(1);
      loadresponse.PendingStates[0].TimeStamp.Should().Be(
          pendingstate2.TimeStamp);
      loadresponse.PendingStates[0].TransactionManager.Should().Be(
          pendingstate2.TransactionManager);
      loadresponse.PendingStates[0].TransactionId.Should().Be(
          pendingstate2.TransactionId);
      AssertTState(loadresponse.PendingStates[0].State, expectedState2);
    }

    public virtual async Task ConfirmOneAndCancelOne(
        bool useTwoSteps = false, bool reverseOrder = false) {
      var stateStorage = await this.stateStorageFactory();
      var loadresponse = await stateStorage.Load();
      var etag = loadresponse.ETag;
      var metadata = loadresponse.Metadata;
      ...
      etag =
          await stateStorage.Store(etag, metadata,
                                   new List<PendingTransactionState<TState>>(){
                                       pendingstate1, pendingstate2},
                                   null, null);
      ...
      if (useTwoSteps) {
        if (reverseOrder) {
          etag = await stateStorage.Store(etag, metadata, emptyPendingStates, 1,
                                          null);
          etag = await stateStorage.Store(etag, metadata, emptyPendingStates,
                                          null, 1);
        } else {
          etag = await stateStorage.Store(etag, metadata, emptyPendingStates, 1,
                                          null);
        ...
        etag =
            await stateStorage.Store(etag, metadata, emptyPendingStates, 1, 1);
      }
    ...
    public virtual async Task PrepareMany(int count) {
      ...
      etag =
          await stateStorage.Store(etag, metadata, pendingstates, null, null);
      ...
      for (int i = 0; i < count; i++) {
        ...
        loadresponse.PendingStates[i].TimeStamp.Should().Be(
            pendingstates[i].TimeStamp);
        loadresponse.PendingStates[i].TransactionManager.Should().Be(
            pendingstates[i].TransactionManager);
        loadresponse.PendingStates[i].TransactionId.Should().Be(
            pendingstates[i].TransactionId);
    ...
    public virtual async Task ConfirmMany(int count, bool useTwoSteps) {
      ...
      if (useTwoSteps) {
        ...
        etag = await stateStorage.Store(etag, metadata, pendingstates, count,
                                        null);
      }
    ...
    public virtual async Task ReplaceMany(int count) {
      ...
      etag =
          await stateStorage.Store(etag, metadata, pendingstates1, null, null);
      etag =
          await stateStorage.Store(etag, metadata, pendingstates2, null, null);
      ...
      for (int i = 0; i < count; i++) {
        loadresponse.PendingStates[i].SequenceId.Should().Be(i + 1);
        loadresponse.PendingStates[i].TimeStamp.Should().Be(
            pendingstates2[i].TimeStamp);
        loadresponse.PendingStates[i].TransactionManager.Should().Be(
            pendingstates2[i].TransactionManager);
        loadresponse.PendingStates[i].TransactionId.Should().Be(
            pendingstates2[i].TransactionId);
    ...
    public virtual async Task GrowingBatch() {
      ...
      etag = await stateStorage.Store(
          etag, metadata,
          new List<PendingTransactionState<TState>>(){
              pendingstate1, pendingstate2, pendingstate3a, pendingstate4a},
          null, null);
      ...
      etag = await stateStorage.Store(
          etag, metadata,
          new List<PendingTransactionState<TState>>(){
              pendingstate3b, pendingstate4b, pendingstate5, pendingstate6,
              pendingstate7, pendingstate8},
          6, null);
      ...
      loadresponse.PendingStates[0].TimeStamp.Should().Be(
          pendingstate7.TimeStamp);
      loadresponse.PendingStates[0].TransactionManager.Should().Be(
          pendingstate7.TransactionManager);
      loadresponse.PendingStates[0].TransactionId.Should().Be(
          pendingstate7.TransactionId);
      AssertTState(loadresponse.PendingStates[0].State, expectedState7);
      loadresponse.PendingStates[1].SequenceId.Should().Be(8);
      loadresponse.PendingStates[1].TimeStamp.Should().Be(
          pendingstate8.TimeStamp);
      loadresponse.PendingStates[1].TransactionManager.Should().Be(
          pendingstate8.TransactionManager);
      loadresponse.PendingStates[1].TransactionId.Should().Be(
          pendingstate8.TransactionId);
    ...
    public virtual async Task ShrinkingBatch() {
      ...
      etag = await stateStorage.Store(
          ...
          new List<PendingTransactionState<TState>>(){
              pendingstate1, pendingstate2, pendingstate3a, pendingstate4a,
              pendingstate5, pendingstate6, pendingstate7, pendingstate8},
          null, null);
      ...
      loadresponse.PendingStates[0].TimeStamp.Should().Be(
          pendingstate4b.TimeStamp);
      loadresponse.PendingStates[0].TransactionManager.Should().Be(
          pendingstate4b.TransactionManager);
      loadresponse.PendingStates[0].TransactionId.Should().Be(
          pendingstate4b.TransactionId);
