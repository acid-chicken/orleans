...
namespace Orleans.Transactions.TestKit {
  public abstract class GoldenPathTransactionTestRunner
    ...
    protected GoldenPathTransactionTestRunner(IGrainFactory grainFactory,
    ...
    public virtual async Task SingleGrainReadTransaction(string grainStates) {
      ...
      // each transaction state should all be 0 since no operation was applied
      // yet
    ...
    RepeatGrainReadWriteTransaction(string grainStates, int grainCount) {
      ...
      List<Guid>grainIds =
          Enumerable.Range(0, grainCount).Select(i => Guid.NewGuid()).ToList();

      List<ITransactionTestGrain>grains =
          grainIds.Select(id => TestGrain(grainStates, id)).ToList();
    ...
    MultiWriteToSingleGrainTransaction(string grainStates) {
      ...
      ITransactionTestGrain grain = RandomTestGrain(grainStates);
      List<ITransactionTestGrain>grains =
          Enumerable.Repeat(grain, concurrentWrites).ToList();
