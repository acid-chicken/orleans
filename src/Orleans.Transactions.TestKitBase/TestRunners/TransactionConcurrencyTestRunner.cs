...
namespace Orleans.Transactions.TestKit {
  public abstract class TransactionConcurrencyTestRunner
      : TransactionTestRunnerBase {
    protected TransactionConcurrencyTestRunner(IGrainFactory grainFactory,
    ...
    public virtual async Task SingleSharedGrainTest(string grainStates) {
      ...
      List<ITransactionTestGrain>transaction1Members =
          new List<ITransactionTestGrain>(new[]{grain1, sharedGrain});
      List<ITransactionTestGrain>transaction2Members =
          new List<ITransactionTestGrain>(new[]{grain2, sharedGrain});

      ITransactionCoordinatorGrain coordinator1 =
          this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(
              Guid.NewGuid());
      ITransactionCoordinatorGrain coordinator2 =
          this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(
              Guid.NewGuid());
      await Task.WhenAll(
    ...
    public virtual async Task TransactionChainTest(string grainStates) {
      ...
      List<ITransactionTestGrain>transaction1Members =
          new List<ITransactionTestGrain>(new[]{grain1, grain2});
      List<ITransactionTestGrain>transaction2Members =
          new List<ITransactionTestGrain>(new[]{grain2, grain3});
      List<ITransactionTestGrain>transaction3Members =
          new List<ITransactionTestGrain>(new[]{grain3, grain4});
      List<ITransactionTestGrain>transaction4Members =
          new List<ITransactionTestGrain>(new[]{grain4, grain5});
      ...
      ITransactionCoordinatorGrain coordinator2 =
          this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(
              Guid.NewGuid());
      ITransactionCoordinatorGrain coordinator3 =
          this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(
              Guid.NewGuid());
      ITransactionCoordinatorGrain coordinator4 =
          this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(
              Guid.NewGuid());
      await Task.WhenAll(
          coordinator1.MultiGrainAdd(transaction1Members, expected),
    ...
    /// <summary>
    /// Single transaction containing two grains is dependent on two other
    /// transaction, one from each grain
    /// </summary>
    /// <param name="grainStates"></param>
    /// <returns></returns>
