...
namespace Orleans.Transactions.TestKit {
  public abstract class GrainFaultTransactionTestRunner
      : TransactionTestRunnerBase {
    public GrainFaultTransactionTestRunner(IGrainFactory grainFactory,
    ...
    public virtual async Task AbortTransactionOnExceptions(string grainStates) {
      ...
      await coordinator.MultiGrainSet(new List<ITransactionTestGrain>{grain},
                                      expected);
    ...
    MultiGrainAbortTransactionOnExceptions(string grainStates) {
      const int grainCount =
          TransactionTestConstants.MaxCoordinatedTransactions - 1;
      const int expected = 5;
      ...
      Func<Task>task = () => coordinator.MultiGrainAddAndThrow(
          new List<ITransactionTestGrain>(){throwGrain}, grains, expected);
      task.ShouldThrow<OrleansTransactionAbortedException>();
...
AbortTransactionExceptionInnerExceptionOnlyContainsOneRootCauseException(
  ...
  const int grainCount =
      TransactionTestConstants.MaxCoordinatedTransactions - throwGrainCount;
  const int expected = 5;

  List<ITransactionTestGrain>throwGrains =
      Enumerable.Range(0, throwGrainCount)
          .Select(i => RandomTestGrain(grainStates))
          .ToList();
  List<ITransactionTestGrain>grains =
      Enumerable.Range(0, grainCount)
          ...
          .ToList();
  ITransactionCoordinatorGrain coordinator =
      this.grainFactory.GetGrain<ITransactionCoordinatorGrain>(Guid.NewGuid());
