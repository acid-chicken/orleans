...
namespace Orleans.Transactions.TestKit.Consistency {
  public class ConsistencyTestHarness {
    ...
    public ConsistencyTestHarness(IGrainFactory grainFactory, int numGrains,
                                  int seed, bool avoidDeadlocks,
                                  bool avoidTimeouts,
      ...
      this.options = new ConsistencyTestOptions(){
          ...
          GrainOffset = (DateTime.UtcNow.Ticks & 0xFFFFFFFF) *
                        ConsistencyTestOptions.MaxGrains,
      };

      this.tuples = new Dictionary<
          int, SortedDictionary<int, Dictionary<string, HashSet<string>>>>();
    ...
    public async Task RunRandomTransactionSequence(int partition, int count,
      ...
      for (int i = 0; i < count; i++) {
        ...
        try {
          var targetgrain = grainFactory.GetGrain<IConsistencyTestGrain>(
              options.GrainOffset + target);
          var stopAfter = options.AvoidTimeouts ? DateTime.UtcNow +
                          TimeSpan.FromSeconds(22) : DateTime.MaxValue;
          var result = await targetgrain.Run(options, 0, $"({partition},{i})",
                                             options.NumGrains, stopAfter);

          if (result.Length > 0) {
            id = result[0].ExecutingTx;

            lock(succeeded) succeeded.Add(id);

            output($"{partition}.{i} g{target} -> {result.Length} tuples");

            foreach(var tuple in result) {
              tuple.ExecutingTx.ShouldBeEquivalentTo(
                  id); // all effects of this transaction must have same id
              lock(tuples) {
                if (!tuples.TryGetValue(tuple.Grain, out var versions)) {
                  tuples.Add(tuple.Grain,
                             versions = new SortedDictionary<
                                 int, Dictionary<string, HashSet<string>>>());
                }
                if (!versions.TryGetValue(tuple.SeqNo, out var writers)) {
                  versions.Add(tuple.SeqNo,
                               writers =
                                   new Dictionary<string, HashSet<string>>());
                }
          ...
          output(
              $"{partition}.{i} g{target} -> aborted {e.GetType().Name} {e.InnerException} {e.TransactionId}");
          ...
          lock(indoubt) indoubt.Add(f.TransactionId, f.Message);
    ...
    public void CheckConsistency(bool tolerateGenericTimeouts = false,
                                 bool tolerateUnknownExceptions = false) {
      foreach(var grainKvp in tuples) {
        var pos = 0;
        Action<string>fail = (msg) => {
          foreach(var kvp1 in grainKvp.Value) foreach(var kvp2 in kvp1.Value)
        ...
        foreach(var seqnoKvp in grainKvp.Value) {
          ...
          if (pos++ != seqno && indoubt.Count == 0 && !timeoutsOccurred)
            fail(
                $"g{grainKvp.Key} is missing version v{pos - 1}, found v{seqno} instead");

          var writers = seqnoKvp.Value;
          if (writers.Count != 1)
            fail($"g{grainKvp.Key} v{seqno} has multiple writers {string.Join(",
                 ", writers.Keys)}");
          ...
          if (seqno == 0) {
            ...
            if (aborted.Contains(writer))
              fail(
                  $"g{grainKvp.Key} v{seqno} written by aborted transaction {writer}");
            if (!timeoutsOccurred &&
                !(succeeded.Contains(writer) || indoubt.ContainsKey(writer)))
              fail(
                  $"g{grainKvp.Key} v{seqno} written by unknown transaction {writer}");
            if (indoubt.Count == 0 && !timeoutsOccurred &&
                !readers.Contains(writer))
              fail($"g{grainKvp.Key} v{seqno} writer {writer} missing");
          }

          // add edges from previous readers to this write
          foreach(var r in readersOfPreviousVersion) if (r != writer) {
          ...
          foreach(var r in readers) if (r != writer) {
      ...
      // due a DFS to find cycles in the ordered-before graph (= violation of
      // serializability)
      DFS();
      ...
      if (!tolerateUnknownExceptions)
        foreach(
            var kvp in
                indoubt) if (kvp.Value
                                 .Contains(
                                     "failure during transaction commit")) true
            .Should()
            .BeFalse($"exception during commit {kvp.Key} {kvp.Value}");
    ...
    private void DFS() {
      foreach(var kvp in orderEdges) if (!marks.ContainsKey(kvp.Key)) {
    ...
    private bool Visit(string node, HashSet<string>edges) {
      if (marks.TryGetValue(node, out var mark)) {
        ...
        marks[node] = false;
        foreach(var n in edges) if (orderEdges.TryGetValue(n, out var edges2)) {
          if (Visit(n, edges2)) {
            output($"{node}");
            return true;
          }
