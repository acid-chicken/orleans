...
namespace Orleans.Statistics {
  internal class LinuxEnvironmentStatistics
    ...
    internal static readonly string[] RequiredFiles =
        new[]{MEMINFO_FILEPATH, CPUSTAT_FILEPATH};
    ...
    public async Task OnStop(CancellationToken ct) {
      ...
      try {
        ...
        _logger.LogError(
            ex, $"Error stopping {nameof(LinuxEnvironmentStatistics)}");
      } finally {
    ...
    private async Task UpdateTotalPhysicalMemory() {
      var memTotalLine =
          await ReadLineStartingWithAsync(MEMINFO_FILEPATH, "MemTotal");

      if (string.IsNullOrWhiteSpace(memTotalLine)) {
        _logger.LogWarning(
            $"Couldn't read 'MemTotal' line from '{MEMINFO_FILEPATH}'");
        return;
      }

      // Format: "MemTotal:       16426476 kB"
      if (!long.TryParse(new string(memTotalLine.Where(char.IsDigit).ToArray()),
                         out var totalMemInKb)) {
    ...
    private async Task UpdateCpuUsage(int i) {
      var cpuUsageLine =
          await ReadLineStartingWithAsync(CPUSTAT_FILEPATH, "cpu  ");
      ...
      if (cpuNumberStrings.Any(n => !long.TryParse(n, out _))) {
        _logger.LogWarning(
            $"Failed to parse '{CPUSTAT_FILEPATH}' output correctly. Line: {cpuUsageLine}");
        return;
      }
      ...
      if (i > 0) {
        ...
        var currentCpuUsage =
            (1.0f - deltaIdleTime / ((float) deltaTotalTime)) * 100f;
    ...
    private async Task UpdateAvailableMemory() {
      var memAvailableLine =
          await ReadLineStartingWithAsync(MEMINFO_FILEPATH, "MemAvailable");

      if (string.IsNullOrWhiteSpace(memAvailableLine)) {
        memAvailableLine =
            await ReadLineStartingWithAsync(MEMINFO_FILEPATH, "MemFree");
        if (string.IsNullOrWhiteSpace(memAvailableLine)) {
          _logger.LogWarning(
              $"Couldn't read 'MemAvailable' or 'MemFree' line from '{MEMINFO_FILEPATH}'");
          return;
        }
      }

      if (!long.TryParse(
              new string(memAvailableLine.Where(char.IsDigit).ToArray()),
              out var availableMemInKb)) {
        _logger.LogWarning(
            $"Couldn't parse meminfo output: '{memAvailableLine}'");
        return;
      }
    ...
    private void WriteToStatistics() {
      ...
      IntValueStatistic.FindOrCreate(
          StatisticNames.RUNTIME_DOT_NET_THREADPOOL_INUSE_WORKERTHREADS, () => {
            ...
            // number of thread pool threads and the number currently active. So
            // max-Available is the actual number in use. If it goes beyond min,
            // it means we are stressing the thread pool.
            ThreadPool.GetAvailableThreads(out var workerThreads,
    ...
    private async Task Monitor(CancellationToken ct) {
      ...
      while (true) {
        ...
        try {
          await Task.WhenAll(i == 0 ? UpdateTotalPhysicalMemory()
                                    : Task.CompletedTask,
                             UpdateCpuUsage(i), UpdateAvailableMemory());

          if (i == 1)
            WriteToStatistics();

          var logStr =
              $"LinuxEnvironmentStatistics: CpuUsage={CpuUsage?.ToString(" 0.0 ")}, TotalPhysicalMemory={TotalPhysicalMemory}, AvailableMemory={AvailableMemory}";
          ...
          await Task.Delay(MONITOR_PERIOD + MONITOR_PERIOD + MONITOR_PERIOD,
                           ct);
        }
        if (i < 2)
          i++;
    ...
    ReadLineStartingWithAsync(string path, string lineStartsWith) {
      using(var fs = new FileStream(
                path, FileMode.Open, FileAccess.Read, FileShare.Read, 512,
                FileOptions.SequentialScan |
                    FileOptions.Asynchronous)) using(var r =
                                                         new StreamReader(
                                                             fs,
                                                             Encoding.ASCII)) {
        string line;
