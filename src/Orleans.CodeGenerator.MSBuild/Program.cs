...
namespace Microsoft.Orleans.CodeGenerator.MSBuild {
  public class Program {
    public static int Main(string[] args) {
      ...
      Console.WriteLine("Orleans.CodeGenerator - command-line = {0}",
                        string.Join(" ", args));
      ...
      try {
        ...
        Console.WriteLine("-- Code Generation FAILED -- \n{0}",
                          LogFormatter.PrintException(ex));
        return 3;
      }
    }
      ...
      using(new AssemblyResolver()) {
        ...
        if (!File.Exists(argsFile))
          throw new ArgumentException(
              $"Arguments file \"{argsFile}\" does not exist.");
        ...
        foreach(var arg in fileArgs) {
          ...
          case nameof(LogLevel):
            if (!Enum.TryParse(ignoreCase
                               : true, value
                               : value, result
                               : out logLevel)) {
              var validValues = string.Join(
                  ", ",
                  Enum.GetNames(typeof (LogLevel)).Select(v => v.ToString()));
              Console.WriteLine(
                  $"ERROR: \"{value}\" is not a valid log level. Valid values are {validValues}");
              return -3;
            }
          ...
          default:
            throw new ArgumentOutOfRangeException(
                $"Key \"{key}\" in argument file is unknown.");
          }
        }
        ...
        var services =
            new ServiceCollection()
                .AddLogging(logging => {
                  logging.SetMinimumLevel(logLevel).AddConsole().AddDebug();
                })
                .BuildServiceProvider();
        cmd.Log = services.GetRequiredService<ILoggerFactory>().CreateLogger(
            "Orleans.CodeGenerator");
        var stopwatch = Stopwatch.StartNew();
        var ok = cmd.Execute(CancellationToken.None).GetAwaiter().GetResult();
        cmd.Log.LogInformation(
            $"Total code generation time: {stopwatch.ElapsedMilliseconds}ms.");
