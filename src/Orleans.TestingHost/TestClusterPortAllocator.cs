...
namespace Orleans.TestingHost {
  public class TestClusterPortAllocator : ITestClusterPortAllocator {
    ...
    ~TestClusterPortAllocator() { Dispose(false); }
      ...
      for (int attempts = 0; attempts < MaxAttempts; attempts++) {
        ...
        if (tcpConnInfoArray.All(endpoint => endpoint.Port < basePort ||
          ...
          for (var i = 0; i < consecutivePortsToCheck; i++) {
            var port = basePort + i;
            var name =
                $"Global.TestCluster.{port.ToString(CultureInfo.InvariantCulture)}";
    ...
    private class MutexManager {
      ...
      public static MutexManager Instance { get; }
      = new MutexManager();
      ...
      public bool Acquire(string name) {
        ...
        _workItems.Add(() => {
          try {
            if (!_mutexes.TryGetValue(name, out var mutex)) {
              ...
              // Failed to acquire: the mutex is already held by another
              // process.
              try {
                mutex.ReleaseMutex();
              } finally {
        ...
        if (!signal.Wait(TimeSpan.FromSeconds(10))) {
          throw new TimeoutException(
              "Timed out while waiting for MutexManager to acquire mutex.");
        }
