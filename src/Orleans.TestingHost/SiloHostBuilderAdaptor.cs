...
namespace Orleans.TestingHost {
  internal class SiloHostBuilderAdaptor : ISiloHostBuilder {
    ...
    public SiloHostBuilderAdaptor(IHostBuilder hostBuilder,
    ...
    public ISiloHostBuilder
    ConfigureAppConfiguration(Action<SiloHostBuilderContext,
                                     IConfigurationBuilder>configureDelegate) {
      this.hostBuilder.ConfigureAppConfiguration(
          (ctx, containerBuilder) =>
              configureDelegate(this.GetContext(ctx), containerBuilder));
      return this;
    }

    public ISiloHostBuilder ConfigureContainer<TContainerBuilder>(
        Action<SiloHostBuilderContext, TContainerBuilder>configureDelegate) {
      this.hostBuilder.ConfigureContainer<TContainerBuilder>(
          (ctx, containerBuilder) =>
              configureDelegate(this.GetContext(ctx), containerBuilder));
      return this;
    }
    ...
    public ISiloHostBuilder ConfigureServices(
        Action<SiloHostBuilderContext, IServiceCollection>configureDelegate) {
      this.siloBuilder.ConfigureServices(
          (ctx, services) => configureDelegate(this.GetContext(ctx), services));
      return this;
    }

    public ISiloHostBuilder UseServiceProviderFactory<TContainerBuilder>(
    ...
    private SiloHostBuilderContext GetContext(HostBuilderContext context) {
      ...
      if (this.Properties.TryGetValue(key, out var result) &&
          result is SiloHostBuilderContext resultContext) {
      ...
      resultContext = new SiloHostBuilderContext(context.Properties){
          ...
          HostingEnvironment =
              new HostingEnvironmentWrapper(context.HostingEnvironment)};
    ...
    private class HostingEnvironmentWrapper : IHostingEnvironment {
      ...
      public HostingEnvironmentWrapper(IHostEnvironment hostEnvironment) =>
