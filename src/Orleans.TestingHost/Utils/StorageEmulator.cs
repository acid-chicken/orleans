...
namespace Orleans.TestingHost.Utils {
  ...
  /// </summary>
  /// <remarks>It might be tricky to implement this as a <see
  /// cref="IDisposable">IDisposable</see>, isolated, autonomous instance, see
  /// at <see
  /// href="http://azure.microsoft.com/en-us/documentation/articles/storage-use-emulator/">Use
  /// the Azure Storage Emulator for Development and Testing</see> for
  /// pointers.</remarks>
  public static class StorageEmulator {
    /// <summary>
    /// The storage emulator process name. One way to enumerate running process
    /// names is Get-Process | Format-Table Id, ProcessName -autosize. If there
    /// were multiple storage emulator processes running, they would named
    /// WASTOR~1, WASTOR~2, ... WASTOR~n.
    /// </summary>
    ...
    /// </summary>
    /// <returns></returns>
    public static bool IsStarted() { return GetStorageEmulatorProcess(); }

    /// <summary>
    /// Checks if the storage emulator exists, i.e. is installed.
    /// </summary>
    public static bool Exists {
      get { return GetStorageEmulatorPath() != null; }
    }
    ...
    public static string Help() {
      if (!IsStarted())
        return "Error happened. Has StorageEmulator.Start() been called?";
    ...
    /// </summary>
    /// <returns><em>TRUE</em> if the process was started successfully.
    ...
    /// </summary>
    /// <returns><em>TRUE</em> if the process was stopped successfully or was
    /// already started. <em>FALSE</em> otherwise.</returns>
    ...
    /// </summary>
    /// <returns><em>TRUE</em> if the process was stopped successfully or was
    /// already stopped. <em>FALSE</em> otherwise.</returns>
    ...
    /// <summary>
    /// Creates a new <see cref="ProcessStartInfo">ProcessStartInfo</see> to be
    /// used as an argument to other operations in this class.
    /// </summary>
    /// <param name="arguments">The arguments.</param>
    /// <returns>A new <see cref="ProcessStartInfo">ProcessStartInfo</see> that
    /// has the given arguments.</returns>
    ...
    /// <summary>
    /// Returns a full path to the storage emulator executable, including the
    /// executable name and file extension.
    /// </summary>
    /// <returns>A full path to the storage emulator executable, or null if not
    /// found.</returns>
    private static string GetStorageEmulatorPath() {
      // Try to take the newest known emulator path. If it does not exist, try
      // an older one.
    ...
    private static string GetProgramFilesBasePath() {
      return Environment.Is64BitOperatingSystem ? Environment
