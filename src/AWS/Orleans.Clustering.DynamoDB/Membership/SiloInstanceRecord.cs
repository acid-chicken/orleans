...
namespace Orleans.Runtime.MembershipService {
  internal class SiloInstanceRecord {
    ...
    internal const string TABLE_VERSION_ROW =
        "VersionRow"; // Range key for version row.
    ...
    public string SuspectingSilos {
      get;
      set;
    }
    public string SuspectingTimes {
      get;
      set;
    }
    ...
    public SiloInstanceRecord(Dictionary<string, AttributeValue>fields) {
      ...
      if (fields.ContainsKey(MEMBERSHIP_VERSION_PROPERTY_NAME) &&
          int.TryParse(fields[MEMBERSHIP_VERSION_PROPERTY_NAME].N,
                       out int version))
    ...
    public static string ConstructSiloIdentity(SiloAddress silo) {
      return string.Format("{0}-{1}-{2}", silo.Endpoint.Address,
                           silo.Endpoint.Port, silo.Generation);
    }
    ...
    GetFields(bool includeKeys = false) {
      var fields = new Dictionary<string, AttributeValue>();

      if (includeKeys) {
        fields.Add(DEPLOYMENT_ID_PROPERTY_NAME,
                   new AttributeValue(DeploymentId));
        fields.Add(SILO_IDENTITY_PROPERTY_NAME,
                   new AttributeValue(SiloIdentity));
      }
      ...
      fields.Add(GENERATION_PROPERTY_NAME,
                 new AttributeValue{N = Generation.ToString()});

      if (!string.IsNullOrWhiteSpace(HostName))
        fields.Add(HOSTNAME_PROPERTY_NAME, new AttributeValue(HostName));

      fields.Add(STATUS_PROPERTY_NAME,
                 new AttributeValue{N = Status.ToString()});
      fields.Add(PROXY_PORT_PROPERTY_NAME,
                 new AttributeValue{N = ProxyPort.ToString()});
      ...
      if (!string.IsNullOrWhiteSpace(SuspectingSilos))
        fields.Add(SUSPECTING_SILOS_PROPERTY_NAME,
                   new AttributeValue(SuspectingSilos));

      if (!string.IsNullOrWhiteSpace(SuspectingTimes))
        fields.Add(SUSPECTING_TIMES_PROPERTY_NAME,
                   new AttributeValue(SuspectingTimes));

      if (!string.IsNullOrWhiteSpace(StartTime))
        fields.Add(START_TIME_PROPERTY_NAME, new AttributeValue(StartTime));

      if (!string.IsNullOrWhiteSpace(IAmAliveTime))
        fields.Add(I_AM_ALIVE_TIME_PROPERTY_NAME,
                   new AttributeValue(IAmAliveTime));

      fields.Add(MEMBERSHIP_VERSION_PROPERTY_NAME,
                 new AttributeValue{N = MembershipVersion.ToString()});
