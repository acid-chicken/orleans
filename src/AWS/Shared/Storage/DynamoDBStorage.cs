...
#endif
{
  ...
  internal class DynamoDBStorage {
    ...
    public DynamoDBStorage(ILogger logger, string service,
    ...
    /// <param name="attributes">The attributes used on the key
    /// definition</param> <param name="secondaryIndexes">(optional) The
    /// secondary index definitions</param> <param
    /// name="ttlAttributeName">(optional) The name of the item attribute that
    /// indicates the item TTL (if null, ttl won't be enabled)</param>
    /// <returns></returns>
    public async Task
    InitializeTable(string tableName, List<KeySchemaElement>keys,
    ...
    private void CreateClient() {
      if (this.service.StartsWith("http://",
        ...
        this.ddbClient = new AmazonDynamoDBClient(
            credentials, new AmazonDynamoDBConfig{ServiceURL = this.service});
      } else if (!string.IsNullOrEmpty(this.accessKey) &&
    ...
    private async Task
    CreateTable(string tableName, List<KeySchemaElement>keys,
                List<AttributeDefinition>attributes,
                List<GlobalSecondaryIndex>secondaryIndexes = null,
                string ttlAttributeName = null) {
      var request = new CreateTableRequest{
        ...
        KeySchema = keys,
        BillingMode = this.useProvisionedThroughput ? BillingMode
                          .PROVISIONED : BillingMode.PAY_PER_REQUEST,
        ProvisionedThroughput =
            this.useProvisionedThroughput ? new ProvisionedThroughput{
                ReadCapacityUnits = readCapacityUnits,
                WriteCapacityUnits = writeCapacityUnits} : null
      };
      ...
      if (secondaryIndexes != null && secondaryIndexes.Count > 0) {
        if (this.useProvisionedThroughput) {
          ...
          secondaryIndexes.ForEach(
              i => { i.ProvisionedThroughput = indexThroughput; });
        }
      ...
      try {
        ...
        if (description.TableStatus != TableStatus.ACTIVE)
          throw new InvalidOperationException(
              $"Failure creating table {tableName}");
      } catch (Exception exc) {
        Logger.Error(ErrorCode.StorageProviderBase,
                     $"Could not create table {tableName}", exc);
        throw;
      }
    }
    ...
    public Task DeleTableAsync(string tableName) {
      try {
        return ddbClient.DeleteTableAsync(
            new DeleteTableRequest{TableName = tableName});
      } catch (Exception exc) {
        Logger.Error(ErrorCode.StorageProviderBase,
                     $"Could not delete table {tableName}", exc);
        throw;
      }
    }
    ...
    /// <param name="fields">The fields/attributes to add or replace in the
    /// table</param> <param name="conditionExpression">Optional conditional
    /// expression</param> <param name="conditionValues">Optional
    /// field/attribute values used in the conditional expression</param>
    /// <returns></returns>
    public Task
    PutEntryAsync(string tableName, Dictionary<string, AttributeValue>fields,
                  string conditionExpression = "",
                  Dictionary<string, AttributeValue>conditionValues = null) {
      if (Logger.IsEnabled(LogLevel.Trace))
        Logger.Trace("Creating {0} table entry: {1}", tableName,
      ...
      try {
        ...
        Logger.Error(ErrorCode.StorageProviderBase,
                     $"Unable to create item to table '{tableName}'", exc);
        throw;
      }
    }
    ...
    /// <param name="fields">The fields/attributes to add or updated in the
    /// table</param> <param name="conditionExpression">Optional conditional
    /// expression</param> <param name="conditionValues">Optional
    /// field/attribute values used in the conditional expression</param> <param
    /// name="extraExpression">Additional expression that will be added in the
    /// end of the upsert expression</param> <param
    /// name="extraExpressionValues">Additional field/attribute that will be
    /// used in the extraExpression</param> <remarks>The fields dictionary item
    /// values will be updated with the values returned from DynamoDB</remarks>
    /// <returns></returns>
    public async Task UpsertEntryAsync(
        string tableName, Dictionary<string, AttributeValue>keys,
        Dictionary<string, AttributeValue>fields,
        string conditionExpression = "",
        Dictionary<string, AttributeValue>conditionValues = null,
        string extraExpression = "",
        Dictionary<string, AttributeValue>extraExpressionValues = null) {
      if (Logger.IsEnabled(LogLevel.Trace))
        Logger.Trace("Upserting entry {0} with key(s) {1} into table {2}",
                     Utils.DictionaryToString(fields),
                     Utils.DictionaryToString(keys), tableName);

      try {
        var request =
            new UpdateItemRequest{TableName = tableName, Key = keys,
                                  ReturnValues = ReturnValue.UPDATED_NEW};

        (request.UpdateExpression, request.ExpressionAttributeValues) =
            ConvertUpdate(fields, conditionValues, extraExpression,
    ...
    /// <param name="keys">The table entry keys for the entry to be
    /// deleted</param> <param name="conditionExpression">Optional conditional
    /// expression</param> <param name="conditionValues">Optional
    /// field/attribute values used in the conditional expression</param>
    /// <returns></returns>
    public Task
    DeleteEntryAsync(string tableName, Dictionary<string, AttributeValue>keys,
                     string conditionExpression = "",
                     Dictionary<string, AttributeValue>conditionValues = null) {
      if (Logger.IsEnabled(LogLevel.Trace))
        Logger.Trace("Deleting table {0}  entry with key(s) {1}", tableName,
                     Utils.DictionaryToString(keys));

      try {
        var request = new DeleteItemRequest{TableName = tableName, Key = keys};
    ...
    /// <param name="toDelete">List of key values for each entry that must be
    /// deleted in the batch</param> <returns></returns>
    public Task DeleteEntriesAsync(
        string tableName,
        IReadOnlyCollection<Dictionary<string, AttributeValue>>toDelete) {
      if (Logger.IsEnabled(LogLevel.Trace))
        Logger.Trace("Deleting {0} table entries", tableName);
    ...
    /// <param name="tableName">The name of the table to search for the
    /// entry</param> <param name="keys">The table entry keys to search
    /// for</param> <param name="resolver">Function that will be called to
    /// translate the returned fields into a concrete type. This Function is
    /// only called if the result is != null</param> <returns>The object
    /// translated by the resolver function</returns>
    public async Task<TResult>ReadSingleEntryAsync<TResult>(
        string tableName, Dictionary<string, AttributeValue>keys,
        Func<Dictionary<string, AttributeValue>, TResult>resolver) where TResult
        : class {
      try {
        var request = new GetItemRequest{TableName = tableName, Key = keys,
                                         ConsistentRead = true};
        ...
        if (Logger.IsEnabled(LogLevel.Debug))
          Logger.Debug("Unable to find table entry for Keys = {0}",
                       Utils.DictionaryToString(keys));
        throw;
      }
    }
    ...
    /// <typeparam name="TResult">The result type</typeparam>
    /// <param name="tableName">The name of the table to search for the
    /// entries</param> <param name="keys">The table entry keys to search
    /// for</param> <param name="keyConditionExpression">the expression that
    /// will filter the keys</param> <param name="resolver">Function that will
    /// be called to translate the returned fields into a concrete type. This
    /// Function is only called if the result is != null and will be called for
    /// each entry that match the query and added to the results list</param>
    /// <param name="indexName">In case a secondary index is used in the
    /// keyConditionExpression</param> <param name="scanIndexForward">In case an
    /// index is used, show if the seek order is ascending (true) or descending
    /// (false)</param> <param name="lastEvaluatedKey">The primary key of the
    /// first item that this operation will evaluate. Use the value that was
    /// returned for LastEvaluatedKey in the previous operation</param>
    /// <returns>The collection containing a list of objects translated by the
    /// resolver function and the LastEvaluatedKey for paged results</returns>
    public async Task<(List<TResult>results,
                       Dictionary<string, AttributeValue>lastEvaluatedKey)>
    QueryAsync<TResult>(
        string tableName, Dictionary<string, AttributeValue>keys,
        string keyConditionExpression,
        Func<Dictionary<string, AttributeValue>, TResult>resolver,
        string indexName = "", bool scanIndexForward = true,
        Dictionary<string, AttributeValue>lastEvaluatedKey = null) where TResult
        : class {
      try {
        var request =
            new QueryRequest{TableName = tableName,
        ...
        foreach(var item in response.Items) { resultList.Add(resolver(item)); }
    ...
    /// keyConditionExpression</param> <param name="scanIndexForward">In case an
    /// index is used, show if the seek order is ascending (true) or descending
    /// (false)</param> <returns>The collection containing a list of objects
    /// translated by the resolver function</returns>
    public async Task<List<TResult>>QueryAllAsync<TResult>(
        string tableName, Dictionary<string, AttributeValue>keys,
        string keyConditionExpression,
        Func<Dictionary<string, AttributeValue>, TResult>resolver,
        string indexName = "", bool scanIndexForward = true) where TResult
        : class {
      ...
      do {
        ...
        (results, lastEvaluatedKey) =
            await QueryAsync(tableName, keys, keyConditionExpression, resolver,
    ...
    /// <typeparam name="TResult">The result type</typeparam>
    /// <param name="tableName">The name of the table to search for the
    /// entries</param> <param name="attributes">The attributes used on the
    /// expression</param> <param name="expression">The filter
    /// expression</param> <param name="resolver">Function that will be called
    /// to translate the returned fields into a concrete type. This Function is
    /// only called if the result is != null and will be called for each entry
    /// that match the query and added to the results list</param> <returns>The
    /// collection containing a list of objects translated by the resolver
    /// function</returns>
    public async Task<List<TResult>>ScanAsync<TResult>(
        string tableName, Dictionary<string, AttributeValue>attributes,
        string expression,
        Func<Dictionary<string, AttributeValue>, TResult>resolver) where TResult
        : class {
    ...
    /// </summary>
    /// <param name="tableName">The name of the table to search for the
    /// entry</param> <param name="toCreate">List of key values for each entry
    /// that must be created or replaced in the batch</param>
    /// <returns></returns>
    public Task PutEntriesAsync(
        string tableName,
        IReadOnlyCollection<Dictionary<string, AttributeValue>>toCreate) {
      if (Logger.IsEnabled(LogLevel.Trace))
        Logger.Trace("Put entries {0} table", tableName);
    ...
    /// only called if the result is != null</param> <returns>The object
    /// translated by the resolver function</returns>
    public async Task<IEnumerable<TResult>>GetEntriesTxAsync<TResult>(
        string tableName, IEnumerable<Dictionary<string, AttributeValue>>keys,
        Func<Dictionary<string, AttributeValue>, TResult>resolver) where TResult
        : class {
      try {
        var request = new TransactGetItemsRequest{
            TransactItems = keys.Select(key => new TransactGetItem{
                                            Get = new Get{TableName = tableName,
                                                          Key = key}})
                                .ToList()};

        var response = await ddbClient.TransactGetItemsAsync(request);

        return response.Responses.Where(r => r?.Item?.Count > 0)
            .Select(r => resolver(r.Item));
      } catch(Exception) {
        if (Logger.IsEnabled(LogLevel.Debug))
          Logger.Debug(
              "Unable to find table entry for Keys = {0}",
    ...
    /// <param name="conditionChecks">Any condition checks to be
    /// performed</param> <returns></returns>
    public Task
    WriteTxAsync(IEnumerable<Put>puts = null, IEnumerable<Update>updates = null,
                 IEnumerable<Delete>deletes = null,
                 IEnumerable<ConditionCheck>conditionChecks = null) {
      try {
        var transactItems = new List<TransactWriteItem>();
        if (puts != null) {
          transactItems.AddRange(
              puts.Select(p => new TransactWriteItem{Put = p}));
        }
        if (updates != null) {
          transactItems.AddRange(
              updates.Select(u => new TransactWriteItem{Update = u}));
        }
        if (deletes != null) {
          transactItems.AddRange(
              deletes.Select(d => new TransactWriteItem{Delete = d}));
        }
        if (conditionChecks != null) {
          transactItems.AddRange(conditionChecks.Select(
              c => new TransactWriteItem{ConditionCheck = c}));
        }
        ...
        if (Logger.IsEnabled(LogLevel.Debug))
          Logger.Debug("Unable to write tx");
        throw;
      }
    }
  }
