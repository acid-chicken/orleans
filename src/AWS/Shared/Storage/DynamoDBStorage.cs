...
#endif
{
  ...
  internal class DynamoDBStorage {
    ...
    /// secondary index definitions</param> <returns></returns>
    public async Task
    ...
    private void CreateClient() {
      if (service.StartsWith("http://", StringComparison.OrdinalIgnoreCase) ||
        ...
        ddbClient = new AmazonDynamoDBClient(
            credentials, new AmazonDynamoDBConfig{ServiceURL = service});
        ...
        ddbClient = new AmazonDynamoDBClient(
            ...
            new AmazonDynamoDBConfig{ServiceURL = service,
                                     RegionEndpoint =
                                         AWSUtils.GetRegionEndpoint(service)});
      } else {
    ...
    CreateTable(string tableName, List<KeySchemaElement>keys,
      ...
      var request = new CreateTableRequest{
          TableName = tableName, AttributeDefinitions = attributes,
          KeySchema = keys,
      ...
      if (secondaryIndexes != null && secondaryIndexes.Count > 0) {
        var indexThroughput =
            new ProvisionedThroughput{ReadCapacityUnits = readCapacityUnits,
                                      WriteCapacityUnits = writeCapacityUnits};
