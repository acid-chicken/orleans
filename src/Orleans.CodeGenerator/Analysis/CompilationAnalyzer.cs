...
namespace Orleans.CodeGenerator.Analysis {
  internal class CompilationAnalyzer {
    ...
    /// </summary>
    private readonly HashSet<IAssemblySymbol>
        assembliesWithForcedSerializability = new HashSet<IAssemblySymbol>();

    /// <summary>
    /// Types whose sub-types are all considered serializable.
    /// </summary>
    private readonly HashSet<INamedTypeSymbol>knownBaseTypes =
        new HashSet<INamedTypeSymbol>();

    /// <summary>
    /// Types which were observed in a grain interface.
    /// </summary>
    private readonly HashSet<ITypeSymbol>dependencyTypes =
        new HashSet<ITypeSymbol>();

    private readonly HashSet<INamedTypeSymbol>grainInterfacesToProcess =
        new HashSet<INamedTypeSymbol>();
    private readonly HashSet<INamedTypeSymbol>grainClassesToProcess =
        new HashSet<INamedTypeSymbol>();
    private readonly HashSet<INamedTypeSymbol>serializationTypesToProcess =
        new HashSet<INamedTypeSymbol>();
    private readonly HashSet<INamedTypeSymbol>fieldOfSerializableType =
        new HashSet<INamedTypeSymbol>();
    ...
    public CompilationAnalyzer(ILogger log, WellKnownTypes wellKnownTypes,
      ...
      this.considerForCodeGenerationAttribute =
          wellKnownTypes.ConsiderForCodeGenerationAttribute;
      this.compilation = compilation;
    }

    public HashSet<INamedTypeSymbol>CodeGenerationRequiredTypes { get; }
    = new HashSet<INamedTypeSymbol>();

    /// <summary>
    /// All assemblies referenced by this compilation.
    /// </summary>
    public HashSet<IAssemblySymbol>ReferencedAssemblies =
        new HashSet<IAssemblySymbol>();

    /// <summary>
    /// Assemblies which should be excluded from code generation (eg, because
    /// they already contain generated code).
    /// </summary>
    public HashSet<IAssemblySymbol>AssembliesExcludedFromCodeGeneration =
        new HashSet<IAssemblySymbol>();

    /// <summary>
    /// Assemblies which should be excluded from metadata generation.
    /// </summary>
    public HashSet<IAssemblySymbol>AssembliesExcludedFromMetadataGeneration =
        new HashSet<IAssemblySymbol>();

    public HashSet<IAssemblySymbol>KnownAssemblies { get; }
    = new HashSet<IAssemblySymbol>();
    public HashSet<INamedTypeSymbol>KnownTypes { get; }
    = new HashSet<INamedTypeSymbol>();

    public(IEnumerable<INamedTypeSymbol>grainClasses,
           IEnumerable<INamedTypeSymbol>grainInterfaces,
           IEnumerable<INamedTypeSymbol>types)
        GetTypesToProcess() => (this.grainClassesToProcess,
                                this.grainInterfacesToProcess,
                                this.GetSerializationTypesToProcess());
    ...
    private IEnumerable<INamedTypeSymbol>GetSerializationTypesToProcess() {
      ...
      while (done.Count != this.serializationTypesToProcess.Count) {
        ...
        foreach(var type in remaining) { yield return type; }
      }
    }
    ...
    public bool IsSerializable(INamedTypeSymbol type) {
      ...
      if (type.IsSerializable ||
          type.HasAttribute(this.serializableAttribute)) {
        if (log.IsEnabled(LogLevel.Debug))
          log.LogTrace($"Type {type} has [Serializable] attribute.");
        result = true;
      }

      if (!result && this.assembliesWithForcedSerializability.Contains(
                         type.ContainingAssembly)) {
        if (log.IsEnabled(LogLevel.Debug))
          log.LogTrace(
              $"Type {type} is declared in an assembly in which all types are considered serializable");
        result = true;
      }

      if (!result && this.KnownTypes.Contains(type)) {
        if (log.IsEnabled(LogLevel.Debug))
          log.LogTrace($"Type {type} is a known type");
        result = true;
      }
      ...
      if (!result && this.dependencyTypes.Contains(type)) {
        if (log.IsEnabled(LogLevel.Debug))
          log.LogTrace(
              $"Type {type} was discovered on a grain method signature or in another serializable type");
        result = true;
      }

      if (!result) {
        for (var current = type; current != null; current = current.BaseType) {
          if (!knownBaseTypes.Contains(current))
            continue;

          if (log.IsEnabled(LogLevel.Debug))
            log.LogTrace($"Type {type} has a known base type");
          result = true;
        }
      }
      ...
      if (!result) {
        foreach(var iface in type.AllInterfaces) {
          if (!knownBaseTypes.Contains(iface))
            continue;

          if (log.IsEnabled(LogLevel.Debug))
            log.LogTrace($"Type {type} has a known base interface");
          result = true;
        }
      }
      ...
      if (!result && this.fieldOfSerializableType.Contains(type)) {
        if (log.IsEnabled(LogLevel.Debug))
          log.LogTrace(
              $"Type {type} is used in a field of another serializable type");
        result = true;
      }

      if (!result) {
        if (log.IsEnabled(LogLevel.Trace))
          log.LogTrace($"Type {type} is not serializable");
      } else {
        ...
        void ExpandGenericArguments(ITypeSymbol typeSymbol) {
          if (typeSymbol is INamedTypeSymbol named &&
              this.fieldOfSerializableType.Add(named)) {
            InspectType(named);
    ...
    public bool
    IsFromKnownAssembly(ITypeSymbol type) => this.KnownAssemblies.Contains(
        type.OriginalDefinition.ContainingAssembly);
    ...
    private void InspectGrainInterface(INamedTypeSymbol type) {
      ...
      foreach(var method in type.GetInstanceMembers<IMethodSymbol>()) {
        ...
        if (!awaitable && !method.ReturnsVoid) {
          var message =
              $"Grain interface {type} has method {method} which returns a non-awaitable type {method.ReturnType}." +
        ...
        if (method.ReturnType is INamedTypeSymbol returnType) {
          foreach(
              var named in ExpandType(returnType).OfType<INamedTypeSymbol>()) {
        ...
        foreach(var param in method.Parameters) {
          if (param.Type is INamedTypeSymbol parameterType) {
            foreach(var named in ExpandType(parameterType)
      ...
      bool IsAwaitable(IMethodSymbol method) {
        ...
        bool HasZeroParameters(IMethodSymbol m) =>
            m.Parameters.Length == 0 && m.TypeParameters.Length == 0;
      }
    }
    ...
    private static IEnumerable<ITypeSymbol>ExpandType(ITypeSymbol symbol) {
      ...
      IEnumerable<ITypeSymbol>ExpandTypeInternal(ITypeSymbol s,
                                                 HashSet<ITypeSymbol>emitted) {
        if (!emitted.Add(s))
          yield break;
        yield return s;
        switch (s) {
        case IArrayTypeSymbol array:
          foreach(var t in ExpandTypeInternal(array.ElementType, emitted))
              yield return t;
          break;
        case INamedTypeSymbol named:
          foreach(var p in named.TypeArguments)
              foreach(var t in ExpandTypeInternal(p, emitted)) yield return t;
          break;
        }

        if (s.BaseType != null) {
          foreach(var t in ExpandTypeInternal(s.BaseType, emitted))
              yield return t;
        }
      }
    }
    ...
    public void InspectType(INamedTypeSymbol type) {
      if (type.HasAttribute(knownBaseTypeAttribute))
        this.AddKnownBaseType(type);
      if (this.wellKnownTypes.IsGrainInterface(type))
        this.InspectGrainInterface(type);
      if (this.wellKnownTypes.IsGrainClass(type))
        this.grainClassesToProcess.Add(type);
      this.serializationTypesToProcess.Add(type);
    }

    public void Analyze() {
      foreach(var reference in this.compilation.References) {
        if (!(this.compilation.GetAssemblyOrModuleSymbol(reference)
                  is IAssemblySymbol asm))
          continue;
        this.ReferencedAssemblies.Add(asm);
      }

      // Recursively all assemblies considered known from the inspected
      // assembly.
      ...
      void ExpandKnownAssemblies(IAssemblySymbol asm) {
        ...
        if (!asm.GetAttributes(this.knownAssemblyAttribute, out var attrs))
          return;
        ...
        foreach(var attr in attrs) {
          ...
          if (param.Kind != TypedConstantKind.Type) {
            throw new ArgumentException(
                $"Unrecognized argument type in attribute [{attr.AttributeClass.Name}({param.ToCSharpString()})]");
          }

          var type = (ITypeSymbol) param.Value;
          if (log.IsEnabled(LogLevel.Debug))
            log.LogDebug(
                $"Known assembly {type.ContainingAssembly} from assembly {asm}");

          // Check if the attribute has the TreatTypesAsSerializable property
          // set.
          var prop =
              attr.NamedArguments
                  .FirstOrDefault(a => a.Key.Equals("TreatTypesAsSerializable"))
                  .Value;
          if (prop.Type != null) {
            var treatAsSerializable = (bool) prop.Value;
            if (treatAsSerializable) {
              // When checking if a type in this assembly is serializable,
              // always respond that it is.
      ...
      void ExpandKnownTypes(IEnumerable<IAssemblySymbol>asm) {
        foreach(var a in asm) {
          if (!a.GetAttributes(this.considerForCodeGenerationAttribute,
                               out var attrs))
            continue;
          ...
          foreach(var attr in attrs) {
            ...
            if (typeParam.Kind != TypedConstantKind.Type) {
              throw new ArgumentException(
                  $"Unrecognized argument type in attribute [{attr.AttributeClass.Name}({typeParam.ToCSharpString()})]");
            }
            ...
            var throwOnFailureParam =
                attr.ConstructorArguments.ElementAtOrDefault(2);
            if (throwOnFailureParam.Type != null) {
              throwOnFailure = (bool) throwOnFailureParam.Value;
              if (throwOnFailure)
                this.CodeGenerationRequiredTypes.Add(type);
            }

            if (log.IsEnabled(LogLevel.Debug))
              log.LogDebug(
                  $"Known type {type}, Throw on failure: {throwOnFailure}");
          }
        }
      }
    }
      ...
      foreach(var asm in this.ReferencedAssemblies) {
        ...
        foreach(var attr in attrs) {
          ...
          if (attr.ConstructorArguments.Length >= 2 &&
              attr.ConstructorArguments[1].Value is bool val) {
            metadataOnly = val;
          } else {
            metadataOnly = false;
          }
          ...
          foreach(var candidate in this.ReferencedAssemblies) {
            ...
            if (string.Equals(assemblyName, candidate.Identity.Name,
                              StringComparison.OrdinalIgnoreCase)) {
              hasGeneratedCode = true;
            } else if (string.Equals(assemblyName,
                                     candidate.Identity.GetDisplayName())) {
              hasGeneratedCode = true;
            } else if (string.Equals(
                           assemblyName,
                           candidate.Identity.GetDisplayName(fullKey
                                                             : true))) {
              hasGeneratedCode = true;
            } else {
    ...
    public void AddAssemblyWithForcedSerializability(IAssemblySymbol asm) =>
        this.assembliesWithForcedSerializability.Add(asm);

    public void AddKnownBaseType(INamedTypeSymbol type) {
      if (log.IsEnabled(LogLevel.Debug))
        this.log.LogDebug($"Added known base type {type}");
    ...
    public void AddDependencyType(ITypeSymbol type) {
      ...
      if (named.IsGenericType && !named.IsUnboundGenericType) {
        ...
        if (SymbolEqualityComparer.Default.Equals(unbound,
                                                  this.wellKnownTypes.Task_1)) {
          return;
        }
      }
