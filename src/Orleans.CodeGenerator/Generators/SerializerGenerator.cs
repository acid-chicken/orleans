...
namespace Orleans.CodeGenerator.Generators {
  ...
  /// global::Orleans.CodeGeneration.SerializerAttribute(typeof(global::MyType))]
  /// internal sealed class
  /// OrleansCodeGenUnitTests_GrainInterfaces_MyTypeSerializer
  /// {
  ///    private readonly global::System.Func&lt;global::MyType,
  ///    global::System.Int32&gt; getField0; private readonly
  ///    global::System.Action&lt;global::MyType, global::System.Int32&gt;
  ///    setField0; public
  ///    OrleansCodeGenUnitTests_GrainInterfaces_MyTypeSerializer(global::Orleans.Serialization.IFieldUtils
  ///    fieldUtils)
  ///    {
  ///        [...]
  ///    }
  ///    [global::Orleans.CodeGeneration.CopierMethodAttribute]
  ///    public global::System.Object DeepCopier(global::System.Object original,
  ///    global::Orleans.Serialization.ICopyContext context)
  ///    {
  ///            [...]
  ///    }
  ///    [global::Orleans.CodeGeneration.SerializerMethodAttribute]
  ///    public void Serializer(global::System.Object untypedInput,
  ///    global::Orleans.Serialization.ISerializationContext context,
  ///    global::System.Type expected)
  ///    {
  ///            [...]
  ///    }
  ///    [global::Orleans.CodeGeneration.DeserializerMethodAttribute]
  ///    public global::System.Object Deserializer(global::System.Type expected,
  ///    global::Orleans.Serialization.IDeserializationContext context)
  ///    {
  ///            [...]
  ///    }
  ///}
  ...
  internal class SerializerGenerator {
    ...
    public SerializerGenerator(CodeGeneratorOptions options,
                               WellKnownTypes wellKnownTypes) {
      this.options = options;
      this.wellKnownTypes = wellKnownTypes;
    }

    private readonly ConcurrentDictionary<ITypeSymbol, bool>
        ShallowCopyableTypes = new ConcurrentDictionary<ITypeSymbol, bool>();

    /// <summary>
    /// Returns the name of the generated class for the provided type.
    /// </summary>
    /// <param name="type">The type.</param>
    /// <returns>The name of the generated class for the provided
    /// type.</returns>
    ...
    internal(TypeDeclarationSyntax, TypeSyntax)
        GenerateClass(SemanticModel model,
                      SerializerTypeDescription description, ILogger logger) {
      var className = GetGeneratedClassName(description.Target);
      var type = description.Target;
      var genericTypes = type.GetHierarchyTypeParameters()
                             .Select(_ => TypeParameter(_.ToString()))
                             .ToArray();

      var attributes = new List<AttributeSyntax>{
          GeneratedCodeAttributeGenerator.GetGeneratedCodeAttributeSyntax(
              wellKnownTypes),
          Attribute(
              wellKnownTypes.ExcludeFromCodeCoverageAttribute.ToNameSyntax()),
          Attribute(wellKnownTypes.SerializerAttribute.ToNameSyntax())
              .AddArgumentListArguments(AttributeArgument(TypeOfExpression(
                  type.WithoutTypeParameters().ToTypeSyntax())))};
      ...
      var classDeclaration =
          ClassDeclaration(className)
              ...
              .AddAttributeLists(
                  AttributeList().AddAttributes(attributes.ToArray()))
              .AddMembers(members.ToArray())
              .AddConstraintClauses(type.GetTypeConstraintSyntax());

      if (genericTypes.Length > 0) {
        classDeclaration =
            classDeclaration.AddTypeParameterListParameters(genericTypes);
      }

      if (this.options.DebuggerStepThrough) {
        var debuggerStepThroughAttribute = Attribute(
            this.wellKnownTypes.DebuggerStepThroughAttribute.ToNameSyntax());
        classDeclaration = classDeclaration.AddAttributeLists(
            AttributeList().AddAttributes(debuggerStepThroughAttribute));
      }

      return (classDeclaration,
              ParseTypeName(type.GetParsableReplacementName(className)));
    }

    private MemberDeclarationSyntax
    GenerateConstructor(string className, List<FieldInfoMember>fields) {
      var body = new List<StatementSyntax>();

      // Expressions for specifying binding flags.
      var bindingFlags =
          SymbolSyntaxExtensions.GetBindingFlagsParenthesizedExpressionSyntax(
              SyntaxKind.BitwiseOrExpression, BindingFlags.Instance,
      ...
      foreach(var field in fields) {
        ...
        var fieldInfo =
            InvocationExpression(
                TypeOfExpression(field.Field.ContainingType.ToTypeSyntax())
                    .Member("GetField"))
                .AddArgumentListArguments(
                    Argument(field.Field.Name.ToLiteralExpression()),
                    Argument(bindingFlags));
        var fieldInfoVariable =
            VariableDeclarator(field.InfoFieldName)
                .WithInitializer(EqualsValueClause(fieldInfo));

        if (!field.IsGettableProperty || !field.IsSettableProperty) {
          body.Add(LocalDeclarationStatement(
              VariableDeclaration(wellKnownTypes.FieldInfo.ToTypeSyntax())
                  .AddVariables(fieldInfoVariable)));
        }

        // Set the getter/setter of the field
        if (!field.IsGettableProperty) {
          var getterType =
              wellKnownTypes.Func_2
                  .Construct(field.Field.ContainingType, field.SafeType)
                  .ToTypeSyntax();

          var getterInvoke = CastExpression(
              getterType,
              InvocationExpression(fieldUtils.Member("GetGetter"))
                  .AddArgumentListArguments(Argument(fieldInfoField)));

          body.Add(ExpressionStatement(AssignmentExpression(
              SyntaxKind.SimpleAssignmentExpression,
              IdentifierName(field.GetterFieldName), getterInvoke)));
        }

        if (!field.IsSettableProperty) {
          if (field.Field.ContainingType != null &&
              field.Field.ContainingType.IsValueType) {
            var setterType =
                wellKnownTypes.ValueTypeSetter_2
                    .Construct(field.Field.ContainingType, field.SafeType)
                    .ToTypeSyntax();
            ...
            body.Add(ExpressionStatement(AssignmentExpression(
                SyntaxKind.SimpleAssignmentExpression,
                IdentifierName(field.SetterFieldName), getValueSetterInvoke)));
          } else {
            var setterType =
                wellKnownTypes.Action_2
                    .Construct(field.Field.ContainingType, field.SafeType)
                    .ToTypeSyntax();
            ...
            body.Add(ExpressionStatement(
                AssignmentExpression(SyntaxKind.SimpleAssignmentExpression,
                                     IdentifierName(field.SetterFieldName),
                                     getReferenceSetterInvoke)));
          }
        }
      }
      ...
      return ConstructorDeclaration(className)
          ...
          .AddParameterListParameters(
              Parameter(fieldUtils.Identifier)
                  .WithType(wellKnownTypes.IFieldUtils.ToTypeSyntax()))
    ...
    GenerateDeserializerMethod(INamedTypeSymbol type,
      ...
      foreach(var field in fields) {
        ...
        body.Add(ExpressionStatement(field.GetSetter(
            resultVariable, CastExpression(field.Type, deserialized))));
      }

      // If the type implements the internal IOnDeserialized lifecycle method,
      // invoke it's method now.
      if (type.HasInterface(wellKnownTypes.IOnDeserialized)) {
        // C#: ((IOnDeserialized)result).OnDeserialized(context);
        var typedResult = ParenthesizedExpression(CastExpression(
            wellKnownTypes.IOnDeserialized.ToTypeSyntax(), resultVariable));
        var invokeOnDeserialized =
            InvocationExpression(typedResult.Member("OnDeserialized"))
                .AddArgumentListArguments(Argument(contextParameter));
        body.Add(ExpressionStatement(invokeOnDeserialized));
      }

      body.Add(
          ReturnStatement(CastExpression(type.ToTypeSyntax(), resultVariable)));
      return MethodDeclaration(wellKnownTypes.Object.ToTypeSyntax(),
                               "Deserializer")
          .AddModifiers(Token(SyntaxKind.PublicKeyword))
          .AddParameterListParameters(
              Parameter(Identifier("expected"))
                  .WithType(wellKnownTypes.Type.ToTypeSyntax()),
              Parameter(Identifier("context"))
                  .WithType(
                      wellKnownTypes.IDeserializationContext.ToTypeSyntax()))
          .AddBodyStatements(body.ToArray())
          .AddAttributeLists(AttributeList().AddAttributes(Attribute(
              wellKnownTypes.DeserializerMethodAttribute.ToNameSyntax())));
    }

    private MemberDeclarationSyntax
    GenerateSerializerMethod(INamedTypeSymbol type, List<FieldInfoMember>fields,
                             SemanticModel model) {
      var contextParameter = IdentifierName("context");

      var body = new List<StatementSyntax>{LocalDeclarationStatement(
          VariableDeclaration(type.ToTypeSyntax())
              .AddVariables(VariableDeclarator("input").WithInitializer(
                  EqualsValueClause(CastExpression(
                      type.ToTypeSyntax(), IdentifierName("untypedInput"))))))};
      ...
      foreach(var field in fields) {
        body.Add(ExpressionStatement(
            InvocationExpression(contextParameter.Member("SerializeInner"))
                .AddArgumentListArguments(
                    Argument(field.GetGetter(inputExpression, forceAvoidCopy
                                             : true)),
                    Argument(TypeOfExpression(field.Type)))));
      }

      return MethodDeclaration(wellKnownTypes.Void.ToTypeSyntax(), "Serializer")
          .AddModifiers(Token(SyntaxKind.PublicKeyword))
          .AddParameterListParameters(
              Parameter(Identifier("untypedInput"))
                  .WithType(wellKnownTypes.Object.ToTypeSyntax()),
              Parameter(Identifier("context"))
                  .WithType(
                      wellKnownTypes.ISerializationContext.ToTypeSyntax()),
              Parameter(Identifier("expected"))
                  .WithType(wellKnownTypes.Type.ToTypeSyntax()))
          .AddBodyStatements(body.ToArray())
          .AddAttributeLists(AttributeList().AddAttributes(Attribute(
              wellKnownTypes.SerializerMethodAttribute.ToNameSyntax())));
    }
    ...
    /// </summary>
    private MemberDeclarationSyntax
    GenerateDeepCopierMethod(INamedTypeSymbol type, List<FieldInfoMember>fields,
                             SemanticModel model) {
      ...
      if (type.HasInterface(wellKnownTypes.ImmutableAttribute)) {
        ...
        var comment = Comment(
            $"// No deep copy required since {typeName} is marked with the [Immutable] attribute.");
        body.Add(ReturnStatement(originalVariable).WithLeadingTrivia(comment));
      } else {
        body.Add(LocalDeclarationStatement(
            VariableDeclaration(type.ToTypeSyntax())
                .AddVariables(VariableDeclarator("input").WithInitializer(
                    EqualsValueClause(ParenthesizedExpression(CastExpression(
                        type.ToTypeSyntax(), originalVariable)))))));
        body.Add(LocalDeclarationStatement(
            VariableDeclaration(type.ToTypeSyntax())
                .AddVariables(VariableDeclarator("result").WithInitializer(
                    EqualsValueClause(
        ...
        body.Add(ExpressionStatement(
            InvocationExpression(context.Member("RecordCopy"))
                .AddArgumentListArguments(Argument(originalVariable),
                                          Argument(resultVariable))));

        // Copy all members from the input to the result.
        foreach(var field in fields) {
          body.Add(ExpressionStatement(field.GetSetter(
              resultVariable, field.GetGetter(inputVariable, context))));
        }
      ...
      return MethodDeclaration(wellKnownTypes.Object.ToTypeSyntax(),
          ...
          .AddModifiers(Token(SyntaxKind.PublicKeyword))
          .AddParameterListParameters(
              Parameter(Identifier("original"))
                  .WithType(wellKnownTypes.Object.ToTypeSyntax()),
              Parameter(Identifier("context"))
                  .WithType(wellKnownTypes.ICopyContext.ToTypeSyntax()))
          .AddBodyStatements(body.ToArray())
          .AddAttributeLists(AttributeList().AddAttributes(
              Attribute(wellKnownTypes.CopierMethodAttribute.ToNameSyntax())));
    }
    ...
    private MemberDeclarationSyntax[] GenerateFields(
      ...
      foreach(var field in fields) {
        ...
        if (!field.IsGettableProperty) {
          ...
          result.Add(FieldDeclaration(VariableDeclaration(getterType)
                                          .AddVariables(fieldGetterVariable))
        ...
        if (!field.IsSettableProperty) {
          if (field.Field.ContainingType != null &&
            ...
            result.Add(FieldDeclaration(VariableDeclaration(setterType)
                                            .AddVariables(fieldSetterVariable))
                           .AddModifiers(Token(SyntaxKind.PrivateKeyword),
                                         Token(SyntaxKind.ReadOnlyKeyword)));
          } else {
            var setterType =
    ...
    GetObjectCreationExpressionSyntax(INamedTypeSymbol type,
                                      SemanticModel model) {
      ...
      if (type.IsValueType) {
        ...
        result = ObjectCreationExpression(type.ToTypeSyntax())
                     .AddArgumentListArguments();
      } else {
        ...
        result = CastExpression(
            type.ToTypeSyntax(),
            InvocationExpression(
                wellKnownTypes.FormatterServices.ToTypeSyntax().Member(
    ...
    /// <summary>
    /// Return the default constructor on <paramref name="type"/> if found or
    /// null if not found.
    /// </summary>
    private IMethodSymbol GetEmptyConstructor(INamedTypeSymbol type,
                                              SemanticModel model) {
      return type.GetDeclaredInstanceMembers<IMethodSymbol>().FirstOrDefault(
          method => method.MethodKind == MethodKind.Constructor &&
                    method.Parameters.Length == 0 &&
                    model.IsAccessible(0, method));
    }

    /// <summary>
    /// Returns a sorted list of the fields of the provided type.
    /// </summary>
    private List<FieldInfoMember>
    GetFields(SemanticModel model, INamedTypeSymbol type, ILogger logger) {
      ...
      foreach(var field in type.GetDeclaredInstanceMembers<IFieldSymbol>()) {
        if (ShouldSerializeField(field)) {
          result.Add(
              new FieldInfoMember(this, model, type, field, result.Count));
        }
      }

      if (type.TypeKind == TypeKind.Class) {
        // Some reference assemblies are compiled without private fields.
        // Warn the user if they are inheriting from a type in one of these
        // assemblies using a heuristic: If the type inherits from a type in a
        // reference assembly and there are no fields declared on those base
        // types, emit a warning.
        var hasUnsupportedRefAsmBase = false;
        var referenceAssemblyHasFields = false;
        var baseType = type.BaseType;
        while (baseType != null && !baseType.Equals(wellKnownTypes.Object) &&
               !baseType.Equals(wellKnownTypes.Attribute)) {
          if (!hasUnsupportedRefAsmBase &&
              baseType.ContainingAssembly.HasAttribute(
                  "ReferenceAssemblyAttribute") &&
              !IsSupportedRefAsmType(baseType)) {
            hasUnsupportedRefAsmBase = true;
          }
          foreach(var field in
                      baseType.GetDeclaredInstanceMembers<IFieldSymbol>()) {
            if (hasUnsupportedRefAsmBase)
        ...
        if (hasUnsupportedRefAsmBase && !referenceAssemblyHasFields) {
          ...
          if (declaration != null) {
            ...
            if (location.IsInSource) {
              ...
              fileLocation = string.Format("{0}({1},{2},{3},{4}): ", pos.Path,
    ...
    /// <summary>
    /// Returns <see langowrd="true"/> if the provided field should be
    /// serialized, <see langword="false"/> otherwise.
    /// </summary>
    public bool ShouldSerializeField(IFieldSymbol symbol) {
      if (symbol.IsStatic)
        return false;
      if (symbol.HasAttribute(wellKnownTypes.NonSerializedAttribute))
        return false;

      ITypeSymbol fieldType = symbol.Type;

      if (fieldType.TypeKind == TypeKind.Pointer)
        return false;
      if (fieldType.TypeKind == TypeKind.Delegate)
        return false;

      if (wellKnownTypes.IntPtr.Equals(fieldType))
        return false;
      if (wellKnownTypes.UIntPtr.Equals(fieldType))
        return false;

      if (symbol.ContainingType.HasBaseType(wellKnownTypes.MarshalByRefObject))
        return false;
    ...
    internal bool IsOrleansShallowCopyable(ITypeSymbol type,
      ...
      if (wellKnownTypes.TimeSpan.Equals(type) ||
          ...
          wellKnownTypes.CorrelationId is
                  WellKnownTypes.Some correlationIdType &&
              correlationIdType.Value.Equals(type) ||
      ...
      if (namedType.IsGenericType &&
          wellKnownTypes.Immutable_1.Equals(namedType.ConstructedFrom)) {
        return ShallowCopyableTypes[type] = true;
      }

      if (type.TypeKind == TypeKind.Struct && !namedType.IsGenericType &&
          !namedType.IsUnboundGenericType) {
        return ShallowCopyableTypes[type] =
                   IsValueTypeFieldsShallowCopyable(type, examining);
      }
    ...
    private bool
    IsValueTypeFieldsShallowCopyable(ITypeSymbol type,
                                     HashSet<ITypeSymbol>examining) {
      foreach(var field in type.GetInstanceMembers<IFieldSymbol>()) {
        ...
        if (!IsOrleansShallowCopyable(fieldType, examining))
          return false;
      }
    ...
    private class FieldInfoMember {
      ...
      public FieldInfoMember(SerializerGenerator generator, SemanticModel model,
                             INamedTypeSymbol targetType, IFieldSymbol field,
                             int ordinal) {
      ...
      /// </summary>
      /// <remarks>
      /// If the field is of type 'dynamic', we represent it as 'object' because
      /// 'dynamic' cannot appear in typeof expressions.
      /// </remarks>
      public ITypeSymbol SafeType =>
          this.Field.Type.TypeKind ==
          TypeKind.Dynamic
        ? this.wellKnownTypes.Object : this.Field.Type;
      ...
      /// <summary>
      /// Gets a value indicating whether or not this field represents a
      /// property with an accessible, non-obsolete getter.
      /// </summary>
      public bool IsGettableProperty =>
          this.Property?.GetMethod != null &&
          this.model.IsAccessible(0, this.Property.GetMethod) &&
          !this.IsObsolete;

      /// <summary>
      /// Gets a value indicating whether or not this field represents a
      /// property with an accessible, non-obsolete setter.
      /// </summary>
      public bool IsSettableProperty =>
          this.Property?.SetMethod != null &&
          this.model.IsAccessible(0, this.Property.SetMethod) &&
          !this.IsObsolete;
      ...
      /// <summary>
      /// Gets the <see cref="Property"/> which this field is the backing
      /// property for, or <see langword="null" /> if this is not the backing
      /// field of an auto-property.
      /// </summary>
      private IPropertySymbol Property {
        get {
          ...
          if (!propertyName.Success || this.Field.ContainingType == null)
            return null;

          var name = propertyName.Groups[1].Value;
          var candidates =
              this.targetType.GetInstanceMembers<IPropertySymbol>()
          ...
          if (candidates.Length > 1)
            return null;
          if (!this.SafeType.Equals(candidates[0].Type))
            return null;
      ...
      private bool IsObsolete =>
          this.Field.HasAttribute(this.wellKnownTypes.ObsoleteAttribute) ||
          this.Property != null &&
              this.Property.HasAttribute(this.wellKnownTypes.ObsoleteAttribute);

      /// <summary>
      /// Returns syntax for retrieving the value of this field, deep copying it
      /// if necessary.
      /// </summary>
      /// <param name="instance">The instance of the containing type.</param>
      /// <param name="serializationContextExpression">The expression used to
      /// retrieve the serialization context.</param> <param
      /// name="forceAvoidCopy">Whether or not to ensure that no copy of the
      /// field is made.</param> <returns>Syntax for retrieving the value of
      /// this field.</returns>
      public ExpressionSyntax
      GetGetter(ExpressionSyntax instance,
                ExpressionSyntax serializationContextExpression = null,
                bool forceAvoidCopy = false) {
        // Retrieve the value of the field.
        var getValueExpression = this.GetValueExpression(instance);

        // Avoid deep-copying the field if possible.
        if (forceAvoidCopy ||
            generator.IsOrleansShallowCopyable(this.SafeType)) {
          // Return the value without deep-copying it.
          return getValueExpression;
        }

        // Addressable arguments must be converted to references before passing.
        // IGrainObserver instances cannot be directly converted to references,
        ...
        if (this.SafeType.HasInterface(this.wellKnownTypes.IAddressable) &&
          ...
          var getAsReference = getValueExpression.Member(
              "AsReference".ToGenericName().AddTypeArgumentListArguments(
                  this.Type));

          // If the value is not a GrainReference, convert it to a
          // strongly-typed GrainReference. C#: (value == null || value is
          // GrainReference) ? value : value.AsReference<TInterface>()
          deepCopyValueExpression = ConditionalExpression(
              ParenthesizedExpression(BinaryExpression(
                  SyntaxKind.LogicalOrExpression,
                  BinaryExpression(
                      SyntaxKind.EqualsExpression, getValueExpression,
                      LiteralExpression(SyntaxKind.NullLiteralExpression)),
                  BinaryExpression(
                      SyntaxKind.IsExpression, getValueExpression,
        ...
        return CastExpression(
            this.Type,
            InvocationExpression(
                serializationContextExpression.Member("DeepCopyInner"))
                .AddArgumentListArguments(Argument(deepCopyValueExpression)));
      }
      ...
      public ExpressionSyntax GetSetter(ExpressionSyntax instance,
                                        ExpressionSyntax value) {
        // If the field is the backing field for an accessible auto-property use
        ...
        if (this.Field.ContainingType != null &&
            this.Field.ContainingType.IsValueType) {
          instanceArg =
              instanceArg.WithRefOrOutKeyword(Token(SyntaxKind.RefKeyword));
        }

        return InvocationExpression(IdentifierName(this.SetterFieldName))
      ...
      private ExpressionSyntax GetValueExpression(ExpressionSyntax instance) {
        ...
        if (this.IsGettableProperty) {
          ...
          result = InvocationExpression(IdentifierName(this.GetterFieldName))
      ...
      public class Comparer : IComparer<FieldInfoMember> {
        ...
        public int Compare(FieldInfoMember x, FieldInfoMember y) {
          return string.Compare(x?.Field.Name, y?.Field.Name,
                                StringComparison.Ordinal);
        }
      }
    }
