...
namespace Orleans.CodeGenerator.Generators {
  ...
  internal class GrainReferenceGenerator {
    ...
    public GrainReferenceGenerator(CodeGeneratorOptions options,
    ...
    GenerateClass(GrainInterfaceDescription description) {
      ...
      var grainTypeArgument =
          TypeOfExpression(grainType.WithoutTypeParameters().ToTypeSyntax());

      var attributes = AttributeList().AddAttributes(
      ...
      var classDeclaration =
          ClassDeclaration(generatedTypeName)
              ...
              .AddMembers(GrainInterfaceCommon
                              .GenerateInterfaceIdProperty(this.wellKnownTypes,
                                                           description)
                              .AddModifiers(Token(SyntaxKind.OverrideKeyword)),
                          GrainInterfaceCommon
                              .GenerateInterfaceVersionProperty(
                                  this.wellKnownTypes, description)
                              .AddModifiers(Token(SyntaxKind.OverrideKeyword)),
    ...
    private MemberDeclarationSyntax[] GenerateConstructors(string className) {
      var baseConstructors = wellKnownTypes.GrainReference.Constructors.Where(
          c => c.DeclaredAccessibility != Accessibility.Private);
    ...
    /// </summary>
    private MemberDeclarationSyntax[] GenerateInvokeMethods(
        GrainInterfaceDescription description) {
      ...
      foreach(var methodDescription in methods) {
        ...
        var methodIdArgument =
            Argument(methodDescription.MethodId.ToHexLiteral());

        // Construct a new object array from all method arguments.
        var parameters =
            method.Parameters.Select((p, i) => (p, GetSanitizedName(p, i)))
                .ToList<(IParameterSymbol Symbol, string Name)>();
        var body = new List<StatementSyntax>();
        foreach(var parameter in parameters) {
          if (parameter.Symbol.Type.HasInterface(
                  wellKnownTypes.IGrainObserver)) {
            body.Add(ExpressionStatement(
                InvocationExpression(
                    wellKnownTypes.GrainFactoryBase.ToNameSyntax().Member(
                        "CheckGrainObserverParamInternal"))
                    .AddArgumentListArguments(
                        Argument(parameter.Name.ToIdentifierName()))));
          }
        }

        // Get the parameters argument value.
        var objectArrayType =
            wellKnownTypes.Object.ToTypeSyntax().GetArrayTypeSyntax();
        ExpressionSyntax args;
        if (method.IsGenericMethod) {
          // Create an arguments array which includes the method's type
          // parameters followed by the method's parameter list.
          var allParameters = new List<ExpressionSyntax>();
          foreach(var typeParameter in method.TypeParameters) {
            allParameters.Add(TypeOfExpression(typeParameter.ToTypeSyntax()));
          }

          allParameters.AddRange(parameters.Select(
              p => GetParameterForInvocation(p.Symbol, p.Name)));

          args = ArrayCreationExpression(objectArrayType)
                     .WithInitializer(
          ...
          args = ArrayCreationExpression(objectArrayType)
                     .WithInitializer(
                         InitializerExpression(
                             ...
                             .AddExpressions(
                                 parameters
                                     .Select((p => GetParameterForInvocation(
                                                  p.Symbol, p.Name)))
                                     .ToArray()));
        }
        ...
        if (method.ReturnsVoid || isOneWayTask) {
          ...
          var invocation =
              InvocationExpression(baseReference.Member("InvokeOneWayMethod"))
          ...
          if (isOneWayTask) {
            if (SymbolEqualityComparer.Default.Equals(wellKnownTypes.Task,
                                                      method.ReturnType)) {
              var done = wellKnownTypes.Task.ToNameSyntax().Member(
                  (object _) => Task.CompletedTask);
              body.Add(ReturnStatement(done));
            } else if (SymbolEqualityComparer.Default.Equals(
                           wellKnownTypes.ValueTaskNamed, method.ReturnType)) {
          ...
          // If the method doesn't return a Task type (eg, it returns
          // ValueTask<T>), then we must make an async method and await the
          // invocation result.
          var isTaskMethod =
              SymbolEqualityComparer.Default.Equals(wellKnownTypes.Task,
                                                    methodReturnType) ||
              methodReturnType.IsGenericType &&
                  SymbolEqualityComparer.Default.Equals(
                      wellKnownTypes.Task_1, methodReturnType.ConstructedFrom);
          asyncMethod = !isTaskMethod;

          var returnType = methodReturnType.IsGenericType
                                 ? methodReturnType
                               .TypeArguments[0] : wellKnownTypes.Object;
          var invokeMethodAsync =
              "InvokeMethodAsync".ToGenericName().AddTypeArgumentListArguments(
                  returnType.ToTypeSyntax());
          var invocation =
              InvocationExpression(MemberAccessExpression(
                                       SyntaxKind.SimpleMemberAccessExpression,
                                       baseReference, invokeMethodAsync))
                  .AddArgumentListArguments(methodIdArgument)
                  .AddArgumentListArguments(Argument(args));
          ...
          var methodResult = asyncMethod ? AwaitExpression(invocation)
              : (ExpressionSyntax) invocation;

          if (this.wellKnownTypes.ValueTask is WellKnownTypes.Some valueTask &&
              SymbolEqualityComparer.Default.Equals(valueTask.Value,
                                                    methodReturnType)) {
          ...
          throw new NotSupportedException(
              $"Method {method} has unsupported return type, {method.ReturnType}.");

        var paramDeclaration = method.Parameters.Select(
            (p, i) => Parameter(GetSanitizedName(p, i).ToIdentifier())
                          .WithType(p.Type.ToTypeSyntax()));
        var methodDeclaration =
            method.GetDeclarationSyntax()
                .WithParameterList(
                    ParameterList().AddParameters(paramDeclaration.ToArray()))
                .WithModifiers(TokenList())
                .WithExplicitInterfaceSpecifier(ExplicitInterfaceSpecifier(
                    method.ContainingType.ToNameSyntax()))
                .AddBodyStatements(body.ToArray())
                // Since explicit implementation is used, constraints must not
                // be specified.
        ...
        if (asyncMethod)
          methodDeclaration =
              methodDeclaration.AddModifiers(Token(SyntaxKind.AsyncKeyword));
      ...
      ExpressionSyntax GetParameterForInvocation(IParameterSymbol arg,
                                                 string name) {
        ...
        if (arg.Type.HasInterface(wellKnownTypes.IAddressable) &&
          ...
          return ConditionalExpression(
              BinaryExpression(SyntaxKind.IsExpression, identifier,
                               wellKnownTypes.Grain.ToTypeSyntax()),
              InvocationExpression(identifier.Member(
                  "AsReference".ToGenericName().AddTypeArgumentListArguments(
                      arg.Type.ToTypeSyntax()))),
              identifier);
        }
      ...
      static string GetSanitizedName(IParameterSymbol parameter, int index) {
        var parameterName =
            string.IsNullOrWhiteSpace(parameter.Name) ? "arg" : parameter.Name;
        return string.Format(CultureInfo.InvariantCulture, "{0}{1:G}",
                             parameterName, index);
      }
    }

    /// <summary>
    /// Returns syntax for the options argument to
    /// GrainReference.InvokeMethodAsync{T} and
    /// GrainReference.InvokeOneWayMethod.
    /// </summary>
    private ArgumentSyntax GetInvokeOptions(IMethodSymbol method) {
      ...
      if (method.GetAttribute(wellKnownTypes.TransactionAttribute,
        ...
        var mapping =
            values.ToDictionary(m => (int) m.ConstantValue, m => m.Name);
        if (!mapping.TryGetValue(txRequirement, out var value)) {
          throw new NotSupportedException(
              $"Transaction requirement {txRequirement} on method {method} was not understood." +
                  $" Known values: {string.Join(",
              ", mapping.Select(kv => $" { kv.Key }({kv.Value}) "))}");
        }
        ...
        default:
          throw new NotSupportedException(
              $"Transaction requirement {value} on method {method} was not understood.");
        }
      }
      ...
      if (options.Count <= 1) {
        ...
        allOptions = options.Aggregate(
            (a, b) => BinaryExpression(SyntaxKind.BitwiseOrExpression, a, b));
      }
    ...
    GenerateIsCompatibleMethod(GrainInterfaceDescription description) {
      ...
      var interfaceIds = new HashSet<int>(new[]{description.InterfaceId}.Concat(
          description.Type.AllInterfaces.Where(wellKnownTypes.IsGrainInterface)
              .Select(wellKnownTypes.GetTypeId)));

      var returnValue = default(BinaryExpressionSyntax);
      foreach(var interfaceId in interfaceIds) {
        var check =
            BinaryExpression(SyntaxKind.EqualsExpression, interfaceIdParameter,
                             interfaceId.ToHexLiteral());

        // If this is the first check, assign it, otherwise OR this check with
        // the previous checks.
        returnValue = returnValue == null
                          ? check
            : BinaryExpression(SyntaxKind.LogicalOrExpression, returnValue,
                               check);
      }
    ...
    private MemberDeclarationSyntax
    GenerateInterfaceNameProperty(GrainInterfaceDescription description) {
      var returnValue = description.Type.Name.ToLiteralExpression();
      return PropertyDeclaration(wellKnownTypes.String.ToTypeSyntax(),
                                 "InterfaceName")
          .WithExpressionBody(ArrowExpressionClause(returnValue))
    ...
    private MethodDeclarationSyntax
    GenerateGetMethodNameMethod(GrainInterfaceDescription description) {
      var method = wellKnownTypes.GrainReference.Method("GetMethodName");
      var methodDeclaration = method.GetDeclarationSyntax().AddModifiers(
          Token(SyntaxKind.OverrideKeyword));
      ...
      var callThrowMethodNotImplemented =
          InvocationExpression(IdentifierName("ThrowMethodNotImplemented"))
              .WithArgumentList(ArgumentList(SeparatedList(new[]{
                  Argument(interfaceIdArgument), Argument(methodIdArgument)})));

      // This method is used directly after its declaration to create blocks for
      // each interface id, comprising primarily of a nested switch statement
      // for each of the methods in the given interface.
      BlockSyntax ComposeInterfaceBlock(INamedTypeSymbol interfaceType,
                                        SwitchStatementSyntax methodSwitch) {
        return Block(methodSwitch.AddSections(
            SwitchSection()
      ...
      var interfaceCases =
          GrainInterfaceCommon.GenerateGrainInterfaceAndMethodSwitch(
              wellKnownTypes, description.Type, methodIdArgument,
              methodType => new StatementSyntax[]{
                  ReturnStatement(methodType.Name.ToLiteralExpression())},
              ComposeInterfaceBlock);

      // Generate the default case, which will throw a NotImplementedException.
      var callThrowInterfaceNotImplemented =
          InvocationExpression(IdentifierName("ThrowInterfaceNotImplemented"))
              .WithArgumentList(ArgumentList(
                  SingletonSeparatedList(Argument(interfaceIdArgument))));
      var defaultCase =
          SwitchSection()
              .AddLabels(DefaultSwitchLabel())
              .AddStatements(
      ...
      var throwInterfaceNotImplemented =
          GrainInterfaceCommon.GenerateMethodNotImplementedFunction(
              wellKnownTypes);
      var throwMethodNotImplemented =
          GrainInterfaceCommon.GenerateInterfaceNotImplementedFunction(
              wellKnownTypes);

      var interfaceIdSwitch = SwitchStatement(interfaceIdArgument)
                                  .AddSections(interfaceCases.ToArray())
                                  .AddSections(defaultCase);
      return methodDeclaration.AddBodyStatements(interfaceIdSwitch,
                                                 throwInterfaceNotImplemented,
