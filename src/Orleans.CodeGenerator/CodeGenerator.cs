...
namespace Orleans.CodeGenerator {
  public class CodeGenerator {
    ...
    public static readonly string Version =
        typeof(CodeGenerator).Assembly.GetName().Version.ToString();
    ...
    public CodeGenerator(Compilation compilation, CodeGeneratorOptions options,
      ...
      this.compilationAnalyzer =
          new CompilationAnalyzer(log, this.wellKnownTypes, compilation);

      var firstSyntaxTree = compilation.SyntaxTrees.FirstOrDefault()
          ?? throw new InvalidOperationException(
                 "Compilation has no syntax trees.");
      this.semanticModelForAccessibility =
          compilation.GetSemanticModel(firstSyntaxTree);
      this.serializerTypeAnalyzer =
          SerializerTypeAnalyzer.Create(this.wellKnownTypes);
      this.serializerGenerator =
          new SerializerGenerator(this.options, this.wellKnownTypes);
      this.grainMethodInvokerGenerator =
          new GrainMethodInvokerGenerator(this.options, this.wellKnownTypes);
      this.grainReferenceGenerator =
          new GrainReferenceGenerator(this.options, this.wellKnownTypes);
    }

    public CompilationUnitSyntax
    GenerateCode(CancellationToken cancellationToken) {
    ...
    private AggregatedModel AnalyzeCompilation() {
      // Inspect the target assembly to discover known assemblies and known
      // types.
      ...
      var types = this.compilationAnalyzer.KnownAssemblies
                      .SelectMany(a => a.GetDeclaredTypes())
      ...
      var (grainClasses, grainInterfaces, serializationTypes) =
          this.compilationAnalyzer.GetTypesToProcess();

      // Process each of the types into the model.
      foreach(var grainInterface in grainInterfaces) this.ProcessGrainInterface(
          model, grainInterface);
      ...
      foreach(var type in serializationTypes) this.ProcessSerializableType(
          model, type);
    ...
    private void AddAssemblyMetadata(AggregatedModel model) {
      ...
      foreach(var asm in this.compilationAnalyzer.ReferencedAssemblies) {
        ...
        if (this.compilationAnalyzer.AssembliesExcludedFromCodeGeneration
                .Contains(asm) ||
            this.compilationAnalyzer.AssembliesExcludedFromMetadataGeneration
                .Contains(asm)) {
          this.log.LogDebug(
              $"Skipping adding known types for assembly {asm.Identity.Name} since a referenced assembly already includes its types.");
          continue;
        }
      ...
      foreach(var asm in assembliesToScan) {
        this.log.LogDebug(
            $"Generating metadata for referenced assembly {asm.Identity.Name}.");
    ...
    private void ValidateModel(AggregatedModel model) {
      // Check that all types which the developer marked as requiring code
      // generation have had code generation.
      foreach(var required
                  in this.compilationAnalyzer.CodeGenerationRequiredTypes) {
        if (!model.Serializers.SerializerTypes.Any(
                t => t.Target.Equals(required))) {
    ...
    private CompilationUnitSyntax GenerateSyntax(AggregatedModel model) {
      var namespaceGroupings =
          new Dictionary<INamespaceSymbol, List<MemberDeclarationSyntax>>();
      ...
      foreach(var grainInterface in model.GrainInterfaces) {
        var nsMembers = GetNamespace(namespaceGroupings,
                                     grainInterface.Type.ContainingNamespace);
        nsMembers.Add(
            grainMethodInvokerGenerator.GenerateClass(grainInterface));
        nsMembers.Add(grainReferenceGenerator.GenerateClass(grainInterface));
      }

      var serializersToGenerate =
          model.Serializers.SerializerTypes
      ...
      foreach(var serializerType in serializersToGenerate) {
        var nsMembers = GetNamespace(namespaceGroupings,
                                     serializerType.Target.ContainingNamespace);
        TypeDeclarationSyntax generated;
        (generated, serializerType.SerializerTypeSyntax) =
            this.serializerGenerator.GenerateClass(
                this.semanticModelForAccessibility, serializerType, this.log);
        nsMembers.Add(generated);
      }

      var compilationMembers = new List<MemberDeclarationSyntax>();

      // Group the generated code by namespace since serialized types, such as
      // the generated GrainReference classes must have a stable namespace.
      foreach(var group in namespaceGroupings) {
        ...
        if (ns.IsGlobalNamespace) {
          ...
          compilationMembers.Add(
              NamespaceDeclaration(ParseName(ns.ToDisplayString()))
                  .AddMembers(members.ToArray()));
        }
      }

      // Add and generate feature populators to tie everything together.
      var (attributes, featurePopulators) =
          FeaturePopulatorGenerator.GenerateSyntax(this.wellKnownTypes, model);
      compilationMembers.AddRange(featurePopulators);

      // Add some attributes detailing which assemblies this generated code
      // targets.
      ...
      List<MemberDeclarationSyntax>GetNamespace(
          Dictionary<INamespaceSymbol, List<MemberDeclarationSyntax>>namespaces,
          INamespaceSymbol ns) {
      ...
      IEnumerable<AttributeSyntax>GetCodeGenerationTargetAttribute() {
        ...
        AttributeSyntax GenerateAttribute(IAssemblySymbol assembly) {
          ...
          this.log.LogTrace(
              $"Adding [assembly: OrleansCodeGenerationTarget(\"{assemblyName}\")]");
          var nameSyntax =
              this.wellKnownTypes.OrleansCodeGenerationTargetAttribute
                  .ToNameSyntax();
          return Attribute(nameSyntax)
              .AddArgumentListArguments(
                  AttributeArgument(assemblyName.ToLiteralExpression()));
        }
      }
    }

    private void ProcessGrainInterface(AggregatedModel model,
                                       INamedTypeSymbol type) {
      var accessible = this.semanticModelForAccessibility.IsAccessible(0, type);

      if (this.log.IsEnabled(LogLevel.Debug)) {
        this.log.LogDebug(
            $"Found grain interface {type.ToDisplayString()}{(accessible ? string.Empty : ",
            but it is inaccessible ")}");
      }

      if (accessible) {
        var genericMethod =
            type.GetInstanceMembers<IMethodSymbol>().FirstOrDefault(
                m => m.IsGenericMethod);
        if (genericMethod != null &&
            this.wellKnownTypes.GenericMethodInvoker is WellKnownTypes.None) {
        ...
        model.GrainInterfaces.Add(new GrainInterfaceDescription(
            type, this.wellKnownTypes.GetTypeId(type),
            this.wellKnownTypes.GetVersion(type), methods));
      }

      // Returns a list of all methods in all interfaces on the provided type.
      IEnumerable<GrainMethodDescription>GetGrainMethodDescriptions(
          INamedTypeSymbol initialType) {
        ...
        foreach(var iface in GetAllInterfaces(initialType)) {
          foreach(
              var method in iface.GetDeclaredInstanceMembers<IMethodSymbol>()) {
            yield return new GrainMethodDescription(
                this.wellKnownTypes.GetMethodId(method), method);
          }
        }
      }
    }

    private void ProcessGrainClass(AggregatedModel model,
                                   INamedTypeSymbol type) {
      var accessible = this.semanticModelForAccessibility.IsAccessible(0, type);

      if (this.log.IsEnabled(LogLevel.Debug)) {
        this.log.LogDebug(
            $"Found grain class {type.ToDisplayString()}{(accessible ? string.Empty : ",
            but it is inaccessible ")}");
      }

      if (accessible) {
        model.GrainClasses.Add(new GrainClassDescription(
            type, this.wellKnownTypes.GetTypeId(type)));
      }
    }
    ...
    private void ProcessSerializableType(AggregatedModel model,
      ...
      if (!ValidForKnownTypes(type)) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping abstract type {type}");
        return;
      }
      ...
      if (!accessible) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping inaccessible type {type}");
        return;
      }
      ...
      if (type.HasBaseType(this.wellKnownTypes.Exception)) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping Exception type {type}");
        return;
      }
      ...
      if (type.HasBaseType(this.wellKnownTypes.Delegate)) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping Delegate type {type}");
        return;
      }
      ...
      if (type.AllInterfaces.Contains(this.wellKnownTypes.IAddressable)) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping IAddressable type {type}");
        return;
      }

      // Account for types that serialize themselves and/or are serializers for
      // other types.
      var selfSerializing = false;
      if (this.serializerTypeAnalyzer.IsSerializer(type,
                                                   out var serializerTargets)) {
        var typeSyntax = type.ToTypeSyntax();
        foreach(var target in serializerTargets) {
          if (this.log.IsEnabled(LogLevel.Trace)) {
            this.log.LogTrace(
                $"{nameof(ProcessSerializableType)} type {type} is a serializer for {target}");
          }
          ...
          serializerModel.SerializerTypes.Add(new SerializerTypeDescription{
              Target = target, SerializerTypeSyntax = typeSyntax,
      ...
      if (selfSerializing) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping serializer generation for self-serializing type {type}");
        return;
      }
      ...
      if (type.HasAttribute(this.wellKnownTypes.GeneratedCodeAttribute)) {
        if (this.log.IsEnabled(LogLevel.Trace)) {
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} type {type} is a generated type and no serializer will be generated for it");
        }
      ...
      if (type.IsStatic) {
        if (this.log.IsEnabled(LogLevel.Trace)) {
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} type {type} is a static type and no serializer will be generated for it");
        }
      ...
      if (type.TypeParameters.Any(
              p => p.ConstraintTypes.Any(
                  c => c.Equals(this.wellKnownTypes.Delegate)))) {
        if (this.log.IsEnabled(LogLevel.Trace))
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} skipping type with Delegate parameter constraint, {type}");
        return;
      }

      var isSerializable = this.compilationAnalyzer.IsSerializable(type);
      if (this.compilationAnalyzer.IsFromKnownAssembly(type) &&
          isSerializable) {
        // Skip types which have fields whose types are inaccessible from
        // generated code.
        foreach(var field in type.GetInstanceMembers<IFieldSymbol>()) {
          ...
          if (!this.serializerGenerator.ShouldSerializeField(field)) {
            if (this.log.IsEnabled(LogLevel.Trace)) {
              this.log.LogTrace(
                  $"{nameof(ProcessSerializableType)} skipping non-serialized field {field} in type {type}");
            }
          ...
          var fieldAccessible =
              this.semanticModelForAccessibility.IsAccessible(0, field.Type);
          if (!fieldAccessible) {
            if (this.log.IsEnabled(LogLevel.Trace)) {
              this.log.LogTrace(
                  $"{nameof(ProcessSerializableType)} skipping type {type} with inaccessible field type {field.Type} (field: {field})");
            }
        ...
        // The serializer generator will fill in the missing
        // SerializerTypeSyntax field with the generated type.
        if (this.log.IsEnabled(LogLevel.Trace)) {
          this.log.LogTrace(
              $"{nameof(ProcessSerializableType)} will generate a serializer for type {type}");
        }

        serializerModel.SerializerTypes.Add(
            new SerializerTypeDescription{Target = type});
      } else if (this.log.IsEnabled(LogLevel.Trace)) {
        this.log.LogTrace(
            $"{nameof(ProcessSerializableType)} will not generate a serializer for type {type}");
      }
    }

    private static void AddKnownType(AggregatedModel model,
                                     INamedTypeSymbol type) {
      // Many types which will never have a serializer generated are still added
      // to known types so that they can be used to identify the type in a
      // serialized payload. For example, when serializing
      // List<SomeAbstractType>, SomeAbstractType must be known. The same
      // applies to interfaces (which are encoded as abstract).
      var serializerModel = model.Serializers;
    ...
    private bool ValidForKnownTypes(INamedTypeSymbol type) {
      ...
      if (this.log.IsEnabled(LogLevel.Trace))
        this.log.LogTrace($"{nameof(ValidForKnownTypes)} adding type {type}");
