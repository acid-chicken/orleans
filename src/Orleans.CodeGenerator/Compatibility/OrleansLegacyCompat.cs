...
namespace Orleans.CodeGenerator.Compatibility {
  internal static class OrleansLegacyCompat {
    public static int GetMethodId(this WellKnownTypes wellKnownTypes,
                                  IMethodSymbol methodInfo) {
      if (GetAttribute(methodInfo, wellKnownTypes.MethodIdAttribute)
              is AttributeData attr) {
    ...
    FormatMethodForMethodIdComputation(IMethodSymbol methodInfo) {
      ...
      if (methodInfo.IsGenericMethod) {
        ...
        foreach(var arg in methodInfo.TypeArguments) {
          if (!first)
            ...
            first = false;
          result.Append(RoslynTypeNameFormatter.Format(
              arg, RoslynTypeNameFormatter.Style.RuntimeTypeNameFormatter));
        }
...

      {
        ...
        foreach(var parameter in parameters) {
          ...
          default:
            result.Append(RoslynTypeNameFormatter.Format(
                parameterType,
                RoslynTypeNameFormatter.Style.RuntimeTypeNameFormatter));
            break;
          }
    ...
    public static int GetTypeId(this WellKnownTypes wellKnownTypes,
                                INamedTypeSymbol type) {
      if (GetAttribute(type, wellKnownTypes.TypeCodeOverrideAttribute)
              is AttributeData attr) {
        return (int) attr.ConstructorArguments.First().Value;
      }
    ...
    private static AttributeData GetAttribute(ISymbol type,
      ...
      foreach(var attr in attrs) {
        if (SymbolEqualityComparer.Default.Equals(attr.AttributeClass,
                                                  attributeType)) {
          return attr;
        }
      }
    ...
    private static int CalculateIdHash(string text) {
      ...
      try {
        ...
        for (var i = 0; i < result.Length; i += 4) {
          var tmp = (result[i] << 24) | (result[i + 1] << 16) |
    ...
    internal static string
    FormatTypeForIdComputation(INamedTypeSymbol symbol) => GetTemplatedName(
        GetFullName(symbol), symbol, symbol.TypeArguments, t => false);

    public static ushort GetVersion(this WellKnownTypes wellKnownTypes,
                                    ISymbol symbol) {
      if (GetAttribute(symbol, wellKnownTypes.VersionAttribute)
              is AttributeData attr) {
    ...
    /// </summary>
    public static bool IsGrainInterface(this WellKnownTypes types,
                                        INamedTypeSymbol type) {
      if (type.TypeKind != TypeKind.Interface)
        return false;

      var orig = type.OriginalDefinition;
      return orig.AllInterfaces.Contains(types.IAddressable) &&
             !IsGrainMarkerInterface(types, orig);

      bool IsGrainMarkerInterface(WellKnownTypes l, INamedTypeSymbol t) {
        return Eq(t, l.IGrainObserver) || Eq(t, l.IAddressable) ||
               Eq(t, l.IGrainExtension) || Eq(t, l.IGrain) ||
               Eq(t, l.IGrainWithGuidKey) || Eq(t, l.IGrainWithIntegerKey) ||
               Eq(t, l.IGrainWithStringKey) ||
               Eq(t, l.IGrainWithGuidCompoundKey) ||
               Eq(t, l.IGrainWithIntegerCompoundKey) || Eq(t, l.ISystemTarget);

        static bool Eq(ISymbol left, ISymbol right) =>
    ...
    /// </summary>
    public static bool IsGrainClass(this WellKnownTypes types,
                                    INamedTypeSymbol type) {
      ...
      bool IsMarkerType(WellKnownTypes l, INamedTypeSymbol t) {
        return SymbolEqualityComparer.Default.Equals(t, l.Grain) ||
               SymbolEqualityComparer.Default.Equals(t, l.GrainOfT);
      }

      bool HasBase(INamedTypeSymbol t, INamedTypeSymbol baseType) {
        if (SymbolEqualityComparer.Default.Equals(t.BaseType, baseType))
          return true;
        if (t.BaseType != null)
          return HasBase(t.BaseType, baseType);
        return false;
      }
    }
    ...
    private static void OrleansTypeKeyString(ITypeSymbol t, StringBuilder sb) {
      ...
      if (namedType != null &&
          IsGenericTypeDefinition(namedType, out var typeParamsLength)) {
        GetBaseTypeKey(t, sb);
        sb.Append('\'');
    ...
    private static void GetBaseTypeKey(ITypeSymbol type, StringBuilder sb) {
      ...
      if (type.DeclaredAccessibility == Accessibility.Public &&
        ...
        OrleansTypeKeyString(
            type.OriginalDefinition?.ContainingType ?? type.ContainingType, sb);
    ...
    GetTemplatedName(ITypeSymbol type, Func<ITypeSymbol, bool>fullName = null) {
      if (fullName == null)
        fullName = _ => true;
      ...
      case INamedTypeSymbol named when named.IsGenericType:
        return GetTemplatedName(GetSimpleTypeName(named, fullName), named,
                                named.TypeArguments, fullName);
      ...
      default:
        throw new NotSupportedException(
            $"Symbol {type} of type {type.GetType()} is not supported.");
      }
    }

    public static string
    GetTemplatedName(string baseName, INamedTypeSymbol type,
                     ImmutableArray<ITypeSymbol>genericArguments,
                     Func<ITypeSymbol, bool>fullName) {
      if (!type.IsGenericType ||
          type.ContainingType != null && type.ContainingType.IsGenericType)
        return baseName;
      var s = baseName;
      s += "<";
    ...
    public static string GetGenericTypeArgs(IEnumerable<ITypeSymbol>args,
                                            Func<ITypeSymbol, bool>fullName) {
      ...
      foreach(var genericParameter in args) {
        ...
        if (genericParameter is INamedTypeSymbol named &&
            !named.IsGenericType) {
    ...
    public static string
    GetSimpleTypeName(ITypeSymbol type,
                      Func<ITypeSymbol, bool>fullName = null) {
      ...
      if (type.ContainingType != null) {
        if (type.ContainingType.IsGenericType) {
          return GetTemplatedName(
                     ...
                     type.ContainingType,
                     named?.TypeArguments ?? default(
                         ImmutableArray<ITypeSymbol>),
                     _ => true) +
                 "." + GetUntemplatedTypeName(type.Name);
        }

        return GetTemplatedName(type.ContainingType) + "." +
               GetUntemplatedTypeName(type.Name);
      }
    ...
    public static string GetFullName(ITypeSymbol t) {
      ...
      return RoslynTypeNameFormatter.Format(
          t, RoslynTypeNameFormatter.Style
                 .FullName); // ?? (t is ITypeParameterSymbol) ? t.Name :
                             // t.GetNamespaceName() + "." + t.Name;
    ...
    static bool IsGenericTypeDefinition(INamedTypeSymbol type,
                                        out int typeParamsLength) {
      if (type.IsUnboundGenericType) {
        typeParamsLength = type.GetHierarchyTypeArguments().Count();
        return true;
      }

      if (type.IsGenericType && type.GetNestedHierarchy().All(
                                    t => SymbolEqualityComparer.Default.Equals(
                                        t.ConstructedFrom, t))) {
        typeParamsLength = type.GetHierarchyTypeArguments().Count();
        return true;
      }
