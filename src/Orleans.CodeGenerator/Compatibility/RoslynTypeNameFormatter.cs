...
namespace Orleans.CodeGenerator.Compatibility {
  /// <summary>
  /// Utility methods for formatting <see cref="ITypeSymbol"/> instances in a
  /// way which can be later parsed by <see cref="Type.GetType(string)"/>.
  /// </summary>
  internal static class RoslynTypeNameFormatter {
    private static readonly char[] SimpleNameTerminators = {'`', '*', '[', '&'};

    public enum Style { FullName, RuntimeTypeNameFormatter }

    /// <summary>
    /// Returns a <see cref="string"/> form of <paramref name="type"/> which can
    /// be parsed by <see cref="Type.GetType(string)"/>.
    /// </summary>
    ...
    private static void Format(StringBuilder builder, ITypeSymbol type,
                               bool isElementType, Style style, int depth) {
      switch (type) {
      case IPointerTypeSymbol pointer:
        Format(builder, pointer.PointedAtType, isElementType
               : true, style
               : style, depth
               : depth + 1);
      ...
      case IArrayTypeSymbol array:
        Format(builder, array.ElementType, isElementType
               : true, style
               : style, depth
               : depth + 1);
      ...
      default:
        throw new NotSupportedException(
            $"Type symbol {type} of type {type.GetType()} (interfaces: {string.Join(",
            ", type.AllInterfaces.Select(i => i.ToString()))}) is not supported");
      }

      // Types which are used as elements are not formatted with their assembly
      // name, since that is added after the element type's adornments.
      if (!isElementType) {
        switch (style) {
        case Style.FullName:
          if (depth != 0)
    ...
    private static void AddClassName(StringBuilder builder,
    ...
    private static List<ITypeSymbol>
    GetHierarchyTypeArguments(ITypeSymbol type) {
      ...
      IEnumerable<ITypeSymbol>Helper(ITypeSymbol t) {
        ...
        if (t is INamedTypeSymbol named)
          foreach(var arg in named.TypeArguments) yield return arg;
      }
    }

    private static void AddGenericParameters(StringBuilder builder,
                                             INamedTypeSymbol type, Style style,
                                             int depth) {
      // Generic type definitions (eg, List<> without parameters) and
      // non-generic types do not include any parameters in their formatting.
      ...
      if (args.Count == 0)
        return;
      if (args.All(a => a is ITypeParameterSymbol))
        return;

      builder.Append('[');
      for (var i = 0; i < args.Count; i++) {
        builder.Append('[');
        Format(builder, args[i], isElementType
               : false, style
               : style, depth
               : depth);
        builder.Append(']');
    ...
    private static void AddGenericArity(StringBuilder builder,
                                        INamedTypeSymbol type) {
    ...
    private static void AddArrayRank(StringBuilder builder,
    ...
    private static void AddAssembly(StringBuilder builder, ITypeSymbol type,
                                    Style style) {
      switch (type) {
      ...
      static bool IsSystemNamespace(INamespaceSymbol ns) {
        if (ns is null || ns.IsGlobalNamespace)
          return false;
        if (ns.ContainingNamespace is INamespaceSymbol parent &&
            !parent.IsGlobalNamespace)
          return IsSystemNamespace(parent);
        return string.Equals(ns.Name, "System", StringComparison.Ordinal) ||
