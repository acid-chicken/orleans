...
#endif
{
  internal static class DbConstantsStore {
    private static readonly Dictionary<
        string, DbConstants>invariantNameToConsts = new Dictionary<string,
                                                                   DbConstants>{
        {AdoNetInvariants.InvariantNameSqlServer,
         new DbConstants(startEscapeIndicator
                         : '[', endEscapeIndicator
                         : ']', unionAllSelectTemplate
                         : " UNION ALL SELECT ",
                           isSynchronousAdoNetImplementation
                         : false, supportsStreamNatively
                         : true, supportsCommandCancellation
                         : true, commandInterceptor
                         : NoOpCommandInterceptor.Instance)},
        {AdoNetInvariants.InvariantNameMySql,
         new DbConstants(startEscapeIndicator
                         ...
                         : " UNION ALL SELECT ",
                           isSynchronousAdoNetImplementation
                         : true, supportsStreamNatively
                         : false, supportsCommandCancellation
                         : false, commandInterceptor
                         : NoOpCommandInterceptor.Instance)},
        {AdoNetInvariants.InvariantNamePostgreSql,
         new DbConstants(
             ...
             : " UNION ALL SELECT ",
               isSynchronousAdoNetImplementation
             : true, // there are some intermittent PostgreSQL problems too, see
                     // more discussion at
                     // https://github.com/dotnet/orleans/pull/2949.
               supportsStreamNatively
             : true,
               supportsCommandCancellation
             : true, // See
                     // https://dev.mysql.com/doc/connector-net/en/connector-net-ref-mysqlclient-mysqlcommandmembers.html.
               commandInterceptor
             : NoOpCommandInterceptor.Instance)
        ...
        {AdoNetInvariants.InvariantNameOracleDatabase,
         new DbConstants(
             ...
             : '\"', endEscapeIndicator
             : '\"', unionAllSelectTemplate
             : " FROM DUAL UNION ALL SELECT ", isSynchronousAdoNetImplementation
             : true, supportsStreamNatively
             : false,
               supportsCommandCancellation
             : false, // Is supported but the remarks sound scary:
                      // https://docs.oracle.com/cd/E11882_01/win.112/e23174/OracleCommandClass.htm#DAFIEHHG.
               commandInterceptor
             : OracleCommandInterceptor.Instance)
    ...
    /// <returns><em>TRUE</em> if cancellation is supported. <em>FALSE</em>
    /// otherwise.</returns>
    public static bool
    SupportsCommandCancellation(this IRelationalStorage storage) {
    ...
    /// </summary>
    /// <param name="adoNetProvider">The ADO.NET provider invariant
    /// string.</param> <returns><em>TRUE</em> if cancellation is supported.
    ...
    /// </summary>
    /// <param name="storage">The storage used.</param>
    /// <returns><em>TRUE</em> if streaming is supported natively.
    /// <em>FALSE</em> otherwise.</returns>
    ...
    /// </summary>
    /// <param name="adoNetProvider">The ADO.NET provider invariant
    /// string.</param> <returns><em>TRUE</em> if streaming is supported
    /// natively. <em>FALSE</em> otherwise.</returns>
    ...
    /// <returns></returns>
    public static bool
    IsSynchronousAdoNetImplementation(this IRelationalStorage storage) {
    ...
    /// </summary>
    /// <param name="adoNetProvider">The ADO.NET provider invariant
    /// string.</param> <returns></returns>
    public static bool
    IsSynchronousAdoNetImplementation(string adoNetProvider) {
      return GetDbConstants(adoNetProvider).IsSynchronousAdoNetImplementation;
    }

    public static ICommandInterceptor
    GetDatabaseCommandInterceptor(string invariantName) {
  ...
  internal class DbConstants {
    ...
    /// <summary>
    /// Indicates whether the ADO.net provider does only support synchronous
    /// operations.
    /// </summary>
    public readonly bool IsSynchronousAdoNetImplementation;

    /// <summary>
    /// Indicates whether the ADO.net provider does streaming operations
    /// natively.
    /// </summary>
    public readonly bool SupportsStreamNatively;

    /// <summary>
    /// Indicates whether the ADO.net provider supports cancellation of
    /// commands.
    /// </summary>
    public readonly bool SupportsCommandCancellation;

    /// <summary>
    /// The character that indicates a start escape key for columns and tables
    /// that are reserved words.
    /// </summary>
    public readonly char StartEscapeIndicator;

    /// <summary>
    /// The character that indicates an end escape key for columns and tables
    /// that are reserved words.
    /// </summary>
    ...
    public DbConstants(char startEscapeIndicator, char endEscapeIndicator,
