...
#endif
{
  ...
  internal static class DbConnectionFactory {
    private static readonly ConcurrentDictionary<string, CachedFactory>
        factoryCache = new ConcurrentDictionary<string, CachedFactory>();

    private static readonly Dictionary<string, Tuple<string, string>>
        providerFactoryTypeMap = new Dictionary<string, Tuple<string, string>>{
            ...
            {AdoNetInvariants.InvariantNamePostgreSql,
             new Tuple<string, string>("Npgsql", "Npgsql.NpgsqlFactory")},
            {AdoNetInvariants.InvariantNameSqlLite,
             new Tuple<string, string>("Microsoft.Data.SQLite",
    ...
    private static CachedFactory GetFactory(string invariantName) {
      ...
      if (!providerFactoryTypeMap.TryGetValue(invariantName,
                                              out providerFactoryDefinition))
      ...
      try {
        ...
        throw new InvalidOperationException(
            $"Unable to find and/or load a candidate assembly for '{invariantName}' invariant name.",
            exc);
      }

      if (asm == null)
        throw new InvalidOperationException(
            $"Can't find database provider factory with '{invariantName}' invariant name. Please make sure that your ADO.Net provider package library is deployed with your application.");

      var providerFactoryType = asm.GetType(providerFactoryDefinition.Item2);
      if (providerFactoryType == null)
        throw new InvalidOperationException(
            $"Unable to load type '{providerFactoryDefinition.Item2}' for '{invariantName}' invariant name.");

      var prop = providerFactoryType.GetFields().SingleOrDefault(
          p => string.Equals(p.Name, "Instance",
                             StringComparison.OrdinalIgnoreCase) &&
               p.IsStatic);
      if (prop == null)
        throw new InvalidOperationException(
            $"Invalid provider type '{providerFactoryDefinition.Item2}' for '{invariantName}' invariant name.");

      var factory = (DbProviderFactory) prop.GetValue(null);
      return new CachedFactory(factory, providerFactoryType.Name, "",
                               providerFactoryType.AssemblyQualifiedName);
    }

    public static DbConnection CreateConnection(string invariantName,
    ...
    private class CachedFactory {
      public CachedFactory(DbProviderFactory factory, string factoryName,
      ...
      /// </summary>
      /// <remarks>For more about <see
      /// href="http://florianreischl.blogspot.fi/2011/08/adonet-connection-pooling-internals-and.html">ConnectionPool</see>
      /// and issues with using this factory. Take these notes into account when
      /// considering robustness of Orleans!</remarks>
      ...
      /// <summary>
      /// The description of the loaded factory, set by a database connector
      /// vendor.
      /// </summary>
