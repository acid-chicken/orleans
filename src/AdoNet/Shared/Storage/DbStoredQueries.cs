...
{
  /// <summary>
  /// This class implements the expected contract between Orleans and the
  /// underlying relational storage. It makes sure all the stored queries are
  /// present and
  /// </summary>
  internal class DbStoredQueries {
    ...
    /// </summary>
    internal const string GetQueriesKey =
        "SELECT QueryKey, QueryText FROM OrleansQuery";
    ...
    /// </summary>
    internal string MembershipReadRowKey =>
        queries[nameof(MembershipReadRowKey)];

    /// <summary>
    /// A query template to retrieve all membership data.
    /// </summary>
    internal string MembershipReadAllKey =>
        queries[nameof(MembershipReadAllKey)];

    /// <summary>
    /// A query template to insert a membership version row.
    /// </summary>
    internal string InsertMembershipVersionKey =>
        queries[nameof(InsertMembershipVersionKey)];

    /// <summary>
    /// A query template to update "I Am Alive Time".
    /// </summary>
    internal string UpdateIAmAlivetimeKey =>
        queries[nameof(UpdateIAmAlivetimeKey)];
    ...
    /// </summary>
    internal string DeleteMembershipTableEntriesKey =>
        queries[nameof(DeleteMembershipTableEntriesKey)];
    ...
    /// </summary>
    internal string UpsertReminderRowKey =>
        queries[nameof(UpsertReminderRowKey)];

    /// <summary>
    /// A query template to delete a reminder row.
    /// </summary>
    internal string DeleteReminderRowKey =>
        queries[nameof(DeleteReminderRowKey)];

    /// <summary>
    /// A query template to delete all reminder rows.
    /// </summary>
    internal string DeleteReminderRowsKey =>
        queries[nameof(DeleteReminderRowsKey)];

#endif

    internal class Converters {
      internal static KeyValuePair<string, string>
      GetQueryKeyAndValue(IDataRecord record) {
        return new KeyValuePair<string, string>(
            record.GetValue<string>("QueryKey"),
            record.GetValue<string>("QueryText"));
      }

      internal static ReminderEntry
      GetReminderEntry(IDataRecord record,
                       IGrainReferenceConverter grainReferenceConverter) {
        // Having non-null field, GrainId, means with the query filter options,
        // an entry was found.
        string grainId =
            record.GetValueOrDefault<string>(nameof(Columns.GrainId));
        if (grainId != null) {
          return new ReminderEntry{
              GrainRef = grainReferenceConverter.GetGrainFromKeyString(grainId),
              ReminderName =
                  record.GetValue<string>(nameof(Columns.ReminderName)),
              StartAt = record.GetValue<DateTime>(nameof(Columns.StartTime)),

              // Use the GetInt64 method instead of the generic GetValue<TValue>
              // version to retrieve the value from the data record GetValue<int>
              // causes an InvalidCastException with oracle data provider. See
              // https://github.com/dotnet/orleans/issues/3561
              Period = TimeSpan.FromMilliseconds(
                  record.GetInt64(nameof(Columns.Period))),
              ETag = GetVersion(record).ToString()};
        }
        return null;
      }

      internal static Tuple<MembershipEntry, int>
      GetMembershipEntry(IDataRecord record) {
        // TODO: This is a bit of hack way to check in the current version if
        // there's membership data or not, but if there's a start time, there's
        // member.
        DateTime? startTime =
            record.GetValueOrDefault<DateTime?>(nameof(Columns.StartTime));
        ...
        if (startTime.HasValue) {
          entry = new MembershipEntry{
              ...
              Status = (SiloStatus) Enum.Parse(
                  typeof (SiloStatus),
                  record.GetInt32(nameof(Columns.Status)).ToString()),
              ProxyPort = record.GetInt32(nameof(Columns.ProxyPort)),
              StartTime = startTime.Value,
              IAmAliveTime =
                  record.GetValue<DateTime>(nameof(Columns.IAmAliveTime))};

          string suspectingSilos =
              record.GetValueOrDefault<string>(nameof(Columns.SuspectTimes));
      ...
      private static SiloAddress GetSiloAddress(IDataRecord record,
                                                string portName) {
        // Use the GetInt32 method instead of the generic GetValue<TValue>
        // version to retrieve the value from the data record GetValue<int>
        // causes an InvalidCastException with orcale data provider. See
        // https://github.com/dotnet/orleans/issues/3561
        ...
        var siloAddress = SiloAddress.New(
            new IPEndPoint(IPAddress.Parse(address), port), generation);
        return siloAddress;
      }
    ...
    internal class Columns {
      ...
      internal Columns(IDbCommand cmd) { command = cmd; }
      ...
      internal List<ICounter>Counters {
        set {
          for (int i = 0; i < value.Count; ++i) {
            ...
            Add($"{StatValue}{i}",
                value[i].IsValueDelta ? value[i].GetDeltaString()
      ...
      internal TimeSpan Period {
        set {
          if (value.TotalMilliseconds <= int.MaxValue) {
            // Original casting when old schema is used.  Here to maintain
