namespace Orleans.CodeGenerator {
  ...
  internal static class GrainReferenceGenerator {
    ...
    /// </summary>
    private static readonly Expression<Action>
        CheckGrainObserverParamInternalExpression = () =>
    ...
    /// type.</returns>
    internal static string GetGeneratedClassName(Type type) =>
        CodeGeneratorCommon.ClassPrefix
        + TypeUtils.GetSuitableClassName(type) + ClassSuffix;
    ...
    /// <param name="onEncounteredType">The callback which is invoked when a
    /// type is encountered.</param> <returns> The generated class.
    /// </returns>
    internal static TypeDeclarationSyntax
    GenerateClass(Type grainType, string generatedTypeName,
                  Action<Type>onEncounteredType) {
      var genericTypes =
      ...
      var markerAttribute =
          SF.Attribute(typeof (GrainReferenceAttribute).GetNameSyntax())
              .AddArgumentListArguments(
                  SF.AttributeArgument(SF.TypeOfExpression(
                      grainType.GetTypeSyntax(includeGenericParameters
                                              : false))));
      var attributes = SF.AttributeList().AddAttributes(
          CodeGeneratorCommon.GetGeneratedCodeAttributeSyntax(),
          SF.Attribute(typeof (SerializableAttribute).GetNameSyntax()),
          SF.Attribute(
              typeof (ExcludeFromCodeCoverageAttribute).GetNameSyntax()),
          markerAttribute);
      ...
      var classDeclaration =
          SF.ClassDeclaration(generatedTypeName)
              ...
              .AddMembers(GenerateInterfaceIdProperty(grainType),
      ...
      foreach(var baseConstructor in baseConstructors) {
        ...
        var declaration =
            baseConstructor.GetDeclarationSyntax(className)
                .WithInitializer(SF.ConstructorInitializer(
    ...
    private static MemberDeclarationSyntax[] GenerateInvokeMethods(
        Type grainType, Action<Type>onEncounteredType) {
      ...
      foreach(var method in methods) {
        ...
        var methodIdArgument = SF.Argument(SF.LiteralExpression(
            SyntaxKind.NumericLiteralExpression, SF.Literal(methodId)));
        ...
        if (method.IsGenericMethodDefinition) {
          ...
          foreach(var typeParameter in method.GetGenericArguments()) {
            allParameters.Add(
                SF.TypeOfExpression(typeParameter.GetTypeSyntax()));
          }
              ...
              SF.ArrayCreationExpression(typeof (object).GetArrayTypeSyntax())
                  .WithInitializer(
                      SF.InitializerExpression(
                            SyntaxKind.ArrayInitializerExpression)
                          .AddExpressions(
                              parameters.Select(GetParameterForInvocation)
                                  .ToArray()));
        }
        ...
        if (method.ReturnType == typeof (void) || isOneWayTask) {
          var invocation = SF.InvocationExpression(
                                 baseReference.Member("InvokeOneWayMethod"))
          ...
          if (isOneWayTask) {
            if (method.ReturnType == typeof (Task)) {
              var done = typeof (Task).GetNameSyntax(true).Member(
                  (object _) => Task.CompletedTask);
              body.Add(SF.ReturnStatement(done));
            } else if (method.ReturnType == typeof (ValueTask)) {
              body.Add(SF.ReturnStatement(
          ...
          var invocation =
              SF.InvocationExpression(
                    baseReference.Member("InvokeMethodAsync", returnType))
                  .AddArgumentListArguments(methodIdArgument)
                  .AddArgumentListArguments(SF.Argument(args));
          ...
          if (method.ReturnType.IsGenericType &&
            ...
            // Wrapping invocation expression with initialization of ValueTask
            // (e.g. new ValueTask<int>(base.InvokeMethod()))
            returnContent =
                SF.ObjectCreationExpression(method.ReturnType.GetTypeSyntax())
                    .AddArgumentListArguments(SF.Argument(
                        SF.ExpressionStatement(invocation).Expression));
          }
        ...
        members.Add(
            method.GetDeclarationSyntax().AddBodyStatements(body.ToArray()));
      }
    ...
    /// <summary>
    /// Returns syntax for the options argument to <see
    /// cref="GrainReference.InvokeMethodAsync{T}"/> and <see
    /// cref="GrainReference.InvokeOneWayMethod"/>.
    /// </summary>
    /// <param name="method">The method which an invoke call is being generated
    /// for.</param> <returns> Argument syntax for the options argument to <see
    /// cref="GrainReference.InvokeMethodAsync{T}"/> and <see
    /// cref="GrainReference.InvokeOneWayMethod"/>, or <see langword="null"/> if
    /// no options are to be specified.
    /// </returns>
    private static ArgumentSyntax GetInvokeOptions(MethodInfo method) {
      ...
      if (GrainInterfaceUtils.IsReadOnly(method)) {
        options.Add(typeof (InvokeMethodOptions)
                        .GetNameSyntax()
                        .Member(InvokeMethodOptions.ReadOnly.ToString()));
      }

      if (GrainInterfaceUtils.IsUnordered(method)) {
        options.Add(typeof (InvokeMethodOptions)
                        .GetNameSyntax()
                        .Member(InvokeMethodOptions.Unordered.ToString()));
      }

      if (GrainInterfaceUtils.IsAlwaysInterleave(method)) {
        options.Add(
            typeof (InvokeMethodOptions)
                .GetNameSyntax()
                .Member(InvokeMethodOptions.AlwaysInterleave.ToString()));
      }

      if (GrainInterfaceUtils.TryGetTransactionOption(
              method, out TransactionOption option)) {
        switch (option) {
        case TransactionOption.Suppress:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(InvokeMethodOptions.TransactionSuppress.ToString()));
          break;
        case TransactionOption.CreateOrJoin:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(
                      InvokeMethodOptions.TransactionCreateOrJoin.ToString()));
          break;
        case TransactionOption.Create:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(InvokeMethodOptions.TransactionCreate.ToString()));
          break;
        case TransactionOption.Join:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(InvokeMethodOptions.TransactionJoin.ToString()));
          break;
        case TransactionOption.Supported:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(InvokeMethodOptions.TransactionSupported.ToString()));
          break;
        case TransactionOption.NotAllowed:
          options.Add(
              typeof (InvokeMethodOptions)
                  .GetNameSyntax()
                  .Member(
                      InvokeMethodOptions.TransactionNotAllowed.ToString()));
          break;
        default:
          throw new NotSupportedException(
              $"Transaction option {options} is not supported.");
        }
      }
      ...
      if (options.Count <= 1) {
        ...
        allOptions =
            options.Aggregate((a, b) => SF.BinaryExpression(
      ...
      return SF.Argument(SF.NameColon("options"), SF.Token(SyntaxKind.None),
                         allOptions);
    }
    ...
    private static ExpressionSyntax GetParameterForInvocation(ParameterInfo arg,
      ...
      if (typeof (IAddressable).IsAssignableFrom(arg.ParameterType) &&
        ...
        return SF.ConditionalExpression(
            SF.BinaryExpression(SyntaxKind.IsExpression, argIdentifier,
                                typeof (Grain).GetTypeSyntax()),
            SF.InvocationExpression(
                argIdentifier.Member("AsReference", arg.ParameterType)),
            argIdentifier);
      }
    ...
    GenerateInterfaceIdProperty(Type grainType) {
      ...
      return SF.PropertyDeclaration(typeof (int).GetTypeSyntax(), property.Name)
          .AddAccessorListAccessors(
              SF.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration)
                  .AddBodyStatements(SF.ReturnStatement(returnValue)))
          .AddModifiers(SF.Token(SyntaxKind.PublicKeyword),
                        SF.Token(SyntaxKind.OverrideKeyword));
    }

    private static MemberDeclarationSyntax
    GenerateInterfaceVersionProperty(Type grainType) {
      ...
      return SF
          .PropertyDeclaration(typeof (ushort).GetTypeSyntax(), property.Name)
          .AddAccessorListAccessors(
              SF.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration)
                  .AddBodyStatements(SF.ReturnStatement(returnValue)))
          .AddModifiers(SF.Token(SyntaxKind.PublicKeyword),
    ...
    private static MemberDeclarationSyntax
    GenerateIsCompatibleMethod(Type grainType) {
      var method =
          TypeUtils.Method((GrainReference _) => _.IsCompatible(default(int)));
      var methodDeclaration = method.GetDeclarationSyntax();
      var interfaceIdParameter =
          method.GetParameters()[0].Name.ToIdentifierName();

      var interfaceIds = new HashSet<int>(
      ...
      foreach(var interfaceId in interfaceIds) {
        var check = SF.BinaryExpression(
            SyntaxKind.EqualsExpression, interfaceIdParameter,
            SF.LiteralExpression(SyntaxKind.NumericLiteralExpression,
                                 SF.Literal(interfaceId)));

        // If this is the first check, assign it, otherwise OR this check with
        // the previous checks.
        returnValue = returnValue == null
                          ? check
            : SF.BinaryExpression(SyntaxKind.LogicalOrExpression, returnValue,
                                  check);
      }
    ...
    private static MemberDeclarationSyntax
    GenerateInterfaceNameProperty(Type grainType) {
      var propertyName =
          TypeUtils.Member((GrainReference _) => _.InterfaceName);
      var returnValue = grainType.GetParseableName().GetLiteralExpression();
      return SF
          .PropertyDeclaration(typeof (string).GetTypeSyntax(),
                               propertyName.Name)
          .AddAccessorListAccessors(
              SF.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration)
                  .AddBodyStatements(SF.ReturnStatement(returnValue)))
    ...
    GenerateGetMethodNameMethod(Type grainType) {
      var method = TypeUtils.Method(
          (GrainReference _) => _.GetMethodName(default(int), default(int)));
      ...
      var interfaceCases =
          CodeGeneratorCommon.GenerateGrainInterfaceAndMethodSwitch(
              grainType, methodIdArgument,
              methodType => new StatementSyntax[]{
                  SF.ReturnStatement(methodType.Name.GetLiteralExpression())});

      // Generate the default case, which will throw a NotImplementedException.
      var errorMessage = SF.BinaryExpression(
          SyntaxKind.AddExpression, "interfaceId=".GetLiteralExpression(),
          interfaceIdArgument);
      var throwStatement = SF.ThrowStatement(
          SF.ObjectCreationExpression(
                typeof (NotImplementedException).GetTypeSyntax())
              .AddArgumentListArguments(SF.Argument(errorMessage)));
      var defaultCase = SF.SwitchSection()
                            .AddLabels(SF.DefaultSwitchLabel())
                            .AddStatements(throwStatement);
      var interfaceIdSwitch = SF.SwitchStatement(interfaceIdArgument)
