...
#endif
{
  public class AzureStoragePolicyOptions {
    ...
    public TimeSpan CreationTimeout {
      get => this.creationTimeout ?? TimeSpan.FromMilliseconds(
          this.PauseBetweenCreationRetries.TotalMilliseconds *
          this.MaxCreationRetries * 3);
      set => SetIfValidTimeout(ref this.creationTimeout, value,
                               nameof(CreationTimeout));
    }

    public TimeSpan OperationTimeout {
      get => this.operationTimeout ?? TimeSpan.FromMilliseconds(
          this.PauseBetweenOperationRetries.TotalMilliseconds *
          this.MaxOperationRetries * 6);
      set => SetIfValidTimeout(ref this.operationTimeout, value,
                               nameof(OperationTimeout));
    }

    public IRetryPolicy CreationRetryPolicy {
      get => this.creationRetryPolicy ?? = new LinearRetry(
          this.PauseBetweenCreationRetries, this.MaxCreationRetries);
      set => this.creationRetryPolicy =
          value ?? throw new ArgumentNullException(nameof(CreationRetryPolicy));
    }

    public IRetryPolicy OperationRetryPolicy {
      get => this.operaionRetryPolicy ?? = new LinearRetry(
          this.PauseBetweenOperationRetries, this.MaxOperationRetries);
      set => this.operaionRetryPolicy =
          value ?? throw new ArgumentNullException(nameof(OperationRetryPolicy));
    }

    private static void SetIfValidTimeout(ref TimeSpan? field, TimeSpan value,
                                          string propertyName) {
      if (value > TimeSpan.Zero ||
          value.Equals(TimeSpan.FromMilliseconds(-1))) {
        field = value;
      } else {
