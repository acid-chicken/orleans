...
namespace Orleans.Storage {
  ...
  /// </summary>
  public class AzureTableGrainStorage : IGrainStorage,
    ...
    // each property can hold 64KB of data and each entity can take 1MB in
    // total, so 15 full properties take 15 * 64 = 960 KB leaving room for the
    // primary key, timestamp etc
    ...
    public AzureTableGrainStorage(string name, AzureTableStorageOptions options,
    ...
    public async Task ReadStateAsync(string grainType,
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace((int)
                     ...
                     grainType, pk, grainReference, this.options.TableName);
      string partitionKey = pk;
      string rowKey = grainType;
      GrainStateRecord record =
          await tableDataManager.Read(partitionKey, rowKey)
      ...
      if (record != null) {
        ...
        if (entity != null) {
          ...
          grainState.State =
              loadedState ?? Activator.CreateInstance(grainState.Type);
    ...
    /// </summary> <see cref="IGrainStorage.WriteStateAsync"/>
    public async Task WriteStateAsync(string grainType,
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace(
            (int) AzureProviderErrorCode.AzureTableProvider_WritingData,
            "Writing: GrainType={0} Pk={1} Grainid={2} ETag={3} to Table={4}",
            grainType, pk, grainReference, grainState.ETag,
            this.options.TableName);

      var entity = new DynamicTableEntity(pk, grainType);
      ConvertToStorageFormat(grainState.State, entity);
      var record =
          new GrainStateRecord{Entity = entity, ETag = grainState.ETag};
      try {
        await DoOptimisticUpdate(() => tableDataManager.Write(record),
                                 grainType, grainReference,
                                 this.options.TableName, grainState.ETag)
            .ConfigureAwait(false);
        grainState.ETag = record.ETag;
      } catch (Exception exc) {
        logger.Error(
            (int) AzureProviderErrorCode.AzureTableProvider_WriteError,
            $"Error Writing: GrainType={grainType} Grainid={grainReference} ETag={grainState.ETag} to Table={this.options.TableName} Exception={exc.Message}",
            exc);
        throw;
      }
    }
    ...
    /// </summary> <remarks> If the <c>DeleteStateOnClear</c> is set to
    /// <c>true</c> then the table row for this grain will be deleted / removed,
    /// otherwise the table row will be cleared by overwriting with default /
    /// null values.
    /// </remarks>
    /// <see cref="IGrainStorage.ClearStateAsync"/>
    public async Task ClearStateAsync(string grainType,
                                      GrainReference grainReference,
                                      IGrainState grainState) {
      if (tableDataManager == null)
        throw new ArgumentException(
      ...
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace(
            ...
            grainType, pk, grainReference, grainState.ETag,
            this.options.DeleteStateOnClear, this.options.TableName);
      var entity = new DynamicTableEntity(pk, grainType);
      var record =
          new GrainStateRecord{Entity = entity, ETag = grainState.ETag};
      ...
      try {
        if (this.options.DeleteStateOnClear) {
          operation = "Deleting";
          await DoOptimisticUpdate(() => tableDataManager.Delete(record),
                                   grainType, grainReference,
                                   this.options.TableName, grainState.ETag)
              .ConfigureAwait(false);
        } else {
        ...
        logger.Error(
            (int) AzureProviderErrorCode.AzureTableProvider_DeleteError,
            string.Format(
                "Error {0}: GrainType={1} Grainid={2} ETag={3} from Table={4} Exception={5}",
                operation, grainType, grainReference, grainState.ETag,
                this.options.TableName, exc.Message),
            exc);
        throw;
      }
    }

    private static async Task DoOptimisticUpdate(Func<Task>updateOperation,
                                                 string grainType,
                                                 GrainReference grainReference,
                                                 string tableName,
                                                 string currentETag) {
    ...
    /// <param name="entity">The Azure table entity the data should be stored
    /// in</param> <remarks> See:
    /// http://msdn.microsoft.com/en-us/library/system.web.script.serialization.javascriptserializer.aspx
    /// for more on the JSON serializer.
    /// </remarks>
    internal void ConvertToStorageFormat(object grainState,
      ...
      if (this.options.UseJson) {
        ...
        string data = Newtonsoft.Json.JsonConvert.SerializeObject(
            grainState, this.JsonSettings);

        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              "Writing JSON data size = {0} for grain id = Partition={1} / Row={2}",
        ...
        byte[] data =
            this.serializationManager.SerializeToByteArray(grainState);

        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              "Writing binary data size = {0} for grain id = Partition={1} / Row={2}",
              data.Length, entity.PartitionKey, entity.RowKey);
      ...
      foreach(var keyValuePair in properties.Zip(
          GetPropertyNames(basePropertyName),
          (property, name) =>
              new KeyValuePair<string, EntityProperty>(name, property))) {
    ...
    private static IEnumerable<string>SplitStringData(string stringData) {
      ...
      while (startIndex < stringData.Length) {
        var chunkSize = Math.Min(MAX_STRING_PROPERTY_LENGTH,
                                 stringData.Length - startIndex);
    ...
    private static IEnumerable<byte[]>SplitBinaryData(byte[] binaryData) {
      ...
      while (startIndex < binaryData.Length) {
        var chunkSize =
            Math.Min(MAX_DATA_CHUNK_SIZE, binaryData.Length - startIndex);
    ...
    GetPropertyNames(string basePropertyName) {
    ...
    ReadBinaryDataChunks(DynamicTableEntity entity) {
      foreach(var binaryDataPropertyName in GetPropertyNames(
          BINARY_DATA_PROPERTY_NAME)) {
        EntityProperty dataProperty;
        if (entity.Properties.TryGetValue(binaryDataPropertyName,
                                          out dataProperty)) {
    ...
    private static IEnumerable<string>
    ReadStringDataChunks(DynamicTableEntity entity) {
      foreach(var stringDataPropertyName in GetPropertyNames(
          STRING_DATA_PROPERTY_NAME)) {
        EntityProperty dataProperty;
        if (entity.Properties.TryGetValue(stringDataPropertyName,
                                          out dataProperty)) {
    ...
    internal object ConvertFromStorageFormat(DynamicTableEntity entity,
      ...
      try {
        if (binaryData.Length > 0) {
          // Rehydrate
          dataValue =
              this.serializationManager.DeserializeFromByteArray<object>(
                  binaryData);
        } else if (!string.IsNullOrEmpty(stringData)) {
          dataValue = Newtonsoft.Json.JsonConvert.DeserializeObject(
              stringData, stateType, this.JsonSettings);
        }
        ...
        if (binaryData.Length > 0) {
          ...
          sb.AppendFormat(
              "Unable to convert from storage format GrainStateEntity.StringData={0}",
              stringData);
        }
        if (dataValue != null) {
          sb.AppendFormat("Data Value={0} Type={1}", dataValue,
                          dataValue.GetType());
        }
    ...
    private string GetKeyString(GrainReference grainReference) {
      var key = String.Format("{0}_{1}", this.clusterOptions.ServiceId,
    ...
    private class GrainStateTableDataManager {
      ...
      public GrainStateTableDataManager(
          string tableName, string storageConnectionString, ILogger logger,
          AzureStoragePolicyOptions storagePolicyOptions) {
        this.logger = logger;
        TableName = tableName;
        tableManager = new AzureTableDataManager<DynamicTableEntity>(
            tableName, storageConnectionString, logger, storagePolicyOptions);
      }

      public Task InitTableAsync() { return tableManager.InitTableAsync(); }

      public async Task<GrainStateRecord>Read(string partitionKey,
                                              string rowKey) {
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              (int) AzureProviderErrorCode.AzureTableProvider_Storage_Reading,
              "Reading: PartitionKey={0} RowKey={1} from Table={2}",
              partitionKey, rowKey, TableName);
        try {
          Tuple<DynamicTableEntity, string>data =
              await tableManager.ReadSingleTableEntryAsync(partitionKey, rowKey)
                  .ConfigureAwait(false);
          if (data == null || data.Item1 == null) {
            if (logger.IsEnabled(LogLevel.Trace))
              logger.Trace(
                  (int) AzureProviderErrorCode.AzureTableProvider_DataNotFound,
                  "DataNotFound reading: PartitionKey={0} RowKey={1} from Table={2}",
                  partitionKey, rowKey, TableName);
            return null;
          }
          DynamicTableEntity stateEntity = data.Item1;
          var record =
              new GrainStateRecord{Entity = stateEntity, ETag = data.Item2};
          if (logger.IsEnabled(LogLevel.Trace))
            logger.Trace(
                (int)
                    AzureProviderErrorCode.AzureTableProvider_Storage_DataRead,
                "Read: PartitionKey={0} RowKey={1} from Table={2} with ETag={3}",
                stateEntity.PartitionKey, stateEntity.RowKey, TableName,
                record.ETag);
          return record;
        } catch (Exception exc) {
          if (AzureTableUtils.TableStorageDataNotFound(exc)) {
            if (logger.IsEnabled(LogLevel.Trace))
              logger.Trace(
                  (int) AzureProviderErrorCode.AzureTableProvider_DataNotFound,
                  "DataNotFound reading (exception): PartitionKey={0} RowKey={1} from Table={2} Exception={3}",
                  partitionKey, rowKey, TableName,
      ...
      public async Task Write(GrainStateRecord record) {
        ...
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              (int) AzureProviderErrorCode.AzureTableProvider_Storage_Writing,
              "Writing: PartitionKey={0} RowKey={1} to Table={2} with ETag={3}",
              entity.PartitionKey, entity.RowKey, TableName, record.ETag);
        string eTag =
            String.IsNullOrEmpty(record.ETag)
      ...
      public async Task Delete(GrainStateRecord record) {
        ...
        if (record.ETag == null) {
          if (logger.IsEnabled(LogLevel.Trace))
            logger.Trace(
                (int) AzureProviderErrorCode.AzureTableProvider_DataNotFound,
                "Not attempting to delete non-existent persistent state: PartitionKey={0} RowKey={1} from Table={2} with ETag={3}",
                entity.PartitionKey, entity.RowKey, TableName, record.ETag);
          return;
        }
        ...
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace(
              (int) AzureProviderErrorCode.AzureTableProvider_Storage_Writing,
              "Deleting: PartitionKey={0} RowKey={1} from Table={2} with ETag={3}",
              entity.PartitionKey, entity.RowKey, TableName, record.ETag);
    ...
    public bool DecodeException(Exception e, out HttpStatusCode httpStatusCode,
                                out string restStatus,
                                bool getRESTErrors = false) {
      return AzureTableUtils.EvaluateException(e, out httpStatusCode,
                                               out restStatus, getRESTErrors);
    }
    ...
    private async Task Init(CancellationToken ct) {
      ...
      try {
        this.logger.LogInformation(
            (int) AzureProviderErrorCode.AzureTableProvider_InitProvider,
            $"AzureTableGrainStorage {name} initializing: {this.options.ToString()}");
        this.logger.LogInformation(
            (int)
                AzureProviderErrorCode.AzureTableProvider_ParamConnectionString,
            $"AzureTableGrainStorage {name} is using DataConnectionString: {ConfigUtilities.RedactConnectionStringInfo(this.options.ConnectionString)}");
        this.JsonSettings = OrleansJsonSerializer.UpdateSerializerSettings(
            OrleansJsonSerializer.GetDefaultSerializerSettings(
                this.typeResolver, this.grainFactory),
            this.options.UseFullAssemblyNames, this.options.IndentJson,
            this.options.TypeNameHandling);
        this.options.ConfigureJsonSerializerSettings?.Invoke(this.JsonSettings);
        this.tableDataManager = new GrainStateTableDataManager(
            this.options.TableName, this.options.ConnectionString, this.logger,
    ...
    public void Participate(ISiloLifecycle lifecycle) {
      lifecycle.Subscribe(
          OptionFormattingUtilities.Name<AzureTableGrainStorage>(this.name),
          this.options.InitStage, Init, Close);
    }
  }
  ...
  public static class AzureTableGrainStorageFactory {
    public static IGrainStorage Create(IServiceProvider services, string name) {
      var optionsSnapshot =
          services
              .GetRequiredService<IOptionsMonitor<AzureTableStorageOptions>>();
      var clusterOptions = services.GetProviderClusterOptions(name);
      return ActivatorUtilities.CreateInstance<AzureTableGrainStorage>(
          services, name, optionsSnapshot.Get(name), clusterOptions);
    }
  }
}
