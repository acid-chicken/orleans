...
namespace Orleans.Configuration {
  ...
  public class AzureTableGrainDirectoryOptionsValidator
      : IConfigurationValidator {
    private AzureTableGrainDirectoryOptions options;

    public AzureTableGrainDirectoryOptionsValidator(
        AzureTableGrainDirectoryOptions options) {
      this.options = options;
    }

    public void ValidateConfiguration() {
      if (!CloudStorageAccount.TryParse(this.options.ConnectionString,
                                        out var ignore))
      ...
      try {
        ...
        throw new OrleansConfigurationException(
            $"Configuration for AzureTableGrainDirectoryOptions is invalid. {nameof(this.options.TableName)} is not valid.",
            ex);
      }
    }
  }
