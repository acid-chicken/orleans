...
namespace Orleans.ServiceBus.Providers {
  /// <summary>
  /// Default event hub data adapter.  Users may subclass to override event data
  /// to stream mapping.
  /// </summary>
  public class EventHubDataAdapter : IEventHubDataAdapter {
    ...
    /// <summary>
    /// Cache data adapter that adapts EventHub's EventData to
    /// CachedEventHubMessage used in cache
    /// </summary>
    ...
    /// <returns></returns>
    public virtual IBatchContainer
    GetBatchContainer(ref CachedMessage cachedMessage) {
      var evenHubMessage =
          new EventHubMessage(cachedMessage, this.serializationManager);
    ...
    /// <returns></returns>
    protected virtual IBatchContainer
    GetBatchContainer(EventHubMessage eventHubMessage) {
    ...
    /// <param name="cachedMessage"></param>
    /// <returns></returns>
    public virtual StreamSequenceToken
    ...
    ToQueueMessage<T>(Guid streamGuid, string streamNamespace,
      ...
      if (token != null)
        throw new ArgumentException(
            ...
            nameof(token));
      return EventHubBatchContainer.ToEventData(this.serializationManager,
                                                streamGuid, streamNamespace,
                                                events, requestContext);
    }

    public virtual CachedMessage
    FromQueueMessage(StreamPosition streamPosition, EventData queueMessage,
      ...
      return new CachedMessage(){
        ...
        StreamNamespace =
            ...
            null ? string.Intern(streamPosition.StreamIdentity.Namespace) : null,
    ...
    public virtual StreamPosition GetStreamPosition(string partition,
      ...
      IStreamIdentity stremIdentity =
          new StreamIdentity(streamGuid, streamNamespace);
      StreamSequenceToken token = new EventHubSequenceTokenV2(
          queueMessage.SystemProperties.Offset,
          queueMessage.SystemProperties.SequenceNumber, 0);
      return new StreamPosition(stremIdentity, token);
    }

    /// <summary>
    /// Get offset from cached message.  Left to derived class, as only it knows
    /// how to get this from the cached message.
    /// </summary>
    public virtual string GetOffset(CachedMessage lastItemPurged) {
      // TODO figure out how to get this from the adapter
      int readOffset = 0;
      return SegmentBuilder.ReadNextString(lastItemPurged.Segment,
                                           ref readOffset); // read offset
    }

    // Placed object message payload into a segment.
    protected virtual ArraySegment<byte>
    EncodeMessageIntoSegment(EventData queueMessage,
                             Func<int, ArraySegment<byte>>getSegment) {
      byte[] propertiesBytes =
          queueMessage.SerializeProperties(this.serializationManager);
      ArraySegment<byte>payload = queueMessage.Body;
      // get size of namespace, offset, partitionkey, properties, and payload
      int size = SegmentBuilder.CalculateAppendSize(
                     queueMessage.SystemProperties.Offset) +
                 SegmentBuilder.CalculateAppendSize(
                     queueMessage.SystemProperties.PartitionKey) +
      ...
      // encode namespace, offset, partitionkey, properties and payload into
      // segment
      int writeOffset = 0;
      SegmentBuilder.Append(segment, ref writeOffset,
                            queueMessage.SystemProperties.Offset);
      SegmentBuilder.Append(segment, ref writeOffset,
                            queueMessage.SystemProperties.PartitionKey);
