...
namespace Orleans.Runtime.MembershipService {
  internal class AzureBasedMembershipTable : IMembershipTable {
    ...
    public async Task InitializeMembershipTable(bool tryInitTableVersion) {
      LogFormatter.SetExceptionDecoder(typeof (StorageException),
                                       AzureTableUtils.PrintStorageException);

      this.tableManager = await OrleansSiloInstanceManager.GetManager(
          this.clusterId, this.loggerFactory, this.options);
    ...
    public async Task<MembershipTableData>ReadRow(SiloAddress key) {
      try {
        ...
        if (logger.IsEnabled(LogLevel.Debug))
          logger.Debug("Read my entry {0} Table=" + Environment.NewLine + "{1}",
                       key.ToLongString(), data.ToString());
        return data;
      } catch (Exception exc) {
        logger.Warn(
            (int) TableStorageErrorCode.AzureTable_20,
    ...
    public async Task<MembershipTableData>ReadAll() {
      try {
        ...
        if (logger.IsEnabled(LogLevel.Trace))
          logger.Trace("ReadAll Table=" + Environment.NewLine + "{0}",
                       data.ToString());

        return data;
      } catch (Exception exc) {
        logger.Warn(
            (int) TableStorageErrorCode.AzureTable_21,
    ...
    public async Task<bool>InsertRow(MembershipEntry entry,
      ...
      try {
        if (logger.IsEnabled(LogLevel.Debug))
          logger.Debug("InsertRow entry = {0}, table version = {1}",
                       entry.ToFullString(), tableVersion);
        var tableEntry = Convert(entry, tableManager.DeploymentId);
        var versionEntry =
            tableManager.CreateTableVersionEntry(tableVersion.Version);
        ...
        if (result == false)
          logger.Warn(
              (int) TableStorageErrorCode.AzureTable_22,
              $"Insert failed due to contention on the table. Will retry. Entry {entry.ToFullString()}, table version = {tableVersion}");
        return result;
      } catch (Exception exc) {
        logger.Warn(
            (int) TableStorageErrorCode.AzureTable_23,
            $"Intermediate error inserting entry {entry.ToFullString()} tableVersion {(tableVersion == null ? " null
            " : tableVersion.ToString())} to the table {tableManager.TableName}.",
            exc);
        throw;
      }
    }
    ...
    public async Task<bool>UpdateRow(MembershipEntry entry, string etag,
      ...
      try {
        if (logger.IsEnabled(LogLevel.Debug))
          logger.Debug("UpdateRow entry = {0}, etag = {1}, table version = {2}",
                       entry.ToFullString(), etag, tableVersion);
        var siloEntry = Convert(entry, tableManager.DeploymentId);
        var versionEntry =
            tableManager.CreateTableVersionEntry(tableVersion.Version);

        bool result = await tableManager.UpdateSiloEntryConditionally(
            siloEntry, etag, versionEntry, tableVersion.VersionEtag);
        if (result == false)
          logger.Warn(
        ...
        logger.Warn(
            (int) TableStorageErrorCode.AzureTable_25,
            $"Intermediate error updating entry {entry.ToFullString()} tableVersion {(tableVersion == null ? " null
            " : tableVersion.ToString())} to the table {tableManager.TableName}.",
            exc);
        throw;
      }
    ...
    Convert(List<Tuple<SiloInstanceTableEntry, string>>entries) {
      try {
        ...
        logger.Error(
            (int) TableStorageErrorCode.AzureTable_60,
            $"Intermediate error parsing SiloInstanceTableEntry to MembershipTableData: {Utils.EnumerableToString(entries, tuple => tuple.Item1.ToString())}.",
            exc);
        throw;
      }
    }
    ...
    private static MembershipEntry Parse(SiloInstanceTableEntry tableEntry) {
      var parse =
          new MembershipEntry{HostName = tableEntry.HostName,
                              Status = (SiloStatus) Enum.Parse(
                                  typeof (SiloStatus), tableEntry.Status)};
      ...
      parse.SiloAddress = SiloAddress.New(
          new IPEndPoint(IPAddress.Parse(tableEntry.Address), port), gen);
      ...
      if (!string.IsNullOrEmpty(tableEntry.SiloName)) {
        ...
        // this is for backward compatability: in a mixed cluster of old and new
        // version, some entries will have the old InstanceName column.
    ...
    private static SiloInstanceTableEntry Convert(MembershipEntry memEntry,
                                                  string deploymentId) {
      var tableEntry = new SiloInstanceTableEntry{
          DeploymentId = deploymentId,
          Address = memEntry.SiloAddress.Endpoint.Address.ToString(),
          Port = memEntry.SiloAddress.Endpoint.Port.ToString(
              CultureInfo.InvariantCulture),
          Generation = memEntry.SiloAddress.Generation.ToString(
              CultureInfo.InvariantCulture),
          HostName = memEntry.HostName, Status = memEntry.Status.ToString(),
          ProxyPort = memEntry.ProxyPort.ToString(CultureInfo.InvariantCulture),
          RoleName = memEntry.RoleName, SiloName = memEntry.SiloName,
          // this is for backward compatability: in a mixed cluster of old and
          // new version, we need to populate both columns.
          InstanceName = memEntry.SiloName,
          UpdateZone =
              memEntry.UpdateZone.ToString(CultureInfo.InvariantCulture),
      ...
      tableEntry.RowKey =
          SiloInstanceTableEntry.ConstructRowKey(memEntry.SiloAddress);

      return tableEntry;
    }

    private static SiloInstanceTableEntry
    ConvertPartial(MembershipEntry memEntry, string deploymentId) {
