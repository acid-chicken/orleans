  ...
  public class AzureBasedReminderTable : IReminderTable {
    ...
    public async Task Init() {
      this.remTableManager = await RemindersTableManager.GetManager(
          ...
          this.loggerFactory, options
          : this.storageOptions);
    }

    private ReminderTableData ConvertFromTableEntryList(
        IEnumerable<Tuple<ReminderTableEntry, string>>entries) {
      var remEntries = new List<ReminderEntry>();
      foreach(var entry in entries) {
        try {
          ReminderEntry converted =
              ConvertFromTableEntry(entry.Item1, entry.Item2);
    ...
    private ReminderEntry ConvertFromTableEntry(ReminderTableEntry tableEntry,
                                                string eTag) {
      try {
        return new ReminderEntry{
            GrainRef = this.grainReferenceConverter.GetGrainFromKeyString(
                tableEntry.GrainReference),
        ...
        this.logger.Error((int) AzureReminderErrorCode.AzureTable_49, error,
                          exc);
        throw;
      } finally {
        ...
        if (!tableEntry.ServiceId.Equals(serviceIdStr)) {
          ...
          this.logger.Warn(
              (int) AzureReminderErrorCode.AzureTable_ReadWrongReminder, error);
          throw new OrleansException(error);
        }
      }
    }

    private static ReminderTableEntry
    ConvertToTableEntry(ReminderEntry remEntry, string serviceId,
                        string deploymentId) {
      string partitionKey = ReminderTableEntry.ConstructPartitionKey(
          serviceId, remEntry.GrainRef);
    ...
    public async Task<ReminderTableData>ReadRows(GrainReference key) {
      try {
        ...
        if (this.logger.IsEnabled(LogLevel.Trace))
          this.logger.Trace("Read for grain {0} Table=" + Environment.NewLine +
                                "{1}",
                            key, data.ToString());
        return data;
      } catch (Exception exc) {
        this.logger.Warn(
            (int) AzureReminderErrorCode.AzureTable_47,
    ...
    public async Task<ReminderTableData>ReadRows(uint begin, uint end) {
      try {
        var entries =
            await this.remTableManager.FindReminderEntries(begin, end);
        ReminderTableData data = ConvertFromTableEntryList(entries);
        if (this.logger.IsEnabled(LogLevel.Trace))
          this.logger.Trace("Read in {0} Table=" + Environment.NewLine + "{1}",
                            RangeFactory.CreateRange(begin, end), data);
        return data;
      } catch (Exception exc) {
        this.logger.Warn(
            (int) AzureReminderErrorCode.AzureTable_40,
    ...
    public async Task<ReminderEntry>ReadRow(GrainReference grainRef,
      ...
      try {
        if (this.logger.IsEnabled(LogLevel.Debug))
          this.logger.Debug("ReadRow grainRef = {0} reminderName = {1}",
                            grainRef, reminderName);
        var result = await this.remTableManager.FindReminderEntry(grainRef,
                                                                  reminderName);
        return result == null ? null
            : ConvertFromTableEntry(result.Item1, result.Item2);
      } catch (Exception exc) {
        this.logger.Warn(
            (int) AzureReminderErrorCode.AzureTable_46,
    ...
    public async Task<string>UpsertRow(ReminderEntry entry) {
      try {
        if (this.logger.IsEnabled(LogLevel.Debug))
          this.logger.Debug("UpsertRow entry = {0}", entry.ToString());
        ReminderTableEntry remTableEntry =
            ConvertToTableEntry(entry, this.remTableManager.ServiceId,
                                this.remTableManager.ClusterId);

        string result = await this.remTableManager.UpsertRow(remTableEntry);
        if (result == null) {
          this.logger.Warn(
              (int) AzureReminderErrorCode.AzureTable_45,
              $"Upsert failed on the reminder table. Will retry. Entry = {entry.ToString()}");
        }
        return result;
      } catch (Exception exc) {
        this.logger.Warn(
            (int) AzureReminderErrorCode.AzureTable_42,
    ...
    public async Task<bool>RemoveRow(GrainReference grainRef,
      ...
      var entry = new ReminderTableEntry{
          PartitionKey = ReminderTableEntry.ConstructPartitionKey(
              this.remTableManager.ServiceId, grainRef),
          RowKey = ReminderTableEntry.ConstructRowKey(grainRef, reminderName),
          ETag = eTag,
      };
      try {
        if (this.logger.IsEnabled(LogLevel.Trace))
          this.logger.Trace("RemoveRow entry = {0}", entry.ToString());

        bool result =
            await this.remTableManager.DeleteReminderEntryConditionally(entry,
                                                                        eTag);
        if (result == false) {
          this.logger.Warn(
              (int) AzureReminderErrorCode.AzureTable_43,
              $"Delete failed on the reminder table. Will retry. Entry = {entry}");
        }
        return result;
      } catch (Exception exc) {
        this.logger.Warn(
            (int) AzureReminderErrorCode.AzureTable_44,
