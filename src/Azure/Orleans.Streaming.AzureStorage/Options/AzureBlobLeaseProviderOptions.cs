...
namespace Orleans.Configuration {
  ...
  public class AzureBlobLeaseProviderOptionsValidator
    ...
    public AzureBlobLeaseProviderOptionsValidator(
        IOptions<AzureBlobLeaseProviderOptions>options) {
      this.options = options.Value;
    }

    /// <summary>
    /// Creates creates validator for named instance of
    /// AzureBlobLeaseProviderOptions options.
    /// </summary>
    /// <param name="services"></param>
    /// <param name="name"></param>
    /// <returns></returns>
    public static IConfigurationValidator Create(IServiceProvider services,
                                                 string name) {
      var options =
          services.GetOptionsByName<AzureBlobLeaseProviderOptions>(name);
    ...
    private AzureBlobLeaseProviderOptionsValidator(
        AzureBlobLeaseProviderOptions options, string name) {
    ...
    public void ValidateConfiguration() {
      ...
      if (this.name != null && string.IsNullOrWhiteSpace(this.name)) {
        throw new OrleansConfigurationException(
            $"Named option {nameof(AzureBlobLeaseProviderOptions)} of name {this.name} is invalid.  Name cannot be empty or whitespace.");
      }
      if (!CloudStorageAccount.TryParse(this.options.DataConnectionString,
                                        out _)) {
        var errorStr =
