...
namespace Orleans.AzureUtils {
  ...
  /// http://msdn.microsoft.com/en-us/library/windowsazure/dd179349.aspx Windows
  /// Azure Queues and Windows Azure Service Bus Queues - Compared and
  /// Contrasted: http://msdn.microsoft.com/en-us/library/hh767287(VS.103).aspx
  ...
  internal static class AzureQueueDefaultPolicies {
    ...
    static AzureQueueDefaultPolicies() {
      ...
      QueueOperationRetryPolicy =
          new LinearRetry(PauseBetweenQueueOperationRetries,
                          MaxQueueOperationRetries); // 5 x 100ms
      QueueOperationTimeout =
          PauseBetweenQueueOperationRetries.Multiply(MaxQueueOperationRetries)
              .Multiply(6); // 3 sec
    }
  }
  ...
  public class AzureQueueDataManager {
    ...
    /// name="visibilityTimeout">A TimeSpan specifying the visibility timeout
    /// interval</param>
    public AzureQueueDataManager(ILoggerFactory loggerFactory, string queueName,
      ...
      queueOperationsClient = GetCloudQueueClient(
          connectionString, AzureQueueDefaultPolicies.QueueOperationRetryPolicy,
          AzureQueueDefaultPolicies.QueueOperationTimeout, logger);
    }
    ...
    public async Task InitQueueAsync() {
      ...
      try {
        ...
        // Not sure if this is a blocking call or not. Did not find an
        // alternative async API. Should probably use BeginListQueuesSegmented.
        var myQueue = queueOperationsClient.GetQueueReference(QueueName);

        // Create the queue if it doesn't already exist.
        bool didCreate = await myQueue.CreateIfNotExistsAsync();
        queue = myQueue;
        logger.Info((int) AzureQueueErrorCode.AzureQueue_01,
                    "{0} Azure storage queue {1}",
                    (didCreate ? "Created"
                     : "Attached to"),
                    QueueName);
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "CreateIfNotExist",
                              AzureQueueErrorCode.AzureQueue_02);
      } finally {
    ...
    public async Task DeleteQueue() {
      ...
      try {
        // that way we don't have first to create the queue to be able later to
        // delete it.
        CloudQueue queueRef =
            queue ?? queueOperationsClient.GetQueueReference(QueueName);
        if (await queueRef.DeleteIfExistsAsync()) {
          logger.Info((int) AzureQueueErrorCode.AzureQueue_03,
                      "Deleted Azure Queue {0}", QueueName);
        }
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "DeleteQueue",
                              AzureQueueErrorCode.AzureQueue_04);
      } finally {
    ...
    public async Task ClearQueue() {
      ...
      try {
        ...
        logger.Info((int) AzureQueueErrorCode.AzureQueue_05,
                    "Cleared Azure Queue {0}", QueueName);
      } catch (Microsoft.Azure.Storage.StorageException exc) {
        if (exc.RequestInformation?.HttpStatusCode !=
            (int) HttpStatusCode.NotFound) {
          ReportErrorAndRethrow(exc, "ClearQueue",
                                AzureQueueErrorCode.AzureQueue_06);
        }
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "ClearQueue",
                              AzureQueueErrorCode.AzureQueue_06);
    ...
    public async Task AddQueueMessage(CloudQueueMessage message) {
      var startTime = DateTime.UtcNow;
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Adding message {0} to queue: {1}", message, QueueName);
      try {
        await queue.AddMessageAsync(message);
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "AddQueueMessage",
                              AzureQueueErrorCode.AzureQueue_07);
      } finally {
    ...
    public async Task<CloudQueueMessage>PeekQueueMessage() {
      var startTime = DateTime.UtcNow;
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Peeking a message from queue: {0}", QueueName);
      try {
        return await queue.PeekMessageAsync();

      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "PeekQueueMessage",
                              AzureQueueErrorCode.AzureQueue_08);
    ...
    public async Task<CloudQueueMessage>GetQueueMessage() {
      var startTime = DateTime.UtcNow;
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Getting a message from queue: {0}", QueueName);
      try {
        // BeginGetMessage and EndGetMessage is not supported in netstandard,
        // may be use GetMessageAsync
        // http://msdn.microsoft.com/en-us/library/ee758456.aspx
        // If no messages are visible in the queue, GetMessage returns null.
        return await queue.GetMessageAsync(messageVisibilityTimeout, options
                                           : null, operationContext
                                           : null);
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "GetQueueMessage",
                              AzureQueueErrorCode.AzureQueue_09);
        return null; // Dummy statement to keep compiler happy
      } finally {
    ...
    public async Task<IEnumerable<CloudQueueMessage>>
    GetQueueMessages(int count = -1) {
      var startTime = DateTime.UtcNow;
      if (count == -1) {
        count = CloudQueueMessage.MaxNumberOfMessagesToPeek;
      }
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("Getting up to {0} messages from queue: {1}", count,
                     QueueName);
      try {
        return await queue.GetMessagesAsync(count, messageVisibilityTimeout,
                                            options
                                            : null, operationContext
                                            : null);
      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "GetQueueMessages",
                              AzureQueueErrorCode.AzureQueue_10);
        return null; // Dummy statement to keep compiler happy
      } finally {
    ...
    public async Task<int>GetApproximateMessageCount() {
      var startTime = DateTime.UtcNow;
      if (logger.IsEnabled(LogLevel.Trace))
        logger.Trace("GetApproximateMessageCount a message from queue: {0}",
                     QueueName);
      try {
        await queue.FetchAttributesAsync();
        return queue.ApproximateMessageCount.HasValue ? queue
            .ApproximateMessageCount.Value : 0;

      } catch (Exception exc) {
        ReportErrorAndRethrow(exc, "FetchAttributes",
                              AzureQueueErrorCode.AzureQueue_12);
    ...
    private void CheckAlertSlowAccess(DateTime startOperation,
      ...
      if (timeSpan > AzureQueueDefaultPolicies.QueueOperationTimeout) {
        logger.Warn((int) AzureQueueErrorCode.AzureQueue_13,
                    "Slow access to Azure queue {0} for {1}, which took {2}.",
                    QueueName, operation, timeSpan);
      }
    }

    private void ReportErrorAndRethrow(Exception exc, string operation,
                                       AzureQueueErrorCode errorCode) {
      var errMsg =
          String.Format("Error doing {0} for Azure storage queue {1} " +
                            Environment.NewLine + "Exception = {2}",
                        operation, QueueName, exc);
      logger.Error((int) errorCode, errMsg, exc);
      throw new AggregateException(errMsg, exc);
    }

    private CloudQueueClient GetCloudQueueClient(string storageConnectionString,
                                                 IRetryPolicy retryPolicy,
                                                 TimeSpan timeout,
                                                 ILogger logger) {
      try {
        var storageAccount =
            AzureQueueUtils.GetCloudStorageAccount(storageConnectionString);
        CloudQueueClient operationsClient =
        ...
        logger.Error(
            (int) AzureQueueErrorCode.AzureQueue_14,
            String.Format("Error creating GetCloudQueueOperationsClient."),
            exc);
        throw;
      }
    }
    ...
    private void ValidateQueueName(string queueName) {
      ...
      if (!(queueName.Length >= 3 && queueName.Length <= 63)) {
        ...
        throw new ArgumentException(
            String.Format(
                "A queue name must be from 3 through 63 characters long, while your queueName length is {0}, queueName is {1}.",
                queueName.Length, queueName),
            queueName);
      }
      ...
      if (!Char.IsLetterOrDigit(queueName.First())) {
        ...
        throw new ArgumentException(
            String.Format(
                "A queue name must start with a letter or number, while your queueName is {0}.",
                queueName),
            queueName);
      }
      ...
      if (!Char.IsLetterOrDigit(queueName.Last())) {
        ...
        // The dash (-) character cannot be the first or last character.
        throw new ArgumentException(
            String.Format(
                "The last letter in the queue name must be alphanumeric, while your queueName is {0}.",
                queueName),
            queueName);
      }

      if (!queueName.All(c => Char.IsLetterOrDigit(c) || c.Equals('-'))) {
        // A queue name can only contain letters, numbers, and the dash (-)
        // character.
        throw new ArgumentException(
            String.Format(
                "A queue name can only contain letters, numbers, and the dash (-) character, while your queueName is {0}.",
                queueName),
            queueName);
      }
      ...
      if (queueName.Contains("--")) {
        ...
        throw new ArgumentException(
            String.Format(
                "Consecutive dash characters are not permitted in the queue name, while your queueName is {0}.",
                queueName),
            queueName);
      }
      ...
      if (queueName.Where(Char.IsLetter).Any(c => !Char.IsLower(c))) {
        ...
        throw new ArgumentException(
            String.Format(
                "All letters in a queue name must be lowercase, while your queueName is {0}.",
                queueName),
            queueName);
      }
    }
