...
namespace Orleans.Providers.Streams.PersistentStreams {
  ...
  public class AzureTableStorageStreamFailureHandler<TEntity>
      : IStreamFailureHandler where TEntity : StreamDeliveryFailureEntity,
                                              new () {
    private static readonly Func<TEntity>DefaultCreateEntity = () =>
        new TEntity();
    ...
    public AzureTableStorageStreamFailureHandler(
        SerializationManager serializationManager, ILoggerFactory loggerFactory,
        bool faultOnFailure, string clusterId, string tableName,
        string storageConnectionString, Func<TEntity>createEntity = null) {
    ...
    /// <summary>
    /// Indicates if the subscription should be put in a faulted state upon
    /// stream failures
    /// </summary>
    ...
    /// <summary>
    /// Should be called when an event could not be delivered to a consumer,
    ...
    /// <returns></returns>
    public Task OnDeliveryFailure(GuidId subscriptionId,
                                  string streamProviderName,
                                  IStreamIdentity streamIdentity,
                                  StreamSequenceToken sequenceToken) {
      return OnFailure(subscriptionId, streamProviderName, streamIdentity,
                       sequenceToken);
    }

    /// <summary>
    /// Should be called when a subscription requested by a consumer could not
    /// be setup, after exhausting retry attempts.
    /// </summary>
    /// <param name="subscriptionId"></param>
    /// <param name="streamProviderName"></param>
    /// <param name="streamIdentity"></param>
    ...
    private async Task OnFailure(GuidId subscriptionId,
                                 string streamProviderName,
                                 IStreamIdentity streamIdentity,
                                 StreamSequenceToken sequenceToken) {
