...
namespace Orleans.Timers.Internal {
  public interface ITimerManager {
    Task<bool>Delay(TimeSpan timeSpan, CancellationToken cancellationToken =
  ...
  internal class TimerManagerImpl : ITimerManager {
    ...
    Delay(TimeSpan timeSpan,
        ...
        TimerManager.Delay(timeSpan, cancellationToken);
  }
  ...
  internal static class TimerManager {
    ...
    Delay(TimeSpan timeSpan,
          CancellationToken cancellationToken = default(CancellationToken)) =>
        DelayUntil(DateTime.UtcNow + timeSpan, cancellationToken);

    public static Task<bool>DelayUntil(
        DateTime dueTime,
        CancellationToken cancellationToken = default(CancellationToken)) {
    ...
    private sealed class DelayTimer : ITimerCallback,
                                      ILinkedListElement<DelayTimer> {
      ...
      DelayTimer ILinkedListElement<DelayTimer>.Next {
        get;
        set;
      }
    }
  ...
  internal static class TimerManager<T>where T : class,
                                                 ITimerCallback,
                                                 ILinkedListElement<T> {
    /// <summary>
    /// The maximum number of times a queue can be denied servicing before
...
#pragma warning restore IDE0052 // Remove unread private members
    ...
    static TimerManager() {
      ...
      QueueChecker = NonCapturingTimer.Create(_ => CheckQueues(), null,
      ...
      try {
        foreach(var queue in allQueues) {
          ...
          if (!queue.Lock.TryGet()) {
            ...
            if (Interlocked.Increment(ref queue.StarvationCount) >
                MAX_STARVATION) {
    ...
    private static void CheckQueueInLock(ThreadLocalQueue queue, DateTime now,
    ...
    /// <summary>
    /// Returns the queue for the current thread, creating and registering one
    /// if it does not yet exist.
    /// </summary>
    /// <returns>The current thread's <see cref="ThreadLocalQueue"/>.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ThreadLocalQueue
    EnsureCurrentThreadHasQueue() {
      return threadLocalQueue ??(threadLocalQueue =
                                    InitializeThreadLocalQueue());
      ...
      ThreadLocalQueue InitializeThreadLocalQueue() {
        ...
        while (true) {
          lock(AllQueuesLock) {
            ...
            // The existing array is full, so copy all values to a new, larger
            // array and register this thread.
    ...
    private sealed class ThreadLocalQueue : ILinkedList<T> {
      public readonly RecursiveInterlockedExchangeLock Lock =
          new RecursiveInterlockedExchangeLock();

      /// <summary>
      /// The number of times that this queue has been starved since it was last
      /// serviced.
      /// </summary>
  ...
  /// </summary>
  /// <typeparam name="TSelf">Self-type. The type implementing this
  /// interface.</typeparam>
  internal interface ILinkedListElement<TSelf>where TSelf
      : ILinkedListElement<TSelf> {
    /// <summary>
  ...
  internal static class LinkedList {
    ...
    public static void AddTail<TList, TElement>(this TList list,
                                                TElement element) where TList
        : class
    , ILinkedList<TElement>where TElement : class
    , ILinkedListElement<TElement> {
    ...
    /// </summary>
    /// <param name="list">The linked list.</param>
    /// <param name="previous">The element before <paramref
    /// name="current"/>.</param> <param name="current">The element to
    /// remove.</param>
    public static void Remove<TList, TElement>(this TList list,
                                               TElement previous,
                                               TElement current) where TList
        : class
    , ILinkedList<TElement>where TElement : class
    , ILinkedListElement<TElement> {
      var next = current.Next;

      // If not removing the first element, point the previous element at the
      // next element.
      if (!(previous is null))
        previous.Next = next;
