...
internal class InvokableObjectManager : IDisposable {
  ...
  private readonly ConcurrentDictionary<ObserverGrainId, LocalObjectData>
      localObjects =
          new ConcurrentDictionary<ObserverGrainId, LocalObjectData>();
  ...
  public InvokableObjectManager(IRuntimeClient runtimeClient,
  ...
  public bool TryRegister(IAddressable obj, ObserverGrainId objectId,
    ...
    return this.localObjects.TryAdd(
        objectId, new LocalObjectData(obj, objectId, invoker));
  }
  ...
  public void Dispatch(Message message) {
    ...
    if (this.localObjects.TryGetValue(observerId, out var objectData)) {
      ...
      this.logger.Error(
          ...
          String.Format("Unexpected target grain in request: {0}. Message={1}",
                        message.TargetGrain, message));
    }
  }
  ...
  private void Invoke(LocalObjectData objectData, Message message) {
    ...
    if (obj == null) {
      //// Remove from the dictionary record for the garbage collected object?
      ///But now we won't be able to detect invalid dispatch IDs anymore.
      this.logger.Warn(
    ...
    if (start) {
      ...
      // override QueueTask as task inlining (as opposed to queueing). this
      // seems implausible to the author, since none of the .NET schedulers do
      // this and it is considered bad form (the OrleansTaskScheduler does not
      // do this).
      //
      ...
      // We pass these options to Task.Factory.StartNew as they make the call
      // identical to Task.Run. See:
      // https://blogs.msdn.microsoft.com/pfxteam/2011/10/24/task-run-vs-task-factory-startnew/
      Task.Factory
          .StartNew(this.dispatchFunc, objectData, CancellationToken.None,
    ...
    while (true) {
      try {
        ...
        try {
          ...
          if (this.logger.IsEnabled(LogLevel.Warning)) {
            this.logger.LogWarning(
                "Exception during message body deserialization in " +
                    nameof(LocalObjectMessagePumpAsync) +
                    " for message: {Message}, Exception: {Exception}",
                message, deserializationException);
          }

          this.runtimeClient.SendResponse(
              message, Response.ExceptionResponse(deserializationException));
          continue;
        }
        ...
        try {
          ...
          // exceptions thrown within this scope are not considered to be thrown
          // from user code and not from runtime code.
          ...
          // the exception needs to be reported in the log or propagated back to
          // the caller.
          caught = exc;
        }
        ...
        // ignore, keep looping.
        this.logger.LogWarning("Exception in " +
                                   nameof(LocalObjectMessagePumpAsync) +
                                   ": {Exception}",
                               outerException);
      } finally {
  ...
  private void
  SendResponseAsync(Message message, object resultObject) {
    if (message.IsExpired) {
      this.messagingTrace.OnDropExpiredMessage(
          message, MessagingStatisticsGroup.Phase.Respond);
      return;
    }
    ...
    try {
      ...
      this.runtimeClient.SendResponse(message,
                                      Response.ExceptionResponse(exc2));
      this.logger.Warn(ErrorCode.ProxyClient_OGC_SendResponseFailed,
  ...
  [System.Diagnostics.CodeAnalysis.SuppressMessage(
      "Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes")]
  private void
  ReportException(Message message, Exception exception) {
    ...
    case Message.Directions.OneWay: {
      this.logger.Error(
          ...
          String.Format(
              ...
              request.MethodId, request.InterfaceId),
          exception);
      break;
    }
    ...
    case Message.Directions.Request: {
      ...
      try {
        ...
        this.runtimeClient.SendResponse(message,
                                        Response.ExceptionResponse(ex2));
        this.logger.Warn(ErrorCode.ProxyClient_OGC_SendExceptionResponseFailed,
    ...
    default:
      throw new InvalidOperationException(
          $"Unrecognized direction for message {message}, request {request}, which resulted in exception: {exception}");
    }
  }
  ...
  public class LocalObjectData {
    ...
    internal LocalObjectData(IAddressable obj, ObserverGrainId observerId,
                             IGrainMethodInvoker invoker) {
