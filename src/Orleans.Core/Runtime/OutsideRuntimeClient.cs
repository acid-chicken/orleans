...
internal class OutsideRuntimeClient : IRuntimeClient,
  ...
  internal ClientMessageCenter MessageCenter {
    get;
    private set;
  }
  ...
  private async Task StreamingInitialize() {
    var implicitSubscriberTable =
        await MessageCenter.GetImplicitStreamSubscriberTable(
            this.InternalGrainFactory);
  ...
  public async Task Start(Func<Exception, Task<bool>>retryFilter = null) {
    ...
    logger.Info(
        ...
        CurrentActivationAddress.ToString() + ", client ID: " + clientId);
  }

  // used for testing to (carefully!) allow two clients in the same process
  private async Task StartInternal(Func<Exception, Task<bool>>retryFilter) {
    var gatewayManager =
        this.ServiceProvider.GetRequiredService<GatewayManager>();
    await ExecuteWithRetries(
        async () => await gatewayManager.StartAsync(CancellationToken.None),
        retryFilter);

    var generation =
        -SiloAddress.AllocateNewGeneration(); // Client generations are negative
    MessageCenter = ActivatorUtilities.CreateInstance<ClientMessageCenter>(
    ...
    CurrentActivationAddress = ActivationAddress.NewActivationAddress(
        MessageCenter.MyAddress, clientId.GrainId);

    await ExecuteWithRetries(
        async () => this.GrainTypeResolver =
            await MessageCenter.GetGrainTypeResolver(this.InternalGrainFactory),
        retryFilter);
  ...
  public GrainReference CreateObjectReference(IAddressable obj,
    ...
    GrainReference reference = GrainReference.NewObserverGrainReference(
        observerId, this.GrainReferenceRuntime);
    if (!localObjects.TryRegister(obj, observerId, invoker)) {
      throw new ArgumentException(
          $"Failed to add new observer {reference} to localObjects collection.",
          "reference");
    }
    return reference;
  }
  ...
  public void DeleteObjectReference(IAddressable obj) {
    ...
    if (!ObserverGrainId.TryParse(reference.GrainId, out var observerId)) {
      throw new ArgumentException(
          $"Reference {reference.GrainId} is not an observer reference");
    }
