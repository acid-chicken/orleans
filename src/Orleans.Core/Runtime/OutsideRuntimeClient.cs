...
internal class OutsideRuntimeClient : IRuntimeClient,
  ...
  public async Task Start(Func<Exception, Task<bool>>retryFilter = null) {
    ...
    logger.Info(
        ErrorCode.ProxyClient_StartDone,
        "{0} Started OutsideRuntimeClient with Global Client ID: {1}", BARS,
        CurrentActivationAddress.ToString() + ", client GUID ID: " + clientId);
  }
  ...
  private async Task StartInternal(Func<Exception, Task<bool>>retryFilter) {
    ...
    transport = ActivatorUtilities.CreateInstance<ClientMessageCenter>(
        this.ServiceProvider, localAddress, generation, clientId);
    transport.RegisterLocalMessageHandler(Message.Categories.Application,
                                          this.HandleMessage);
    transport.Start();
    CurrentActivationAddress =
        ActivationAddress.NewActivationAddress(transport.MyAddress, clientId);
