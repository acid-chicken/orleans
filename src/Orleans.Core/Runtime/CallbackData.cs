...
namespace Orleans.Runtime {
  internal class CallbackData {
    ...
    public CallbackData(SharedCallbackData shared,
                        TaskCompletionSource<object>ctx, Message msg) {
      this.shared = shared;
      this.context = ctx;
      this.Message = msg;
    ...
    public Message Message {
      get; // might hold metadata used by response pipeline
    }

    public bool IsCompleted => this.completed == 1;

    public bool IsExpired(long currentTimestamp) {
      return currentTimestamp - this.stopwatch.GetRawTimestamp() >
             this.shared.ResponseTimeoutStopwatchTicks;
    }
    ...
    public void OnTimeout(TimeSpan timeout) {
      ...
      this.shared.Logger.Warn(ErrorCode.Runtime_Error_100157,
                              "{0} About to break its promise.", errorMsg);

      var error = Message.CreatePromptExceptionResponse(
          msg, new TimeoutException(errorMsg));
    ...
    public void OnTargetSiloFail() {
      ...
      var error = Message.CreatePromptExceptionResponse(
          msg, new SiloUnavailableException(errorMsg));
    ...
    public void DoCallback(Message response) {
      ...
      if (Interlocked.CompareExchange(ref this.completed, 1, 0) == 0) {
        ...
        // do callback outside the CallbackData lock. Just not a good practice
        // to hold a lock for this unrelated operation.
