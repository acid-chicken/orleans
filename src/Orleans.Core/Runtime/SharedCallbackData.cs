...
namespace Orleans.Runtime {
  internal class SharedCallbackData {
    ...
    public SharedCallbackData(
        Action<Message>unregister, ILogger logger,
    ...
    public TimeSpan ResponseTimeout {
      ...
      set {
        ...
        this.ResponseTimeoutStopwatchTicks =
    ...
    public void
    ResponseCallback(Message message, TaskCompletionSource<object>context) {
      Response response;
          ...
          if (rejection == null) {
            if (string.IsNullOrEmpty(message.RejectionInfo)) {
              message.RejectionInfo =
                  "Unable to send request - no rejection info available";
            }
            rejection =
                new OrleansMessageRejectionException(message.RejectionInfo);
          }
          break;
        }
