...
namespace Orleans.Runtime {
  ...
  internal sealed class GrainClassData {
    ...
    internal GrainClassData(int grainTypeCode, string grainClass,
                            bool isGeneric) {
      ...
      genericClassNames =
          new Dictionary<string,
                         string>(); // TODO: initialize only for generic classes
    }

    internal string GetClassName(string typeArguments) {
      // Knowing whether the grain implementation is generic allows for
      // non-generic grain classes to implement one or more generic grain
      // interfaces. For generic grain classes, the assumption that they take
      // the same generic arguments as the implemented generic interface(s)
      // still holds.
      if (!isGeneric || String.IsNullOrWhiteSpace(typeArguments)) {
        return GrainClass;
      } else {
        lock(this) {
          if (!genericClassNames.TryGetValue(typeArguments,
                                             out var className)) {
    ...
    internal long GetTypeCode(Type interfaceType) {
      if (interfaceType.IsGenericType && this.IsGeneric) {
        string args = TypeUtils.GetGenericTypeArgs(
            interfaceType.GetGenericArguments(), t => true);
    ...
    public override int GetHashCode() { return GrainTypeCode; }
