...
namespace Orleans.Runtime {
  /// <summary>
  /// Internal data structure that holds a grain interfaces to grain classes
  /// map.
  /// </summary>
  [Serializable]
  internal class GrainInterfaceMap {
    ...
    public GrainInterfaceMap(bool localTestMode,
      ...
      if (localTestMode) // if we are running in test mode, we'll build a list
                         // of loaded grain assemblies to help with
    ...
    internal void AddEntry(Type iface, Type grain, PlacementStrategy placement,
                           bool primaryImplementation) {
      lock(this) {
        ...
        var grainInterfaceData =
            GetOrAddGrainInterfaceData(iface, isGenericGrainClass);
        ...
        grainInterfaceData.AddImplementation(implementation,
                                             primaryImplementation);
        if (primaryImplementation) {
          primaryImplementations[grainInterfaceData.GrainInterface] = grainName;
        } else {
          if (!primaryImplementations.ContainsKey(
                  grainInterfaceData.GrainInterface))
            primaryImplementations.Add(grainInterfaceData.GrainInterface,
                                       grainName);
        }
    ...
    private GrainInterfaceData
    GetOrAddGrainInterfaceData(Type iface, bool isGenericGrainClass) {
      ...
      var interfaceName =
          TypeUtils.GetRawClassName(TypeUtils.GetFullName(iface));
      grainInterfaceData =
          new GrainInterfaceData(interfaceId, version, iface, interfaceName);
    ...
    internal bool TryGetPrimaryImplementation(string grainInterface,
      ...
      lock(this) {
        return primaryImplementations.TryGetValue(grainInterface,
                                                  out grainClass);
      }
    }
    ...
    internal bool TryGetTypeInfo(int typeCode, out string grainClass,
    ...
    internal static string GetTypeKey(Type interfaceType,
                                      bool isGenericGrainClass) {
      if (isGenericGrainClass && interfaceType.IsGenericType) {
        ...
        return TypeUtils.GetTemplatedName(
            TypeUtils.GetFullName(interfaceType), interfaceType,
            interfaceType.GetGenericArguments(), t => false);
      }
    }

    public void AddToUnorderedList(Type grainClass) {
      var grainClassTypeCode =
          GrainInterfaceUtils.GetGrainClassTypeCode(grainClass);
    ...
    public IGrainTypeResolver GetGrainTypeResolver() {
      return new GrainTypeResolver(this.typeToInterfaceData, this.table,
                                   this.loadedGrainAsemblies, this.unordered);
    }
  }
}
