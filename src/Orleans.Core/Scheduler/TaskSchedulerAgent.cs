...
namespace Orleans.Runtime {
  internal abstract class TaskSchedulerAgent : IDisposable {
    public enum FaultBehavior {
      ...
      IgnoreFault     // Allow the agent to stop if it faults, but take no other
                      // action (other than logging)
    }

    public enum AgentState { Stopped, Running, StopRequested }
    ...
    private void
    CurrentDomain_DomainUnload(object sender, EventArgs e) {
      try {
    ...
    public virtual void Start() {
      ...
      lock(Lockable) {
        ...
        LogStatus(Log, "Starting AsyncAgent {0} on managed thread {1}", Name,
      ...
      if (Log.IsEnabled(LogLevel.Debug))
        Log.Debug("Started asynch agent " + this.Name);
    }
    ...
    private async Task StartAsync() {
      ...
      try {
        ...
        if (!handled) {
          if (this.OnFault == FaultBehavior.RestartOnFault &&
              !this.Cts.IsCancellationRequested) {
            try {
              if (Log.IsEnabled(LogLevel.Debug))
                Log.Debug("Run completed on agent " + this.Name +
                          " - restarting");
              this.Start();
            } catch (Exception exc) {
              this.Log.Error(ErrorCode.Runtime_Error_100027,
                             "Unable to restart AsynchAgent", exc);
    ...
    protected bool HandleFault(Exception ex) {
      ...
      if (OnFault == FaultBehavior.RestartOnFault &&
        ...
        try {
          ...
          Log.Error(ErrorCode.Runtime_Error_100027,
                    "Unable to restart AsynchAgent", exc);
          State = AgentState.Stopped;
        }
      }
    ...
    private void LogExecutorError(Exception exc) {
      var logMessagePrefix =
          $"Asynch agent {Name} encountered unexpected exception";
      switch (OnFault) {
      ...
      case FaultBehavior.IgnoreFault:
        Log.Error(ErrorCode.Runtime_Error_100025,
                  $"{logMessagePrefix} The executor will exit.", exc);
        break;
      case FaultBehavior.RestartOnFault:
        Log.Error(ErrorCode.Runtime_Error_100026,
                  $"{logMessagePrefix} The Stage will be restarted.", exc);
        break;
      default:
    ...
    private static void LogStatus(ILogger log, string msg,
                                  params object[] args) {
      if (IsStarting) {
        ...
        // Changes in agent threads during all operations aside for initial
        // creation are usually important diag events.
        log.Info(msg, args);
      }
    }
