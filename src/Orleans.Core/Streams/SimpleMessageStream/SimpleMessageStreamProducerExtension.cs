...
namespace Orleans.Providers.Streams.SimpleMessageStream {
  /// <summary>
  /// Multiplexes messages to multiple different producers in the same grain
  /// over one grain-extension interface.
  ///
  /// On the silo, we have one extension per activation and this extension
  /// multiplexes all streams on this activation
  ///     (different stream ids and different stream providers).
  /// On the client, we have one extension per stream (we bind an extension for
  /// every StreamProducer, therefore every stream has its own extension).
  /// </summary>
  [Serializable]
  internal class SimpleMessageStreamProducerExtension
      : IStreamProducerExtension {
    private readonly Dictionary<StreamId, StreamConsumerExtensionCollection>
        remoteConsumers;
    ...
    internal SimpleMessageStreamProducerExtension(
        IStreamProviderRuntime providerRt, IStreamPubSub pubsub, ILogger logger,
        bool fireAndForget, bool optimizeForImmutable) {
      providerRuntime = providerRt;
      streamPubSub = pubsub;
      fireAndForgetDelivery = fireAndForget;
      optimizeForImmutableData = optimizeForImmutable;
      remoteConsumers =
          new Dictionary<StreamId, StreamConsumerExtensionCollection>();
      this.logger = logger;
    }

    internal void AddStream(StreamId streamId) {
      StreamConsumerExtensionCollection obs;
      // no need to lock on _remoteConsumers, since on the client we have one
      // extension per stream (per StreamProducer) so this call is only made
      // once, when StreamProducer is created.
      if (remoteConsumers.TryGetValue(streamId, out obs))
        return;
    ...
    internal void
    AddSubscribers(StreamId streamId,
                   ICollection<PubSubSubscriptionState>newSubscribers) {
      if (logger.IsEnabled(LogLevel.Debug))
        logger.Debug("{0} AddSubscribers {1} for stream {2}",
                     providerRuntime.ExecutingEntityIdentity(),
                     Utils.EnumerableToString(newSubscribers), streamId);
      ...
      if (remoteConsumers.TryGetValue(streamId, out consumers)) {
        foreach(var newSubscriber in newSubscribers) {
          consumers.AddRemoteSubscriber(newSubscriber.SubscriptionId,
                                        newSubscriber.Consumer,
                                        newSubscriber.Filter);
        }
      } else {
        // We got an item when we don't think we're the subscriber. This is a
        // normal race condition. We can drop the item on the floor, or pass it
        // to the rendezvous, or log a warning.
      }
    }
    ...
    public Task AddSubscriber(GuidId subscriptionId, StreamId streamId,
      ...
      if (logger.IsEnabled(LogLevel.Debug)) {
        logger.Debug("{0} AddSubscriber {1} for stream {2}",
    ...
    public Task RemoveSubscriber(GuidId subscriptionId, StreamId streamId) {
      if (logger.IsEnabled(LogLevel.Debug)) {
        logger.Debug("{0} RemoveSubscription {1}",
                     providerRuntime.ExecutingEntityIdentity(), subscriptionId);
      }

      foreach(StreamConsumerExtensionCollection consumers in
                  remoteConsumers.Values) {
    ...
    internal class StreamConsumerExtensionCollection {
      ...
      internal StreamConsumerExtensionCollection(IStreamPubSub pubSub,
                                                 ILogger logger) {
        this.streamPubSub = pubSub;
        this.logger = logger;
        this.consumers = new ConcurrentDictionary<
            GuidId,
            Tuple<IStreamConsumerExtension, IStreamFilterPredicateWrapper>>();
      }

      internal void AddRemoteSubscriber(GuidId subscriptionId,
                                        IStreamConsumerExtension streamConsumer,
                                        IStreamFilterPredicateWrapper filter) {
      ...
      internal Task DeliverItem(StreamId streamId, object item,
        ...
        foreach(KeyValuePair<GuidId, Tuple<IStreamConsumerExtension,
                                           IStreamFilterPredicateWrapper>>
                    subscriptionKvp in consumers) {
          IStreamConsumerExtension remoteConsumer = subscriptionKvp.Value.Item1;

          // Apply filter(s) to see if we should forward this item to this
          // consumer
          ...
          Task task = DeliverToRemote(
              remoteConsumer, streamId, subscriptionKvp.Key, item,
              optimizeForImmutableData, fireAndForgetDelivery);
          if (fireAndForgetDelivery)
            task.Ignore();
          else
            tasks.Add(task);
        }
        // If there's no subscriber, presumably we just drop the item on the
        // floor
        return fireAndForgetDelivery ? Task.CompletedTask : Task.WhenAll(tasks);
      }

      private async Task DeliverToRemote(
          IStreamConsumerExtension remoteConsumer, StreamId streamId,
          GuidId subscriptionId, object item, bool optimizeForImmutableData,
          bool fireAndForgetDelivery) {
        try {
          if (optimizeForImmutableData)
            await remoteConsumer.DeliverImmutable(subscriptionId, streamId,
                                                  new Immutable<object>(item),
                                                  null, null);
          else
            await remoteConsumer.DeliverMutable(subscriptionId, streamId, item,
                                                null, null);
          ...
          this.logger.LogWarning(
              ex,
              "Failed to deliver message to consumer on {SubscriptionId} for stream {StreamId}.",
      ...
      internal Task CompleteStream(StreamId streamId,
        ...
        var tasks = fireAndForgetDelivery ? null : new List<Task>();
        foreach(KeyValuePair<GuidId, Tuple<IStreamConsumerExtension,
                                           IStreamFilterPredicateWrapper>>kvp in
                    consumers) {
          IStreamConsumerExtension remoteConsumer = kvp.Value.Item1;
          GuidId subscriptionId = kvp.Key;
          Task task = NotifyComplete(remoteConsumer, subscriptionId, streamId,
      ...
      private async Task NotifyComplete(IStreamConsumerExtension remoteConsumer,
        ...
        try {
          ...
          this.logger.LogWarning(
              ex,
              "Failed to notify consumer of stream completion on {SubscriptionId} for stream {StreamId}.",
              subscriptionId, streamId);
        }
      }
      ...
      internal Task ErrorInStream(StreamId streamId, Exception exc,
        ...
        foreach(KeyValuePair<GuidId, Tuple<IStreamConsumerExtension,
          ...
          IStreamConsumerExtension remoteConsumer = kvp.Value.Item1;
          GuidId subscriptionId = kvp.Key;
          Task task = NotifyError(remoteConsumer, subscriptionId, exc, streamId,
                                  fireAndForgetDelivery);
          if (fireAndForgetDelivery)
            task.Ignore();
          else
      ...
      private async Task NotifyError(IStreamConsumerExtension remoteConsumer,
                                     GuidId subscriptionId, Exception exc,
                                     StreamId streamId,
                                     bool fireAndForgetDelivery) {
        try {
          ...
          this.logger.LogWarning(
              ex,
              "Failed to notify consumer of stream error on {SubscriptionId} for stream {StreamId}. Error: {ErrorException}",
              subscriptionId, streamId, exc);
        }
      }
    }
  }
