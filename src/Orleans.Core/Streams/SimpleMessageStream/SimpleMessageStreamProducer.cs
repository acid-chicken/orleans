...
namespace Orleans.Providers.Streams.SimpleMessageStream {
  internal class SimpleMessageStreamProducer<T>
      : IInternalAsyncBatchObserver<T> {
    ...
    internal SimpleMessageStreamProducer(
        StreamImpl<T>stream, string streamProviderName,
        IStreamProviderRuntime providerUtilities, bool fireAndForgetDelivery,
        bool optimizeForImmutableData, IStreamPubSub pubSub, bool isRewindable,
    ...
    private async Task<ISet<PubSubSubscriptionState>>RegisterProducer() {
      var tup = await providerRuntime.BindExtension<
          SimpleMessageStreamProducerExtension, IStreamProducerExtension>(
          () => new SimpleMessageStreamProducerExtension(
              providerRuntime, pubSub, this.logger, fireAndForgetDelivery,
      ...
      // Notify streamRendezvous about new stream streamProducer. Retreave the
      // list of RemoteSubscribers.
      return await pubSub.RegisterProducer(stream.StreamId, streamProviderName,
                                           myGrainReference);
    }
    ...
    private async Task ConnectToRendezvous() {
      if (isDisposed)
        throw new ObjectDisposedException(
            string.Format("{0}-{1}", GetType(), "ConnectToRendezvous"));

      // the caller should check _connectedToRendezvous before calling this
      // method.
    ...
    public async Task OnNextAsync(T item, StreamSequenceToken token) {
      if (token != null && !IsRewindable)
        throw new ArgumentNullException(
            "token",
            "Passing a non-null token to a non-rewindable IAsyncBatchObserver.");

      if (isDisposed)
        throw new ObjectDisposedException(
            string.Format("{0}-{1}", GetType(), "OnNextAsync"));

      if (!connectedToRendezvous) {
        if (!this.optimizeForImmutableData) {
          // In order to avoid potential concurrency errors, synchronously copy
          // the input before yielding the thread. DeliverItem below must also
          // be take care to avoid yielding before copying for non-immutable
          // objects.
    ...
    public Task OnNextBatchAsync(IEnumerable<T>batch,
      ...
      throw new NotImplementedException(
          "We still don't support OnNextBatchAsync()");
    }

    public async Task OnCompletedAsync() {
      if (isDisposed)
        throw new ObjectDisposedException(
            string.Format("{0}-{1}", GetType(), "OnCompletedAsync"));
    ...
    public async Task OnErrorAsync(Exception exc) {
      if (isDisposed)
        throw new ObjectDisposedException(
            string.Format("{0}-{1}", GetType(), "OnErrorAsync"));
    ...
    public async Task Cleanup() {
      ...
      if (connectedToRendezvous) {
        try {
          await pubSub.UnregisterProducer(stream.StreamId, streamProviderName,
                                          myGrainReference);
          connectedToRendezvous = false;
        } catch (Exception exc) {
          logger.Warn(
