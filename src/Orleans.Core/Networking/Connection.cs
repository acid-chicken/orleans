...
namespace Orleans.Runtime.Messaging {
  internal abstract class Connection {
    private static readonly Func<ConnectionContext, Task>OnConnectedDelegate =
        context => OnConnectedAsync(context);
    private static readonly Action<object>OnConnectionClosedDelegate = state =>
        ((Connection) state)
            .CloseInternal(new ConnectionAbortedException("Connection closed"));
    private static readonly UnboundedChannelOptions
        OutgoingMessageChannelOptions =
            new UnboundedChannelOptions{SingleReader = true,
                                        SingleWriter = false,
                                        AllowSynchronousContinuations = false};

#if NETCOREAPP
    private static readonly ObjectPool<MessageHandler>MessageHandlerPool =
        ObjectPool.Create(new MessageHandlerPoolPolicy());
#else
    ...
    protected Connection(ConnectionContext connection,
      ...
      this.outgoingMessages =
          Channel.CreateUnbounded<Message>(OutgoingMessageChannelOptions);
      this.outgoingMessageWriter = this.outgoingMessages.Writer;

      // Set the connection on the connection context so that it can be
    ...
    protected CounterStatistic MessageReceivedCounter {
      get;
      set;
    }
    protected CounterStatistic MessageSentCounter {
      get;
      set;
    }
    ...
    public static void ConfigureBuilder(ConnectionBuilder builder) =>
        builder.Run(OnConnectedDelegate);

    /// <summary>
    /// Start processing this connection.
    /// </summary>
    /// <returns>A <see cref="Task"/> which completes when the connection
    /// terminates and has completed processing.</returns>
    public async Task Run() {
      RequestContext.Clear();

      Exception error = default;
      try {
        // Eventually calls through to OnConnectedAsync (unless the connection
        // delegate has been misconfigured)
    ...
    RunInternal() => Task.WhenAll(this.ProcessIncoming(),
        ...
        try {
          ...
          if (this.Log.IsEnabled(LogLevel.Information)) {
            this.Log.LogInformation(
                ...
                this.RemoteEndPoint, Environment.StackTrace);
          }
          ...
          if (exception is null) {
            ...
            var abortedException = exception
                as ConnectionAbortedException
          ...
          this.Log.LogWarning(
              innerException,
              "Exception closing connection with remote endpoint {EndPoint}: {Exception}",
              this.RemoteEndPoint, innerException);
        }
      }
    }
    ...
    public override string ToString() =>
        $"Local: {this.LocalEndPoint}, Remote: {this.RemoteEndPoint}, ConnectionId: {this.Context.ConnectionId}";
    ...
    private async Task ProcessIncoming() {
      ...
      var serializer =
          this.shared.ServiceProvider.GetRequiredService<IMessageSerializer>();
      try {
        if (this.Log.IsEnabled(LogLevel.Information)) {
          this.Log.LogInformation(
          ...
          if (buffer.Length >= requiredBytes) {
            do {
              try {
                ...
                (requiredBytes, headerLength, bodyLength) =
                    serializer.TryRead(ref buffer, out message);
                if (requiredBytes == 0) {
                  MessagingStatisticsGroup.OnMessageReceive(
                      this.MessageReceivedCounter, message,
                      bodyLength + headerLength, headerLength,
                      this.ConnectionDirection);
#if NETCOREAPP
                  var handler = MessageHandlerPool.Get();
                  handler.Set(message, this);
                  ThreadPool.UnsafeQueueUserWorkItem(handler, preferLocal
                                                     : true);
#else
                  ThreadPool.UnsafeQueueUserWorkItem(this.handleMessageCallback,
                                                     message);
#endif
                  message = null;
                }
              } catch (Exception exception)
                  when(this.HandleReceiveMessageFailure(message, exception)) {
              }
    ...
    private async Task ProcessOutgoing() {
      ...
      try {
        ...
        while (true) {
          ...
          Message message = default;
          try {
            while (inflight.Count < inflight.Capacity &&
                   reader.TryRead(out message) &&
                   this.PrepareMessageForSend(message)) {
              inflight.Add(message);
              var (headerLength, bodyLength) =
                  serializer.Write(ref output, message);
              MessagingStatisticsGroup.OnMessageSend(
                  this.MessageSentCounter, message, headerLength + bodyLength,
                  headerLength, this.ConnectionDirection);
            }
    ...
    private void RerouteMessage(Message message) {
      ...
      ThreadPool.UnsafeQueueUserWorkItem(
          msg => this.RetryMessage((Message) msg), message);
    }
    ...
    /// </summary>
    /// <returns><see langword="true"/> if the exception should not be caught
    /// and <see langword="false"/> if it should be caught.</returns>
    private bool HandleReceiveMessageFailure(Message message,
                                             Exception exception) {
      this.Log.LogWarning(
          "Exception reading message {Message} from remote endpoint {Remote} to local endpoint {Local}: {Exception}",
          message, this.RemoteEndPoint, this.LocalEndPoint, exception);

      // If deserialization completely failed, rethrow the exception so that it
      // can be handled at another level.
      if (message?.Headers is null) {
        // Returning false here informs the caller that the exception should not
        // be caught.
        return false;
      }
      ...
      if (message.Direction == Message.Directions.Request) {
        ...
        // If the message was a response, propagate the exception to the
        // intended recipient.
    ...
    private void OnMessageSerializationFailure(Message message,
                                               Exception exception) {
      // we only get here if we failed to serialize the msg (or any other
      // catastrophic failure). Request msg fails to serialize on the sender, so
      // we just enqueue a rejection msg. Response msg fails to serialize on the
      // responding silo, so we try to send an error response back.
      this.Log.LogWarning(
      ...
      if (message.Direction == Message.Directions.Request) {
        ...
        // If we failed sending an original response, turn the response body
        // into an error and reply with it. unless we have already tried sending
...
#if NETCOREAPP
    private sealed class MessageHandlerPoolPolicy
        : PooledObjectPolicy<MessageHandler> {
