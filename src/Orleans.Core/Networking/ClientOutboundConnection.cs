...
namespace Orleans.Runtime.Messaging {
  internal sealed class ClientOutboundConnection : Connection {
    ...
    public ClientOutboundConnection(SiloAddress remoteSiloAddress,
      ...
      this.remoteSiloAddress =
          remoteSiloAddress ?? throw new ArgumentNullException(
              nameof(remoteSiloAddress));
      this.MessageReceivedCounter =
          MessagingStatisticsGroup.GetMessageReceivedCounter(
              this.remoteSiloAddress);
      this.MessageSentCounter = MessagingStatisticsGroup.GetMessageSendCounter(
          this.remoteSiloAddress);
    }

    protected override ConnectionDirection ConnectionDirection =>
        ConnectionDirection.ClientToGateway;
    ...
    protected override async Task RunInternal() {
      ...
      try {
        ...
        await ConnectionPreamble.Write(
            this.Context, this.messageCenter.ClientId,
            this.connectionOptions.ProtocolVersion, siloAddress
            : null);

        if (this.connectionOptions.ProtocolVersion >=
            NetworkProtocolVersion.Version2) {
          var (_, protocolVersion, siloAddress) =
              await ConnectionPreamble.Read(this.Context);
          this.Log.LogInformation(
              "Established connection to {Silo} with protocol version {ProtocolVersion}",
              siloAddress, protocolVersion.ToString());
        }
    ...
    protected override bool PrepareMessageForSend(Message msg) {
      ...
      if (!this.IsValid) {
        // Recycle the message we've dequeued. Note that this will recycle
        // messages that were queued up to be sent when the gateway connection
        // is declared dead
      ...
      if (msg.TargetGrain.IsSystemTarget)
        msg.TargetActivation =
            ActivationId.GetSystemActivation(msg.TargetGrain, msg.TargetSilo);
      ...
      if (msg.Direction == Message.Directions.Request) {
        if (this.Log.IsEnabled(LogLevel.Debug))
          this.Log.Debug(
              ...
              "Client is rejecting message: {Message}. Reason = {Reason}", msg,
              reason);
        // Done retrying, send back an error instead
        this.SendRejection(
            msg, Message.RejectionTypes.Transient,
            $"Client is rejecting message: {msg}. Reason = {reason}");
      } else {
        this.Log.Info(
            ErrorCode.Messaging_OutgoingMS_DroppingMessage,
            "Client is dropping message: {<essage}. Reason = {Reason}", msg,
            reason);
