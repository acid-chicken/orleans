...
namespace Orleans.Runtime.Messaging {
  internal sealed class ConnectionManager {
    ...
    private readonly ConcurrentDictionary<SiloAddress, ConnectionEntry>
        connections = new ConcurrentDictionary<SiloAddress, ConnectionEntry>();
    ...
    private readonly TaskCompletionSource<int>closedTaskCompletionSource =
        new TaskCompletionSource<int>(
            TaskCreationOptions.RunContinuationsAsynchronously);

    public ConnectionManager(IOptions<ConnectionOptions>connectionOptions,
    ...
    public ImmutableArray<SiloAddress>
    GetConnectedAddresses() => this.connections.Keys.ToImmutableArray();

    public ValueTask<Connection>GetConnection(SiloAddress endpoint) {
      if (this.connections.TryGetValue(endpoint, out var entry) &&
          entry.TryGetConnection(out var connection)) {
        var pendingAttempt = entry.PendingConnection;
        if (!entry.HasSufficientConnections &&
            (pendingAttempt is null || pendingAttempt.IsCompleted)) {
      ...
      while (true) {
        ...
        lock(this.lockObj) {
          ...
          foreach(var c in entry.Connections) {
            if (!c.IsValid)
              entry.Connections = entry.Connections.Remove(c);
          }

          // If there are sufficient connections available then return an
          // existing connection.
          if (entry.HasSufficientConnections &&
              entry.TryGetConnection(out var connection)) {
            return connection;
          ...
          // If there is no pending attempt then start one, otherwise the
          // pending attempt will be awaited before reevaluating.
    ...
    private void OnConnectionFailed(SiloAddress address, DateTime lastFailure) {
      lock(this.lockObj) {
        ...
        if (entry.LastFailure.HasValue) {
          var ticks =
              Math.Max(lastFailure.Ticks, entry.LastFailure.Value.Ticks);
    ...
    public void OnConnected(SiloAddress address, Connection connection) {
      lock(this.lockObj) {
        var entry = this.GetOrCreateEntry(address);
        var newConnections = entry.Connections.Contains(connection)
                                 ? entry.Connections
      ...
      this.trace.LogInformation(
          "Connection {Connection} established with {Silo}", connection,
          address);
    }

    public void OnConnectionTerminated(SiloAddress address,
      ...
      lock(this.lockObj) {
        if (this.connections.TryGetValue(address, out var entry)) {
          ...
          if (entry.Connections.Length == 0 &&
              entry.PendingConnection is null) {
      ...
      if (exception != null) {
        this.trace.LogWarning(
            ...
            connection, address, exception);
      } else {
        this.trace.LogDebug(
            "Connection {Connection} to endpoint {EndPoint} closed", connection,
            address);
      }
    }
    ...
    private ConnectionEntry GetOrCreateEntry(SiloAddress address) {
      lock(this.lockObj) {
        if (!this.connections.TryGetValue(address, out var entry)) {
          ...
          entry =
              new ConnectionEntry(this.connectionOptions, address,
                                  ImmutableArray<Connection>.Empty, default);
    ...
    private async Task<Connection>ConnectAsync(SiloAddress address) {
      ...
      try {
        if (this.trace.IsEnabled(LogLevel.Information)) {
          this.trace.LogInformation(
              "Establishing connection to endpoint {EndPoint}", address);
        }

        // Cancel pending connection attempts either when the host terminates or
        // after the configured time limit.
        openConnectionCancellation =
            CancellationTokenSource.CreateLinkedTokenSource(
                this.cancellation.Token);
        openConnectionCancellation.CancelAfter(
            this.connectionOptions.OpenConnectionTimeout);

        var connection =
            await this.connectionFactory
                .ConnectAsync(address, openConnectionCancellation.Token)
                .AsTask()
                .WithCancellation(openConnectionCancellation.Token);

        if (this.trace.IsEnabled(LogLevel.Information)) {
          this.trace.LogInformation("Connected to endpoint {EndPoint}",
                                    address);
        }
        ...
        throw new ConnectionFailedException(
            $"Unable to connect to endpoint {address}. See {nameof(exception.InnerException)}",
            exception);
      } finally {
    ...
    public void Abort(SiloAddress endpoint) {
      ...
      if (entry is ConnectionEntry && !entry.Connections.IsDefault) {
        var exception = new ConnectionAbortedException(
            $"Aborting connection to {endpoint}");
    ...
    public async Task Close(CancellationToken ct) {
      try {
        ...
        var connectionAbortedException =
            new ConnectionAbortedException("Stopping");
        var cycles = 0;
        while (this.ConnectionCount > 0) {
          foreach(var entry in this.connections.Values.ToImmutableList()) {
            if (entry.Connections.IsDefaultOrEmpty)
              continue;
            foreach(var connection in entry.Connections) {
              try {
          ...
          if (++cycles > 100 && cycles % 500 == 0 && this.ConnectionCount > 0) {
            this.trace?.LogWarning(
                "Waiting for {NumRemaining} connections to terminate",
                this.ConnectionCount);
          }
        }
      } catch (Exception exception) {
        this.trace?.LogWarning("Exception during shutdown: {Exception}",
                               exception);
      } finally {
    ...
    private void StartConnection(SiloAddress address, Connection connection) {
      ThreadPool.UnsafeQueueUserWorkItem(this.StartConnectionCore,
                                         (address, connection));
    }
    ...
    private async Task RunConnectionAsync(SiloAddress address,
                                          Connection connection) {
      Exception error = default;
      try {
        using(this.BeginConnectionScope(connection)) { await connection.Run(); }
      } catch (Exception exception) {
        error = exception;
      } finally {
    ...
    private class ConnectionEntry {
      ...
      public ConnectionEntry(ConnectionOptions connectionOptions,
      ...
      public TimeSpan RemainingRetryDelay {
        get {
          ...
          if (lastFailure.HasValue) {
            ...
            var retryAfter = lastFailure.Value.Add(
                this.connectionOptions.ConnectionRetryDelay);
      ...
      public bool HasSufficientConnections {
        get {
          ...
          return !connections.IsDefaultOrEmpty &&
                 connections.Length >=
                     this.connectionOptions.ConnectionsPerEndpoint;
        }
      }
      ...
      public void ThrowIfRecentConnectionFailure() {
        ...
        if (remainingDelay > TimeSpan.Zero) {
          throw new ConnectionFailedException(
              $"Unable to connect to {this.Endpoint}, will retry after {remainingDelay.TotalMilliseconds}ms");
        }
      }
    }
  }
