...
namespace Orleans.Runtime.Messaging {
  internal static class ConnectionPreamble {
    ...
    Write(ConnectionContext connection, GrainId nodeIdentity,
          NetworkProtocolVersion protocolVersion, SiloAddress siloAddress) {
      var output = connection.Transport.Output;
      var outputWriter = new PrefixingBufferWriter<byte, PipeWriter>(
          sizeof(int), 1024, MemoryPool<byte>.Shared);
      outputWriter.Reset(output);
      var writer =
          new BinaryTokenStreamWriter2<PrefixingBufferWriter<byte, PipeWriter>>(
              outputWriter);
      ...
      if (length > MaxPreambleLength) {
        throw new InvalidOperationException(
            $"Created preamble of length {length}, which is greater than maximum allowed size of {MaxPreambleLength}.");
      }
    ...
    private static void
    WriteLength(PrefixingBufferWriter<byte, PipeWriter>outputWriter,
                int length) {
    ...
    internal static async
        ValueTask<(GrainId, NetworkProtocolVersion, SiloAddress)>
        Read(ConnectionContext connection) {
      ...
      if (length > MaxPreambleLength) {
        throw new InvalidOperationException(
            $"Remote connection sent preamble length of {length}, which is greater than maximum allowed size of {MaxPreambleLength}.");
      }
      ...
      try {
        ...
        default:
          throw new NotSupportedException(
              "Unexpected token while reading connection preamble. Expected SiloAddress, encountered " +
              token);
        }
