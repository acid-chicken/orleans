...
namespace Orleans.CodeGeneration {
  internal static class GrainInterfaceUtils {
    ...
    internal class RulesViolationException : ArgumentException {
      ...
      protected RulesViolationException(SerializationInfo info,
                                        StreamingContext context)
          : base(info, context) {
        this.Violations = info.GetValue(nameof(Violations),
    ...
    public static bool IsGrainInterface(Type t) {
      ...
      if (t == typeof (IGrainObserver) || t == typeof (IAddressable) ||
          t == typeof (IGrainExtension))
        return false;
      if (t == typeof (IGrain) || t == typeof (IGrainWithGuidKey) ||
    ...
    public static MethodInfo[] GetMethods(Type grainType,
                                          bool bAllMethods = true) {
      ...
      foreach(var methodInfo in infos) if (!methodInfos.Contains(
                                               methodInfo, MethodComparer))
    ...
    public static bool TryGetTransactionOption(MethodInfo methodInfo,
                                               out TransactionOption option) {
      option = TransactionOption.Suppress;
      TransactionAttribute transactionAttribute =
          methodInfo.GetCustomAttribute<TransactionAttribute>(true);
    ...
    public static Dictionary<int, Type>
    GetRemoteInterfaces(Type type, bool checkIsGrainInterface = true) {
      ...
      foreach(Type interfaceType in interfaces.Where(
          i => !checkIsGrainInterface || IsGrainInterface(i)))
    ...
    internal static string FormatMethodForIdComputation(MethodInfo methodInfo) {
      ...
      foreach(ParameterInfo info in parameters) {
        ...
        if (pt.IsGenericParameter) {
          ...
          strMethodId.Append(
              RuntimeTypeNameFormatter.Format(info.ParameterType));
        }
    ...
    public static bool IsTaskBasedInterface(Type type) {
      ...
      // An interface is task-based if it has at least one method that returns a
      // Task or at least one parent that's task-based.
      return methods.Any(m => IsTaskType(m.ReturnType)) ||
             type.GetInterfaces().Any(IsTaskBasedInterface);
    }
    ...
    internal static bool TryValidateInterfaceRules(Type type,
                                                   out List<string>violations) {
    ...
    internal static void ValidateInterfaceRules(Type type) {
      ...
      if (!TryValidateInterfaceRules(type, out violations)) {
        ...
        throw new RulesViolationException(
            string.Format("{0} does not conform to the grain interface rules.",
                          type.FullName),
            violations);
      }
    }
    ...
    internal static void ValidateInterface(Type type) {
      if (!IsGrainInterface(type))
        throw new ArgumentException(
            String.Format("{0} is not a grain interface", type.FullName));

      ValidateInterfaceRules(type);
    }

    private static bool ValidateInterfaceMethods(Type type,
                                                 List<string>violations) {
      bool success = true;
      ...
      foreach(MethodInfo method in methods) {
        ...
        if (IsPureObserverInterface(method.DeclaringType)) {
          if (method.ReturnType != typeof (void)) {
            ...
            violations.Add(String.Format(
                "Method {0}.{1} must return void because it is defined within an observer interface.",
                type.FullName, method.Name));
          }
        } else if (!IsTaskType(method.ReturnType)) {
          success = false;
          violations.Add(String.Format(
              "Method {0}.{1} must return Task or Task<T> because it is defined within a grain interface.",
              type.FullName, method.Name));
        }
        ...
        foreach(ParameterInfo parameter in parameters) {
          if (parameter.IsOut) {
            success = false;
            violations.Add(String.Format(
                "Argument {0} of method {1}.{2} is an output parameter. Output parameters are not allowed in grain interfaces.",
    ...
    private static bool ValidateInterfaceProperties(Type type,
                                                    List<string>violations) {
      bool success = true;
      ...
      foreach(PropertyInfo property in properties) {
        success = false;
        violations.Add(String.Format(
            "Properties are not allowed on grain interfaces:  {0}.{1}.",
    ...
    private static bool IsPureObserverInterface(Type t) {
      ...
      foreach(Type iface in t.GetInterfaces()) {
        if (iface == typeof (IAddressable)) // skip IAddressable that will be in
                                            // the list regardless
          continue;

        if (iface == typeof (IGrainExtension))
          // Skip IGrainExtension, it's just a marker that can go on observer or
          // grain interfaces
          continue;
    ...
    private static void GetMethodsImpl(Type grainType, Type serviceType,
    ...
    private static int GetTypeCode(Type grainInterfaceOrClass) {
      var attr = grainInterfaceOrClass
                     .GetCustomAttributes<TypeCodeOverrideAttribute>(false)
                     .FirstOrDefault();
      if (attr != null)
        return attr.TypeCode;

      var fullName = TypeUtils.GetTemplatedName(
          TypeUtils.GetFullName(grainInterfaceOrClass), grainInterfaceOrClass,
          grainInterfaceOrClass.GetGenericArguments(), t => false);
