...
internal static class SetExtensions {
  ...
  /// <param name="separator">Before each element, or space if
  /// unspecified</param> <returns></returns>
  public static string ToStrings<T>(this IEnumerable<T>list,
                                    Func<T, object>toString = null,
                                    string separator = " ") {
    if (list == null)
      return "";
    ...
    // Do NOT use Aggregate for string concatenation. It is very inefficient,
    // will reallocate and copy lots of intermediate strings. toString = toString
    // ?? (x => x);
    return list.Aggregate("", (s, x) => s + separator + toString(x));
  }

  public static T GetValueOrAddNew<T, TU>(this Dictionary<TU, T>dictionary,
                                          TU key) where T : new() {
    T result;
