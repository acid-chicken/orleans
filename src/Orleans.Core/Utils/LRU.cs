...
namespace Orleans.Runtime {
  // This class implements an LRU cache of values. It keeps a bounded set of
  // values and will flush "old" values
  internal class LRU<TKey, TValue>: IEnumerable<KeyValuePair<TKey, TValue>> {
    ...
    // The following machinery is used to notify client objects when a key and
    // its value is being flushed from the cache. The client's event handler is
    // called after the key has been removed from the cache, but when the cache
    // is in a consistent state so that other methods on the cache may freely be
    // invoked.
    ...
    // We want this to be a reference type so that we can update the values in
    // the cache without having to call AddOrUpdate, which is a nuisance
    ...
    public LRU(int maxSize, TimeSpan maxAge, FetchValueDelegate f) {
      if (maxSize <= 0) {
        throw new ArgumentOutOfRangeException(
            "maxSize", "LRU maxSize must be greater than 0");
      }
      MaximumSize = maxSize;
    ...
    private void AdjustSize() {
      while (cache.Count >= MaximumSize) {
        ...
        KeyValuePair<TKey, TimestampedValue>entryToFree = cache.FirstOrDefault(
            kvp => kvp.Value.Generation == generationToDelete);
    ...
    public IEnumerator<KeyValuePair<TKey, TValue>>GetEnumerator() {
      return cache
          .Select(p => new KeyValuePair<TKey, TValue>(p.Key, p.Value.Value))
          .GetEnumerator();
    }
