...
namespace Orleans.LogConsistency {
  /// <summary>
  /// Functionality for use by log view adaptors that use custom consistency or
  /// replication protocols. Abstracts communication between replicas of the
  /// log-consistent grain in different clusters.
  /// </summary>
  public interface ILogConsistencyProtocolServices {
    ...
    /// <summary>
    /// The id of this cluster. Returns "I" if no multi-cluster network is
    /// present.
    /// </summary>
    /// <returns></returns>
    ...
    /// <param name="where">The context from which the callback was
    /// called</param> <param name="e">The caught exception</param>
    void CaughtUserCodeException(string callback, string where, Exception e);

    /// <summary> Output the specified message at the specified log level.
    /// </summary>
  ...
  public class ProtocolTransportException : OrleansException {
    ...
    public ProtocolTransportException(string msg) : base(msg) {}
    public ProtocolTransportException(string msg, Exception exc)
        : base(msg, exc) {}

    protected ProtocolTransportException(SerializationInfo info,
