...
public class LifecycleSubject : ILifecycleSubject {
  ...
  protected virtual void PerfMeasureOnStart(int stage, TimeSpan elapsed) {
    if (this.logger != null && this.logger.IsEnabled(LogLevel.Trace)) {
      this.logger.LogTrace(
          ...
          "Starting lifecycle stage {Stage} took {Elapsed} Milliseconds", stage,
  ...
  public virtual async Task OnStart(CancellationToken ct) {
    ...
    try {
      foreach(IGrouping<int, OrderedObserver>observerGroup in this.subscribers
        ...
        if (ct.IsCancellationRequested) {
          throw new OrleansLifecycleCanceledException(
              "Lifecycle start canceled by request");
        }

        var stage = observerGroup.Key;
        this.highStage = stage;
        var stopWatch = ValueStopwatch.StartNew();
        await Task.WhenAll(observerGroup.Select(
            orderedObserver => CallOnStart(orderedObserver, ct)));
        stopWatch.Stop();
    ...
    static Task CallOnStart(OrderedObserver observer,
                            CancellationToken cancellationToken) {
      try {
        return observer.Observer?.OnStart(cancellationToken)
            ?? Task.CompletedTask;
      } catch (Exception ex) {
  ...
  protected virtual void PerfMeasureOnStop(int stage, TimeSpan elapsed) {
    if (this.logger != null && this.logger.IsEnabled(LogLevel.Trace)) {
      this.logger.LogTrace(
          (int) ErrorCode.SiloStartPerfMeasure,
          "Stopping lifecycle stage {Stage} took {Elapsed} Milliseconds", stage,
          elapsed.TotalMilliseconds);
    }
  }
  ...
  public virtual async Task OnStop(CancellationToken ct) {
    ...
    foreach(IGrouping<int, OrderedObserver>observerGroup in this
                .subscribers
      ...
      if (ct.IsCancellationRequested && !loggedCancellation) {
        this.logger?.LogWarning(
            "Lifecycle stop operations canceled by request.");
      ...
      try {
        var stopwatch = ValueStopwatch.StartNew();
        await Task.WhenAll(observerGroup.Select(
            orderedObserver => CallOnStop(orderedObserver, ct)));
        stopwatch.Stop();
        this.PerfMeasureOnStop(stage, stopwatch.Elapsed);
      } catch (Exception ex) {
        this.logger?.LogError(
            (int) ErrorCode.LifecycleStopFailure,
            "Stopping lifecycle encountered an error at stage {Stage}. Continuing to stop. Exception: {Exception}",
            this.highStage, ex);
      }
    ...
    static Task CallOnStop(OrderedObserver observer,
                           CancellationToken cancellationToken) {
      try {
        return observer.Observer?.OnStop(cancellationToken)
            ?? Task.CompletedTask;
      } catch (Exception ex) {
        return Task.FromException(ex);
      }
  ...
  public virtual IDisposable Subscribe(string observerName, int stage,
  ...
  private class OrderedObserver : IDisposable {
    public ILifecycleObserver Observer {
      get;
      private set;
    }
