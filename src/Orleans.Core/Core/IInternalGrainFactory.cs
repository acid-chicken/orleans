...
internal interface IInternalGrainFactory : IGrainFactory {
  ...
  /// </summary>
  /// <typeparam name="TGrainObserverInterface">The interface which
  /// interface.</typeparam> <param name="obj">The object.</param> <returns>A
  /// reference to the provided object.</returns>
  TGrainObserverInterface CreateObjectReference<TGrainObserverInterface>(
  ...
  TGrainInterface GetSystemTarget<TGrainInterface>(GrainType grainType,
  ...
  /// <returns>A reference to the specified system target.</returns>
  TGrainInterface GetSystemTarget<TGrainInterface>(GrainId grainId)
      where TGrainInterface : ISystemTarget;
