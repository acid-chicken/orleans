...
internal static class GrainCasterFactory {
  /// <summary>
  /// The cached <see cref="MethodInfo"/> for <see
  /// cref="GrainExtensions.AsWeaklyTypedReference"/>.
  /// </summary>
  ...
  /// <summary>
  /// The cached <see cref="MethodInfo"/> for checking if one type is assignable
  /// from another.
  /// </summary>
  ...
  private static readonly MethodInfo GetTypeMethodInfo =
      TypeUtils.Method((object o) => o.GetType());

  /// <summary>
  /// The cached <see cref="MethodInfo"/> for <see
  /// cref="Type.GetTypeFromHandle"/>.
  /// </summary>
  private static readonly MethodInfo GetTypeFromHandleMethodInfo =
      TypeUtils.Method(() =>
                           Type.GetTypeFromHandle(default(RuntimeTypeHandle)));

  /// <summary>
  /// Creates a grain reference caster delegate for the provided grain interface
  /// type and concrete grain reference type.
  /// </summary>
  /// <param name="interfaceType">The interface type.</param>
  /// <param name="grainReferenceType">The grain reference implementation
  /// type.</param> <returns>A grain reference caster delegate.</returns>
  public static GrainReferenceFactory.GrainReferenceCaster
  CreateGrainReferenceCaster(Type interfaceType, Type grainReferenceType) {
    ...
    var method = new DynamicMethod(
        "caster_" + grainReferenceType.Name, typeof (object),
        new[]{typeof (IAddressable)}, typeof (GrainFactory).Module, true);
    ...
    // C#: var grainReferenceType =
    // Type.GetTypeFromHandle(<grainReferenceType>.TypeHandle);
    ...
    // C#: var interfaceType =
    // Type.GetTypeFromHandle(<interfaceType>.TypeHandle);
    il.DeclareLocal(typeof (Type));
    il.Emit(OpCodes.Ldtoken, interfaceType);
    EmitCall(il, GetTypeFromHandleMethodInfo);
    il.Emit(OpCodes.Stloc_2);

    // C#: if (grainReferenceType.IsAssignableFrom(grainRef.GetType())) return
    // grainRef;
    ...
    // Otherwise, cast the input to a GrainReference and wrap it in the target
    // type by calling the copy constructor. C#: result = new
    // <grainReferenceType>((GrainReference)result);
    var grainRefConstructor = TypeUtils.GetConstructorThatMatches(
        grainReferenceType, new[]{typeof (GrainReference)});
    il.Emit(OpCodes.Ldloc_0);
    ...
    return (GrainReferenceFactory.GrainReferenceCaster) method.CreateDelegate(
        typeof (GrainReferenceFactory.GrainReferenceCaster));
  }
  ...
  private static bool
  IsGrainReferenceCopyConstructor(ConstructorInfo constructor) {
    var parameters = constructor.GetParameters();
    return parameters.Length == 1 &&
           parameters[0].ParameterType == typeof (GrainReference);
  }
}
}