...
internal class GrainReferenceFactory {
  ...
  /// <summary>
  /// Casts an <see cref="IAddressable"/> to a concrete <see
  /// cref="GrainReference"/> implementation.
  /// </summary>
  /// <param name="existingReference">The existing <see cref="IAddressable"/>
  /// reference.</param> <returns>The concrete <see cref="GrainReference"/>
  /// implementation.</returns>
  internal delegate object GrainReferenceCaster(IAddressable existingReference);

  public GrainReferenceFactory(TypeMetadataCache typeCache,
  ...
  public object CreateGrainReference(Type interfaceType, GrainId grainId) {
    var untypedGrainReference = GrainReference.FromGrainId(
        grainId, this.grainReferenceRuntime,
        interfaceType.IsGenericType ? TypeUtils.GenericTypeArgsString(
            interfaceType.UnderlyingSystemType.FullName)
        : null);
  ...
  public object Cast(IAddressable grain, Type interfaceType) {
    ...
    GrainReferenceCaster MakeCaster(Type interfaceType) {
      var grainReferenceType =
          this.typeCache.GetGrainReferenceType(interfaceType);
      return GrainCasterFactory.CreateGrainReferenceCaster(interfaceType,
                                                           grainReferenceType);
    }
  }
}
