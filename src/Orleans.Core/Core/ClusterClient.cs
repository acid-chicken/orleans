...
internal class ClusterClient : IInternalClusterClient {
  ...
  public ClusterClient(IRuntimeClient runtimeClient,
                       ILoggerFactory loggerFactory,
                       IOptions<ClientMessagingOptions>clientMessagingOptions) {
    this.runtimeClient = runtimeClient;
    this.clusterClientLifecycle = new ClusterClientLifecycle(
        loggerFactory.CreateLogger<LifecycleSubject>());

    // set PropagateActivityId flag from node cofnig
    RequestContext.PropagateActivityId |=
        clientMessagingOptions.Value.PropagateActivityId;

    // register all lifecycle participants
    IEnumerable<ILifecycleParticipant<IClusterClientLifecycle>>
    ...
    IKeyedServiceCollection<string,
                            ILifecycleParticipant<IClusterClientLifecycle>>
        namedLifecycleParticipantCollections =
            this.ServiceProvider.GetService<IKeyedServiceCollection<
                string, ILifecycleParticipant<IClusterClientLifecycle>>>();
        ...
        // It is fine for this field to be null in the case that the client is
        // not the host.
        this.applicationLifetime =
            runtimeClient.ServiceProvider.GetService<IHostApplicationLifetime>()
                as ClientApplicationLifetime;
  }
  ...
  /// <inheritdoc />
  IStreamProviderRuntime IInternalClusterClient.StreamProviderRuntime =>
  ...
  public IStreamProvider GetStreamProvider(string name) {
    ...
    return this.runtimeClient.ServiceProvider
        .GetRequiredServiceByName<IStreamProvider>(name);
  }
    ...
    using(await this.initLock.LockAsync().ConfigureAwait(false)) {
      ...
      if (this.state == LifecycleState.Starting) {
        throw new InvalidOperationException(
            "A prior connection attempt failed. This instance must be disposed.");
      }
  ...
  public TGrainInterface GetGrain<TGrainInterface>(
      ...
      : IGrainWithGuidKey {
    return this.InternalGrainFactory.GetGrain<TGrainInterface>(
        primaryKey, grainClassNamePrefix);
  }
  ...
  public TGrainInterface GetGrain<TGrainInterface>(
    ...
    return this.InternalGrainFactory.GetGrain<TGrainInterface>(
        primaryKey, keyExtension, grainClassNamePrefix);
  }
  ...
  CreateObjectReference<TGrainObserverInterface>(IGrainObserver obj) where
    ...
    return ((IGrainFactory) this.runtimeClient.InternalGrainFactory)
        .CreateObjectReference<TGrainObserverInterface>(obj);
  }

  /// <inheritdoc />
  public Task DeleteObjectReference<TGrainObserverInterface>(
      IGrainObserver obj) where TGrainObserverInterface : IGrainObserver {
    return this.InternalGrainFactory
        .DeleteObjectReference<TGrainObserverInterface>(obj);
  }
  ...
  /// <inheritdoc />
  TGrainInterface IInternalGrainFactory.GetSystemTarget<TGrainInterface>(
      GrainType grainType, SiloAddress destination) {
    return this.InternalGrainFactory.GetSystemTarget<TGrainInterface>(
  ...
  /// <inheritdoc />
  TGrainInterface
      IInternalGrainFactory.Cast<TGrainInterface>(IAddressable grain) {
  ...
  private void ThrowIfDisposedOrNotInitialized() {
    ...
    if (!this.IsInitialized)
      throw new InvalidOperationException(
          $"Client is not initialized. Current client state is {this.state}.");
  }
  ...
  GetGrain<TGrainInterface>(Type grainInterfaceType,
      ...
      : IGrain => this.InternalGrainFactory.GetGrain<TGrainInterface>(
            grainInterfaceType, grainPrimaryKey);
  ...
  GetGrain<TGrainInterface>(Type grainInterfaceType, Guid grainPrimaryKey,
      ...
      : IGrain => this.InternalGrainFactory.GetGrain<TGrainInterface>(
            grainInterfaceType, grainPrimaryKey, keyExtension);
