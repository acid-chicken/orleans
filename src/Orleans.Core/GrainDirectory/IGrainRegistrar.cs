...
namespace Orleans.GrainDirectory {
  /// <summary>
  /// A grain registrar takes responsibility of coordinating the registration of
  /// a grains, possibly involving multiple clusters. The grain registrar is
  /// called only on the silo that is the owner for that grain.
  /// </summary>
  interface IGrainRegistrar {
    ...
    /// </summary>
    /// <returns>true if synchronous methods should be used, false if
    /// asynchronous methods should be used</returns>
    ...
    /// <param name="singleActivation">If true, use single-activation
    /// registration</param> <returns>The address registered for the grain's
    /// single activation.</returns>
    ...
    /// registration</param> <returns>The address registered for the grain's
    /// single activation.</returns>
    Task<AddressAndTag>RegisterAsync(ActivationAddress address,
    ...
    /// <summary>
    /// Invalidates registrations that are caches, i.e. point to activations in
    /// other clusters
    /// </summary>
    /// <param name="address">the remote activation to remove from the
    /// cache</param>
    ...
    /// <param name="cause">The reason for unregistration</param>
    Task UnregisterAsync(List<ActivationAddress>addresses,
