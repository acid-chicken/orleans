...
namespace Orleans.Runtime {
  ...
  */
  ...
  internal class StageAnalysisStatisticsGroup {
    ...
    private readonly Dictionary<string, List<ThreadTrackingStatistic>>
        stageGroups;

    public StageAnalysisStatisticsGroup(
      ...
      this.PerformStageAnalysis =
          statisticsOptions.Value.CollectionLevel.PerformStageAnalysis();
      if (this.PerformStageAnalysis) {
        StringValueStatistic.FindOrCreate(StatisticNames.STAGE_ANALYSIS,
                                          StageAnalysisInfo);
      }
    }
    ...
    private static readonly List<string>stages =
        new List<string>(){"Runtime.IncomingMessageAgent.Application",
    ...
    private static readonly HashSet<string>waitingStages =
        new HashSet<string>(){// stages[1], // if we know there is no waiting in
                              // the WorkerPoolThreads, we can remove it from
                              // waitingStages and get more accuarte measurements
                              stages[2], stages[3]};
    ...
    private string StageAnalysisInfo() {
      try {
        lock(stageGroups) {
          ...
          Dictionary<string, double>cpuPerRequest =
              new Dictionary<string,
                             double>(); // CPU time per request for each stage
          Dictionary<string, double>wallClockPerRequest =
              new Dictionary<string, double>(); // Wallclock time per request
                                                // for each stage
          Dictionary<string, double>numberOfRequests =
              new Dictionary<string,
                             double>(); // Number of requests for each stage
          foreach(var keyVal in stageGroups) {
            ...
            if (GetNumberOfRequests(name) > 0) {
              ...
              wallClockPerRequest.Add(name,
                                      GetWallClockPerStagePerRequest(name));
              numberOfRequests.Add(name, GetNumberOfRequests(name));
            }
          }
          cpuPerRequest.Add(
              "Untracked.ThreadPoolThread",
              GetCpuPerStagePerRequest("Untracked.ThreadPoolThread"));
          numberOfRequests.Add(
              "Untracked.ThreadPoolThread",
              GetNumberOfRequests("Untracked.ThreadPoolThread"));

          double elapsedWallClock = GetMaxWallClock();
          double elapsedCPUClock = GetTotalCPU();

          // Idle time estimation
          double untrackedProportionTime =
              1 - elapsedCPUClock / (cores * elapsedWallClock);
          ...
          foreach(var stage in wallClockPerRequest.Keys) {
            if (!waitingStages.Contains(stage)) {
              double ratio =
                  wallClockPerRequest[stage] / cpuPerRequest[stage] - 1;
          ...
          // Wait time estimation - implementation of strategy 2 from the
          // "local-throughput.pdf" "Coping with Practical Measurements".
          var waitTimes =
              new Dictionary<string,
                             double>(); // Wait time per request for each stage
          foreach(var stage in wallClockPerRequest.Keys) {
            waitTimes.Add(stage,
                          waitingStages.Contains(stage)
                              ? Math.Max(wallClockPerRequest[stage] -
                                             avgRatio * cpuPerRequest[stage] -
                                             cpuPerRequest[stage],
                                         0)
                              : 0);
          }
          ...
          foreach(var stage in wallClockPerRequest.Keys) {
            beta.Add(stage, cpuPerRequest[stage] /
                                (cpuPerRequest[stage] + waitTimes[stage]));
            s.Add(stage, 1000.0 / (cpuPerRequest[stage] + waitTimes[stage]));
            lambda.Add(stage,
                       1000.0 * numberOfRequests[stage] / elapsedWallClock);
          }

          // Final equation thread allocation - implementation of theorem 2 from
          // the "local-throughput.pdf" "Incorporating Ready Time".
          var throughputThreadAllocation =
              new Dictionary<string, double>(); // Thread allocation suggestion
                                                // for each stage
          foreach(var stage in wallClockPerRequest.Keys) {
            // cores is p
            ...
            // waitTimes is w
            throughputThreadAllocation.Add(
                stage,
                cores * numberOfRequests[stage] *
                    (cpuPerRequest[stage] * (1 + stableReadyTimeProportion) +
                     waitTimes[stage]) /
                    cpuSum);
          }

          double sum1 = 0;
          foreach(var stage in s.Keys) sum1 +=
              lambda[stage] * beta[stage] / s[stage];

          double sum2 = 0;
          foreach(var stage in s.Keys) sum2 +=
              Math.Sqrt(lambda[stage] * beta[stage] / s[stage]);

          var latencyThreadAllocation =
              new Dictionary<string, double>(); // Latency thread allocation
                                                // suggestion for each stage
          foreach(var stage in wallClockPerRequest.Keys)
              latencyThreadAllocation.Add(
                  stage, lambda[stage] / s[stage] +
                             Math.Sqrt(lambda[stage]) * (cores - sum1) /
                                 (Math.Sqrt(s[stage] * beta[stage]) * sum2));

          var latencyPenalizedThreadAllocationConst =
              new Dictionary<string, double>();
          var latencyPenalizedThreadAllocationCoef =
              new Dictionary<string, double>();

          foreach(var stage in wallClockPerRequest.Keys) {
            latencyPenalizedThreadAllocationConst.Add(stage,
                                                      lambda[stage] / s[stage]);
            latencyPenalizedThreadAllocationCoef.Add(
                stage,
                Math.Sqrt(lambda[stage] / (lambda[firstStage] * s[stage])));
          }

          double sum3 = 0;
          foreach(var stage in s.Keys) sum3 +=
              beta[stage] * Math.Sqrt(lambda[stage] / s[stage]);
          ...
          foreach(var stage in stages.Intersect(wallClockPerRequest.Keys)) {
            ...
            sb.AppendLine("  Measured average CPU per request:        " +
                          cpuPerRequest[stage].ToString("F3") + " ms");
            sb.AppendLine("  Measured average Wall-clock per request: " +
                          wallClockPerRequest[stage].ToString("F3") + " ms");
            sb.AppendLine("  Measured number of requests:             " +
                          numberOfRequests[stage].ToString("F0") + " requests");
            sb.AppendLine("  lambda:                                  " +
                          lambda[stage].ToString("F3") +
                          " arrival rate requests/sec");
            sb.AppendLine("  s:                                       " +
                          s[stage].ToString("F3") +
                          " per thread service rate requests/sec");
            sb.AppendLine("  beta:                                    " +
                          beta[stage].ToString("F3") + " per thread CPU usage");
            sb.AppendLine("  Estimated wait time:                     " +
                          waitTimes[stage].ToString("F3") + " ms");
            sb.AppendLine("  Throughput thread allocation:            " +
                          Math.Ceiling(throughputThreadAllocation[stage]) +
                          " threads  (rounded up from " +
                          throughputThreadAllocation[stage].ToString("F3") +
                          ")");
            sb.AppendLine("  Latency thread allocation:               " +
                          Math.Ceiling(latencyThreadAllocation[stage]) +
                          " threads  (rounded up from " +
                          latencyThreadAllocation[stage].ToString("F3") + ")");
            sb.AppendLine(
                "  Regularlized latency thread allocation:  " +
                latencyPenalizedThreadAllocationConst[stage].ToString("F3") +
                " + " +
                latencyPenalizedThreadAllocationCoef[stage].ToString("F3") +
                " / sqrt(eta) threads  (rounded this value up)");
          }

          var cpuBreakdown = new Dictionary<string, double>();
          foreach(var stage in cpuPerRequest.Keys) {
            double val = (numberOfRequests[stage] * cpuPerRequest[stage]) /
                         (cores * elapsedWallClock);
            cpuBreakdown.Add(stage == "ThreadPoolThread"
                                 ? "ThreadPoolThread.AsynchronousReceive"
                                 : stage,
                             val);
          }
    ...
    private double GetTotalCPU() {
      double total = 0;
      foreach(var keyVal in stageGroups) foreach(var statistics in keyVal.Value)
    ...
    /// <summary>
    /// gets total wallclock which is the wallclock of the stage with maximum
    /// wallclock time
    /// </summary>
    private double GetMaxWallClock() {
      double maxTime = 0;
      foreach(var keyVal in stageGroups) foreach(
          var statistics in keyVal.Value) maxTime =
          Math.Max(maxTime,
                   statistics.ExecutingWallClockTime.Elapsed.TotalMilliseconds);
    ...
    /// </summary>
    /// <param name="stageName">name of a stage from thread tracking
    /// statistics</param> <returns>number of requests</returns>
    private double GetNumberOfRequests(string stageName) {
      ...
      foreach(var tts in groups) num += tts.NumRequests;
    ...
    /// </summary>
    /// <param name="stageName">name of a stage from thread tracking
    /// statistics</param> <returns>average milliseconds of wallclock time per
    /// request</returns>
    private double GetWallClockPerStagePerRequest(string stageName) {
      ...
      foreach(var statistics in groups) if (stageName == "ThreadPoolThread") {
        sum += statistics.ProcessingWallClockTime.Elapsed.TotalMilliseconds;
      }
      else {
        sum += statistics.ProcessingWallClockTime.Elapsed.TotalMilliseconds;

        // We need to add the pure Take time, since in the
        // GetCpuPerStagePerRequest we includes both processingCPUCycleTime and
        // the Take time.
    ...
    /// </summary>
    /// <param name="stageName">name of a stage from thread tracking
    /// statistics</param> <returns>average milliseconds of cpu time per
    /// request</returns>
