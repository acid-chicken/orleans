...
namespace Orleans.Runtime {
  class IntValueStatistic : ICounter<long> {
    private static readonly Dictionary<string, IntValueStatistic>dict =
        new Dictionary<string, IntValueStatistic>();
    ...
    public static IntValueStatistic
    FindOrCreate(StatisticName name, Func<long>f,
    ...
    public static void Delete(StatisticName name){lock(dict){
        if(dict.TryGetValue(name.Name, out var stat)){dict.Remove(name.Name);
