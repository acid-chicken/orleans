...
namespace Orleans.Runtime {
  public class FloatValueStatistic : ICounter<float> {
    private static readonly Dictionary<string, FloatValueStatistic>dict =
        new Dictionary<string, FloatValueStatistic>();
    ...
    public static FloatValueStatistic FindOrCreate(StatisticName name,
                                                   Func<float>f) {
      return FindOrCreate(name, f, CounterStorage.LogOnly);
    }

    public static FloatValueStatistic
    FindOrCreate(StatisticName name, Func<float>f, CounterStorage storage) {
      lock(dict) {
    ...
    public static void Delete(StatisticName name) {
      lock(dict) {
        if (dict.TryGetValue(name.Name, out var stat)) {
          ...
          // Null the fetcher delegate to prevent memory leaks via undesirable
          // reference capture by the fetcher lambda.
          stat.fetcher = null;
    ...
    public static FloatValueStatistic CreateDoNotRegister(string name,
                                                          Func<float>f) {
      return new FloatValueStatistic(name,
                                     f){Storage = CounterStorage.DontStore};
    }
    ...
    public static void AddCounters(List<ICounter>list,
                                   Func<ICounter, bool>predicate) {
      lock(dict) { list.AddRange(dict.Values.Where(predicate)); }
    }
    ...
    public string GetDeltaString() { throw new NotSupportedException(); }

    public void ResetCurrent() { throw new NotSupportedException(); }

    public string GetDisplayString() { return ToString(); }

    public override string ToString() { return $"{Name}={GetValueString()}"; }
