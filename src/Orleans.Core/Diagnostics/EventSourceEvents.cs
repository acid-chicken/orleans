...
namespace Orleans.Runtime {
  ...
  internal sealed class OrleansCallBackDataEvent : EventSource {
    public static readonly OrleansCallBackDataEvent Log =
        new OrleansCallBackDataEvent();
    ...
    [Event(1)]
    public void
    OnTimeout(Guid activityId) {
      this.WriteEventWithRelatedActivityId(1, activityId);
    }

    [NonEvent]
    public void
    OnTargetSiloFail(Message message) {
      if (this.IsEnabled()) {
    ...
    [Event(2)]
    public void
    OnTargetSiloFail(Guid activityId) {
      this.WriteEventWithRelatedActivityId(2, activityId);
    }

    [NonEvent]
    public void
    DoCallback(Message message) {
      if (this.IsEnabled()) {
  ...
  internal sealed class OrleansOutsideRuntimeClientEvent : EventSource {
    public static readonly OrleansOutsideRuntimeClientEvent Log =
        new OrleansOutsideRuntimeClientEvent();
    ...
    [Event(2)]
    public void
    ReceiveResponse(Guid activityId) {
  ...
  internal sealed class OrleansDispatcherEvent : EventSource {
    public static readonly OrleansDispatcherEvent Log =
        new OrleansDispatcherEvent();
    ...
    [Event(1)]
    public void
    ReceiveMessage(Guid activityId) =>
        WriteEventWithRelatedActivityId(1, activityId);
  }

  [EventSource(Name = "Microsoft-Orleans-InsideRuntimeClient")]
  internal sealed class OrleansInsideRuntimeClientEvent : EventSource {
    public static readonly OrleansInsideRuntimeClientEvent Log =
        new OrleansInsideRuntimeClientEvent();
    ...
    [Event(1)]
    public void
    SendRequest(Guid activityId) => WriteEventWithRelatedActivityId(1,
                                                                    activityId);
  ...
  internal sealed class OrleansGatewayAcceptorEvent : EventSource {
    public static readonly OrleansGatewayAcceptorEvent Log =
        new OrleansGatewayAcceptorEvent();
  ...
  internal sealed class OrleansIncomingMessageAcceptorEvent : EventSource {
    public static readonly OrleansIncomingMessageAcceptorEvent Log =
        new OrleansIncomingMessageAcceptorEvent();
  ...
  internal sealed class OrleansIncomingMessageAgentEvent : EventSource {
    public static readonly OrleansIncomingMessageAgentEvent Log =
        new OrleansIncomingMessageAgentEvent();
