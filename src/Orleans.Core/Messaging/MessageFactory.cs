...
namespace Orleans.Runtime {
  internal class MessageFactory {
    ...
    public Message CreateMessage(InvokeMethodRequest request,
                                 InvokeMethodOptions options) {
      var message = new Message{
          Category = Message.Categories.Application,
          Direction = (options & InvokeMethodOptions.OneWay) != 0
                          ? Message.Directions.OneWay
                          : Message.Directions.Request,
          Id = CorrelationId.GetNext(),
          IsReadOnly = (options & InvokeMethodOptions.ReadOnly) != 0,
          IsUnordered = (options & InvokeMethodOptions.Unordered) != 0,
          IsAlwaysInterleave =
              (options & InvokeMethodOptions.AlwaysInterleave) != 0,
          BodyObject = request,
      ...
      if (options.IsTransactional()) {
        ...
        message.RequestContextData?.Remove(
            TransactionContext.Orleans_TransactionContext_Key);
      }
    ...
    private void SetTransaction(Message message, InvokeMethodOptions options) {
      // clear transaction info if transaction operation requires new
      // transaction.
      ITransactionInfo transactionInfo =
          TransactionContext.GetTransactionInfo();

      // enforce join transaction calls
      if (options.IsTransactionOption(InvokeMethodOptions.TransactionJoin) &&
          transactionInfo == null) {
        throw new NotSupportedException(
            "Call cannot be made outside of a transaction.");
      }

      // enforce not allowed transaction calls
      if (options.IsTransactionOption(
              InvokeMethodOptions.TransactionNotAllowed) &&
          transactionInfo != null) {
        throw new NotSupportedException(
            "Call cannot be made within a transaction.");
      }

      // clear transaction context if creating a transaction or transaction is
      // suppressed
      if (options.IsTransactionOption(InvokeMethodOptions.TransactionCreate) ||
          options.IsTransactionOption(
              InvokeMethodOptions.TransactionSuppress)) {
        transactionInfo = null;
      }

      bool isTransactionRequired =
          options.IsTransactionOption(InvokeMethodOptions.TransactionCreate) ||
          options.IsTransactionOption(
              InvokeMethodOptions.TransactionCreateOrJoin) ||
      ...
      if (transactionInfo == null) {
        // if we're leaving a transaction context, make sure it's been cleared
    ...
    public Message CreateResponseMessage(Message request) {
      var response =
          new Message{Category = request.Category,
      ...
      if (request.TargetGrain != null) {
        ...
        if (request.TargetActivation != null) {
          ...
          response.SendingActivation = ActivationId.GetSystemActivation(
              request.TargetGrain, request.TargetSilo);
        }
      }
    ...
    public Message CreateRejectionResponse(Message request,
                                           Message.RejectionTypes type,
                                           string info, Exception ex = null) {
      ...
      if (this.logger.IsEnabled(LogLevel.Debug))
        this.logger.Debug("Creating {0} rejection with info '{1}' for {2} at:" +
                              Environment.NewLine + "{3}",
                          type, info, this, Utils.GetStackTrace());
      return response;
    }
  }
}