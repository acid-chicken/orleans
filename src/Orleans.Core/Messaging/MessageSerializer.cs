...
namespace Orleans.Runtime.Messaging {
  internal sealed class MessageSerializer : IMessageSerializer {
    ...
    public MessageSerializer(SerializationManager serializationManager,
                             SharedMemoryPool memoryPool, int maxHeaderSize,
                             int maxBodySize) {
      this.headersSerializer = new HeadersSerializer(serializationManager);
      this.objectSerializer =
          new OrleansSerializer<object>(serializationManager);
      this.memoryPool = memoryPool.Pool;
      this.maxHeaderLength = maxHeaderSize;
      this.maxBodyLength = maxBodySize;
    }

    public(int RequiredBytes, int HeaderLength, int BodyLength)
        TryRead(ref ReadOnlySequence<byte>input, out Message message) {
      message = default;
      ...
      var bodyLength =
          BinaryPrimitives.ReadInt32LittleEndian(lengthBytes.Slice(4));
      ...
      try {
        ...
        message = new Message{Headers = headersContainer};

        // Body deserialization is more likely to fail than header
        // deserialization. Separating the two allows for these kinds of errors
    ...
    public(int HeaderLength, int BodyLength) Write<TBufferWriter>(
        ref TBufferWriter writer, Message message) where TBufferWriter
        : IBufferWriter<byte> {
      if (!(this.bufferWriter is
                PrefixingBufferWriter<byte, TBufferWriter>buffer)) {
        this.bufferWriter = buffer =
            new PrefixingBufferWriter<byte, TBufferWriter>(
                FramingLength, MessageSizeHint, this.memoryPool);
      }
      ...
      BinaryPrimitives.WriteInt32LittleEndian(lengthFields.Slice(4),
                                              bodyLength);
    ...
    private void
    ThrowIfLengthsInvalid(int headerLength, int bodyLength) {
      if (headerLength <= 0 || headerLength > this.maxHeaderLength) {
        throw new OrleansException(
            $"Invalid header size: {headerLength} (max configured value is {this.maxHeaderLength}, see {nameof(MessagingOptions.MaxMessageHeaderSize)})");
      }

      if (bodyLength < 0 || bodyLength > this.maxBodyLength) {
        throw new OrleansException(
            $"Invalid body size: {bodyLength} (max configured value is {this.maxBodyLength}, see {nameof(MessagingOptions.MaxMessageBodySize)})");
      }
    }

    private sealed class OrleansSerializer<T> {
      private readonly SerializationManager serializationManager;
      private readonly BinaryTokenStreamReader2 reader =
          new BinaryTokenStreamReader2();
      ...
      public OrleansSerializer(SerializationManager serializationManager) {
        ...
        this.deserializationContext = new DeserializationContext(
            serializationManager){StreamReader = this.reader};
      }

      public void Deserialize(ReadOnlySequence<byte>input, out T value) {
        reader.PartialReset(input);
        try {
          value = (T) SerializationManager.DeserializeInner(
              this.serializationManager, typeof (T),
              this.deserializationContext, this.reader);
        } finally {
          this.deserializationContext.Reset();
        }
      }

      public void Serialize<TBufferWriter>(TBufferWriter output,
                                           T value) where TBufferWriter
          : IBufferWriter<byte> {
        ...
        if (streamWriter is BinaryTokenStreamWriter2<TBufferWriter>writer) {
          ...
          this.serializationContext.StreamWriter = writer =
              new BinaryTokenStreamWriter2<TBufferWriter>(output);
        }

        try {
          SerializationManager.SerializeInner(
              this.serializationManager, value, typeof (T),
              this.serializationContext, writer);
        } finally {
          writer.Commit();
    ...
    private sealed class HeadersSerializer {
      ...
      public void Deserialize(ReadOnlySequence<byte>input,
                              out Message.HeadersContainer value) {
        try {
          reader.PartialReset(input);
          value =
              (Message.HeadersContainer) Message.HeadersContainer.Deserializer(
                  null, this.deserializationContext);
        } finally {
          this.deserializationContext.Reset();
        }
      }
      ...
      public void Serialize<TBufferWriter>(TBufferWriter output,
                                           Message.HeadersContainer value) where
        ...
        try {
          Message.HeadersContainer.Serializer(value, this.serializationContext,
                                              null);
        } finally {
          writer.Commit();
          this.serializationContext.Reset();
