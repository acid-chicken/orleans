...
namespace Orleans.Messaging {
  /// <summary>
  /// The GatewayManager class holds the list of known gateways, as well as
  /// maintaining the list of "dead" gateways.
  ///
  /// The known list can come from one of two places: the full list may appear
  /// in the client configuration object, or the config object may contain an
  /// IGatewayListProvider delegate. If both appear, then the delegate takes
  /// priority.
  /// </summary>
  internal class GatewayManager : IGatewayListListener, IDisposable {
    ...
    public GatewayManager(IOptions<GatewayOptions>gatewayOptions,
      ...
      logger.Info(ErrorCode.GatewayManager_FoundKnownGateways,
                  "Found {0} knownGateways from Gateway listProvider {1}",
                  knownGateways.Count, Utils.EnumerableToString(knownGateways));

      if (ListProvider is IGatewayListObservable) {
        ((IGatewayListObservable) ListProvider)
            .SubscribeToGatewayNotificationEvents(this);
      }

      roundRobinCounter = this.gatewayOptions.PreferedGatewayIndex >= 0
                              ? this.gatewayOptions.PreferedGatewayIndex
                              : rand.Next(knownGateways.Count);

      this.knownGateways = cachedLiveGateways =
          knownGateways.Select(gw => gw.ToGatewayAddress()).ToList();
    ...
    public void Stop() {
      ...
      if (ListProvider != null && ListProvider is IGatewayListObservable) {
        Utils.SafeExecute(
            () => ((IGatewayListObservable) ListProvider)
                      .UnSubscribeFromGatewayNotificationEvents(this),
            logger);
      }
    }
    ...
    public void MarkAsDead(SiloAddress gateway) {
      lock(lockable) {
        ...
        copy.Remove(gateway);
        // swap the reference, don't mutate cachedLiveGateways, so we can access
        // cachedLiveGateways without the lock.
    ...
    ///
    /// Note that if a list provider delegate was given, the delegate is invoked
    /// every time this method is called. This method performs caching to avoid
    /// hammering the ultimate data source.
    ///
    /// This implementation does a simple round robin selection. It assumes that
    /// the gateway list from the provider is in the same order every time.
    /// </summary>
    /// <returns></returns>
    public SiloAddress GetLiveGateway() {
      ...
      if (count > 0) {
        lock(lockable) {
          // Round-robin through the known gateways and take the next live one,
    ...
    public List<SiloAddress>GetLiveGateways() {
      // Never takes a lock and returns the cachedLiveGateways list quickly
      // without any operation. Asynchronously starts gateway refresh only when
      // it is empty.
          ...
          lock(this.lockable) {
            if (cachedLiveGateways.Count == 0 && knownGateways.Count > 0) {
              this.logger.LogWarning(
                  "All known gateways have been marked dead locally. Expediting gateway refresh and resetting all gateways to live status.");
    ...
    internal void ExpediteUpdateLiveGatewaysSnapshot() {
      // If there is already an expedited refresh call in place, don't call
      // again, until the previous one is finished. We don't want to issue too
      // many Gateway refresh calls.
      if (ListProvider == null || gatewayRefreshCallInitiated)
        return;

      // Initiate gateway list refresh asynchronously. The Refresh timer will
      // keep ticking regardless. We don't want to block the client with
      // synchronously Refresh call. Client's call will fail with "No Gateways
      // found" but we will try to refresh the list quickly.
    ...
    public void
    GatewayListNotification(IEnumerable<Uri>gateways) {
      try {
        UpdateLiveGatewaysSnapshot(gateways.Select(gw => gw.ToGatewayAddress()),
                                   ListProvider.MaxStaleness);
      } catch (Exception exc) {
        logger.Error(
            ErrorCode.ProxyClient_GetGateways,
            "Exception occurred during GatewayListNotification -> UpdateLiveGatewaysSnapshot",
            exc);
      }
    }
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage(
        "Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes")]
    internal async Task
    RefreshSnapshotLiveGateways_TimerCallback(object context) {
      try {
        if (ListProvider is null)
          return;

        // the listProvider.GetGateways() is not under lock.
        var allGateways = await ListProvider.GetGateways();
        var refreshedGateways =
            allGateways.Select(gw => gw.ToGatewayAddress()).ToList();
        if (logger.IsEnabled(LogLevel.Debug)) {
          logger.LogDebug("Discovered {GatewayCount} gateways: {Gateways}",
                          refreshedGateways.Count,
                          Utils.EnumerableToString(refreshedGateways));
        }

        // the next one will grab the lock.
        UpdateLiveGatewaysSnapshot(refreshedGateways,
                                   ListProvider.MaxStaleness);
      } catch (Exception exc) {
        logger.Error(
            ErrorCode.ProxyClient_GetGateways,
            "Exception occurred during RefreshSnapshotLiveGateways_TimerCallback -> listProvider.GetGateways()",
            exc);
      }
    }
    ...
    private void
    UpdateLiveGatewaysSnapshot(IEnumerable<SiloAddress>refreshedGateways,
                               TimeSpan maxStaleness) {
      // this is a short lock, protecting the access to knownDead and
      // cachedLiveGateways.
      lock(lockable) {
        // now take whatever listProvider gave us and exclude those we think are
        // dead.
        ...
        this.knownGateways =
            refreshedGateways as List<SiloAddress>?? refreshedGateways.ToList();
        foreach(SiloAddress trial in knownGateways) {
          var address = trial.Generation == 0
                            ? trial
                            : SiloAddress.New(trial.Endpoint, 0);

          // We consider a node to be dead if we recorded it is dead due to
          // socket error and it was recorded (diedAt) not too long ago (less
          // than maxStaleness ago). The latter is to cover the case when the
          // Gateway provider returns an outdated list that does not yet reflect
          // the actually recently died Gateway. If it has passed more than
          // maxStaleness - we assume maxStaleness is the upper bound on Gateway
          // provider freshness.
          var isDead = false;
        ...
        if (logger.IsEnabled(LogLevel.Information)) {
          logger.Info(
              ErrorCode.GatewayManager_FoundKnownGateways,
              ...
              Utils.EnumerableToString(cachedLiveGateways), prevRefresh);
        }
        ...
        if (!isLiveGateway) {
          if (logger.IsEnabled(LogLevel.Information)) {
            this.logger.LogInformation(
                "Aborting connection to {Endpoint} because it has been marked as dead",
                address);
          }
