...
namespace Orleans.Messaging {
  ...
  internal class GatewayManager : IGatewayListListener, IDisposable {
    ...
    private readonly Dictionary<SiloAddress, DateTime>knownDead =
        new Dictionary<SiloAddress, DateTime>();
    ...
    public async Task StartAsync(CancellationToken cancellationToken) {
      ...
      if (knownGateways.Count == 0) {
        var err =
            $"Could not find any gateway in {this.gatewayListProvider.GetType().FullName}. Orleans client cannot initialize.";
      ...
      this.logger.LogInformation(
          ...
          "Found {GatewayCount} gateways: {Gateways}", knownGateways.Count,
      ...
      this.roundRobinCounter = this.gatewayOptions.PreferedGatewayIndex >= 0
                                   ? this.gatewayOptions.PreferedGatewayIndex
                                   : this.rand.Next(knownGateways.Count);
      this.knownGateways = this.cachedLiveGateways =
    ...
    internal void ExpediteUpdateLiveGatewaysSnapshot() {
      ...
      if (gatewayListProvider == null || gatewayRefreshCallInitiated)
        return;
    ...
    GatewayListNotification(IEnumerable<Uri>gateways) {
      try {
        UpdateLiveGatewaysSnapshot(gateways.Select(gw => gw.ToGatewayAddress()),
                                   gatewayListProvider.MaxStaleness);
      } catch (Exception exc) {
        logger.Error(
