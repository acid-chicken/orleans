...
namespace Orleans.Serialization {
  internal static class BinaryTokenStreamReaderExtensinons {
    internal static IdSpan ReadIdSpan<TReader>(this TReader @this) where TReader
    ...
    /// <returns>Data from current position in stream, converted to the
    /// appropriate output type.</returns>
    internal static GrainId
    ReadGrainId<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
    ...
    /// <returns>Data from current position in stream, converted to the
    /// appropriate output type.</returns>
    internal static ActivationId
    ReadActivationId<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
      UniqueKey key = @this.ReadUniqueKey();
      return ActivationId.GetActivationId(key);
    }

    internal static UniqueKey
    ReadUniqueKey<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
    ...
    internal static CorrelationId
    ReadCorrelationId<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
      return new CorrelationId(@this.ReadLong());
    }

    internal static GrainDirectoryEntryStatus
    ReadMultiClusterStatus<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
      byte val = @this.ReadByte();
      return (GrainDirectoryEntryStatus) val;
    }

    /// <summary> Read an <c>ActivationAddress</c> value from the stream.
    /// </summary> <returns>Data from current position in stream, converted to
    /// the appropriate output type.</returns>
    internal static ActivationAddress
    ReadActivationAddress<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
    ...
    internal static SerializationTokenType
    PeekToken<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
      return (SerializationTokenType) @this.PeekByte();
    }

    /// <summary> Read a <c>SerializationTokenType</c> value from the stream.
    /// </summary> <returns>Data from current position in stream, converted to
    /// the appropriate output type.</returns>
    internal static SerializationTokenType
    ReadToken<TReader>(this TReader @this) where TReader
        : IBinaryTokenStreamReader {
      return (SerializationTokenType) @this.ReadByte();
    }

    internal static bool
    TryReadSimpleType<TReader>(this TReader @this, out object result,
                               out SerializationTokenType token) where TReader
        : IBinaryTokenStreamReader {
    ...
    internal static Type ReadSpecifiedTypeHeader<TReader>(
        this TReader @this,
        SerializationManager serializationManager) where TReader
        : IBinaryTokenStreamReader {
      ...
      case SerializationTokenType.Tuple + 1:
        return typeof (Tuple<>).MakeGenericType(
            @this.ReadGenericArguments(serializationManager, 1));
      case SerializationTokenType.Tuple + 2:
        return typeof (Tuple<, >).MakeGenericType(
            @this.ReadGenericArguments(serializationManager, 2));
      case SerializationTokenType.Tuple + 3:
        return typeof (Tuple<, , >)
            .MakeGenericType(
                @this.ReadGenericArguments(serializationManager, 3));
      case SerializationTokenType.Tuple + 4:
        return typeof (Tuple<, , , >)
            .MakeGenericType(
                @this.ReadGenericArguments(serializationManager, 4));
      case SerializationTokenType.Tuple + 5:
        return typeof (Tuple<, , , , >)
            .MakeGenericType(
                @this.ReadGenericArguments(serializationManager, 5));
      case SerializationTokenType.Tuple + 6:
        return typeof (Tuple<, , , , , >)
            .MakeGenericType(
                @this.ReadGenericArguments(serializationManager, 6));
      case SerializationTokenType.Tuple + 7:
        return typeof (Tuple<, , , , , , >)
            .MakeGenericType(
                @this.ReadGenericArguments(serializationManager, 7));
      ...
      case SerializationTokenType.Stack:
        return typeof (Stack<>).MakeGenericType(
            @this.ReadGenericArguments(serializationManager, 1));
      case SerializationTokenType.Queue:
        return typeof (Queue<>).MakeGenericType(
            @this.ReadGenericArguments(serializationManager, 1));
      ...
      case SerializationTokenType.NamedType:
        ...
        try {
          ...
          throw new TypeAccessException("Named type \"" + typeName +
                                        "\" is invalid: " + ex.Message);
        }
      default:
        break;
      }
      ...
      throw new SerializationException(
          "Unexpected '" + token + "' found when expecting a type reference");
    }
    /// <summary> Read a <c>Type</c> value from the stream. </summary>
    /// <param name="this">The IBinaryTokenStreamReader to read from</param>
    /// <param name="serializationManager">The serialization manager used to
    /// resolve type names.</param> <param name="expected">Expected Type, if
    /// known.</param> <returns>Data from current position in stream, converted
    /// to the appropriate output type.</returns>
    private static Type ReadFullTypeHeader<TReader>(
        this TReader @this, SerializationManager serializationManager,
        Type expected = null) where TReader : IBinaryTokenStreamReader {
      var token = @this.ReadToken();
      ...
      throw new SerializationException(
          "Invalid '" + token +
          "'token in input stream where full type header is expected");
    }

    private static Type[] ReadGenericArguments(
        this IBinaryTokenStreamReader @this,
        SerializationManager serializationManager, int n) {
      var args = new Type[n];
      for (var i = 0; i < n; i++) {
  ...
  public class BinaryTokenStreamReader : IBinaryTokenStreamReader {
    ...
    private static readonly ArraySegment<byte>emptySegment =
        new ArraySegment<byte>(new byte[0]);
    private static readonly byte[] emptyByteArray = new byte[0];

    /// <summary>
    /// Create a new BinaryTokenStreamReader to read from the specified input
    /// byte array.
    /// </summary>
    ...
    /// buffers.
    /// </summary>
    /// <param name="buffs">The list of ArraySegments to use for the
    /// data.</param>
    ...
    private void InitializeCurrentSegment(int segmentIndex) {
      ...
      currentSegmentOffsetPlusCount =
          currentSegmentOffset + currentSegmentCount;
    }
    ...
    public int CurrentPosition =>
        currentOffset + totalProcessedBytes - currentSegmentOffset;
    ...
    private bool
    TryCheckLengthFast(int n, out byte[] res, out int offset,
                       out bool safeToUse) {
      safeToUse = false;
      res = null;
      offset = 0;
    ...
    private byte[] CheckLength(int n, out int offset, out bool safeToUse) {
      ...
      if ((CurrentPosition + n > totalLength)) {
        throw new SerializationException(String.Format(
            "Attempt to read past the end of the input stream: CurrentPosition={0}, n={1}, totalLength={2}",
      ...
      while (i < n) {
        ...
        var bytesFromThisBuffer =
            Math.Min(segmentOffsetPlusCount - currentOffset, n - i);
        Buffer.BlockCopy(currentBuffer, currentOffset, temp, i,
                         bytesFromThisBuffer);
        ...
        if (currentOffset >= segmentOffsetPlusCount) {
          if (currentSegmentIndex >= buffersCount) {
            throw new SerializationException(String.Format(
                "Attempt to read past buffers.Count: currentSegmentIndex={0}, buffers.Count={1}.",
                currentSegmentIndex, buffers.Count));
          }
    ...
    /// <summary> Read the next bytes from the stream. </summary>
    /// <param name="destination">Output array to store the returned data
    /// in.</param> <param name="offset">Offset into the destination array to
    /// write to.</param> <param name="count">Number of bytes to read.</param>
    public void ReadByteArray(byte[] destination, int offset, int count) {
      if (offset + count > destination.Length) {
        throw new ArgumentOutOfRangeException(
            "count", "Reading into an array that is too small");
      }

      var buffOffset = 0;
      var buff =
          count == 0 ? emptyByteArray : CheckLength(count, out buffOffset);
