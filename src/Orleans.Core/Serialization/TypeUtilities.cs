...
namespace Orleans.Serialization {
  ...
  internal static class TypeUtilities {
    ...
    static readonly ConcurrentDictionary<Type, string>typeNameCache =
        new ConcurrentDictionary<Type, string>();
    static readonly ConcurrentDictionary<Type, string>typeKeyStringCache =
        new ConcurrentDictionary<Type, string>();
    static readonly ConcurrentDictionary<Type, byte[]>typeKeyCache =
        new ConcurrentDictionary<Type, byte[]>();

    static readonly ConcurrentDictionary<Type, bool>shallowCopyableTypes =
    ...
    private static bool IsShallowCopyableInternal(Type t) {
      ...
      if (t.IsConstructedGenericType) {
        ...
        if (def == typeof (Nullable<>) || def == typeof (Tuple<>) ||
            def == typeof (Tuple<, >) || def == typeof (Tuple<, , >) ||
            def == typeof (Tuple<, , , >) || def == typeof (Tuple<, , , , >) ||
            def == typeof (Tuple<, , , , , >) ||
            def == typeof (Tuple<, , , , , , >) ||
            def == typeof (Tuple<, , , , , , , >))
          return Array.TrueForAll(t.GenericTypeArguments,
                                  a => IsOrleansShallowCopyable(a));
      }

      if (t.IsValueType && !t.IsGenericTypeDefinition)
        return Array.TrueForAll(t.GetFields(BindingFlags.Instance |
                                            BindingFlags.Public |
                                            BindingFlags.NonPublic),
                                f => IsOrleansShallowCopyable(f.FieldType));
    ...
    private static string GetBaseTypeKey(Type t) {
      ...
      if (t.IsNestedPublic) {
        return namespacePrefix + OrleansTypeKeyString(t.DeclaringType) + "." +
               t.Name;
      }
    ...
    GetLocationSafe(this Assembly a) {
      if (a.IsDynamic) {
        return "dynamic";
      }
    ...
    /// <summary>
    /// Returns <see langword="true"/> if a type is accessible from C# code from
    /// the specified assembly, and <see langword="false"/> otherwise.
    /// </summary>
    ...
    public static bool IsAccessibleFromAssembly(Type type, Assembly assembly) {
      ...
      // Obsolete types can be accessed, however obsolete types which have
      // IsError set cannot.
      ...
      if (type.IsArray)
        return IsAccessibleFromAssembly(type.GetElementType(), assembly);

      // Pointer and ref types are not accessible.
      if (type.IsPointer || type.IsByRef)
        return false;

      // Generic types are only accessible if their generic arguments are
      // accessible.
      if (type.IsConstructedGenericType) {
        foreach(var parameter in type.GetGenericArguments()) {
          if (!IsAccessibleFromAssembly(parameter, assembly))
            return false;
        }
      } else if (type.IsGenericTypeDefinition) {
        // Guard against unrepresentable type constraints, which appear when
        // generating code for some languages, such as F#.
        foreach(var parameter in type.GetTypeInfo().GenericTypeParameters) {
          foreach(
              var constraint in parameter.GetGenericParameterConstraints()) {
            if (constraint == typeof (Array) ||
                constraint == typeof (Delegate) || constraint == typeof (Enum))
              return false;
          }
        }
      }

      // Internal types are accessible only if the declaring assembly exposes
      // its internals to the target assembly.
      if (type.IsNotPublic || type.IsNestedAssembly ||
          type.IsNestedFamORAssem) {
        if (!AreInternalsVisibleTo(type.Assembly, assembly))
          return false;
      }
      ...
      // Nested types are otherwise accessible if their declaring type is
      // accessible.
      if (type.IsNested) {
    ...
    /// <summary>
    /// Returns true if <paramref name="fromAssembly"/> has exposed its
    /// internals to <paramref name="toAssembly"/>, false otherwise.
    /// </summary>
    /// <param name="fromAssembly">The assembly containing internal
    /// types.</param> <param name="toAssembly">The assembly requiring access to
    /// internal types.</param> <returns> true if <paramref
    /// name="fromAssembly"/> has exposed its internals to <paramref
    /// name="toAssembly"/>, false otherwise
    /// </returns>
    private static bool AreInternalsVisibleTo(Assembly fromAssembly,
                                              Assembly toAssembly) {
      ...
      // Check InternalsVisibleTo attributes on the from-assembly, pointing to
      // the to-assembly.
      var fullName = toAssembly.GetName().FullName;
      var shortName = toAssembly.GetName().Name;
      var internalsVisibleTo =
          fromAssembly.GetCustomAttributes<InternalsVisibleToAttribute>();
      foreach(var attr in internalsVisibleTo) {
        if (string.Equals(attr.AssemblyName, fullName,
                          StringComparison.Ordinal))
          return true;
        if (string.Equals(attr.AssemblyName, shortName,
                          StringComparison.Ordinal))
          return true;
      }
