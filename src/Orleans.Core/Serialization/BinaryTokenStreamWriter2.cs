...
namespace Orleans.Serialization {
  ...
  /// </summary>
  internal sealed class BinaryTokenStreamWriter2<TBufferWriter>
      : IBinaryTokenStreamWriter where TBufferWriter : IBufferWriter<byte> {
    private static readonly Dictionary<Type, SerializationTokenType>typeTokens;
    private static readonly Dictionary<
        Type, Action<BinaryTokenStreamWriter2<TBufferWriter>, object>>writers;
    ...
    static BinaryTokenStreamWriter2() {
      ...
      typeTokens[typeof (ActivationAddress)] =
      ...
      typeTokens[typeof (Dictionary<string, object>)] =
      ...
      typeTokens[typeof (KeyValuePair<, >)] =
      ...
      typeTokens[typeof (Tuple<, , , , , , >)] =
          SerializationTokenType.Tuple + 7;

      writers = new Dictionary<
          Type, Action<BinaryTokenStreamWriter2<TBufferWriter>, object>>();
      ...
      writers[typeof (GrainId)] = (stream, obj) => {
        stream.Write(SerializationTokenType.GrainId);
        stream.Write((GrainId) obj);
      };
      writers[typeof (ActivationId)] = (stream, obj) => {
      ...
      writers[typeof (ActivationAddress)] = (stream, obj) => {
      ...
      writers[typeof (CorrelationId)] = (stream, obj) => {
    ...
    public void
    PartialReset(TBufferWriter output) {
      this.output = output;
...
public void Write(decimal d) { this.Write(Decimal.GetBits(d)); }
...
Write(string s) {
  if (s is null) {
    ...
    // Attempt a fast write. Note that we could accurately determine the
    // required bytes to encode the input here, but that requires an additional
    // scan over the string. We could determine an upper bound here, too, but
    // that may be wasteful. Instead, we will fall back to a slower and more
    // accurate method if it is not.
    var writableSpan = this.TryGetContiguous(256);
    enc.Convert(s, writableSpan.Slice(4), true, out var charsUsed,
    ...
    // Otherwise, try again more slowly, overwriting whatever data was just
...
private void
WriteStringInternalSlower(string s) {
...
public void Write(char c) { this.Write(Convert.ToInt16(c)); }

public void Write(bool b) {
  this.Write((byte)(b ? SerializationTokenType.True
                    : SerializationTokenType.False));
}

public void WriteNull() { this.Write((byte) SerializationTokenType.Null); }
...
public void WriteTypeHeader(Type t, Type expected = null) {
  ...
  if (t.GetTypeInfo().IsGenericType) {
    if (typeTokens.TryGetValue(t.GetGenericTypeDefinition(), out token)) {
      this.Write((byte) token);
      foreach(var tp in t.GetGenericArguments()) { this.WriteTypeHeader(tp); }
      return;
    }
  }
...
public void Write(TimeSpan ts) { this.Write(ts.Ticks); }

public void Write(DateTime dt) { this.Write(dt.ToBinary()); }

public void Write(Guid id) { this.Write(id.ToByteArray()); }
...
/// <returns>Returns <c>true</c> if the value was successfully written to the
...
private Span<byte>WritableSpan =>
    this.currentBuffer.Slice(this.currentOffset).Span;

[MethodImpl(MethodImplOptions.AggressiveInlining)]
public void
EnsureContiguous(int length) {
  ...
  if (this.currentBuffer.Length < length)
    ThrowTooLarge(length);

  void ThrowTooLarge(int l) => throw new InvalidOperationException(
      $"Requested buffer length {l} cannot be satisfied by the writer.");
#endif
}

[MethodImpl(MethodImplOptions.AggressiveInlining)]
public Span<byte>
TryGetContiguous(int length) {
  // The current buffer is adequate.
  if (this.currentOffset + length > this.currentBuffer.Length) {
    // The current buffer is inadequate, allocate another.
    this.Allocate(length);
...
private void WriteMultiSegment(in ReadOnlySpan<byte>source) {
  ...
  while (true) {
    ...
    var writeSize =
        Math.Min(this.currentBuffer.Length - this.currentOffset, input.Length);
...
public void Write(List<ArraySegment<byte>>b) {
  foreach(var segment in b) { this.Write(segment); }
}
