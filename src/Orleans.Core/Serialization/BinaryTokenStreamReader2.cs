...
namespace Orleans.Serialization {
  ...
  internal sealed class BinaryTokenStreamReader2 : IBinaryTokenStreamReader {
    ...
    public int CurrentPosition {
      [MethodImpl(MethodImplOptions.AggressiveInlining)] get =>
          (int) this.previousBuffersSize + this.bufferPos;
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void
    PartialReset(ReadOnlySequence<byte>input) {
      this.input = input;
    ...
    MoveNext() {
      ...
      // If this is the first call to MoveNext then nextSequencePosition is
      // invalid and must be moved to the second position.
      if (this.nextSequencePosition.Equals(this.input.Start))
        this.input.TryGet(ref this.nextSequencePosition, out _);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public short
    ReadInt16() => unchecked((short) ReadUInt16());

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ushort
    ReadUInt16() {
      const int width = 2;
      if (this.bufferPos + width > this.bufferSize)
        return ReadSlower();

      var result = BinaryPrimitives.ReadUInt16LittleEndian(
          this.currentSpan.Span.Slice(this.bufferPos, width));
    ...
    ReadUInt32() {
      ...
      var result = BinaryPrimitives.ReadUInt32LittleEndian(
          this.currentSpan.Span.Slice(this.bufferPos, width));
      this.bufferPos += width;
      return result;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public long
    ReadInt64() => (long) this.ReadUInt64();
    ...
    ReadUInt64() {
      ...
      var result = BinaryPrimitives.ReadUInt64LittleEndian(
          this.currentSpan.Slice(this.bufferPos, width).Span);
      this.bufferPos += width;
      return result;
    ...
    private static void
    ThrowInsufficientData() => throw new InvalidOperationException(
        "Insufficient data present in buffer.");

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
#if NETSTANDARD2_1
    public float
    ReadFloat() => BitConverter.Int32BitsToSingle(ReadInt32());
#else
    public float
    ReadFloat() =>
        BitConverter.ToSingle(BitConverter.GetBytes(this.ReadInt32()), 0);
#endif

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
#if NETSTANDARD2_1
    public double
    ReadDouble() => BitConverter.Int64BitsToDouble(ReadInt64());
#else
    public double
    ReadDouble() =>
        BitConverter.ToDouble(BitConverter.GetBytes(this.ReadInt64()), 0);
#endif

    public decimal ReadDecimal() {
      var parts = new[]{this.ReadInt32(), this.ReadInt32(), this.ReadInt32(),
                        this.ReadInt32()};
    ...
    public void ReadBytes(in Span<byte>destination) {
      if (this.bufferPos + destination.Length <= this.bufferSize) {
        this.currentSpan.Slice(this.bufferPos, destination.Length)
      ...
      void CopySlower(in Span<byte>d) {
        ...
        while (true) {
          var writeSize =
              Math.Min(dest.Length, this.currentSpan.Length - this.bufferPos);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool
    TryReadBytes(int length, out ReadOnlySpan<byte>bytes) {
    ...
    public string ReadString() {
      ...
      if (this.bufferSize - this.bufferPos >= n) {
        var s = Encoding.UTF8.GetString(
            this.currentSpan.Slice(this.bufferPos, n).Span);
        this.bufferPos += n;
        return s;
      } else if (n <= 256) {
    ...
    /// <returns>Data from current position in stream, converted to the
    /// appropriate output type.</returns>
    public char ReadChar() { return Convert.ToChar(ReadInt16()); }
