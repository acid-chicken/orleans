...
namespace Orleans.Serialization {
  ...
  public class OrleansJsonSerializer : IExternalSerializer {
    ...
    GetDefaultSerializerSettings(ITypeResolver typeResolver,
                                 IGrainFactory grainFactory) {
      var serializationBinder =
          new OrleansJsonSerializationBinder(typeResolver);
      ...
      settings.Converters.Add(
          new GrainReferenceConverter(grainFactory, serializationBinder));
    ...
    /// Can be used by any provider, allowing the users to use a standard set of
    ...
    public static JsonSerializerSettings
    UpdateSerializerSettings(JsonSerializerSettings settings,
                             IProviderConfiguration config) {
      bool useFullAssemblyNames =
          config.GetBoolProperty(UseFullAssemblyNamesProperty, false);
      bool indentJson = config.GetBoolProperty(IndentJsonProperty, false);
      TypeNameHandling typeNameHandling = config.GetEnumProperty(
          TypeNameHandlingProperty, settings.TypeNameHandling);
      return UpdateSerializerSettings(settings, useFullAssemblyNames,
                                      indentJson, typeNameHandling);
    }

    public static JsonSerializerSettings
    UpdateSerializerSettings(JsonSerializerSettings settings,
                             bool useFullAssemblyNames, bool indentJson,
                             TypeNameHandling? typeNameHandling) {
      if (useFullAssemblyNames) {
        settings.TypeNameAssemblyFormatHandling =
    ...
    /// <inheritdoc />
    public bool IsSupportedType(Type itemType) { return true; }
    ...
    public object DeepCopy(object source, ICopyContext context) {
      ...
      var serializationContext = new SerializationContext(
          context.GetSerializationManager()){StreamWriter = outputWriter};
    ...
    public object Deserialize(Type expectedType,
      ...
      return JsonConvert.DeserializeObject(str, expectedType,
                                           this.settings.Value);
    }
    ...
    /// <param name="expectedType">The type the deserializer should
    /// expect</param>
    public void Serialize(object item, ISerializationContext context,
      ...
      var str =
          JsonConvert.SerializeObject(item, expectedType, this.settings.Value);
      writer.Write(str);
    }
  }
  ...
  public class GrainIdConverter : JsonConverter {
    ...
    public override object ReadJson(JsonReader reader, Type objectType,
      ...
      GrainId grainId =
          LegacyGrainId.FromParsableString(jo["GrainId"].ToObject<string>());
      return grainId;
    }
  }
  ...
  public class SiloAddressConverter : JsonConverter {
    ...
    public override object ReadJson(JsonReader reader, Type objectType,
      ...
      JObject jo = JObject.Load(reader);
      SiloAddress addr =
          SiloAddress.FromParsableString(jo["SiloAddress"].ToObject<string>());
      return addr;
    }
  }
  ...
  public class UniqueKeyConverter : JsonConverter {
    ...
    public override object ReadJson(JsonReader reader, Type objectType,
      ...
      JObject jo = JObject.Load(reader);
      UniqueKey addr =
          UniqueKey.Parse(jo["UniqueKey"].ToObject<string>().AsSpan());
      return addr;
    }
  }
  ...
  public class GrainReferenceConverter : JsonConverter {
    ...
    public GrainReferenceConverter(
        IGrainFactory grainFactory,
        OrleansJsonSerializationBinder serializationBinder) {
      this.grainFactory = grainFactory;

      // Create a serializer for internal serialization which does not have a
      // specified GrainReference serializer. This internal serializer will use
      // GrainReference's ISerializable implementation for serialization and
      // deserialization.
      this.internalSerializer =
          JsonSerializer.Create(new JsonSerializerSettings{
              ...
              Converters = {new IPAddressConverter(), new IPEndPointConverter(),
                            new GrainIdConverter(), new SiloAddressConverter(),
    ...
    public override object ReadJson(JsonReader reader, Type objectType,
                                    object existingValue,
                                    JsonSerializer serializer) {
      // Deserialize using the internal serializer which will use the concrete
      // GrainReference implementation's ISerializable constructor.
