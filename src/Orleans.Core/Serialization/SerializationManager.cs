...
namespace Orleans.Serialization {
  ...
  public sealed class SerializationManager : IDisposable {
    ...
    private readonly Dictionary<KeyedSerializerId, IKeyedSerializer>
        keyedSerializers =
            new Dictionary<KeyedSerializerId, IKeyedSerializer>();
    private readonly List<IKeyedSerializer>orderedKeyedSerializers =
        new List<IKeyedSerializer>();
    private readonly ConcurrentDictionary<Type, IExternalSerializer>
        typeToExternalSerializerDictionary;
    private readonly CachedReadConcurrentDictionary<string, Type>types;
    private readonly Dictionary<string, string>typeKeysToQualifiedNames =
        new Dictionary<string, string>();
    private readonly Dictionary<Type, DeepCopier>copiers;
    private readonly Dictionary<Type, Serializer>serializers;

    private readonly CachedReadConcurrentDictionary<Type, IKeyedSerializer>
        typeToKeyedSerializer =
            new CachedReadConcurrentDictionary<Type, IKeyedSerializer>();
    private readonly Dictionary<Type, Deserializer>deserializers;
    private readonly
        ConcurrentDictionary<Type, Func<GrainReference, GrainReference>>
            grainRefConstructorDictionary;

    private readonly IExternalSerializer fallbackSerializer;
    private readonly ILogger logger;

    // Semi-constants: type handles for simple types
    private static readonly RuntimeTypeHandle shortTypeHandle =
        typeof(short).TypeHandle;
    private static readonly RuntimeTypeHandle intTypeHandle =
        typeof(int).TypeHandle;
    private static readonly RuntimeTypeHandle longTypeHandle =
        typeof(long).TypeHandle;
    private static readonly RuntimeTypeHandle ushortTypeHandle =
        typeof(ushort).TypeHandle;
    private static readonly RuntimeTypeHandle uintTypeHandle =
        typeof(uint).TypeHandle;
    private static readonly RuntimeTypeHandle ulongTypeHandle =
        typeof(ulong).TypeHandle;
    private static readonly RuntimeTypeHandle byteTypeHandle =
        typeof(byte).TypeHandle;
    private static readonly RuntimeTypeHandle sbyteTypeHandle =
        typeof(sbyte).TypeHandle;
    private static readonly RuntimeTypeHandle floatTypeHandle =
        typeof(float).TypeHandle;
    private static readonly RuntimeTypeHandle doubleTypeHandle =
        typeof(double).TypeHandle;
    private static readonly RuntimeTypeHandle charTypeHandle =
        typeof(char).TypeHandle;
    private static readonly RuntimeTypeHandle boolTypeHandle =
        typeof(bool).TypeHandle;
    private static readonly RuntimeTypeHandle objectTypeHandle =
        typeof(object).TypeHandle;
    private static readonly RuntimeTypeHandle byteArrayTypeHandle =
        typeof(byte[]).TypeHandle;
    ...
    internal IRuntimeClient RuntimeClient => this.runtimeClient ??(
        this.runtimeClient = this.serviceProvider.GetService<IRuntimeClient>());
    ...
    public SerializationManager(
        ...
        ILoggerFactory loggerFactory, ITypeResolver typeResolver,
      ...
      typeToExternalSerializerDictionary =
          new ConcurrentDictionary<Type, IExternalSerializer>();
      ...
      fallbackSerializer = GetFallbackSerializer(
          serviceProvider,
          SerializationProviderOptions.FallbackSerializationProvider);

      RegisterSerializationProviders(
          SerializationProviderOptions.SerializationProviders);
    }

    internal SerializationProviderOptions SerializationProviderOptions { get; }

    public void
    RegisterSerializers(IApplicationPartManager applicationPartManager) {
      var serializerFeature =
          applicationPartManager.CreateAndPopulateFeature<SerializerFeature>();
      this.RegisterSerializers(serializerFeature);

      var grainInterfaceFeature =
          applicationPartManager
              .CreateAndPopulateFeature<GrainInterfaceFeature>();
      this.RegisterGrainReferenceSerializers(grainInterfaceFeature);
    }

    private void RegisterGrainReferenceSerializers(
    ...
    private void RegisterSerializers(SerializerFeature serializerFeature) {
      var typeSerializer =
          new BuiltInTypes.DefaultTypeSerializer(this.typeResolver);

      // ReSharper disable once PossibleMistakenCallToGetType.2
      // GetType() returns a RuntimeType, which is an inaccessible subclass of
      // Type which is used at runtime.
      this.Register(typeof (Type).GetType(), typeSerializer.CopyType,
                    typeSerializer.SerializeType,
                    typeSerializer.DeserializeType);
      this.Register(typeof (Type), typeSerializer.CopyType,
                    typeSerializer.SerializeType,
                    typeSerializer.DeserializeType);

      foreach(var serializer in serializerFeature.SerializerDelegates) {
        this.Register(serializer.Target, serializer.Delegates.DeepCopy,
    ...
    /// <summary>
    /// Register a Type with the serialization system to use the specified
    /// DeepCopier, Serializer and Deserializer functions.
    /// </summary>
    ...
    public void Register(Type t, DeepCopier cop, Serializer ser,
                         Deserializer deser) {
    ...
    private object InitializeSerializer(Type serializerType) {
      ...
      // If the type lacks a Serializer attribute then it's a self-serializing
      // type and all serialization methods must be static
      if (!serializerType
               .GetCustomAttributes(typeof (SerializerAttribute), true)
               .Any())
        return null;
      ...
      var serializer = ActivatorUtilities.GetServiceOrCreateInstance(
          this.ServiceProvider, serializerType);
      return serializer;
    }

    /// <summary>
    /// Register a Type with the serialization system to use the specified
    /// DeepCopier, Serializer and Deserializer functions. If <c>forcOverride ==
    /// true</c> then this definition will replace any any previous functions
    ...
    /// <param name="ser">Serializer function for this type.</param>
    /// <param name="deser">Deserializer function for this type.</param>
    /// <param name="forceOverride">Whether these functions should replace any
    /// previously registered functions for this Type.</param>
    private void Register(Type t, DeepCopier cop, Serializer ser,
                          Deserializer deser, bool forceOverride) {
      if ((ser == null) && (deser != null)) {
        throw new OrleansException(
            "Deserializer without serializer for class " + t.OrleansTypeName());
      }
      if ((ser != null) && (deser == null)) {
        throw new OrleansException(
            "Serializer without deserializer for class " + t.OrleansTypeName());
      }
      ...
      lock(registeredTypes) {
        if (registeredTypes.Contains(t)) {
          if (cop != null) {
            lock(copiers) {
              DeepCopier current;
              if (forceOverride || !copiers.TryGetValue(t, out current) ||
                  (current == null)) {
                copiers[t] = cop;
              }
            }
          ...
          if (ser != null) {
            lock(serializers) {
              ...
              if (forceOverride ||
                  !serializers.TryGetValue(t, out currentSer) ||
                  (currentSer == null)) {
                serializers[t] = ser;
              }
            }
            lock(deserializers) {
              ...
              if (forceOverride ||
                  !deserializers.TryGetValue(t, out currentDeser) ||
                  (currentDeser == null)) {
          ...
          types[name] = t;
          if (cop != null) {
            lock(copiers) { copiers[t] = cop; }
          }
          if (ser != null) {
            lock(serializers) { serializers[t] = ser; }
          }
          if (deser != null) {
            lock(deserializers) { deserializers[t] = deser; }
          }

          if (logger.IsEnabled(LogLevel.Trace))
            logger.Trace("Registered type {0} as {1}", t, name);
        }
      }

      // Register any interfaces this type implements, in order to support
      // passing values that are statically of the interface type but
      // dynamically of this (implementation) type
      foreach(var iface in t.GetInterfaces()) { Register(iface); }
    ...
    /// <summary>
    /// This method registers a type that has no specific serializer or
    /// deserializer. For instance, abstract base types and interfaces need to
    /// be registered this way.
    /// </summary>
    /// <param name="t">Type to be registered.</param>
    ...
    /// <summary>
    /// Registers <paramref name="serializerType"/> as the serializer for
    /// <paramref name="type"/>.
    /// </summary>
    /// <param name="type">The type serialized by the provided serializer
    /// type.</param> <param name="serializerType">The type containing
    /// serialization methods for <paramref name="type"/>.</param> <param
    /// name="overrideExisting">Whether or not to override existing
    /// registrations for the provided <paramref name="type"/>.</param>
    private void Register(Type type, Type serializerType,
                          bool overrideExisting = true) {
      GetSerializationMethods(serializerType, out var copier,
                              out var serializer, out var deserializer);

      if (serializer == null && deserializer == null && copier == null) {
        var msg =
            $"No serialization methods found on type {serializerType.GetParseableName(TypeFormattingOptions.LogFormat)}.";
        logger.Warn(ErrorCode.SerMgr_SerializationMethodsMissing, msg);
      ...
      if (serializer != null ^ deserializer != null) {
        var msg =
            ...
            $"Instead, serializer = {serializer?.ToString() ?? " null
            "} and deserializer = {deserializer?.ToString() ?? " null "}";
        logger.Warn(ErrorCode.SerMgr_SerializationMethodsMissing, msg);
        throw new ArgumentException(msg);
      }
      ...
      try {
        if (type.IsGenericTypeDefinition) {
          object DeepCopyGeneric(object obj, ICopyContext context) {
            var concrete =
                this.RegisterConcreteSerializer(obj.GetType(), serializerType);
            return concrete.DeepCopy(obj, context);
          }

          void SerializeGeneric(object obj, ISerializationContext context,
                                Type exp) {
            var concrete =
                this.RegisterConcreteSerializer(obj.GetType(), serializerType);
          ...
          object DeserializeGeneric(Type expected,
                                    IDeserializationContext context) {
            var concrete =
                this.RegisterConcreteSerializer(expected, serializerType);
            return concrete.Deserialize(expected, context);
          }

          this.Register(
              type, copier != null ? DeepCopyGeneric
              : default(DeepCopier), serializer != null ? SerializeGeneric
              : default(Serializer), deserializer != null ? DeserializeGeneric
              : default(Deserializer), overrideExisting);
        } else {
          var serializerInstance = this.InitializeSerializer(serializerType);
          this.Register(
              type, CreateDelegate<DeepCopier>(copier, serializerInstance),
              CreateDelegate<Serializer>(serializer, serializerInstance),
              CreateDelegate<Deserializer>(deserializer, serializerInstance),
              overrideExisting);
        }
      } catch (ArgumentException) {
        logger.Warn(ErrorCode.SerMgr_ErrorBindingMethods,
      ...
      if (this.logger.IsEnabled(LogLevel.Trace))
        this.logger.Trace(
            ...
            type.Name, serializerType.Assembly.GetName().Name);
    }

    /// <summary>
    /// Registers <see cref="GrainReference"/> serializers for the provided
    /// <paramref name="type"/>.
    /// </summary>
    /// <param name="type">
    /// The type.
    ...
    private void RegisterGrainReferenceSerializers(Type type) {
      ...
      Register(type, GrainReferenceSerializer.CopyGrainReference,
               GrainReferenceSerializer.SerializeGrainReference,
               (expected, context) => {
                 Func<GrainReference, GrainReference>ctorDelegate;
                 var deserialized =
                     (GrainReference)
                         GrainReferenceSerializer.DeserializeGrainReference(
                             expected, context);
                 if (expected.IsConstructedGenericType == false) {
                   return defaultCtorDelegate(deserialized);
                 }

                 if (!grainRefConstructorDictionary.TryGetValue(
                         expected, out ctorDelegate)) {
                   ctorDelegate = CreateGrainRefConstructorDelegate(
                       type, expected.GenericTypeArguments);
    ...
    private static Func<GrainReference, GrainReference>
    CreateGrainRefConstructorDelegate(Type type, Type[] genericArgs) {
      ...
      var constructor = TypeUtils.GetConstructorThatMatches(
          type, new[]{typeof (GrainReference)});
      var method =
          new DynamicMethod(".ctor_" + type.Name, typeof (GrainReference),
                            new[]{typeof (GrainReference)},
                            typeof (SerializationManager).Module, true);
      var il = method.GetILGenerator();
      il.Emit(OpCodes.Ldarg_0);
      il.Emit(OpCodes.Newobj, constructor);
      il.Emit(OpCodes.Ret);
      return (Func<GrainReference, GrainReference>)
          method.CreateDelegate(typeof (Func<GrainReference, GrainReference>));
    }

    private SerializerMethods
    RegisterConcreteSerializer(Type concreteType, Type genericSerializerType) {
      MethodInfo copier;
      MethodInfo serializer;
      MethodInfo deserializer;

      var concreteSerializerType = genericSerializerType.MakeGenericType(
          concreteType.GetGenericArguments());
      var typeAlreadyRegistered = false;

      lock(registeredTypes) {
        typeAlreadyRegistered =
            registeredTypes.Contains(concreteSerializerType);
      }

      if (typeAlreadyRegistered) {
        return new SerializerMethods(GetCopier(concreteSerializerType),
                                     GetSerializer(concreteSerializerType),
                                     GetDeserializer(concreteSerializerType));
      }

      GetSerializationMethods(concreteSerializerType, out copier,
                              out serializer, out deserializer);
      var serializerInstance =
          this.InitializeSerializer(concreteSerializerType);
      var concreteCopier =
      ...
      this.Register(concreteType, concreteCopier, concreteSerializer,
                    concreteDeserializer, true);

      return new SerializerMethods(concreteCopier, concreteSerializer,
                                   concreteDeserializer);
    }

    internal static void GetSerializationMethods(Type type,
                                                 out MethodInfo copier,
                                                 out MethodInfo serializer,
                                                 out MethodInfo deserializer) {
      copier = null;
      serializer = null;
      deserializer = null;
      foreach(var method in type.GetMethods(
        ...
        if (method.GetCustomAttributes(typeof (CopierMethodAttribute), true)
                .Any()) {
          copier = method;
        } else if (method
                       .GetCustomAttributes(typeof (SerializerMethodAttribute),
                                            true)
                       .Any()) {
          serializer = method;
        } else if (method
                       .GetCustomAttributes(
                           typeof (DeserializerMethodAttribute), true)
                       .Any()) {
          deserializer = method;
        }
      }
    ...
    private T CreateDelegate<T>(MethodInfo methodInfo, object target) where T
        : class {
      if(this.ServiceProvider == null)
        return default(T);
    ...
    internal DeepCopier GetCopier(Type t) {
      lock(copiers) {
        ...
        if (t.IsGenericType &&
            this.copiers.TryGetValue(t.GetGenericTypeDefinition(), out copier))
          return copier;
      }
    ...
    /// <summary>
    /// Deep copy the specified object, using DeepCopier functions previously
    /// registered for this type.
    /// </summary>
    ...
    public object DeepCopy(object original) {
      ...
      if (timer != null) {
        timer.Stop();
        context.SerializationManager.serializationStatistics.CopyTimeStatistic
            .IncrementBy(timer.ElapsedTicks);
      }
    ...
    internal void DeepCopyElementsInPlace(object[] args) {
      ...
      for (var i = 0; i < args.Length; i++)
        args[i] = DeepCopyInner(args[i], context);
    ...
    private object DeepCopierHelper(Type t, object original,
      ...
      if (originalArray != null) {
        ...
        if (t.TypeHandle.Equals(byteArrayTypeHandle) &&
            (originalArray.Rank == 1)) {
          var source = (byte[]) original;
          if (source.Length > this.LargeObjectSizeThreshold) {
            logger.Info(
                ErrorCode.Ser_LargeObjectAllocated,
                "Large byte array of size {0} is being copied. This will result in an allocation on the large object heap. " +
                    ...
                    "Please consider using Immutable<byte[]> instead.",
                source.Length);
          }
        ...
        if (et.IsOrleansShallowCopyable()) {
          // Only check the size for primitive types because otherwise
          // Buffer.ByteLength throws
          if (et.IsPrimitive && Buffer.ByteLength(originalArray) >
                                    this.LargeObjectSizeThreshold) {
            logger.Info(
                ErrorCode.Ser_LargeObjectAllocated,
        ...
        // We assume that all arrays have lower bound 0. In .NET 4.0, it's hard
        // to create an array with a non-zero lower bound.
        ...
        if (rank == 1) {
          for (var i = 0; i < lengths[0]; i++)
            copyArray.SetValue(
                DeepCopyInner(originalArray.GetValue(i), context), i);
        } else if (rank == 2) {
          for (var i = 0; i < lengths[0]; i++)
            for (var j = 0; j < lengths[1]; j++)
              copyArray.SetValue(
                  DeepCopyInner(originalArray.GetValue(i, j), context), i, j);
        } else {
          ...
          for (var i = 0; i < originalArray.Length; i++) {
            ...
            copyArray.SetValue(
                DeepCopyInner(originalArray.GetValue(index), context), index);
          }
        }
        return copyArray;
      }
      ...
      throw new OrleansException(
          "No copier found for object of type " + t.OrleansTypeName() +
    ...
    /// <returns>true if <paramref name="t"/> is serializable, false
    /// otherwise.</returns>
    ...
    internal Serializer GetSerializer(Type t) {
      lock(serializers) {
        ...
        if (t.IsGenericType)
          if (this.serializers.TryGetValue(t.GetGenericTypeDefinition(),
                                           out ser))
            return ser;
      }
    ...
    /// <summary>
    /// Serialize the specified object, using Serializer functions previously
    /// registered for this type.
    /// </summary>
    ...
    public void Serialize(object raw, IBinaryTokenStreamWriter stream) {
      ...
      if (timer != null) {
        timer.Stop();
        serializationStatistics.SerTimeStatistic.IncrementBy(
            timer.ElapsedTicks);
      }
    }
    ...
    public static void SerializeInner<T>(T obj,
                                         ISerializationContext context) =>
        SerializeInner(obj, context, typeof(T));
    ...
    public static void
    SerializeInner(object obj, ISerializationContext context, Type expected) {
      var sm = context.GetSerializationManager();
    ...
    SerializeInner<TObject, TContext, TWriter>(SerializationManager sm,
                                               TObject obj, Type expected,
                                               TContext context,
                                               TWriter writer) where TContext
        : ISerializationContext where TWriter : IBinaryTokenStreamWriter {
      ...
      if (writer.TryWriteSimpleObject(obj))
        return;

      // Check for primitives
      // At this point, we're either an object or a non-trivial value type

      // Start by checking to see if we're a back-reference, and recording us
      // for possible future back-references if not
      if (!t.IsValueType) {
      ...
      // If we're simply a plain old unadorned, undifferentiated object, life is
      // easy
      ...
      if (obj is Exception && !sm.fallbackSerializer.IsSupportedType(t)) {
        // Exceptions should always be serializable, and thus handled by the
        // prior if. In case someone creates a non-serializable exception,
        // though, we don't want to throw and return a serialization
        // exception... Note that the "!t.IsSerializable" is redundant in this
        // if, but it's there in case this code block moves.
        var rawException = obj as Exception;
        var foo = new Exception(
            String.Format("Non-serializable exception of type {0}: {1}" +
                              Environment.NewLine + "at {2}",
      ...
      throw new ArgumentException(
          "No serializer found for object of type " + t.OrleansTypeKeyString() +
          " from assembly " + t.Assembly.FullName +
          ". Perhaps you need to mark it [Serializable] or define a custom serializer for it?");
    }

    private static void WriteEnum(object obj, IBinaryTokenStreamWriter stream,
                                  Type type) {
      var t = Enum.GetUnderlyingType(type).TypeHandle;
      if (t.Equals(byteTypeHandle) || t.Equals(sbyteTypeHandle))
        stream.Write(Convert.ToByte(obj));
      else if (t.Equals(shortTypeHandle) || t.Equals(ushortTypeHandle))
        stream.Write(Convert.ToInt16(obj));
      else if (t.Equals(intTypeHandle) || t.Equals(uintTypeHandle))
        stream.Write(Convert.ToInt32(obj));
      else if (t.Equals(longTypeHandle) || t.Equals(ulongTypeHandle))
        stream.Write(Convert.ToInt64(obj));
      else
        throw new NotSupportedException(
            $"Serialization of type {type.GetParseableName()} is not supported.");
    }
    ...
    private static object ReadEnum(IBinaryTokenStreamReader stream, Type type) {
      ...
      if (t.Equals(shortTypeHandle) || t.Equals(ushortTypeHandle))
        return Enum.ToObject(type, stream.ReadShort());
      if (t.Equals(intTypeHandle) || t.Equals(uintTypeHandle))
        return Enum.ToObject(type, stream.ReadInt());
      if (t.Equals(longTypeHandle) || t.Equals(ulongTypeHandle))
        return Enum.ToObject(type, stream.ReadLong());
      throw new NotSupportedException(
          $"Deserialization of type {type.GetParseableName()} is not supported.");
    }

    // We assume that all lower bounds are 0, since creating an array with lower
    // bound !=0 is hard in .NET 4.0+
    private static void SerializeArray(Array array,
                                       ISerializationContext context,
                                       Type expected, Type et) {
      var writer = context.StreamWriter;
    ...
    /// <returns>Object of the required Type, rehydrated from the input
    /// stream.</returns>
    ...
    public object Deserialize(Type t, IBinaryTokenStreamReader stream) {
      ...
      if (this.serializationStatistics.CollectSerializationStats) {
        timer = new Stopwatch();
        timer.Start();
        context.SerializationManager.serializationStatistics.Deserializations
            .Increment();
      }
      object result = null;
      ...
      if (timer != null) {
        timer.Stop();
        context.SerializationManager.serializationStatistics.DeserTimeStatistic
            .IncrementBy(timer.ElapsedTicks);
      }
      return result;
    }
    ...
    /// <returns>Object of the required Type, rehydrated from the input
    /// stream.</returns>
    public static object DeserializeInner(Type expected,
                                          IDeserializationContext context) {
      var sm = context.GetSerializationManager();
    ...
    DeserializeInner<TContext, TReader>(SerializationManager sm, Type expected,
                                        TContext context,
                                        TReader reader) where TContext
        : IDeserializationContext where TReader : IBinaryTokenStreamReader {
      var previousOffset = context.CurrentObjectOffset;
      context.CurrentObjectOffset = context.CurrentPosition;

      try {
        // NOTE: we don't check that the actual dynamic result implements the
        // expected type. We'll allow a cast exception higher up to catch this.
        ...
        if (token == SerializationTokenType.ExpectedType) {
          if (expected == null) {
            throw new SerializationException(
                "ExpectedType token encountered but no expected type provided");
          }
          ...
          if (!sm.keyedSerializers.TryGetValue(serializerId,
                                               out var keyedSerializer)) {
            throw new SerializationException(
                $"Specified serializer {serializerId} not configured.");
          }

          return keyedSerializer.Deserialize(expected, context);
        } else {
          throw new SerializationException("Unexpected token '" + token +
                                           "' introducing type specifier");
        }
    ...
    private static object DeserializeArray(Type resultType,
                                           IDeserializationContext context) {
      var reader = context.StreamReader;
    ...
    private static int[] ReadArrayLengths(int n,
                                          IBinaryTokenStreamReader stream) {
    ...
    internal Deserializer GetDeserializer(Type t) {
      ...
      if (t.IsGenericType) {
        lock(deserializers) {
          if (deserializers.TryGetValue(t.GetGenericTypeDefinition(),
                                        out deser))
            return deser;
        }
      }
    ...
    /// <summary>
    /// Deserialize data from the specified byte[] and rehydrate back into
    /// objects.
    /// </summary>
    ...
    internal static void
    SerializeMessageHeaders(Message.HeadersContainer headers,
      ...
      if (timer != null) {
        ...
        sm.serializationStatistics.HeaderSerTime.IncrementBy(
            timer.ElapsedTicks);
      }
    }

    internal static Message.HeadersContainer
    DeserializeMessageHeaders(IDeserializationContext context) {
      var sm = context.GetSerializationManager();
      Stopwatch timer = null;
      if (sm.serializationStatistics.CollectSerializationStats) {
        timer = new Stopwatch();
      ...
      var headers = (Message.HeadersContainer)
          des(typeof (Message.HeadersContainer), context);

      if (timer != null) {
        timer.Stop();
        sm.serializationStatistics.HeaderDesers.Increment();
        sm.serializationStatistics.HeaderDeserTime.IncrementBy(
            timer.ElapsedTicks);
      }
    ...
    private bool
    TryLookupExternalSerializer(Type t, out IExternalSerializer serializer) {
      ...
      // the associated serializer will be null if there are no external
      // serializers that handle this type
      ...
      // add the serializer to the dictionary, even if it's null to signify that
      // we already performed the search and found none
      if (typeToExternalSerializerDictionary.TryAdd(t, serializer) &&
          serializer != null) {
        // we need to register the type, otherwise exceptions are thrown about
        // types not being found
        Register(t, serializer.DeepCopy, serializer.Serialize,
    ...
    private bool TryLookupKeyedSerializer(Type type,
                                          out IKeyedSerializer serializer) {
      if (this.orderedKeyedSerializers.Count == 0) {
        serializer = null;
        return false;
      }

      if (this.typeToKeyedSerializer.TryGetValue(type, out serializer))
        return true;
    ...
    internal void FallbackSerializer(object raw, ISerializationContext context,
                                     Type t) {
      Stopwatch timer = null;
      if (this.serializationStatistics.CollectSerializationStats) {
        timer = new Stopwatch();
      ...
      if (this.serializationStatistics.CollectSerializationStats) {
        timer.Stop();
        serializationStatistics.FallbackSerTimeStatistic.IncrementBy(
            timer.ElapsedTicks);
      }
    }

    private object FallbackDeserializer(IDeserializationContext context,
                                        Type expectedType) {
      Stopwatch timer = null;
      if (this.serializationStatistics.CollectSerializationStats) {
        timer = new Stopwatch();
      ...
      if (timer != null) {
        timer.Stop();
        serializationStatistics.FallbackDeserTimeStatistic.IncrementBy(
            timer.ElapsedTicks);
      }
    ...
    private IExternalSerializer
    GetFallbackSerializer(IServiceProvider serviceProvider, Type fallbackType) {
      IExternalSerializer serializer;
      if (fallbackType != null) {
        serializer = (IExternalSerializer) ActivatorUtilities.CreateInstance(
            serviceProvider, fallbackType);
      } else {
    ...
    private object FallbackSerializationDeepCopy(object obj,
                                                 ICopyContext context) {
      Stopwatch timer = null;
      if (this.serializationStatistics.CollectSerializationStats) {
        timer = new Stopwatch();
      ...
      if (this.serializationStatistics.CollectSerializationStats) {
        timer.Stop();
        serializationStatistics.FallbackCopiesTimeStatistic.IncrementBy(
            timer.ElapsedTicks);
      }
      return retVal;
    }
    ...
    internal Type ResolveTypeName(string typeName) {
      ...
      // Attempt to find the qualified type name from the key and resolve the
      // type from the name.
      if (this.typeKeysToQualifiedNames.TryGetValue(
              typeName, out var fullyQualifiedName) &&
          this.typeResolver.TryResolveType(fullyQualifiedName, out result)) {
      ...
      if (typeName[typeName.Length - 1] == ']') {
        ...
        // The rank of the array will be the length of the string, minus the
        // index of the [, minus 1; it's the number of commas between the [ and
        // the ]
        var rank = typeName.Length - j - 1;
        var baseName = typeName.Substring(0, j);
        var baseType = ResolveTypeName(baseName);
        return rank == 1 ? baseType.MakeArrayType()
                         : baseType.MakeArrayType(rank);
      }
      ...
      throw new TypeAccessException("Type string \"" + typeName +
                                    "\" cannot be resolved.");
    }
    ...
    public void LogRegisteredTypes() {
      ...
      var report = String.Format(
          "Registered artifacts for {0} types:" + Environment.NewLine + "{1}",
          count, lines);
    ...
    /// </summary>
    /// <param name="providerTypes">The list of types that implement <see
    /// cref="IExternalSerializer"/></param>
    private void RegisterSerializationProviders(List<Type>providerTypes) {
      ...
      providerTypes.ForEach(type => {
        try {
          var serializer = ActivatorUtilities.CreateInstance(
              serviceProvider, type) as IExternalSerializer;
          externalSerializers.Add(serializer);
        } catch (Exception exception) {
          logger.Error(ErrorCode.SerMgr_ErrorLoadingAssemblyTypes,
      ...
      if (this.ServiceProvider != null) {
        foreach(var serializer
                    in this.ServiceProvider.GetServices<IKeyedSerializer>()) {
