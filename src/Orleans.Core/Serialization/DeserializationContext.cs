...
namespace Orleans.Serialization {
  public static class DeserializationContextExtensions {
    ...
    /// <returns></returns>
    public static IDeserializationContext
    CreateNestedContext(this IDeserializationContext context, int position,
                        BinaryTokenStreamReader reader) {
      return new DeserializationContext.NestedDeserializationContext(
          context, position, reader);
    }
  }

  public sealed class DeserializationContext : SerializationContextBase,
    ...
    /// <inheritdoc />
    public IBinaryTokenStreamReader StreamReader {
      get;
      set;
    }

    /// <inheritdoc />
    public int CurrentObjectOffset {
      get;
      set;
    }
    ...
    internal void Reset() {
      if (this.taggedObjects is null ||
          this.taggedObjects.Count >
    ...
    internal class NestedDeserializationContext : IDeserializationContext {
      ...
      /// <param name="position">The position, relative to the outer-most
      /// context, at which this context begins.</param> <param
      /// name="reader"></param>
      public NestedDeserializationContext(IDeserializationContext parent,
                                          int position,
                                          BinaryTokenStreamReader reader) {
      ...
      public int CurrentPosition =>
          this.position + this.StreamReader.CurrentPosition;
      public void RecordObject(object obj,
                               int offset) => this.parent.RecordObject(obj,
                                                                       offset);
      public void RecordObject(object obj) =>
          this.RecordObject(obj, this.CurrentObjectOffset);
      public object FetchReferencedObject(int offset) =>
          this.parent.FetchReferencedObject(offset);
      public object DeserializeInner(Type expected) =>
          SerializationManager.DeserializeInner(expected, this);
    }
  }
}
