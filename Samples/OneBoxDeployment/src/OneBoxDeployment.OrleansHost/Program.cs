...
namespace OneBoxDeployment.OrleansHost {
  ...
  public static class Program {
    private static AutoResetEvent ProceedWithClosing =
    ...
    public static ISiloHost BuildOrleansHost(string[] args,
      ...
      var configuration =
          new ConfigurationBuilder()
              .AddJsonFile("appsettings.orleanshost.json", optional
                           : true, reloadOnChange
                           : true)
              .AddJsonFile($"appsettings.orleanshost.{environmentName}.json",
                           optional
      ...
      // If cluster configuration is null, the application is likely started
      // from the command line, so try read one from a JSON configuration file.
      // Also, test sets args == null.
      if (clusterConfig == null) {
        clusterConfig =
            configuration.GetSection("ClusterConfig").Get<ClusterConfig>();
      }
      ...
      var siloBuilder =
          new SiloHostBuilder()
              ...
              .ConfigureApplicationParts(
                  parts =>
                      parts.AddApplicationPart(typeof (TestStateGrain).Assembly)
                          .WithReferences())
              .UseAdoNetReminderService(options => {
                options.Invariant =
                    clusterConfig.ReminderConfigs[0].AdoNetConstant;
                options.ConnectionString =
                    clusterConfig.ReminderConfigs[0].ConnectionString;
              })
              .AddAdoNetGrainStorage(
                  clusterConfig.StorageConfigs[0].Name, options => {
                    options.Invariant =
                        clusterConfig.StorageConfigs[0].AdoNetConstant;
                    options.ConnectionString =
                        clusterConfig.StorageConfigs[0].ConnectionString;
                  });
