...
namespace OneBoxDeployment.IntegrationTests.Tests {
  ...
  /// </summary>
  public sealed class TestStateGrainTests
      : IAssemblyFixture<IntegrationTestFixture> {
    /// <summary>
    ...
    /// <summary>
    /// Increments the target state of two grains by a random integer number and
    /// checks the result.
    /// </summary>
    [Fact]
    public async Task
    IncrementTwoRandomStatesByRandomNumber() {
      var rand = new Random();

      // First get the base situation for later comparison. It's done with a
      // random grain ID to reduce risk to inadvert data dependencies.
      var grainId1 = rand.Next(0, 10_000);
      var grainId2 = rand.Next(10_001, 20_000);
      var grain1InitialResponse =
          await TestStateClient.IncrementByAsync(grainId1, 0)
              .ConfigureAwait(false);
      var grain2InitialResponse =
          await TestStateClient.IncrementByAsync(grainId2, 0)
              .ConfigureAwait(false);
      ...
      var grain1IncrementedResponse =
          await TestStateClient.IncrementByAsync(grainId1, grainIncrement1)
              .ConfigureAwait(false);
      var grain2IncrementedResponse =
          await TestStateClient.IncrementByAsync(grainId2, grainIncrement2)
              .ConfigureAwait(false);
      ...
      var initialState1 =
          await grain1InitialResponse.Content.DeserializeAsync<int>()
              .ConfigureAwait(false);
      var initialState2 =
          await grain2InitialResponse.Content.DeserializeAsync<int>()
              .ConfigureAwait(false);
      var incrementedState1 =
          await grain1IncrementedResponse.Content.DeserializeAsync<int>()
              .ConfigureAwait(false);
      var incrementedState2 =
          await grain2IncrementedResponse.Content.DeserializeAsync<int>()
              .ConfigureAwait(false);
      Assert.Equal(initialState1 + grainIncrement1, incrementedState1);
      Assert.Equal(initialState2 + grainIncrement2, incrementedState2);

      // There were four calls to the API and it should log four events of this
      // kind.
