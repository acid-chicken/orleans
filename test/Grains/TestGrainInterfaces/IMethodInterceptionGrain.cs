...
namespace UnitTests.GrainInterfaces {
  ...
  public interface IMethodInterceptionGrain : IGrainWithIntegerKey,
                                              IMethodFromAnotherInterface {
    [MethodId(14142)] Task<string>One();
    [MethodId(-14142)] Task<string>Echo(string someArg);
  ...
  public interface IOutgoingMethodInterceptionGrain : IGrainWithIntegerKey {
    Task<Dictionary<string, object>>EchoViaOtherGrain(
        IMethodInterceptionGrain otherGrain, string message);
    Task<string>ThrowIfGreaterThanZero(int value);
  }

  public interface IGenericMethodInterceptionGrain<in T>
      : IGrainWithIntegerKey, IMethodFromAnotherInterface {
    Task<string>GetInputAsString(T input);
  }

  public interface IMethodFromAnotherInterface { Task<string>SayHello(); }

  public interface ITrickyMethodInterceptionGrain
      : IGenericMethodInterceptionGrain<string>,
        IGenericMethodInterceptionGrain<bool> {
  ...
  public interface IHungryGrain<T>: IGrainWithIntegerKey {
    [TestMethodTag("hungry-eat")] Task Eat(T food);

    [TestMethodTag("hungry-eatwith")] Task EatWith<U>(T food, U condiment);
  }

  public interface IOmnivoreGrain : IGrainWithIntegerKey {
    [TestMethodTag("omnivore-eat")] Task Eat<T>(T food);
  }
  ...
  public interface ICaterpillarGrain : IHungryGrain<Apple>, IOmnivoreGrain {
    [TestMethodTag("caterpillar-eat")] new Task Eat<T>(T food);
  }
