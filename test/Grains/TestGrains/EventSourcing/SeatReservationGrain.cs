...
///
/// This means we persist the latest state only ... we are not truly "event
/// sourcing". It is not necessary here to persist all events, as the state
/// already stores all the successful reservations.
///
/// </summary>

[StorageProvider(ProviderName = "Default")]
[LogConsistencyProvider(ProviderName = "StateStorage")]
public class SeatReservationGrain
    : JournaledGrain<ReservationState, SeatReservation>,
      ISeatReservationGrain{

          public async Task<bool>Reserve(int seatnumber, string userid){
              // first, enqueue the request
              RaiseEvent(
                  new SeatReservation(){Seat = seatnumber, UserId = userid});
...
public class ReservationState {
  public Dictionary<int, SeatReservation>Reservations {
    get;
    set;
  }
