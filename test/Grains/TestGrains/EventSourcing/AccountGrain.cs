...
public class AccountGrain
    : JournaledGrain<AccountGrain.GrainState, Transaction>,
      IAccountGrain{/// <summary>
                    /// The state of this grain is just the current balance.
                    /// </summary>
                    [Serializable]
                    public class GrainState{public uint Balance{get;
set;
}
...
public Task<uint>Balance() { return Task.FromResult(State.Balance); }

public Task Deposit(uint amount, Guid guid, string description) {
  RaiseEvent(new DepositTransaction(){Guid = guid, IssueTime = DateTime.UtcNow,
...
public Task<bool>Withdraw(uint amount, Guid guid, string description) {
  ...
  // (conditional events commit only if the version hasn't already changed in
  // the meantime) this is important so we can guarantee that we never overdraw
  // even if racing with other clusters, of in transient duplicate grain
  // situations
  return RaiseConditionalEvent(new WithdrawalTransaction(){
      Guid = guid, IssueTime = DateTime.UtcNow, WithdrawalAmount = amount,
      Description = description});
}
...
/// A variant of the same grain that does not persist the log, but only the
/// latest grain state (so it does not do true event sourcing).
