...
/// <summary>
/// A class used by many different unit tests for the various log consistency
/// providers. The content of this class is pretty arbitrary and messy; (don't
/// use this as an introduction on how to use JournaledGrain) it started from
/// SimpleGrain, but a lot of stuff got added over time
/// </summary>
[Serializable]
public class MyGrainState {
  ...
  public MyGrainState() { Reservations = new Dictionary<string, int>(); }
  ...
  public void Apply(AddReservation x) {
    Reservations[x.Val.ToString()] = x.Val;
  }
  public void Apply(RemoveReservation x) {
    Reservations.Remove(x.Val.ToString());
  }
}
...
/// </summary>
public abstract class LogTestGrain : JournaledGrain<MyGrainState, object>,
  ...
  public Task<int>GetALocal() { return Task.FromResult(TentativeState.A); }
  ...
  public Task<AB>GetBothLocal() {
    return Task.FromResult(
        new AB(){A = TentativeState.A, B = TentativeState.B});
  }
  ...
  public Task SynchronizeGlobalState() { return RefreshNow(); }
  ...
  public async Task<bool>Update(IReadOnlyList<object>updates,
                                int expectedversion) {
