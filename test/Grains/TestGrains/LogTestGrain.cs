...
public class MyGrainState
{
    ...
    public void Apply(UpdateA x) {
        A = x.Val;
    }
    public void Apply(UpdateB x) {
        B = x.Val;
    }
    public void Apply(IncrementA x) {
        A++;
    }
...
public abstract class LogTestGrain : JournaledGrain<MyGrainState,object>, UnitTests.GrainInterfaces.ILogTestGrain
{
    ...
    public async Task<Tuple<int, bool>> SetAConditional(int x)
    {
        ...
        bool success = await RaiseConditionalEvent(new UpdateA() {
            Val = x
        });
    ...
    {
        return Task.FromResult(new AB() {
            A = TentativeState.A, B = TentativeState.B
        });
    }

    public Task AddReservationLocal(int val)
    {
        RaiseEvent(new AddReservation() {
            Val = val
        });
        return Task.CompletedTask;
    ...
    {
        RaiseEvent(new RemoveReservation() {
            Val = val
        });
        return Task.CompletedTask;
