...
namespace UnitTests.Grains {
  public class OutgoingMethodInterceptionGrain
      : Grain,
        IOutgoingMethodInterceptionGrain {
    public async Task<Dictionary<string, object>>
    EchoViaOtherGrain(IMethodInterceptionGrain otherGrain, string message) {
      return new Dictionary<string, object>{["result"] =
                                                await otherGrain.Echo(message)};
    }
  ...
  public class MethodInterceptionGrain : Grain,
                                         IMethodInterceptionGrain,
                                         IIncomingGrainCallFilter {
    public Task<string>One() {
      throw new InvalidOperationException(
          "Not allowed to actually invoke this method!");
    }

    [MessWithResult]
    public Task<string>
    Echo(string someArg) => Task.FromResult(someArg);
    ...
    async Task
        IIncomingGrainCallFilter.Invoke(IIncomingGrainCallContext context) {
      var methodInfo = context.ImplementationMethod;
      if (methodInfo.Name == nameof(One) &&
          methodInfo.GetParameters().Length == 0) {
        // Short-circuit the request and return to the caller without actually
        // invoking the grain method.
        context.Result = "intercepted one with no args";
        return;
      }

      if (methodInfo.Name == nameof(IncorrectResultType)) {
        // This method has a string return type, but we are setting the result
        // to a Guid. This should result in an invalid cast exception.
      ...
      // To prove that the MethodInfo is from the implementation and not the
      // interface, we check for this attribute which is only present on the
      // implementation. This could be done in a simpler fashion, but this
      // demonstrates a potential usage scenario.
      var shouldMessWithResult =
          methodInfo.GetCustomAttribute<MessWithResultAttribute>();
    ...
    [Serializable]
    public class MyDomainSpecificException
        : Exception{
          public MyDomainSpecificException(){}

          public MyDomainSpecificException(string message) : base(message){}

          protected MyDomainSpecificException(SerializationInfo info,
  ...
  public class GenericMethodInterceptionGrain<T>
      : Grain, IGenericMethodInterceptionGrain<T>, IIncomingGrainCallFilter {
    public Task<string>SayHello() => Task.FromResult("Hello");

    public Task<string>
    GetInputAsString(T input) => Task.FromResult(input.ToString());
    public async Task Invoke(IIncomingGrainCallContext context) {
      if (context.ImplementationMethod.Name == nameof(GetInputAsString)) {
        context.Result =
            $"Hah! You wanted {context.Arguments[0]}, but you got me!";
        return;
      }
  ...
  public class TrickyInterceptionGrain : Grain,
                                         ITrickyMethodInterceptionGrain,
                                         IIncomingGrainCallFilter {
    public Task<string>SayHello() => Task.FromResult("Hello");

    public Task<string>GetInputAsString(string input) => Task.FromResult(input);

    public Task<string>GetInputAsString(bool input) =>
        Task.FromResult(input.ToString(CultureInfo.InvariantCulture));
  ...
  public class GrainCallFilterTestGrain : Grain,
    ...
    public Task<string>GetRequestContext() =>
        Task.FromResult((string) RequestContext.Get(Key) + "4");
    ...
    public async Task Invoke(IIncomingGrainCallContext ctx) {
      ...
      while (attemptsRemaining > 0) {
        try {
          ...
          if (string.Equals(ctx.ImplementationMethod?.Name,
                            nameof(ThrowIfGreaterThanZero)) &&
  ...
  public class CaterpillarGrain : Grain,
                                  ICaterpillarGrain,
                                  IIncomingGrainCallFilter {
    Task IIncomingGrainCallFilter.Invoke(IIncomingGrainCallContext ctx) {
      ...
      if (RequestContext.Get("tag") is string tag) {
        var ifaceTag =
            ctx.InterfaceMethod.GetCustomAttribute<TestMethodTagAttribute>()
            ?.Tag;
        var implTag = ctx.ImplementationMethod
                          .GetCustomAttribute<TestMethodTagAttribute>()
            ?.Tag;
        if (!string.Equals(tag, ifaceTag, StringComparison.Ordinal) ||
            !string.Equals(tag, implTag, StringComparison.Ordinal)) {
          throw new Exception(
              $"Expected method tags to be equal to request context tag: RequestContext: {tag} Interface: {ifaceTag} Implementation: {implTag}");
        }
      }
    ...
    public Task
    Eat(Apple food) => Task.CompletedTask;

    [TestMethodTag("omnivore-eat")] Task
        IOmnivoreGrain.Eat<T>(T food) => Task.CompletedTask;

    [TestMethodTag("caterpillar-eat")]
    public Task
    Eat<T>(T food) => Task.CompletedTask;

    [TestMethodTag("hungry-eatwith")]
    public Task
    EatWith<U>(Apple food, U condiment) => Task.CompletedTask;
  }
}
