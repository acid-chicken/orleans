...
namespace UnitTests.Grains {
  ...
  public class ReminderTestGrain2 : Grain, IReminderTestGrain2, IRemindable {
    ...
    private static long aCCURACY =
        50 *
        TimeSpan.TicksPerMillisecond; // when we use ticks to compute sequence
                                      // numbers, we might get wrong results as
                                      // timeouts don't happen with precision of
                                      // ticks  ... we keep this as a leeway

    private ILogger logger;
    private string myId; // used to distinguish during debugging between
                         // multiple activations of the same grain
    ...
    public ReminderTestGrain2(IServiceProvider services,
      ...
      this.unvalidatedReminderRegistry =
          new UnvalidatedReminderRegistry(services);
    ...
    public async Task<IGrainReminder>StartReminder(string reminderName,
                                                   TimeSpan? p = null,
                                                   bool validate = false) {
      ...
      if (validate)
        r = await RegisterOrUpdateReminder(
            reminderName, usePeriod - TimeSpan.FromSeconds(2), usePeriod);
      else
      ...
      File.Delete(
          fileName); // if successfully started, then remove any old data
    ...
    public Task ReceiveReminder(string reminderName, TickStatus status) {
      ...
      // hence, this activation may receive a reminder that it didn't register
      // itself, but the previous activation (incarnation of the grain)
      // registered... so, play it safe
      if (!this.sequence.ContainsKey(reminderName)) {
        // allReminders.Add(reminderName, r); // not using allReminders at the
        // moment
        // counters.Add(reminderName, 0);
        this.sequence.Add(reminderName,
                          0); // we'll get upto date to the latest sequence
                              // number while processing this tick
      }

      // calculating tick sequence number

      // we do all arithmetics on DateTime by converting into long because we
      // dont have divide operation on DateTime using dateTime.Ticks is not
      // accurate as between two invocations of ReceiveReminder(), there maybe <
      // period.Ticks if # of ticks between two consecutive ReceiveReminder() is
      // larger than period.Ticks, everything is fine... the problem is when its
      // less thus, we reduce our accuracy by ACCURACY ... here, we are
      // preparing all used variables for the given accuracy
      long now = status.CurrentTickTime.Ticks /
                 aCCURACY; // DateTime.UtcNow.Ticks / ACCURACY;
      ...
      if (sequenceNumber < this.sequence[reminderName]) {
        this.logger.Info(
            "ReceiveReminder: {0} Incorrect tick {1} vs. {2} with status {3}.",
            reminderName, this.sequence[reminderName], sequenceNumber, status);
        return Task.CompletedTask;
      }

      this.sequence[reminderName] = sequenceNumber;
      this.logger.Info("ReceiveReminder: {0} Sequence # {1} with status {2}.",
                       reminderName, this.sequence[reminderName], status);
    ...
    public async Task StopReminder(string reminderName) {
      ...
      // we dont reset counter as we want the test methods to be able to read it
      // even after stopping the reminder
      ...
      if (this.allReminders.TryGetValue(reminderName, out reminder)) {
        ...
        // during failures, there may be reminders registered by an earlier
        // activation that we dont have cached locally therefore, we need to
        ...
        if (this.allReminders.TryGetValue(reminderName, out reminder)) {
          ...
          throw new OrleansException(
              string.Format("Could not find reminder {0} in grain {1}",
                            reminderName, this.IdentityString));
        }
      }
    }
    ...
    private async Task GetMissingReminders() {
      ...
      this.logger.Info("Got missing reminders {0}",
                       Utils.EnumerableToString(reminders));
    ...
    public static TimeSpan GetDefaultPeriod(ILogger log) {
      ...
      log.Info("Using reminder period of {0} in ReminderTestGrain",
               reminderPeriod);
  ...
  public class ReminderTestCopyGrain : Grain,
                                       IReminderTestCopyGrain,
                                       IRemindable {
    private readonly IReminderRegistry unvalidatedReminderRegistry;
    Dictionary<string, IGrainReminder>allReminders;
    Dictionary<string, long>sequence;
    ...
    private ILogger logger;
    private long myId; // used to distinguish during debugging between multiple
                       // activations of the same grain
    ...
    public async Task<IGrainReminder>StartReminder(string reminderName,
                                                   ...
                                                   bool validate = false) {
      TimeSpan usePeriod = p ?? this.period;
      this.logger.Info("Starting reminder {0} for {1}", reminderName,
                       this.Identity);
      IGrainReminder r = null;
      if (validate)
        r = await RegisterOrUpdateReminder(
            reminderName,
            /*TimeSpan.FromSeconds(3)*/ usePeriod - TimeSpan.FromSeconds(2),
            usePeriod);
      else
        r = await this.unvalidatedReminderRegistry.RegisterOrUpdateReminder(
            reminderName, usePeriod - TimeSpan.FromSeconds(2), usePeriod);
      ...
      File.Delete(GetFileName(
          reminderName)); // if successfully started, then remove any old data
      this.logger.Info("Started reminder {0}.", r);
      return r;
    }
    ...
    public Task ReceiveReminder(string reminderName, TickStatus status) {
      ...
      // do switch-ing here
      if (sequenceNumber < this.sequence[reminderName]) {
        this.logger.Info("{0} Incorrect tick {1} vs. {2} with status {3}.",
                         reminderName, this.sequence[reminderName],
                         sequenceNumber, status);
        return Task.CompletedTask;
      }

      this.sequence[reminderName] = sequenceNumber;
      this.logger.Info("{0} Sequence # {1} with status {2}.", reminderName,
                       this.sequence[reminderName], status);

      File.WriteAllText(GetFileName(reminderName),
                        this.sequence[reminderName].ToString());
  ...
  public class WrongReminderGrain : Grain, IReminderGrainWrong {
    ...
    public async Task<bool>StartReminder(string reminderName) {
      this.logger.Info("Starting reminder {0}.", reminderName);
      IGrainReminder r = await RegisterOrUpdateReminder(
          reminderName, TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(3));
      this.logger.Info("Started reminder {0}. It shouldn't have succeeded!", r);
      return true;
    }
  }

  internal class UnvalidatedReminderRegistry
      : GrainServiceClient<IReminderService>,
        IReminderRegistry {
    public UnvalidatedReminderRegistry(IServiceProvider serviceProvider)
        : base(serviceProvider) {}

    public Task<IGrainReminder>RegisterOrUpdateReminder(string reminderName,
                                                        TimeSpan dueTime,
                                                        TimeSpan period) {
      return this.GrainService.RegisterOrUpdateReminder(
          this.CallingGrainReference, reminderName, dueTime, period);
    }
    ...
    public Task<IGrainReminder>GetReminder(string reminderName) {
      return this.GrainService.GetReminder(this.CallingGrainReference,
                                           reminderName);
    }
