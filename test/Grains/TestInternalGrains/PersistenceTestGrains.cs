...
namespace UnitTests.Grains {
  internal static class TestRuntimeEnvironmentUtility {
    public static string CaptureRuntimeEnvironment() {
      var callStack =
          Utils.GetStackTrace(1); // Don't include this method in stack trace
      return String.Format("   TaskScheduler={0}" + Environment.NewLine +
                           ...
                           TaskScheduler.Current, RuntimeContext.Current,
                           Thread.CurrentThread.Name,
                           Thread.CurrentThread.ManagedThreadId, callStack);
    }
  }
  ...
  public class PersistenceTestGrainState {
    ...
    public SortedDictionary<int, int>SortedDict {
      get;
      set;
    }
  }
  ...
  [Serializable]
  public class PersistenceGenericGrainState<T>{public T Field1{get;
  set;
}
public string Field2 {
  get;
...
public SortedDictionary<T, T>SortedDict {
  get;
  set;
}
}
...
public class PersistenceTestGrain
    : Grain<PersistenceTestGrainState>,
      IPersistenceTestGrain{
          public override Task OnActivateAsync(){return Task.CompletedTask;
}
...
public Task<bool>CheckStateInit() {
  ...
  // Assert.AreEqual(0, State.Field3.Count, "Field3 = {0}", String.Join("'",
  // State.Field3));
...
public Task<int>GetValue() { return Task.FromResult(State.Field1); }

public async Task DoDelete() { await ClearStateAsync(); }
}

[Orleans.Providers.StorageProvider(ProviderName = "test1")]
public class PersistenceTestGenericGrain<T>
    : PersistenceTestGrain,
      IPersistenceTestGenericGrain<T>{
          //...
      }

      [Orleans.Providers.StorageProvider(ProviderName = "ErrorInjector")]
      public class PersistenceProviderErrorGrain
    : Grain<PersistenceTestGrainState>,
      IPersistenceProviderErrorGrain {
  public override Task OnActivateAsync() { return Task.CompletedTask; }
  ...
  public Task<string>
  GetActivationId() => Task.FromResult(this.Data.ActivationId.ToString());
}

[Orleans.Providers.StorageProvider(ProviderName = "ErrorInjector")]
public class PersistenceUserHandledErrorGrain
    : Grain<PersistenceTestGrainState>,
      IPersistenceUserHandledErrorGrain {
  private readonly SerializationManager serializationManager;
  private readonly ILogger logger;

  public PersistenceUserHandledErrorGrain(
      ILoggerFactory loggerFactory, SerializationManager serializationManager) {
    this.logger = loggerFactory.CreateLogger(
        $"{this.GetType().Name}-{this.IdentityString}");
  ...
  public async Task DoWrite(int val, bool recover) {
    ...
    try {
      ...
      this.logger.Warn(
          0,
          "Grain is handling error in DoWrite - Resetting value to " + original,
          exc);
  ...
  public async Task<int>DoRead(bool recover) {
    ...
    try {
      ...
      this.logger.Warn(
          0,
          "Grain is handling error in DoRead - Resetting value to " + original,
          exc);
      State = (PersistenceTestGrainState) original;
    }
    return State.Field1;
...
public class PersistenceProviderErrorProxyGrain
    : Grain,
      IPersistenceProviderErrorProxyGrain {
  public Task<int>
  GetValue(IPersistenceProviderErrorGrain other) => other.GetValue();

  public Task
  DoWrite(int val, IPersistenceProviderErrorGrain other) => other.DoWrite(val);

  public Task<int>
  DoRead(IPersistenceProviderErrorGrain other) => other.DoRead();
...
[Orleans.Providers.StorageProvider(ProviderName = "test1")]
public class PersistenceErrorGrain
    : Grain<PersistenceTestGrainState>,
      IPersistenceErrorGrain{
          public override Task OnActivateAsync(){return Task.CompletedTask;
}
...
public async Task DoWriteError(int val, bool errorBeforeUpdate) {
  if (errorBeforeUpdate)
    throw new ApplicationException("Before Update");
  State.Field1 = val;
  await WriteStateAsync();
...
public async Task<int>DoReadError(bool errorBeforeRead) {
  if (errorBeforeRead)
    throw new ApplicationException("Before Read");
...
public class BadProviderTestGrain : Grain<PersistenceTestGrainState>,
                                    IBadProviderTestGrain {
  private readonly ILogger logger;
...
[Orleans.Providers.StorageProvider(ProviderName = "test1")]
public class PersistenceNoStateTestGrain : Grain,
                                           IPersistenceNoStateTestGrain {
  private readonly ILogger logger;
...
public class GrainStorageTestGrain
    : Grain<PersistenceTestGrainState>,
      IGrainStorageTestGrain,
      IGrainStorageTestGrain_LongKey{
          public override Task OnActivateAsync(){return Task.CompletedTask;
}
...
public Task DoDelete() {
  return ClearStateAsync(); // Automatically marks this grain as
                            // DeactivateOnIdle
}
}

[Orleans.Providers.StorageProvider(ProviderName = "GrainStorageForTest")]
public class GrainStorageGenericGrain<T>
    : Grain<PersistenceGenericGrainState<T>>,
      IGrainStorageGenericGrain<T>{
          public override Task OnActivateAsync(){return Task.CompletedTask;
}

public Task<T>GetValue() { return Task.FromResult(State.Field1); }
...
[Orleans.Providers.StorageProvider(ProviderName = "GrainStorageForTest")]
public class GrainStorageTestGrainExtendedKey
    : Grain<PersistenceTestGrainState>,
...
public class AWSStorageTestGrain : Grain<PersistenceTestGrainState>,
                                   IAWSStorageTestGrain,
                                   IAWSStorageTestGrain_LongKey {
  public override Task OnActivateAsync() { return Task.CompletedTask; }
...
[Orleans.Providers.StorageProvider(ProviderName = "DDBStore")]
public class AWSStorageGenericGrain<T>
    : Grain<PersistenceGenericGrainState<T>>,
...
[Orleans.Providers.StorageProvider(ProviderName = "DDBStore")]
public class AWSStorageTestGrainExtendedKey
    : Grain<PersistenceTestGrainState>,
...
[Orleans.Providers.StorageProvider(ProviderName = "MemoryStore")]
    ...
    public class MemoryStorageTestGrain
    : Grain<MemoryStorageTestGrain.NestedPersistenceTestGrainState>,
...
public Task DoDelete() { return ClearStateAsync(); }
...
public class UserState {
  public UserState() { Friends = new List<IUser>(); }
...
[Serializable]
public class DerivedUserState : UserState{public int Field1{get;
set;
}
public int Field2 {
  get;
...
[Orleans.Providers.StorageProvider(ProviderName = "MemoryStore")]
    ...
    : Grain<DerivedUserState>,
      IUser{public Task SetName(string name){State.Name = name;
return WriteStateAsync();
}
...
public Task<string>GetName() { return Task.FromResult(State.Name); }
...
public Task<List<IUser>>GetFriends() { return Task.FromResult(State.Friends); }

public async Task<string>GetFriendsStatuses() {
  var sb = new StringBuilder();
  var promises = new List<Task<string>>();

  foreach(var friend in State.Friends) promises.Add(friend.GetStatus());

  var friends = await Task.WhenAll(promises);

  foreach(var f in friends) { sb.AppendLine(f); }
...
public class StateForIReentrentGrain {
  ...
  public Dictionary<string, int>DictOne {
    get;
    set;
  }
  public Dictionary<string, int>DictTwo {
    get;
    set;
  }
}
...
[Orleans.Providers.StorageProvider(ProviderName = "MemoryStore")]
[Reentrant]
public class ReentrentGrainWithState : Grain<StateForIReentrentGrain>,
                                       IReentrentGrainWithState {
  ...
  // When reentrant grain is doing WriteStateAsync, etag violations are
  // posssible due to concurent writes. The solution is to serialize all writes,
  // and make sure only a single write is outstanind at any moment in time. No
  // deadlocks are posssible with that approach, since all WriteStateAsync go to
  // the store, which does not issue call into grains, thus cycle of calls is
  // not posssible. Implementaton: need to use While and not if, due to the same
  // "early check becomes later invalid" standard problem, like in conditional
  // variables.
  ...
  public async Task Task_Delay(bool doStart) {
    var wrapper = new Task(async () => {
      logger.Info("Before Task.Delay #1 TaskScheduler.Current=" +
                  TaskScheduler.Current);
      await DoDelay(1);
      logger.Info("After Task.Delay #1 TaskScheduler.Current=" +
                  TaskScheduler.Current);
      await DoDelay(2);
      logger.Info("After Task.Delay #2 TaskScheduler.Current=" +
                  TaskScheduler.Current);
    });
  ...
  private async Task DoDelay(int i) {
    logger.Info("Before Task.Delay #{0} TaskScheduler.Current={1}", i,
                TaskScheduler.Current);
    await Task.Delay(1);
    logger.Info("After Task.Delay #{0} TaskScheduler.Current={1}", i,
                TaskScheduler.Current);
  }
...
internal class NonReentrentStressGrainWithoutState
    : Grain,
      INonReentrentStressGrainWithoutState {
  ...
  private readonly Tuple<string, Severity>[] overridesOn = {
      ...
      new Tuple<string, Severity>("Scheduler.ActivationTaskScheduler",
                                  Severity.Verbose3)};
  ...
  private NonReentrentStressGrainWithoutState(GrainId identity,
                                              IGrainRuntime runtime)
      : base(identity, runtime) {}

  public static NonReentrentStressGrainWithoutState
  Create(GrainId identity, IGrainRuntime runtime) =>
  ...
  public override Task OnActivateAsync() {
    ...
    logger = loggerFactory.CreateLogger(
        $"NonReentrentStressGrainWithoutState-{_id}");
  ...
  private async Task SetOne(int iter, int level) {
    ...
    if (level > 0) {
      ...
      Log("SetOne {0}-{1}_7 => {2}. After await SetOne call.", iter, level,
          nextLevel);
  ...
  private void CheckRuntimeEnvironment(string str) {
    ...
    if (executing) {
      var errorMsg = string.Format(
          "Found out that grain {0} is already in the middle of execution." +
              "\n Single threaded-ness violation!" + "\n {1} \n Call Stack={2}",
          this._id, TestRuntimeEnvironmentUtility.CaptureRuntimeEnvironment(),
          callStack);
      this.logger.Error(1, "\n\n\n\n" + errorMsg + "\n\n\n\n");
    ...
    // Assert.IsFalse(executing, "Found out that this grain is already in the
    // middle of execution. Single threaded-ness violation!");
    executing = true;
...
[Orleans.Providers.StorageProvider(ProviderName = "MemoryStore")]
internal class InternalGrainWithState : Grain<InternalGrainStateData>,
                                        IInternalGrainWithState {
  private ILogger logger;
...
public interface IBaseStateData // Note: I am deliberately not using IGrainState
                                // here.
    {int Field1{get;
set;
}
...
[Serializable]
public class StateInheritanceTestGrainData
    : IBaseStateData{private int Field2{get;
set;
}
...
[Orleans.Providers.StorageProvider(ProviderName = "MemoryStore")]
public class StateInheritanceTestGrain : Grain<StateInheritanceTestGrainData>,
                                         IStateInheritanceTestGrain {
  private ILogger logger;
...
public class SerializationTestGrain : Grain, ISerializationTestGrain {
  ...
  public SerializationTestGrain(ILoggerFactory loggerFactory,
  ...
  public Task Test_Serialize_Func() {
    ...
    // Fails
    // TestSerializeFuncPtr("Instance Func In Grain Class",
    // instanceFuncInGrainClass); TestSerializeFuncPtr("Func Lambda - Instance
    // field", instanceFilterFunc);
  ...
  public Task Test_Serialize_Predicate() {
    ...
    // Fails
    // TestSerializePredicate("Predicate Lambda - Instance field",
    // instancePredicate);
  ...
  public Task Test_Serialize_Predicate_Class_Param(IMyPredicate pred) {
    // Works OK
    TestSerializePredicate("Predicate Class Instance passed as param",
                           pred.FilterFunc);
  ...
  private void TestSerializeFuncPtr(string what, Func<int, bool>func1) {
    object obj2 =
        this.serializationManager.RoundTripSerializationForTesting(func1);
    var func2 = (Func<int, bool>) obj2;

    foreach(var val in new[]{_staticFilterValue1, _staticFilterValue2,
                             _staticFilterValue3, _staticFilterValue4}) {
  ...
  private void TestSerializePredicate(string what, Predicate<int>pred1) {
    object obj2 =
        this.serializationManager.RoundTripSerializationForTesting(pred1);
  ...
  public bool PredFuncInstance(int i) { return i == _instanceFilterValue2; }

  public static bool PredFuncStatic(int i) { return i == _staticFilterValue2; }
}
}