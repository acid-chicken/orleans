...
namespace UnitTests.Grains {
  ...
  public class EchoGrain : Grain<EchoTaskGrainState>,
                           IEchoGrain {
    ...
    public Task<string>GetLastEcho() { return Task.FromResult(State.LastEcho); }
    ...
    EchoNullable(DateTime? value) => Task.FromResult(value);
  }

  [StorageProvider(ProviderName = "MemoryStore")]
  internal class EchoTaskGrain : Grain<EchoTaskGrainState>,
                                 IEchoTaskGrain,
                                 IDebuggerHelperTestGrain {
    private readonly IInternalGrainFactory internalGrainFactory;
    private ILogger logger;

    public EchoTaskGrain(IInternalGrainFactory internalGrainFactory,
                         ILogger<EchoTaskGrain>logger) {
      this.internalGrainFactory = internalGrainFactory;
      this.logger = logger;
    }

    public Task<int>GetMyIdAsync() { return Task.FromResult(State.MyId); }
    public Task<string>GetLastEchoAsync() {
    ...
    public async Task<string>AwaitAVGrainCallErrorAsync(string data) {
      ...
      IEchoGrain avGrain =
          GrainFactory.GetGrain<IEchoGrain>(this.GetPrimaryKey());
    ...
    public Task<int>BlockingCallTimeoutAsync(TimeSpan delay) {
      ...
      Thread.Sleep(delay);
      logger.Info(
          "IEchoGrainAsync.BlockingCallTimeout Awoke from sleep after {0}",
          sw.Elapsed);
      throw new InvalidOperationException(
          "Timeout should have been returned to caller before " + delay);
    }
    ...
    public async Task PingOtherSiloAsync() {
      ...
      SiloAddress siloAddress = silos.Where(pair => !pair.Key.Equals(mySilo))
                                    .Select(pair => pair.Key)
                                    .First();
      logger.Info("Sending Ping to remote silo {0}", siloAddress);

      await GetSiloControlReference(siloAddress)
          .Ping("PingOtherSilo-" + siloAddress);
    ...
    public async Task PingClusterMemberAsync() {
      ...
      var oracle =
          this.internalGrainFactory.GetSystemTarget<IMembershipService>(
              Constants.MembershipOracleType, siloAddress);
    ...
    public Task OrleansDebuggerHelper_GetGrainInstance_Test() {
      ...
      result = OrleansDebuggerHelper.GetGrainInstance(
          this.AsReference<IDebuggerHelperTestGrain>());
      Assert.Same(this, result);

      result = OrleansDebuggerHelper.GetGrainInstance(
          this.GrainFactory.GetGrain<IEchoGrain>(Guid.NewGuid()));
      Assert.Null(result);
  ...
  [StorageProvider(ProviderName = "MemoryStore")]
  public class BlockingEchoTaskGrain : Grain<EchoTaskGrainState>,
    ...
    public Task<int>GetMyId() { return Task.FromResult(State.MyId); }
    ...
    public async Task<string>CallMethodTask_Await(string data) {
      ...
      logger.Info(name + " Data=" + data);
      IEchoTaskGrain avGrain =
          GrainFactory.GetGrain<IEchoTaskGrain>(this.GetPrimaryKey());
...
#pragma warning disable 1998
    public async Task<string>CallMethodTask_Block(string data) {
      ...
      // Note: We deliberately use .Result here in this test case to block
...
#pragma warning restore 1998
  ...
  [Reentrant]
  [StorageProvider(ProviderName = "MemoryStore")]
  public class ReentrantBlockingEchoTaskGrain
      : Grain<EchoTaskGrainState>,
