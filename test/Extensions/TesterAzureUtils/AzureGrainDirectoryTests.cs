...
namespace Tester.AzureUtils {
  ...
  public class AzureTableGrainDirectoryTests
      : GrainDirectoryTests<AzureTableGrainDirectory> {
    protected override AzureTableGrainDirectory GetGrainDirectory() {
      ...
      var loggerFactory = TestingUtils.CreateDefaultLoggerFactory(
          "AzureGrainDirectoryTests.log");

      var directory = new AzureTableGrainDirectory(
          directoryOptions, Options.Create(clusterOptions), loggerFactory);
    ...
    UnregisterMany() {
      ...
      for (var i = 0; i < N; i++) {
        var addr = new GrainAddress{ActivationId = Guid.NewGuid().ToString("N"),
                                    GrainId = "user/someraondomuser_" +
      ...
      // Modify the Rth entry locally, to simulate another activation tentative
      // by another silo
  ...
  public abstract class GrainDirectoryTests<T>where T : IGrainDirectory {
    ...
    [SkippableFact]
    public async Task
    RegisterLookupUnregisterLookup() {
      ...
      Assert.Equal(expected,
                   await this.grainDirectory.Lookup(expected.GrainId));
    ...
    DoNotOverrideEntry() {
      ...
      var differentActivation = new GrainAddress{
          ActivationId = Guid.NewGuid().ToString("N"),
          GrainId = expected.GrainId, SiloAddress = "10.0.23.12:1000@5678"};

      var differentSilo = new GrainAddress{
          ActivationId = expected.ActivationId, GrainId = expected.GrainId,
          SiloAddress = "10.0.23.14:1000@4583"};

      Assert.Equal(expected, await this.grainDirectory.Register(expected));
      Assert.Equal(expected,
                   await this.grainDirectory.Register(differentActivation));
    ...
    [SkippableFact]
    public async Task
    DoNotDeleteDifferentActivationIdEntry() {
      var expected = new GrainAddress{
          ActivationId = Guid.NewGuid().ToString("N"),
          GrainId = "user/someraondomuser_" + Guid.NewGuid().ToString("N"),
          SiloAddress = "10.0.23.12:1000@5678"};
    ...
    public async Task
    LookupNotFound() {
      Assert.Null(await this.grainDirectory.Lookup(
          "user/someraondomuser_" + Guid.NewGuid().ToString("N")));
    }
  }
}
