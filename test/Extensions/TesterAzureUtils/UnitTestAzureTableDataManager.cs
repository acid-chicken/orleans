...
namespace Tester.AzureUtils {
  ...
  public class UnitTestAzureTableData : TableEntity {
    ...
    public UnitTestAzureTableData(string data, string partitionKey,
                                  string rowKey) {
      StringData = data;
    ...
    public UnitTestAzureTableData Clone() {
      return new UnitTestAzureTableData{StringData = this.StringData,
  ...
  internal class UnitTestAzureTableDataManager
      : AzureTableDataManager<UnitTestAzureTableData> {
    ...
    public UnitTestAzureTableDataManager(string storageConnectionString,
        ...
        : base(INSTANCE_TABLE_NAME, storageConnectionString,
               loggerFactory.CreateLogger<UnitTestAzureTableDataManager>(),
               new AzureStoragePolicyOptions()) {
      InitTableAsync()
          .WithTimeout(new AzureStoragePolicyOptions().CreationTimeout)
          .Wait();
    }
  }
}
