...
using AzureStoragePolicyOptions =
    Orleans.Clustering.AzureStorage.AzureStoragePolicyOptions;
...
namespace Tester.AzureUtils.Persistence {
  /// <summary>
  /// Base_PersistenceGrainTests - a base class for testing persistence
  /// providers
  /// </summary>
  public abstract class Base_PersistenceGrainTests_AzureStore
    ...
    public static IProviderConfiguration
    GetNamedProviderConfigForShardedProvider(
        IEnumerable<KeyValuePair<string, IProviderConfiguration>>providers,
        string providerName) {
      var providerConfig =
          providers.Where(o => o.Key.Equals(providerName)).Select(o => o.Value);

      return providerConfig.First();
    }

    public Base_PersistenceGrainTests_AzureStore(
        ITestOutputHelper output, BaseTestClusterFixture fixture,
      ...
      this.basicPersistenceTestsRunner =
          new GrainPersistenceTestsRunner(output, fixture, grainNamespace);
    }
    ...
    protected Task Grain_GuidKey_AzureStore_Read_Write() {
      return this.basicPersistenceTestsRunner
          .Grain_GuidKey_GrainStorage_Read_Write();
    }

    protected Task Grain_LongKey_AzureStore_Read_Write() {
      return this.basicPersistenceTestsRunner
          .Grain_LongKey_GrainStorage_Read_Write();
    }

    protected Task Grain_LongKeyExtended_AzureStore_Read_Write() {
      return this.basicPersistenceTestsRunner
          .Grain_LongKeyExtended_GrainStorage_Read_Write();
    }

    protected Task Grain_GuidKeyExtended_AzureStore_Read_Write() {
      return this.basicPersistenceTestsRunner
          .Grain_GuidKeyExtended_GrainStorage_Read_Write();
    }

    protected Task Grain_Generic_AzureStore_Read_Write() {
      return this.basicPersistenceTestsRunner
          .Grain_Generic_GrainStorage_Read_Write();
    }

    protected Task Grain_Generic_AzureStore_DiffTypes() {
      return this.basicPersistenceTestsRunner
          .Grain_Generic_GrainStorage_DiffTypes();
    }
    ...
    protected void Persistence_Perf_Activate() {
      ...
      RunPerfTest(n, testName, target, grainNoState => grainNoState.PingAsync(),
    ...
    Persistence_Silo_StorageProvider_Azure(string providerName) {
      ...
      foreach(var silo in silos) {
        ...
        List<string>providers =
            (await testHooks.GetStorageProviderNames()).ToList();
        Assert.True(providers.Contains(providerName),
                    $"No storage provider found: {providerName}");
      }
    }
    ...
    protected void
    RunPerfTest(int n, string testName, TimeSpan target,
      ...
      IMemoryStorageTestGrain[] memoryStoreGrains =
          new IMemoryStorageTestGrain[n];
      ...
      for (int i = 0; i < n; i++) {
        ...
        azureStoreGrains[i] =
            this.GrainFactory.GetGrain<IGrainStorageTestGrain>(id);
        memoryStoreGrains[i] =
            this.GrainFactory.GetGrain<IMemoryStorageTestGrain>(id);
      }

      TimeSpan baseline, elapsed;

      elapsed = baseline = TestUtils.TimeRun(
          n, TimeSpan.Zero, testName + " (No state)",
          () =>
              RunIterations(testName, n, i => actionNoState(noStateGrains[i])));

      elapsed = TestUtils.TimeRun(
          n, baseline, testName + " (Local Memory Store)",
          () => RunIterations(testName, n, i => actionMemory(memoryGrains[i])));

      elapsed = TestUtils.TimeRun(
          n, baseline, testName + " (Dev Store Grain Store)",
          () => RunIterations(testName, n,
                              i => actionMemoryStore(memoryStoreGrains[i])));

      elapsed = TestUtils.TimeRun(
          n, baseline, testName + " (Azure Table Store)",
          () => RunIterations(testName, n,
                              i => actionAzureTable(azureStoreGrains[i])));

      if (elapsed > target.Multiply(timingFactor)) {
        string msg =
            string.Format("{0}: Elapsed time {1} exceeds target time {2}",
                          testName, elapsed, target);
