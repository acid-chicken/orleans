...
namespace UnitTests.Streaming.Reliability {
  ...
  public class StreamReliabilityTests : TestClusterPerTest {
    ...
    public const string SMS_STREAM_PROVIDER_NAME =
        StreamTestsConstants.SMS_STREAM_PROVIDER_NAME;
    public const string AZURE_QUEUE_STREAM_PROVIDER_NAME =
    ...
    public class ClientBuilderConfigurator : IClientBuilderConfigurator {
      public void Configure(IConfiguration configuration,
        ...
        clientBuilder
            .UseAzureStorageClustering(gatewayOptions => {
    ...
    public override void Dispose() {
      ...
      foreach(var g in _usedGrains) { promises.Add(g.ClearGrain()); }
    ...
    Baseline_StreamRel() {
      ...
      StreamTestUtils.LogStartTest(testName, _streamId, _streamProviderName,
                                   logger, HostedCluster);
      StreamTestUtils.LogEndTest(testName, logger);
    }

    [SkippableFact, TestCategory("Functional")]
    public async Task
    Baseline_StreamRel_RestartSilos() {
      // This test case is just a sanity-check that the silo test config is OK.
      ...
      Assert.NotEqual(
          silos,
          this.HostedCluster.Silos); // Should be different silos after restart
    ...
    public async Task
    AQ_Baseline_StreamRel() {
      // This test case is just a sanity-check that the AzureQueue test config
      // is OK.
    ...
    [SkippableFact(Skip = "Ignore"), TestCategory("Failures"),
     TestCategory("Streaming"), TestCategory("Reliability")]
    public async Task
    SMS_AddMany_Consumers() {
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_PubSub_MultiConsumerSameGrain() {
      const string testName = "SMS_PubSub_MultiConsumerSameGrain";
      await Test_PubSub_MultiConsumerSameGrain(testName,
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_PubSub_MultiProducerSameGrain() {
      const string testName = "SMS_PubSub_MultiProducerSameGrain";
      await Test_PubSub_MultiProducerSameGrain(testName,
                                               SMS_STREAM_PROVIDER_NAME);
    }
    ...
    // TODO: This test fails because the resubscribe to streams after restart
    // creates a new subscription, losing the events on the previous
    // subscription.  Should be fixed when 'renew' subscription feature is added.
    // - jbragg
    [SkippableFact, TestCategory("Functional"), TestCategory("Failures")]
    public async Task
    SMS_StreamRel_AllSilosRestart_PubSubCounts() {
      const string testName = "SMS_StreamRel_AllSilosRestart_PubSubCounts";
      await Test_AllSilosRestart_PubSubCounts(testName,
                                              SMS_STREAM_PROVIDER_NAME);
    }
    // AQ_StreamRel_AllSilosRestart_PubSubCounts not required - does not use
    // PubSub

    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_StreamRel_AllSilosRestart() {
      const string testName = "SMS_StreamRel_AllSilosRestart";

      await Test_AllSilosRestart(testName, SMS_STREAM_PROVIDER_NAME);
    }
    [SkippableFact, TestCategory("Functional"), TestCategory("Azure"),
     TestCategory("AzureQueue")]
    public async Task
    AQ_StreamRel_AllSilosRestart() {
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_StreamRel_SiloDies_Consumer() {
    ...
    [SkippableFact, TestCategory("Functional"), TestCategory("Azure"),
     TestCategory("AzureQueue")]
    public async Task
    AQ_StreamRel_SiloDies_Consumer() {
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_StreamRel_SiloDies_Producer() {
    ...
    [SkippableFact, TestCategory("Functional"), TestCategory("Azure"),
     TestCategory("AzureQueue")]
    public async Task
    AQ_StreamRel_SiloDies_Producer() {
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_StreamRel_SiloRestarts_Consumer() {
    ...
    [SkippableFact, TestCategory("Functional"), TestCategory("Azure"),
     TestCategory("AzureQueue")]
    public async Task
    AQ_StreamRel_SiloRestarts_Consumer() {
      const string testName = "AQ_StreamRel_SiloRestarts_Consumer";
      await Test_SiloRestarts_Consumer(testName,
                                       AZURE_QUEUE_STREAM_PROVIDER_NAME);
    }

    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_StreamRel_SiloRestarts_Producer() {
    ...
    [SkippableFact, TestCategory("Functional"), TestCategory("Azure"),
     TestCategory("AzureQueue")]
    public async Task
    AQ_StreamRel_SiloRestarts_Producer() {
      const string testName = "AQ_StreamRel_SiloRestarts_Producer";
      await Test_SiloRestarts_Producer(testName,
                                       AZURE_QUEUE_STREAM_PROVIDER_NAME);
    }
...
#if USE_GENERICS
    private async Task<IStreamReliabilityTestGrain<int>>
    Do_BaselineTest(long consumerGrainId, long producerGrainId)
#else
    private async Task<IStreamReliabilityTestGrain>
    Do_BaselineTest(long consumerGrainId, long producerGrainId)
#endif
    {
      logger.Info("Initializing: ConsumerGrain={0} ProducerGrain={1}",
                  consumerGrainId, producerGrainId);
      ...
      logger.Info("AddConsumer: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
      await consumerGrain.AddConsumer(_streamId, _streamProviderName);
      logger.Info("BecomeProducer: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
...
#if USE_GENERICS
    private async Task<IStreamReliabilityTestGrain<int>[]>
    Do_AddConsumerGrains(long baseId, int numGrains)
#else
    private async Task<IStreamReliabilityTestGrain[]>
    Do_AddConsumerGrains(long baseId, int numGrains)
#endif
    {
      ...
      logger.Info("AddConsumer: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
      await Task.WhenAll(
          grains.Select(g => g.AddConsumer(_streamId, _streamProviderName)));
    ...
    private async Task Test_AddMany_Consumers(string testName,
      ...
      // Messages received by new consumer grains
      await Task.WhenAll(
          grains2.Select(g => CheckReceivedCounts(when2, g, numLoops, 0)));

      StreamTestUtils.LogEndTest(testName, logger);
    }

    private async Task Test_PubSub_MultiConsumerSameGrain(
        string testName, string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      ...
      // Note: Only semantics guarenteed for producer is that they will have
      // been registered by time that first msg is sent.
      await producerGrain.SendItem(0);
      await StreamTestUtils.CheckPubSubCounts(
          this.InternalClient, output, when, 1, 0, _streamId,
          _streamProviderName, StreamTestsConstants.StreamReliabilityNamespace);

      logger.Info("AddConsumer x 2 : StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
      await consumerGrain.AddConsumer(_streamId, _streamProviderName);
      when = "After first AddConsumer";
      await StreamTestUtils.CheckPubSubCounts(
          this.InternalClient, output, when, 1, 1, _streamId,
          _streamProviderName, StreamTestsConstants.StreamReliabilityNamespace);

      await consumerGrain.AddConsumer(_streamId, _streamProviderName);
      when = "After second AddConsumer";
      await StreamTestUtils.CheckPubSubCounts(
          this.InternalClient, output, when, 1, 2, _streamId,
          _streamProviderName, StreamTestsConstants.StreamReliabilityNamespace);

      StreamTestUtils.LogEndTest(testName, logger);
    }

    private async Task Test_PubSub_MultiProducerSameGrain(
        string testName, string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
    ...
    private async Task Test_PubSub_Unsubscribe(string testName,
                                               string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      ...
      // Grain Producer -> Grain 2 x Consumer
      // Note: PubSub should only count distinct grains, even if a grain has
      // multiple consumer handles
      ...
      await producerGrain.BecomeProducer(_streamId, _streamProviderName);
      when = "After BecomeProducer";
      // Note: Only semantics guarenteed are that producer will have been
      // registered by time that first msg is sent.
      await producerGrain.SendItem(0);
      await StreamTestUtils.CheckPubSubCounts(
          this.InternalClient, output, when, 1, 0, _streamId,
          _streamProviderName, StreamTestsConstants.StreamReliabilityNamespace);
      ...
      logger.Info("RemoveConsumer: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
      ...
      await CheckConsumerCounts(when, consumerGrain, 1);
#if REMOVE_PRODUCER
      logger.Info("RemoveProducer: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
    ...
    [SkippableFact, TestCategory("Functional")]
    public async Task
    SMS_AllSilosRestart_UnsubscribeConsumer() {
      ...
      long consumerGrainId = random.Next();
      var consumerGrain =
          this.GrainFactory.GetGrain<IStreamUnsubscribeTestGrain>(
              consumerGrainId);

      logger.Info("Subscribe: StreamId={0} Provider={1}", _streamId,
                  _streamProviderName);
    ...
    private async Task Test_AllSilosRestart(string testName,
                                            string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
    ...
    private async Task Test_AllSilosRestart_PubSubCounts(
        string testName, string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      ...
      CheckSilosRunning(when, numExpectedSilos);
      // Note: It is not guaranteed that the list of producers will not get
      // modified / cleaned up during silo shutdown, so can't assume count will
      // be 1 here. Expected == -1 means don't care.
      await StreamTestUtils.CheckPubSubCounts(
          this.InternalClient, output, when, -1, 1, _streamId,
          _streamProviderName, StreamTestsConstants.StreamReliabilityNamespace);
    ...
    private async Task Test_SiloDies_Consumer(string testName,
                                              string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      string when;
      ...
      output.WriteLine(
          "Consumer grain is located on silo {0} ; Producer on same silo = {1}",
          siloAddress, sameSilo);

      // Kill the silo containing the consumer grain
      SiloHandle siloToKill = this.HostedCluster.Silos.First(
          s => s.SiloAddress.Equals(siloAddress));
    ...
    private async Task Test_SiloDies_Producer(string testName,
                                              string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      string when;
      ...
      output.WriteLine(
          "Producer grain is located on silo {0} ; Consumer on same silo = {1}",
          siloAddress, sameSilo);
    ...
    private async Task Test_SiloRestarts_Consumer(string testName,
                                                  string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      string when;
    ...
    private async Task Test_SiloRestarts_Producer(string testName,
                                                  string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      string when;
    ...
    private async Task Test_SiloJoins(string testName,
                                      string streamProviderName) {
      _streamId = Guid.NewGuid();
      _streamProviderName = streamProviderName;
      ...
      output.WriteLine("Grain silo locations: Producer={0} Consumer={1}",
                       producerLocation, consumerLocation);
      ...
      //}
      // expectedReceived += numLoops;
      // await CheckConsumerProducerStatus(when, producerGrainId,
      // consumerGrainId, true, true); await CheckReceivedCounts(when,
      // consumerGrain, expectedReceived, 0);

      // Find a Consumer Grain on the new silo
      IStreamReliabilityTestGrain newConsumer =
          CreateGrainOnSilo(newSilo.SiloAddress);
      await newConsumer.AddConsumer(_streamId, _streamProviderName);
      output.WriteLine(
          "Grain silo locations: Producer={0} OldConsumer={1} NewConsumer={2}",
          producerLocation, consumerLocation, newSilo.SiloAddress);
      ...
      await CheckReceivedCounts(when + "-Old", consumerGrain, expectedReceived,
                                0);
    ...
    private async Task RestartAllSilos() {
        ...
        // Note: Needed to reinitialize client in this test case to connect to
        // new silos this.HostedCluster.InitializeClient();
    ...
    private async Task StopSilo(SiloHandle silo, bool kill, bool restart) {
      ...
      if (restart) {
        ...
        logger.Info("Restarted new {0} silo {1}", siloType,
                    newSilo.SiloAddress);

        Assert.NotEqual(oldSilo,
                        newSilo.SiloAddress); //"Should be different silo
    ...
    protected async Task CheckConsumerProducerStatus(string when,
                                                     long producerGrainId,
                                                     long consumerGrainId,
                                                     bool expectIsProducer,
                                                     bool expectIsConsumer) {
      await CheckConsumerProducerStatus(when, producerGrainId, consumerGrainId,
    ...
    protected async Task CheckConsumerProducerStatus(string when,
      ...
      output.WriteLine("Grain {0} HandleCount={1} ObserverCount={2}",
                       consumerGrainId, consumerHandleCount,
                       consumerObserverCount);
    ...
    private void CheckSilosRunning(string when, int expectedNumSilos) {
      Assert.Equal(expectedNumSilos,
                   this.HostedCluster.GetActiveSilos().Count());
    }
    protected async Task<bool>CheckGrainCounts() {
      ...
      IManagementGrain mgmtGrain =
          this.GrainFactory.GetGrain<IManagementGrain>(0);

      SimpleGrainStatistic[] grainStats =
          await mgmtGrain.GetSimpleGrainStatistics();
      output.WriteLine("Found grains " + Utils.EnumerableToString(grainStats));

      var grainLocs =
          grainStats.Where(gs => gs.GrainType == grainType).ToArray();

      Assert.True(grainLocs.Length > 0, "Found too few grains");
      Assert.True(grainLocs.Length <= 2,
                  "Found too many grains " + grainLocs.Length);

      bool sameSilo = grainLocs.Length == 1;
      if (sameSilo) {
        StreamTestUtils.Assert_AreEqual(output, 2, grainLocs[0].ActivationCount,
                                        "Num grains on same Silo " +
                                            grainLocs[0].SiloAddress);
      } else {
        StreamTestUtils.Assert_AreEqual(output, 1, grainLocs[0].ActivationCount,
                                        "Num grains on Silo " +
                                            grainLocs[0].SiloAddress);
        StreamTestUtils.Assert_AreEqual(output, 1, grainLocs[1].ActivationCount,
                                        "Num grains on Silo " +
                                            grainLocs[1].SiloAddress);
      }
      return sameSilo;
    }

#if USE_GENERICS
    protected async Task CheckReceivedCounts<T>(
        string when, IStreamReliabilityTestGrain<T>consumerGrain,
        int expectedReceivedCount, int expectedErrorsCount)
#else
    protected async Task
    CheckReceivedCounts(string when, IStreamReliabilityTestGrain consumerGrain,
                        int expectedReceivedCount, int expectedErrorsCount)
#endif
    {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        output.WriteLine("After {0}s ReceivedCount={1} for grain {2}", i,
                         receivedCount, pk);
      ...
      StreamTestUtils.Assert_AreEqual(
          output, expectedReceivedCount, receivedCount,
          "ReceivedCount for stream {0} for grain {1} {2}", _streamId, pk,
          when);

      int errorsCount = await consumerGrain.GetErrorsCount();
      StreamTestUtils.Assert_AreEqual(
          output, expectedErrorsCount, errorsCount,
          "ErrorsCount for stream {0} for grain {1} {2}", _streamId, pk, when);
    }
#if USE_GENERICS
    protected async Task CheckConsumerCounts<T>(
        string when, IStreamReliabilityTestGrain<T>consumerGrain,
        int expectedConsumerCount)
#else
    protected async Task
    CheckConsumerCounts(string when, IStreamReliabilityTestGrain consumerGrain,
                        int expectedConsumerCount)
#endif
    {
      int consumerCount = await consumerGrain.GetConsumerCount();
      StreamTestUtils.Assert_AreEqual(
          output, expectedConsumerCount, consumerCount,
          "ConsumerCount for stream {0} {1}", _streamId, when);
    }
  }
}
