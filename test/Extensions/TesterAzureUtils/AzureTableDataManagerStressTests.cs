...
namespace Tester.AzureUtils {
  ...
  public class AzureTableDataManagerStressTests : AzureStorageBasicTests {
    ...
    [SkippableFact]
    public void
    AzureTableDataManagerStressTests_WriteAlot_SinglePartition() {
    ...
    [SkippableFact]
    public void
    AzureTableDataManagerStressTests_WriteAlot_MultiPartition() {
      const string testName =
          "AzureTableDataManagerStressTests_WriteAlot_MultiPartition";
      const int iterations = 2000;
      const int batchSize = 1000;
    ...
    [SkippableFact]
    public void
    AzureTableDataManagerStressTests_ReadAll_SinglePartition() {
      ...
      output.WriteLine(
          "AzureTable_ReadAll completed. ReadAll {0} entries in {1} at {2} RPS",
          count, sw.Elapsed, count / sw.Elapsed.TotalSeconds);

      Assert.True(count >= iterations,
                  $"ReadAllshould return some data: Found={count}");
    }

    [SkippableFact]
    public void
    AzureTableDataManagerStressTests_ReadAllTableEntities() {
      const string testName =
          "AzureTableDataManagerStressTests_ReadAllTableEntities";
      const int iterations = 2000;
      ...
      int count = data.Count();
      output.WriteLine(
          "AzureTable_ReadAllTableEntities completed. ReadAll {0} entries in {1} at {2} RPS",
          count, sw.Elapsed, count / sw.Elapsed.TotalSeconds);
      ...
      manager.ClearTableAsync().WaitWithThrow(
          new AzureStoragePolicyOptions().CreationTimeout);
      sw.Stop();
      output.WriteLine(
          "AzureTable_ReadAllTableEntities clear. Cleared table of {0} entries in {1} at {2} RPS",
          count, sw.Elapsed, count / sw.Elapsed.TotalSeconds);
    }

    private void WriteAlot_Async(string testName, int numPartitions,
                                 int iterations, int batchSize) {
      output.WriteLine("Iterations={0}, Batch={1}, Partitions={2}", iterations,
                       batchSize, numPartitions);
      ...
      for (int i = 0; i < iterations; i++) {
        ...
        if ((i % batchSize) == 0 && i > 0) {
          Task.WhenAll(promises).WaitWithThrow(
              new AzureStoragePolicyOptions().CreationTimeout);
          promises.Clear();
          output.WriteLine("{0} has written {1} rows in {2} at {3} RPS",
                           testName, i, sw.Elapsed,
                           i / sw.Elapsed.TotalSeconds);
        }
      }
      Task.WhenAll(promises).WaitWithThrow(
          new AzureStoragePolicyOptions().CreationTimeout);
      sw.Stop();
      output.WriteLine(
          "{0} completed. Wrote {1} entries to {2} partition(s) in {3} at {4} RPS",
          testName, iterations, numPartitions, sw.Elapsed,
