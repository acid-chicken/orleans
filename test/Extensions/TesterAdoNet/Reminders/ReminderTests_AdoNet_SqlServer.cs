...
namespace Tester.AdoNet.Reminders {
  ...
  public class ReminderTests_AdoNet_SqlServer
      : ReminderTests_Base,
        IClassFixture<ReminderTests_AdoNet_SqlServer.Fixture> {
    private const string TestDatabaseName = "OrleansTest";
    private static string AdoInvariant =
      ...
      protected override void ConfigureTestCluster(TestClusterBuilder builder) {
        string connectionString =
            RelationalStorageForTesting
                .SetupInstance(AdoInvariant, TestDatabaseName)
                .Result.CurrentConnectionString;
        builder.ConfigureHostConfiguration(
            config => config.AddInMemoryCollection(
                new Dictionary<string, string>{[ConnectionStringKey] =
                                                   connectionString}));
      ...
      public void Configure(ISiloBuilder hostBuilder) {
        hostBuilder.UseAdoNetReminderService(options => {
          options.ConnectionString =
              hostBuilder.GetConfigurationValue(ConnectionStringKey);
    ...
    public ReminderTests_AdoNet_SqlServer(Fixture fixture) : base(fixture) {
      ...
      var controlProxy =
          fixture.GrainFactory.GetGrain<IReminderTestGrain2>(Guid.NewGuid());
      controlProxy.EraseReminderTable().WaitWithThrow(
    ...
    [SkippableFact]
    public async Task
    Rem_Sql_Basic_StopByRef() {
      await Test_Reminders_Basic_StopByRef();
    }

    [SkippableFact]
    public async Task
    Rem_Sql_Basic_ListOps() {
    ...
    [SkippableFact]
    public async Task
    Rem_Sql_1J_MultiGrainMultiReminders() {
      await Test_Reminders_1J_MultiGrainMultiReminders();
    }

    [SkippableFact]
    public async Task
    Rem_Sql_ReminderNotFound() {
