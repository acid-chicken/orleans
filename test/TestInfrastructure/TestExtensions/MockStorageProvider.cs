...
namespace UnitTests.StorageTests {
  public static class SiloBuilderExtensions {
    ...
    AddTestStorageProvider<T>(this ISiloBuilder builder, string name) where T
        : IGrainStorage {
      return builder.AddTestStorageProvider(
          name, (sp, n) => ActivatorUtilities.CreateInstance<T>(sp));
    }

    public static ISiloBuilder AddTestStorageProvider<T>(
        this ISiloBuilder builder, string name,
        Func<IServiceProvider, string, T>createInstance) where T
        : IGrainStorage {
      return builder.ConfigureServices(services => {
        services.AddSingletonNamedService<IGrainStorage>(
            name, (sp, n) => createInstance(sp, n));

        if (typeof (ILifecycleParticipant<ISiloLifecycle>)
                .IsAssignableFrom(typeof (T))) {
          services
              .AddSingletonNamedService<ILifecycleParticipant<ISiloLifecycle>>(
                  name,
                  (svc, n) =>
                      (ILifecycleParticipant<ISiloLifecycle>)
                          svc.GetRequiredServiceByName<IGrainStorage>(name));
        }

        if (typeof (IControllable).IsAssignableFrom(typeof (T))) {
          services.AddSingletonNamedService<IControllable>(
              name, (svc, n) =>
                        (IControllable)
                            svc.GetRequiredServiceByName<IGrainStorage>(name));
        }
  ...
  public class MockStorageProvider : IControllable,
                                     IGrainStorage {
    ...
    public MockStorageProvider(ILoggerFactory loggerFactory,
                               SerializationManager serializationManager)
        : this(Guid.NewGuid().ToString(), 2, loggerFactory,
               serializationManager) {}

    public MockStorageProvider(string name, ILoggerFactory loggerFactory,
                               SerializationManager serializationManager)
        : this(name, 2, loggerFactory, serializationManager) {}

    public MockStorageProvider(string name, int numKeys,
      ...
      this.logger = loggerFactory.CreateLogger(
          string.Format("Storage.{0}-{1}", this.GetType().Name, this._id));
    ...
    public void SetValue(SetValueArgs args) {
      SetValue(args.StateType, args.GrainType, args.GrainReference, args.Name,
               args.Val);
    }

    private void SetValue(Type stateType, string grainType,
                          GrainReference grainReference, string name,
                          object val) {
      lock(StateStore) {
        this.logger.Info("Setting stored value {0} for {1} to {2}", name,
                         grainReference, val);
    ...
    public object GetLastState() { return LastState; }

    public T GetLastState<T>() { return (T) LastState; }

    private object GetLastState(string grainType, GrainReference grainReference,
                                IGrainState grainState) {
      lock(StateStore) {
        var keys = MakeGrainStateKeys(grainType, grainReference);
        var storedStateRow = StateStore.ReadRow(keys);
        if (!storedStateRow.ContainsKey(stateStoreKey)) {
          storedStateRow[stateStoreKey] =
    ...
    public virtual Task ReadStateAsync(string grainType,
      ...
      lock(StateStore) {
        ...
        grainState.State = this.serializationManager.DeepCopy(
            storedState); // Read current state data
      }
      return Task.CompletedTask;
    }

    public virtual Task WriteStateAsync(string grainType,
                                        GrainReference grainReference,
                                        IGrainState grainState) {
      logger.Info(0, "WriteStateAsync for {0} {1}", grainType, grainReference);
      Interlocked.Increment(ref writeCount);
      lock(StateStore) {
        var storedState = this.serializationManager.DeepCopy(
            grainState.State); // Store current state data
        var stateStore =
            new Dictionary<string, object>{{stateStoreKey, storedState}};
        StateStore.WriteRow(MakeGrainStateKeys(grainType, grainReference),
                            stateStore, grainState.ETag);
    ...
    public virtual Task ClearStateAsync(string grainType,
                                        GrainReference grainReference,
                                        IGrainState grainState) {
    ...
    private static IList<Tuple<string, string>>
    MakeGrainStateKeys(string grainType, GrainReference grainReference) {
      return new[]{Tuple.Create("GrainType", grainType),
