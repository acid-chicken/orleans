...
namespace Orleans.TestingHost.Tests {
  ...
  public class TestClusterTests : IDisposable, IAsyncLifetime {
    ...
    public TestClusterTests(ITestOutputHelper output) { this.output = output; }
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    CanInitializeWithLegacyConfiguration() {
      var builder = new TestClusterBuilder(2);
      builder.ConfigureHostConfiguration(
          TestDefaultConfiguration.ConfigureHostConfiguration);
    ...
    public void Dispose() { this.testCluster?.StopAllSilos(); }
