...
namespace Orleans.TestingHost {
  ...
  /// NOTE: this class has to and will be removed entirely. This requires the
  /// tests that currently rely on it, to assert using different mechanisms,
  /// such as with grains.
  /// </summary>
  internal class AppDomainTestHooks : MarshalByRefObject {
    private readonly ISiloHost host;

    public AppDomainTestHooks(ISiloHost host) { this.host = host; }

    /// <summary>Find the named storage provider loaded in this silo. </summary>
    internal IStorageProvider
    GetStorageProvider(string name) => CheckReturnBoundaryReference(
        "storage provider",
        this.host.Services.GetRequiredServiceByName<IStorageProvider>(name));

    private static T CheckReturnBoundaryReference<T>(string what, T obj) where T
        : class {
      if(obj == null)
        return null;
      if(obj is MarshalByRefObject || obj is ISerializable) {
        // Reference to the provider can safely be passed across app-domain
        // boundary in unit test process
        return obj;
      }
    ...
    public IDictionary<GrainId, IGrainInfo>
    GetDirectoryForTypeNamesContaining(string expr) {
      var x = new Dictionary<GrainId, IGrainInfo>();
      LocalGrainDirectory localGrainDirectory =
          this.host.Services.GetRequiredService<LocalGrainDirectory>();
      ...
      foreach(var kvp in localGrainDirectory.DirectoryPartition.GetItems()) {
        if (kvp.Key.IsSystemTarget() || kvp.Key.IsClient() ||
            !kvp.Key.IsLegacyGrain())
    ...
    internal void BlockSiloCommunication(IPEndPoint destination,
    ...
    private bool ShouldDrop(Message msg) {
      if (simulatedMessageLoss != null) {
        ...
        simulatedMessageLoss.TryGetValue(msg.TargetSilo.Endpoint,
                                         out blockedpercentage);
