...
[assembly:System.Reflection.AssemblyVersionAttribute("2.0.0.0")]
  ...
  public class RoslynTypeNameFormatterTests {
    ...
    private static readonly Type[] Types = {
        ...
        typeof(
            Generic<int>.NestedMultiGeneric<Generic<int>.NestedGeneric<bool>, double>)};
    ...
    public RoslynTypeNameFormatterTests(ITestOutputHelper output) {
      ...
      var metas =
          new[]{
              ...
              .Select(a => MetadataReference.CreateFromFile(
                          a.Location, MetadataReferenceProperties.Assembly));
      var metadataReferences = metas.Concat(GetGlobalReferences()).ToArray();

      var syntaxTrees = new[]{CSharpSyntaxTree.ParseText(testCode, path
                                                         : "TestProgram.cs")};
      var assemblyName =
          typeof (RoslynTypeNameFormatterTests).Assembly.GetName().Name;
      var options =
          new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary)
              .WithMetadataImportOptions(MetadataImportOptions.All);
      this.compilation = CSharpCompilation.Create(assemblyName, syntaxTrees,
                                                  metadataReferences, options);
      ...
      IEnumerable<MetadataReference>GetGlobalReferences() {
        ...
        return new List<MetadataReference>{
            ...
            MetadataReference.CreateFromFile(Path.Combine(
                assemblyPath, "System.Runtime.Serialization.Formatters.dll"))};
      }
    }
    ...
    private string GetSource() {
      ...
      using(var stream = type.Assembly.GetManifestResourceStream(
                $"{type.Namespace}.{type.Name}.cs")) using(StreamReader reader =
                                                               new StreamReader(
                                                                   stream)) {
    ...
    /// <summary>
    /// Tests that various strings formatted with <see
    /// cref="RoslynTypeNameFormatter"/> match their corresponding <see
    /// cref="Type.FullName"/> values.
    /// </summary>
    [Fact]
    public void
    FullNameMatchesClr(){foreach(var(type, symbol)
                                     in GetTypeSymbolPairs(nameof(Types))){
        this.output.WriteLine($"Type: {RuntimeTypeNameFormatter.Format(type)}");
    var expected = type.FullName;
    var actual = RoslynTypeNameFormatter.Format(
        symbol, RoslynTypeNameFormatter.Style.FullName);
    this.output.WriteLine(
        $"Expected FullName: {expected}\nActual FullName:   {actual}");
...
/// <summary>
/// Tests that the ITypeSymbol id generation algorithm generates ids which match
/// the Type-based generator.
/// </summary>
[Fact]
public void
TypeCodesMatch() {
  ...
  foreach(var (type, symbol) in GetTypeSymbolPairs(nameof(Grains))) {
    ...
    {
      var expected =
          TypeUtils.GetTemplatedName(TypeUtils.GetFullName(type), type,
                                     type.GetGenericArguments(), t => false);
...
/// <summary>
/// Tests that the IMethodSymbol id generation algorithm generates ids which
/// match the MethodInfo-based generator.
/// </summary>
[Fact]
public void
MethodIdsMatch() {
  ...
  foreach(var (type, typeSymbol) in GetTypeSymbolPairs(nameof(Grains))) {
    ...
    var methodSymbols =
        methods.Select(m => typeSymbol.GetMembers(m.Name).SingleOrDefault())
            .OfType<IMethodSymbol>();

    foreach(var (method, methodSymbol)
                in methods.Zip(methodSymbols, ValueTuple.Create)) {
      this.output.WriteLine(
          $"IMethodSymbol: {methodSymbol}, MethodInfo: {method}");
      Assert.NotNull(methodSymbol);

      {
        var expected = GrainInterfaceUtils.FormatMethodForIdComputation(method);
        var actual = OrleansLegacyCompat.FormatMethodForMethodIdComputation(
            methodSymbol);
        this.output.WriteLine(
            $"Expected format: {expected}\nActual format:   {actual}");
        Assert.Equal(expected, actual);
...
private IEnumerable<(Type, ITypeSymbol)>GetTypeSymbolPairs(string fieldName) {
  var typesMember =
      compilation.Assembly.GlobalNamespace.GetMembers("CodeGenerator")
          .First()
          .GetMembers("Tests")
          ...
          .First();
  var declaratorSyntax = Assert.IsType<VariableDeclaratorSyntax>(
  ...
  var creationExpressionSyntax = Assert.IsType<InitializerExpressionSyntax>(
      declaratorSyntax.Initializer.Value);
...
public interface IMyGrainInterfaceWithTypeCodeOverride : IGrainWithGuidKey {
  [MethodId(1)] Task One(int a, int b, int c);

  [MethodId(2)] Task<int>Two();
}

[TypeCodeOverride(2)]
public class MyGrainClassWithTypeCodeOverride
    : Grain,
      IMyGrainInterfaceWithTypeCodeOverride {
  [MethodId(12234)]
  public Task
  One(int a, int b, int c) => throw new NotImplementedException();

  [MethodId(-41243)]
  public Task<int>
  Two() => throw new NotImplementedException();
}
}
