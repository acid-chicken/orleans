...
namespace Tester.Directories {
  public class MultipleGrainDirectoriesTests : TestClusterPerTest {
    ...
    CheckPreconditionsOrThrow() => TestUtils.CheckForAzureStorage();
    ...
    public async Task
    PingGrain() {
      var grainOnPrimary = await GetGrainOnPrimary().WithTimeout(
          TimeSpan.FromSeconds(5), "Could not get a grain on the primary silo");
      var grainOnSecondary = await GetGrainOnSecondary().WithTimeout(
          TimeSpan.FromSeconds(5),
          "Could not get a grain on the secondary silo");
      ...
      Assert.Equal(++primaryCounter,
                   await grainOnSecondary.ProxyPing(grainOnPrimary));
      Assert.Equal(++secondaryCounter,
      ...
      // Activation on the primary silo should still be there, another
      // activation should be created for the other one
    ...
    private async Task<IAzureTableDirectoryGrain>GetGrainOnPrimary() {
      while (true) {
        var grain = this.GrainFactory.GetGrain<IAzureTableDirectoryGrain>(
            Guid.NewGuid());
        var instanceId = await grain.GetRuntimeInstanceId();
        if (instanceId.Contains(
                HostedCluster.Primary.SiloAddress.Endpoint.ToString()))
          return grain;
      }
    }
