...
namespace UnitTests.General {
  ...
  public class GrainCallFilterTests
      : OrleansTestingBase,
        IClassFixture<GrainCallFilterTests.Fixture> {
    public class Fixture
        : BaseTestClusterFixture{protected override void ConfigureTestCluster(
              TestClusterBuilder builder){builder.ConfigureHostConfiguration(
              TestDefaultConfiguration.ConfigureHostConfiguration);
    builder
        .AddSiloBuilderConfigurator<SiloInvokerTestSiloBuilderConfigurator>();
      ...
      hostBuilder
          .AddIncomingGrainCallFilter(context => {
            if (string.Equals(
                    ...
                    nameof(IGrainCallFilterTestGrain.GetRequestContext))) {
  ...
  private class ClientConfigurator : IClientBuilderConfigurator {
    public void Configure(IConfiguration configuration,
      ...
      clientBuilder.AddOutgoingGrainCallFilter(RetryCertainCalls)
          .AddOutgoingGrainCallFilter(async ctx => {
            if (ctx.InterfaceMethod?.DeclaringType ==
                typeof (IOutgoingMethodInterceptionGrain) &&ctx.InterfaceMethod?
                    .Name ==
                nameof(IOutgoingMethodInterceptionGrain.EchoViaOtherGrain)) {
          ...
          try {
            ...
            return;
              ...
              when(attemptsRemaining > 1 &&
                   ctx.Grain is IOutgoingMethodInterceptionGrain) {
            if (string.Equals(ctx.InterfaceMethod?.Name,
                              nameof(IOutgoingMethodInterceptionGrain
                                         .ThrowIfGreaterThanZero)) &&
                ctx.Arguments[0] is int value) {
...
public class GrainCallFilterWithDependencies : IIncomingGrainCallFilter {
  ...
  public GrainCallFilterWithDependencies(
      SerializationManager serializationManager, Silo silo,
  ...
  public Task Invoke(IIncomingGrainCallContext context) {
    if (string.Equals(context.ImplementationMethod.Name,
                      nameof(IGrainCallFilterTestGrain.GetRequestContext))) {
      if (RequestContext.Get(GrainCallFilterTestConstants.Key)
              is string value) {
...
public GrainCallFilterTests(Fixture fixture) { this.fixture = fixture; }

/// <summary>
/// Ensures that grain call filters are invoked around method calls in the
/// correct order.
/// </summary>
/// <returns>A <see cref="Task"/> representing the work performed.</returns>
[Fact]
public async Task
GrainCallFilter_Outgoing_Test() {
  var grain =
      this.fixture.GrainFactory.GetGrain<IOutgoingMethodInterceptionGrain>(
          random.Next());
  var grain2 = this.fixture.GrainFactory.GetGrain<IMethodInterceptionGrain>(
      random.Next());
...
[Fact]
public async Task
GrainCallFilter_Incoming_Order_Test() {
  var grain = this.fixture.GrainFactory.GetGrain<IGrainCallFilterTestGrain>(
      random.Next());
...
[Fact]
public async Task
GrainCallFilter_Incoming_Stream_Test() {
...
[Fact]
public async Task
GrainCallFilter_Incoming_Retry_Test() {
  ...
  await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
      () => grain.ThrowIfGreaterThanZero(2));
}
...
[Fact]
public async Task
GrainCallFilter_Outgoing_Retry_Test() {
  var grain =
      this.fixture.GrainFactory.GetGrain<IOutgoingMethodInterceptionGrain>(0);
...
[Fact]
public async Task
GrainCallFilter_Incoming_GrainLevel_Test() {
  ...
  Assert.Equal("eat more oats",
               result); // Grain interceptors should receive the MethodInfo of
...
[Fact]
public async Task
GrainCallFilter_Incoming_GenericGrain_Test() {
  var grain =
      this.fixture.GrainFactory.GetGrain<IGenericMethodInterceptionGrain<int>>(
          0);
...
/// <summary>
/// Tests filters on grains which implement multiple of the same generic
/// interface.
/// </summary>
[Fact]
public async Task
GrainCallFilter_Incoming_ConstructedGenericInheritance_Test() {
  var grain =
      this.fixture.GrainFactory.GetGrain<ITrickyMethodInterceptionGrain>(0);
...
[Fact]
public async Task
GrainCallFilter_Incoming_ExceptionHandling_Test() {
  var grain = this.fixture.GrainFactory.GetGrain<IMethodInterceptionGrain>(
      random.Next());

  // This grain method throws, but the exception should be handled by one of the
  // filters and converted into a specific message.
...
[Fact]
public async Task
GrainCallFilter_Incoming_FilterThrows_Test() {
  var grain = this.fixture.GrainFactory.GetGrain<IMethodInterceptionGrain>(
      random.Next());

  var exception =
      await Assert
          .ThrowsAsync<MethodInterceptionGrain.MyDomainSpecificException>(
              () => grain.FilterThrows());
...
/// <summary>
/// Tests that if a grain call filter sets an incorrect result type for <see
/// cref="Orleans.IGrainCallContext.Result"/>, an exception is thrown on the
/// caller.
/// </summary>
[Fact]
public async Task
GrainCallFilter_Incoming_SetIncorrectResultType_Test() {
  var grain = this.fixture.GrainFactory.GetGrain<IMethodInterceptionGrain>(
      random.Next());
  ...
  await Assert.ThrowsAsync<InvalidCastException>(
      () => grain.IncorrectResultType());
}

/// <summary>
/// Tests that <see cref="IIncomingGrainCallContext.ImplementationMethod"/> and
/// <see cref="IGrainCallContext.InterfaceMethod"/> are non-null for a call made
/// to a grain and that they match the correct methods.
/// </summary>
[Fact]
public async Task
GrainCallFilter_Incoming_GenericInterface_ConcreteGrain_Test() {
  var id = random.Next();
