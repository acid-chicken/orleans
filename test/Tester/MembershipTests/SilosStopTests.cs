...
namespace UnitTests.MembershipTests {
  public class SilosStopTests : TestClusterPerTest {
    private class BuilderConfigurator : ISiloConfigurator,
      ...
      public void Configure(IConfiguration configuration,
        ...
        clientBuilder.UseStaticClustering(
            new IPEndPoint(IPAddress.Loopback, clusterOptions.BaseGatewayPort));
      }
    }
    ...
    public async Task
    SiloUngracefulShutdown_OutstandingRequestsBreak() {
    ...
    private async Task<ILongRunningTaskGrain<bool>>
    GetGrainOnTargetSilo(SiloHandle siloHandle) {
      const int maxRetry = 10;
      for (int i = 0; i < maxRetry; i++) {
        var grain =
            GrainFactory.GetGrain<ILongRunningTaskGrain<bool>>(Guid.NewGuid());
