  ...
  public class StallConnectionTests : TestClusterPerTest {
    ...
    public class SiloConfigurator : ISiloConfigurator {
      public void Configure(ISiloBuilder hostBuilder) {
        hostBuilder.Configure<ConnectionOptions>(
            options => options.OpenConnectionTimeout = Timeout);
      }
    }
    ...
    public class ClientConfigurator
        : IClientBuilderConfigurator{public void Configure(
              ...
              clientBuilder.Configure<ClientMessagingOptions>(
                  options => options.ResponseTimeout = Timeout);
  }
}

[Fact, TestCategory("Functional")]
public async Task
ConnectToGwAfterStallConnectionOpened() {
  ...
  using(stalledSocket = new Socket(gwEndpoint.AddressFamily, SocketType.Stream,
                                   ProtocolType.Tcp)) {
...
[Fact, TestCategory("Functional")]
public async Task
SiloJoinAfterStallConnectionOpened() {
  Socket stalledSocket;
  var siloEndpoint = this.HostedCluster.Primary.SiloAddress.Endpoint;

  // Stall connection to GW
  using(stalledSocket = new Socket(siloEndpoint.AddressFamily,
...
private async Task<bool>WaitForClusterSize(int expectedSize) {
  ...
  var timeout = TestCluster.GetLivenessStabilizationTime(
      new Orleans.Configuration.ClusterMembershipOptions());
  var stopWatch = Stopwatch.StartNew();
  do {
