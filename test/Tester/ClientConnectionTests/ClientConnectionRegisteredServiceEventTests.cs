...
public class ClientConnectionRegisteredServiceEventTests : TestClusterPerTest {
  ...
  private readonly EventNotifier<GatewayCountChangedEventArgs>
      gatewayCountChangedNotifier;

  public ClientConnectionRegisteredServiceEventTests() {
    this.clusterConnectionLostNotifier =
        this.HostedCluster.ServiceProvider
            .GetRequiredService<EventNotifier<EventArgs>>();
    this.gatewayCountChangedNotifier =
        this.HostedCluster.ServiceProvider
            .GetRequiredService<EventNotifier<GatewayCountChangedEventArgs>>();
  }
  ...
  public class EventNotifier<TEventArgs> {
    private ImmutableList<Action<TEventArgs>>subscribers =
        ImmutableList<Action<TEventArgs>>.Empty;

    public void Subscribe(Action<TEventArgs>action) =>
        ImmutableInterlocked.Update(ref this.subscribers,
                                    (subs, sub) => subs.Add(sub), action);

    public void Unsubscribe(Action<TEventArgs>action) =>
        ImmutableInterlocked.Update(ref this.subscribers,
                                    (subs, sub) => subs.Remove(sub), action);

    public void Notify(TEventArgs arg) {
      foreach(var sub in this.subscribers) { sub(arg); }
    }
  }
  ...
  var gatewayConnectionCountNotifier =
      new EventNotifier<GatewayCountChangedEventArgs>();

  clientBuilder.ConfigureServices(
      s => s.AddSingleton(clusterConnectionLostNotifier));
  clientBuilder.ConfigureServices(
      s => s.AddSingleton(gatewayConnectionCountNotifier));
  clientBuilder.Configure<GatewayOptions>(
      options => options.GatewayListRefreshPeriod = TimeSpan.FromSeconds(1));
  clientBuilder.AddClusterConnectionLostHandler(
      (s, e) => clusterConnectionLostNotifier.Notify(e));
  clientBuilder.AddGatewayCountChangedHandler(
      (s, e) => gatewayConnectionCountNotifier.Notify(e));
}
}

[Fact, TestCategory("SlowBVT")]
public async Task
EventSendWhenDisconnectedFromCluster() {
  var runtime = this.HostedCluster.ServiceProvider
...
[Fact, TestCategory("SlowBVT")]
public async Task
GatewayChangedEventSentOnDisconnectAndReconnect() {
  ...
  try {
    ...
    bool reconnected;
    do {
      this.Client.GetGrain<ITestGrain>(Guid.NewGuid().GetHashCode())
          .SetLabel("test")
          .Ignore();
      reconnected =
          await regainedGatewaySemaphore.WaitAsync(TimeSpan.FromSeconds(1));
