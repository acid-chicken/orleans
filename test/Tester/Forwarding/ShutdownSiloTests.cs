...
namespace Tester.Forwarding {
  public class ShutdownSiloTests : TestClusterPerTest {
    ...
    public static readonly TimeSpan DeactivationTimeout =
    ...
    internal class SiloBuilderConfigurator : ISiloConfigurator {
      public void Configure(ISiloBuilder hostBuilder) {
        hostBuilder
            ...
            .ConfigureServices(
                services =>
                    services.AddSingleton<PlacementStrategy,
    ...
    public ShutdownSiloTests() { this.EnsurePreconditionsMet(); }

    [Fact(Skip = "https://github.com/dotnet/orleans/issues/2287"),
     TestCategory("Forward"), TestCategory("Functional")]
    public async Task
    SiloGracefulShutdown_ForwardPendingRequest() {
      var grain = await GetLongRunningTaskGrainOnSecondary<bool>();

      var tasks = new List<Task<string>>();
      for (int i = 0; i < 100; i++) {
        tasks.Add(
            grain.GetRuntimeInstanceIdWithDelay(TimeSpan.FromMilliseconds(50)));
      }
      ...
      Assert.Equal(results[99],
                   HostedCluster.Primary.SiloAddress.ToLongString());
    }

    [SkippableFact, TestCategory("GracefulShutdown"),
     TestCategory("Functional")]
    public async Task
    SiloGracefulShutdown_PendingRequestTimers() {
    ...
    [SkippableFact, TestCategory("GracefulShutdown"),
     TestCategory("Functional")]
    public async Task
    SiloGracefulShutdown_StuckTimers() {
      var grain = await GetTimerRequestGrainOnSecondary();
    ...
    [SkippableFact, TestCategory("GracefulShutdown"),
     TestCategory("Functional")]
    public async Task
    SiloGracefulShutdown_StuckActivation() {
      var grain = await GetTimerRequestGrainOnSecondary();
    ...
    private async Task<ILongRunningTaskGrain<T>>
    GetLongRunningTaskGrainOnSecondary<T>() {
      while (true) {
        var grain =
            GrainFactory.GetGrain<ILongRunningTaskGrain<T>>(Guid.NewGuid());
        var instanceId = await grain.GetRuntimeInstanceId();
        if (instanceId.Contains(HostedCluster.SecondarySilos[0]
