...
namespace UnitTests.StreamingTests {
  public class PullingAgentManagementTests
      : OrleansTestingBase,
        IClassFixture<PullingAgentManagementTests.Fixture> {
                      ...
                      ob.Configure<IOptions<ClusterOptions>>((options, dep) => {
                        ...
                        options.QueueNames =
                            Enumerable.Range(0, 8)
                                .Select(num => $"{dep.Value.ClusterId}-{num}")
                                .ToList();
                      })));
...
#pragma warning restore 618
    ...
    private async Task
    ValidateAgentsState(StreamLifecycleOptions.RunState expectedState) {
      var mgmt = this.fixture.GrainFactory.GetGrain<IManagementGrain>(0);

      var states = await mgmt.SendControlCommandToProvider(
          adapterType, adapterName,
      ...
      var numAgents = await mgmt.SendControlCommandToProvider(
          adapterType, adapterName,
      ...
      if (expectedState == StreamLifecycleOptions.RunState.AgentsStarted) {
        Assert.Equal(HashRingStreamQueueMapperOptions.DEFAULT_NUM_QUEUES,
                     totalNumAgents);
      } else {
