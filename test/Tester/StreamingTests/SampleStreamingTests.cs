...
namespace UnitTests.StreamingTests {
  ...
  public class SampleSmsStreamingTests
      : OrleansTestingBase,
        IClassFixture<SampleSmsStreamingTests.Fixture> {
    ...
    private const string StreamProvider =
    ...
    public void
    SampleStreamingTests_StreamTypeMismatch_ShouldThrowOrleansException() {
      var streamId = Guid.NewGuid();
      var streamNameSpace = "SmsStream";
      var stream = this.fixture.Client.GetStreamProvider(StreamProvider)
                       .GetStream<int>(streamId, streamNameSpace);
      Assert.Throws<Orleans.Runtime.OrleansException>(() => {
        this.fixture.Client.GetStreamProvider(StreamProvider)
            .GetStream<string>(streamId, streamNameSpace);
      });
    }
    ...
    SampleStreamingTests_1() {
      this.logger.Info(
          "************************ SampleStreamingTests_1 *********************************");
      var runner = new SampleStreamingTests(StreamProvider, this.logger,
                                            this.fixture.HostedCluster);
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    MultipleImplicitSubscriptionTest() {
      this.logger.Info(
          "************************ MultipleImplicitSubscriptionTest *********************************");
      var streamId = Guid.NewGuid();
      const int nRedEvents = 5, nBlueEvents = 3;

      var provider = this.fixture.HostedCluster.ServiceProvider
                         .GetServiceByName<IStreamProvider>(
                             StreamTestsConstants.SMS_STREAM_PROVIDER_NAME);
      ...
      var grain = this.fixture.GrainFactory
                      .GetGrain<IMultipleImplicitSubscriptionGrain>(streamId);
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    FilteredImplicitSubscriptionGrainTest() {
      this.logger.Info(
          $"************************ {nameof(FilteredImplicitSubscriptionGrainTest)} *********************************");

      var streamNamespaces = new[]{"red1", "red2", "blue3", "blue4"};
      var events = new[]{3, 5, 2, 4};
      var testData = streamNamespaces
                         .Zip(events, (s, e) => new {Namespace = s, Events = e,
      ...
      foreach(var item in testData) {
        var grain =
            this.fixture.GrainFactory
                .GetGrain<IFilteredImplicitSubscriptionGrain>(item.StreamId);
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    FilteredImplicitSubscriptionWithExtensionGrainTest() {
      logger.Info(
          $"************************ {nameof(FilteredImplicitSubscriptionWithExtensionGrainTest)} *********************************");
      ...
      for (int i = 0; i < redEvents.Length; i++) {
        var grain =
            this.fixture.GrainFactory
                .GetGrain<IFilteredImplicitSubscriptionWithExtensionGrain>(
                    streamId, "red" + i, null);
  ...
  public class SampleStreamingTests {
    ...
    public SampleStreamingTests(string streamProvider, ILogger logger,
                                TestCluster cluster) {
    ...
    public async Task StreamingTests_Consumer_Producer(Guid streamId) {
      ...
      var consumer =
          this.cluster.GrainFactory.GetGrain<ISampleStreaming_ConsumerGrain>(
              Guid.NewGuid());
      await consumer.BecomeConsumer(streamId, StreamNamespace, streamProvider);

      var producer =
          this.cluster.GrainFactory.GetGrain<ISampleStreaming_ProducerGrain>(
              Guid.NewGuid());
      ...
      await TestingUtils.WaitUntilAsync(
          lastTry => CheckCounters(producer, consumer, lastTry), _timeout);
    ...
    public async Task StreamingTests_Producer_InlineConsumer(Guid streamId) {
      ...
      var consumer =
          this.cluster.GrainFactory
              .GetGrain<ISampleStreaming_InlineConsumerGrain>(Guid.NewGuid());
      await consumer.BecomeConsumer(streamId, StreamNamespace, streamProvider);
    ...
    CheckCounters(ISampleStreaming_ProducerGrain producer,
                  ISampleStreaming_ConsumerGrain consumer, bool assertIsTrue) {
      var numProduced = await producer.GetNumberProduced();
      var numConsumed = await consumer.GetNumberConsumed();
      this.logger.Info("CheckCounters: numProduced = {0}, numConsumed = {1}",
