...
namespace UnitTests.StreamingTests {
  [TestCategory("BVT")]
  public class MemoryStreamBatchingTests
      : StreamBatchingTestRunner,
        IClassFixture<MemoryStreamBatchingTests.Fixture> {
        ...
        public void Configure(ISiloBuilder hostBuilder) =>
            hostBuilder.AddMemoryGrainStorage("PubSubStore")
                .AddMemoryStreams<DefaultMemoryMessageBodySerializer>(
                    StreamBatchingTestConst.ProviderName, b => {
                      b.ConfigurePartitioning(partitionCount);
                      b.ConfigurePullingAgent(
                          ob => ob.Configure(
                              options => options.BatchContainerBatchSize = 10));
                      b.ConfigureStreamPubSub(StreamPubSubType.ImplicitOnly);
                    });
      }
    }
