...
namespace Tester.GeoClusterTests {
  ...
  public class MultiClusterDataTests {
    ...
    public void
    MultiClusterData_Configuration() {
      var ts1 = new DateTime(year : 2011, month : 1, day : 1, hour : 0,
                             minute : 0, second : 0, kind
                             : DateTimeKind.Utc);
      var ts2 = new DateTime(year : 2012, month : 2, day : 2, hour : 0,
                             minute : 0, second : 0, kind
                             : DateTimeKind.Utc);

      var conf1 =
          new MultiClusterConfiguration(ts1, new string[]{"A"}.ToList());
      var conf2 = new MultiClusterConfiguration(
          ts2, new string[]{"A", "B", "C"}.ToList());
    ...
    MultiClusterData_Gateways() {
      ...
      var G1 = new GatewayEntry(){SiloAddress = siloAddress1, ClusterId = "1",
                                  HeartbeatTimestamp = ts1,
                                  Status = GatewayStatus.Active};
      var G2 = new GatewayEntry(){SiloAddress = siloAddress1, ClusterId = "1",
                                  HeartbeatTimestamp = ts3,
                                  Status = GatewayStatus.Inactive};
      var H1 = new GatewayEntry(){SiloAddress = siloAddress2, ClusterId = "2",
                                  HeartbeatTimestamp = ts2,
                                  Status = GatewayStatus.Active};
      var H2 = new GatewayEntry(){SiloAddress = siloAddress2, ClusterId = "2",
                                  HeartbeatTimestamp = ts3,
                                  Status = GatewayStatus.Inactive};
    ...
    private void TestAlgebraicProperties(MultiClusterData A,
                                         MultiClusterData B) {
      MultiClusterData D;
    ...
    private void AssertEffect(MultiClusterData what, MultiClusterData to,
                              MultiClusterData expectedMerge,
                              MultiClusterData expectedDelta = null) {
    ...
    private bool CheckEquality(MultiClusterData one, MultiClusterData other) {
      ...
      if ((one.Configuration != null) &&
          !one.Configuration.Equals(other.Configuration))
        return false;
