...
public class StorageFacetTests : IClassFixture<StorageFacetTests.Fixture> {
  ...
  public class Fixture : BaseTestClusterFixture {
    ...
    private class TestSiloBuilderConfigurator : ISiloConfigurator {
      public void Configure(ISiloBuilder hostBuilder) {
        ...
        // - Call extension functions from each implementation assembly to
        // register it's classes.
  ...
  public StorageFacetTests(Fixture fixture) { this.fixture = fixture; }

  [Fact, TestCategory("BVT"), TestCategory("Facet")]
  public Task
  ExampleStorageFacetHappyPath() {
    return ExampleStorageHappyPath<IStorageFacetGrain>();
  }

  [Fact, TestCategory("BVT"), TestCategory("Facet")]
  public Task
  ExampleStorageFactoryHappyPath() {
    return ExampleStorageHappyPath<IStorageFactoryGrain>();
  }

  private async Task ExampleStorageHappyPath<TGrainInterface>() where
      TGrainInterface : IStorageFacetGrain {
    IStorageFacetGrain grain =
        this.fixture.GrainFactory.GetGrain<TGrainInterface>(0);
  ...
  [Fact, TestCategory("BVT"), TestCategory("Facet")]
  public Task
  ExampleStorageFacetDefaultPath() {
    return ExampleStorageDefaultPath<IStorageDefaultFacetGrain>();
  }

  [Fact, TestCategory("BVT"), TestCategory("Facet")]
  public Task
  ExampleStorageFactoryDefaultPath() {
    return ExampleStorageDefaultPath<IStorageDefaultFactoryGrain>();
  }

  private async Task ExampleStorageDefaultPath<TGrainInterface>() where
      TGrainInterface : IStorageFacetGrain {
    IStorageFacetGrain grain =
        this.fixture.GrainFactory.GetGrain<TGrainInterface>(0);
    string[] names = await grain.GetNames();
    ...
    Assert.Equal("second", names[1]);
    Assert.Equal(2, info.Length);
    Assert.Equal("Table:FirstState-ActivateCalled:True, StateType:String",
                 info[0]);
    Assert.Equal("Table:second-ActivateCalled:True, StateType:String", info[1]);
  }
}
