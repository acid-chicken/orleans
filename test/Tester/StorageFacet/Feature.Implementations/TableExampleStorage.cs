...
namespace Tester.StorageFacet.Implementations {
  public class TableExampleStorage<TState>
      : IExampleStorage<TState>, ILifecycleParticipant<IGrainLifecycle> {
    ...
    public Task Save() { return Task.CompletedTask; }
    ...
    public void Participate(IGrainLifecycle lifecycle) {
      lifecycle.Subscribe(
          OptionFormattingUtilities.Name<TableExampleStorage<TState>>(
              this.Name),
          GrainLifecycleStage.SetupState, LoadState);
    }
  ...
  public class TableExampleStorageFactory : IExampleStorageFactory {
    ...
    public IExampleStorage<TState>Create<TState>(IExampleStorageConfig config) {
      var storage = this.context.ActivationServices
                        .GetRequiredService<TableExampleStorage<TState>>();
  ...
  public static class TableExampleStorageExtensions {
    public static void UseTableExampleStorage(this ISiloBuilder builder,
                                              string name) {
      builder.ConfigureServices(services => {
        services.AddTransientNamedService<IExampleStorageFactory,
                                          TableExampleStorageFactory>(name);
