...
namespace UnitTests.Directory {
  ...
  public class CachedGrainLocatorTests {
    ...
    public CachedGrainLocatorTests(ITestOutputHelper output) {
      this.loggerFactory =
          new LoggerFactory(new[]{new XunitLoggerProvider(output)});
      this.lifecycle = new SiloLifecycleSubject(
          this.loggerFactory.CreateLogger<SiloLifecycleSubject>());

      this.grainDirectory = Substitute.For<IGrainDirectory>();
      this.grainDirectoryResolver = Substitute.For<IGrainDirectoryResolver>();
      this.grainDirectoryResolver.Resolve(Arg.Any<LegacyGrainId>())
          .Returns(this.grainDirectory);
      this.grainDirectoryResolver.Directories.Returns(
          new[]{this.grainDirectory});
      this.localGrainDirectory = Substitute.For<ILocalGrainDirectory>();
      this.mockMembershipService = new MockClusterMembershipService();

      this.grainLocator =
          new CachedGrainLocator(this.grainDirectoryResolver,
    ...
    [Fact]
    public void
    ConvertActivationAddressToGrainAddress() {
    ...
    [Fact]
    public async Task
    RegisterWhenNoOtherEntryExists() {
      var expected = GenerateActivationAddress();
      var grainAddress = expected.ToGrainAddress();
      ...
      Assert.True(
          this.grainLocator.TryLocalLookup(expected.Grain, out var results));
    ...
    RegisterWhenOtherEntryExists() {
      ...
      // Now should be in cache
      Assert.True(this.grainLocator.TryLocalLookup(expectedAddr.Grain,
                                                   out var results));
      Assert.Single(results);
    ...
    [Fact]
    public async Task
    RegisterWhenOtherEntryExistsButSiloIsDead() {
      var expectedAddr = GenerateActivationAddress();
      var expectedGrainAddr = expectedAddr.ToGrainAddress();
      ...
      this.mockMembershipService.UpdateSiloStatus(expectedAddr.Silo,
                                                  SiloStatus.Active);
      this.mockMembershipService.UpdateSiloStatus(outdatedAddr.Silo,
                                                  SiloStatus.Dead);
      await this.lifecycle.OnStart();
      await WaitUntilClusterChangePropagated();

      // First returns the outdated entry, then the new one
      this.grainDirectory.Register(expectedGrainAddr)
          .Returns(outdatedGrainAddr, expectedGrainAddr);
    ...
    LookupPopulateTheCache() {
      ...
      Assert.False(
          this.grainLocator.TryLocalLookup(expected.Grain, out var unused));
      ...
      Assert.True(this.grainLocator.TryLocalLookup(expected.Grain,
    ...
    [Fact]
    public async Task
    LookupWhenEntryExistsButSiloIsDead() {
      var outdatedAddr = GenerateActivationAddress();
      var outdatedGrainAddr = outdatedAddr.ToGrainAddress();
      ...
      this.grainDirectory.Lookup(outdatedGrainAddr.GrainId)
          .Returns(outdatedGrainAddr);
      ...
      Assert.False(
          this.grainLocator.TryLocalLookup(outdatedAddr.Grain, out var unused));
    ...
    [Fact]
    public async Task
    LocalLookupWhenEntryExistsButSiloIsDead() {
      var outdatedAddr = GenerateActivationAddress();
      var outdatedGrainAddr = outdatedAddr.ToGrainAddress();
      ...
      await this.grainDirectory.DidNotReceive().Lookup(
          outdatedGrainAddr.GrainId);
    ...
    CleanupWhenSiloIsDead() {
      ...
      this.grainDirectory.Register(expectedGrainAddr)
          .Returns(expectedGrainAddr);
      this.grainDirectory.Register(outdatedGrainAddr)
          .Returns(outdatedGrainAddr);
      ...
      await this.grainDirectory.Received(1).UnregisterSilos(
          Arg.Is<List<string>>(
              list => list.Count == 1 &&
                      list.Contains(outdatedGrainAddr.SiloAddress)));

      // Cache should have been cleaned
      Assert.False(this.grainLocator.TryLocalLookup(outdatedAddr.Grain,
                                                    out var unused1));
      Assert.True(this.grainLocator.TryLocalLookup(expectedAddr.Grain,
                                                   out var unused2));
    ...
    [Fact]
    public async Task
    UnregisterCallDirectoryAndCleanCache() {
      var expectedAddr = GenerateActivationAddress();
      var expectedGrainAddr = expectedAddr.ToGrainAddress();
      ...
      await this.grainLocator.Unregister(expectedAddr,
                                         UnregistrationCause.Force);
      Assert.False(
          this.grainLocator.TryLocalLookup(expectedAddr.Grain, out var unused));
    }

    private int generation = 0;
    private ActivationAddress GenerateActivationAddress() {
      var grainId = LegacyGrainId.GetGrainIdForTesting(Guid.NewGuid());
      var siloAddr = SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 5000),
                                     ++generation);
    ...
    private async Task WaitUntilClusterChangePropagated() {
      await Until(() => this.mockMembershipService.CurrentVersion ==
                        ((CachedGrainLocator.ITestAccessor) this.grainLocator)
