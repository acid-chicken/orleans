...
namespace NonSilo.Tests.Membership {
  /// <summary>
  /// An in-memory implementation of <see cref="IMembershipTable"/> for testing
  /// purposes.
  /// </summary>
  public class InMemoryMembershipTable : IMembershipTable {
    ...
    private ImmutableList<(MembershipEntry, string)>entries =
        ImmutableList<(MembershipEntry, string)>.Empty;
    private TableVersion version = new TableVersion(0, "0");

    public InMemoryMembershipTable() {}

    public InMemoryMembershipTable(TableVersion version,
    ...
    public Action OnReadAll {
      get;
      set;
    }

    public void ClearCalls() { lock(this.tableLock) this.calls.Clear(); }
    ...
    public Task CleanupDefunctSiloEntries(DateTimeOffset beforeDate) {
      lock(this.tableLock) {
        ...
        var newEntries =
            ImmutableList.CreateBuilder<(MembershipEntry, string)>();
        foreach(var (entry, etag) in this.entries) {
          if (entry.Status == SiloStatus.Dead &&
              new DateTime(
                  Math.Max(entry.IAmAliveTime.Ticks, entry.StartTime.Ticks),
                  DateTimeKind.Utc) < beforeDate) {
            continue;
          }
    ...
    public Task InitializeMembershipTable(bool tryInitTableVersion) {
      lock(this.tableLock) {
        this.calls.Add(
            (nameof(InitializeMembershipTable), tryInitTableVersion));
      }
    ...
    public Task<bool>InsertRow(MembershipEntry entry,
      ...
      lock(this.tableLock) {
        ...
        if (this.entries.Any(
                e => e.Item1.SiloAddress.Equals(entry.SiloAddress))) {
          return Task.FromResult(false);
        }

        this.version = new TableVersion(tableVersion.Version,
    ...
    public Task<MembershipTableData>ReadRow(SiloAddress key) {
      lock(this.tableLock) {
        ...
        var result = new MembershipTableData(
            this.entries.Where(e => e.Item1.SiloAddress.Equals(key))
                .Select(e => Tuple.Create(e.Item1, e.Item2))
                .ToList(),
            this.version);
        return Task.FromResult(result);
      }
    ...
    public Task<bool>UpdateRow(MembershipEntry entry, string etag,
      ...
      lock(this.tableLock) {
        ...
        this.ValidateVersion(tableVersion);
        var existingEntry = this.entries.FirstOrDefault(
            e => e.Item1.SiloAddress.Equals(entry.SiloAddress));
        if (existingEntry.Item1 is null)
          return Task.FromResult(false);

        if (!etag.Equals(existingEntry.Item2)) {
          throw new InvalidOperationException(
              $"Mismatching row etag. Required: {existingEntry.Item2}, Provided: {etag}");
        }

        this.version = new TableVersion(tableVersion.Version,
                                        tableVersion.Version.ToString());
        this.entries = this.entries.Replace(existingEntry,
                                            (entry, this.version.VersionEtag));
        return Task.FromResult(true);
      }
    }
