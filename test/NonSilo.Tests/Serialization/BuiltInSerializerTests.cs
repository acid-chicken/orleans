...
{
    public ConcurrentDictionary<BuiltInSerializerTests.SerializerToUse, SerializationTestEnvironment> Environments {
        get;
    } =
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public void Serialize_ComplexStruct(SerializerToUse serializerToUse)
    {
        ...
        var expected = new SomeStruct(10) {
            Id = Guid.NewGuid(), PublicValue = 6, ValueWithPrivateGetter = 7
        };
...
#pragma warning restore xUnit2017 // Do not use Contains() to check if a value exists in a collection
    ...
    public void Serialize_UnserializableException()
    {
        ...
        Exception source = Assert.Throws<UnserializableException>((Action)(() => {
    ...
    public void Serialize_UnserializableException_IlFallback(SerializerToUse serializerToUse)
    {
        ...
        // throw the exception so that stack trace is populated
        var source = Assert.Throws<UnserializableException>((Action)(() => {
            throw new UnserializableException(Message);
        }));
        object deserialized = OrleansSerializationLoop(environment.SerializationManager, source);
