...
namespace UnitTests.General {
  ...
  public class Identifiertests {
    ...
    public void
    UniqueKeyToByteArrayWithKeyExt() {
      var key = UniqueKey.NewKey(Guid.NewGuid(), category
                                 : UniqueKey.Category.KeyExtGrain, keyExt
                                 : "hello world");
    ...
    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    UniqueKeyToByteArrayWithoutKeyExt() {
      var key = UniqueKey.NewKey(Guid.NewGuid(), category
                                 : UniqueKey.Category.Client);
    ...
    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    SiloAddressGetUniformHashCodes() {
      int numberofHash = 3;
      var siloAddress =
          SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 8080), 26);
    ...
    ID_IsSystem() {
      ...
      Assert.True(
          testGrain
              .IsSystemTarget); // System grain ID is not flagged as a system ID

      GrainId sGrain =
          (GrainId) this.environment.SerializationManager.DeepCopy(testGrain);
      output.WriteLine("Testing GrainID " + sGrain);
      Assert.True(sGrain.IsSystemTarget); // String round-trip grain ID is not
                                          // flagged as a system ID
      Assert.Equal(testGrain, sGrain);    // Should be equivalent GrainId object
      Assert.Same(testGrain,
                  sGrain); // Should be same / intern'ed GrainId object

      ActivationId testActivation = ActivationId.GetSystemActivation(
          testGrain,
          SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 2456), 0));
      output.WriteLine("Testing ActivationID " + testActivation);
      Assert.True(
          testActivation
              .IsSystem); // System activation ID is not flagged as a system ID
    }

    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    UniqueKeyKeyExtGrainCategoryDisallowsNullKeyExtension() {
      Assert.Throws<ArgumentNullException>(
          () => UniqueKey.NewKey(Guid.NewGuid(), category
                                 : UniqueKey.Category.KeyExtGrain, keyExt
                                 : null));
    }
    ...
    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    UniqueKeyKeyExtGrainCategoryDisallowsEmptyKeyExtension() {
    ...
    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    UniqueKeyKeyExtGrainCategoryDisallowsWhiteSpaceKeyExtension() {
      Assert.Throws<ArgumentException>(
          () => UniqueKey.NewKey(Guid.NewGuid(), category
                                 : UniqueKey.Category.KeyExtGrain, keyExt
                                 : " \t\n\r"));
    ...
    [Fact, TestCategory("BVT"), TestCategory("Identifiers")]
    public void
    UniqueKeySerializationShouldReproduceAnIdenticalObject(){
    ...
    BinaryTokenStreamReader reader =
        new BinaryTokenStreamReader(writer.ToBytes());
    var actual = reader.ReadUniqueKey();
    Assert.Equal(expected,
                 actual); // UniqueKey.Serialize() and UniqueKey.Deserialize()
                          // failed to reproduce an identical object (case #1).
  }

  {
    var kx = random.Next().ToString(CultureInfo.InvariantCulture);
    var expected = UniqueKey.NewKey(Guid.NewGuid(), category
                                    : UniqueKey.Category.KeyExtGrain, keyExt
                                    : kx);
    BinaryTokenStreamWriter writer = new BinaryTokenStreamWriter();
    writer.Write(expected);
    ...
    Assert.Equal(expected,
                 actual); // UniqueKey.Serialize() and UniqueKey.Deserialize()
                          // failed to reproduce an identical object (case #2).
  }

  {
    ...
    Assert.Equal(expected,
                 actual); // UniqueKey.Serialize() and UniqueKey.Deserialize()
                          // failed to reproduce an identical object (case #3).
  }
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
ParsingUniqueKeyStringificationShouldReproduceAnIdenticalObject() {
  ...
  Assert.Equal(expected1,
               actual1); // UniqueKey.ToString() and UniqueKey.Parse() failed to
                         // reproduce an identical object (case 1).
  ...
  Assert.Equal(expected3,
               actual3); // UniqueKey.ToString() and UniqueKey.Parse() failed to
                         // reproduce an identical object (case 3).
  ...
  Assert.Equal(expected4,
               actual4); // UniqueKey.ToString() and UniqueKey.Parse() failed to
                         // reproduce an identical object (case 4).

  pk = random.Next();
  string kx5 = "case 5";
  UniqueKey expected5 =
      UniqueKey.NewKey(pk, category
                       : UniqueKey.Category.KeyExtGrain, keyExt
                       : kx5);
  string str5 = expected5.ToHexString();
  UniqueKey actual5 = UniqueKey.Parse(str5.AsSpan());
  Assert.Equal(expected5,
               actual5); // UniqueKey.ToString() and UniqueKey.Parse() failed to
                         // reproduce an identical object (case 5).
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
GrainIdShouldEncodeAndDecodePrimaryKeyGuidCorrectly() {
...
[Fact, TestCategory("SlowBVT"), TestCategory("Functional"),
 TestCategory("Identifiers")]
public void
GrainId_ToFromPrintableString() {
  ...
  Assert.Equal(
      grainId,
      roundTripped); // GrainId.ToPrintableString -- Guid key + Extended Key
  ...
  Assert.Equal(
      grainId,
      roundTripped); // GrainId.ToPrintableString -- Int64 key + Extended Key
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
UniqueTypeCodeDataShouldStore32BitsOfInformation() {
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
UniqueKeysShouldPreserveTheirPrimaryKeyValueIfItIsGuid() {
  const int all32Bits = unchecked((int) 0xffffffff);
  var expectedKey1 = Guid.NewGuid();
  const string expectedKeyExt1 = "1";
  var uk1 = UniqueKey.NewKey(expectedKey1, UniqueKey.Category.KeyExtGrain,
                             all32Bits, expectedKeyExt1);
  string actualKeyExt1;
  var actualKey1 = uk1.PrimaryKeyToGuid(out actualKeyExt1);
  Assert.Equal(expectedKey1,
               actualKey1); //"UniqueKey objects should preserve the value of
                            //their primary key (Guid case #1).");
  Assert.Equal(expectedKeyExt1,
               actualKeyExt1); //"UniqueKey objects should preserve the value of
                               //their key extension (Guid case #1).");

  var expectedKey2 = Guid.NewGuid();
  const string expectedKeyExt2 = "2";
  var uk2 = UniqueKey.NewKey(expectedKey2, UniqueKey.Category.KeyExtGrain,
                             all32Bits, expectedKeyExt2);
  string actualKeyExt2;
  var actualKey2 = uk2.PrimaryKeyToGuid(out actualKeyExt2);
  Assert.Equal(expectedKey2,
               actualKey2); // "UniqueKey objects should preserve the value of
                            // their primary key (Guid case #2).");
  Assert.Equal(expectedKeyExt2,
               actualKeyExt2); // "UniqueKey objects should preserve the value
                               // of their key extension (Guid case #2).");
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
UniqueKeysShouldPreserveTheirPrimaryKeyValueIfItIsLong() {
  const int all32Bits = unchecked((int) 0xffffffff);
  ...
  var expectedKey =
      unchecked((long)((((ulong)((uint) n1)) << 32) | ((uint) n2)));
  var uk = UniqueKey.NewKey(expectedKey, UniqueKey.Category.KeyExtGrain,
                            all32Bits, expectedKeyExt);

  string actualKeyExt;
  var actualKey = uk.PrimaryKeyToLong(out actualKeyExt);

  Assert.Equal(expectedKey,
               actualKey); // "UniqueKey objects should preserve the value of
                           // their primary key (long case).");
  Assert.Equal(expectedKeyExt,
               actualKeyExt); // "UniqueKey objects should preserve the value of
                              // their key extension (long case).");
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
ID_HashCorrectness() {
  // This tests that our optimized Jenkins hash computes the same value as the
  ...
  for (int i = 0; i < testCount; i++) {
    ...
    Assert.Equal(
        referenceHash,
        optimizedHash); //  "Optimized hash value doesn't match the reference
                        //  value for inputs {0}, {1}, {2}", u1, u2, u3
  }
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
ID_Interning_GrainID() {
  Guid guid = new Guid();
  ...
  GrainId gid3 = (GrainId) this.environment.SerializationManager
                     .RoundTripSerializationForTesting(gid1);
...
ID_Interning_string_equals() {
  ...
  // Round-trip through Serializer
  string r3 = (string) this.environment.SerializationManager
                  .RoundTripSerializationForTesting(r1);
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
ID_Intern_FindOrCreate_derived_class() {
  ...
  Assert.NotSame(
      obj3,
      r3); // FindOrCreate should not replace previous object of same class

  // FindOrCreate should not replace cached instances with instances of most
  // derived class
  var r4 = interner.FindOrCreate(1, _ => obj2);
  Assert.Same(obj1, r4); // FindOrCreate return previously cached object
  Assert.NotSame(
      obj2, r4); // FindOrCreate should not replace previously cached object

  // FindOrCreate should not replace cached instances with instances of less
  // derived class
  var r5 = interner.FindOrCreate(2, _ => obj1);
  Assert.NotSame(
      obj1, r5); // FindOrCreate should not replace previously cached object
  Assert.Same(obj2, r5); // FindOrCreate return previously cached object
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
Interning_SiloAddress() {
  // string addrStr1 = "1.2.3.4@11111@1";
  SiloAddress a1 =
      SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 1111), 12345);
  SiloAddress a2 =
      SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 1111), 12345);
  ...
  // Round-trip through Serializer
  SiloAddress a3 = (SiloAddress) this.environment.SerializationManager
                       .RoundTripSerializationForTesting(a1);
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
Interning_SiloAddress2() {
  SiloAddress a1 =
      SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 1111), 12345);
  SiloAddress a2 =
      SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 2222), 12345);
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
Interning_SiloAddress_Serialization() {
  SiloAddress a1 =
      SiloAddress.New(new IPEndPoint(IPAddress.Loopback, 1111), 12345);
...
[Fact, TestCategory("BVT"), TestCategory("Identifiers")]
public void
SiloAddress_ToFrom_ParsableString() {
  ...
  Assert.Equal(address1,
               addressObj1); // SiloAddress equal after To-From-ParsableString
  ...
  Assert.Equal(
      addressStr2,
      addressStr2Out); // SiloAddress equal after From-To-ParsableString
}
...
internal string GrainIdToGuidString(GrainId grainId) {
  ...
  int pkStartIdx =
      grainIdFullString.IndexOf(pkIdentifierStr, StringComparison.Ordinal) +
      pkIdentifierStr.Length + 1;
  string pkGuidString =
      grainIdFullString.Substring(pkStartIdx, Guid.Empty.ToString().Length);
  return pkGuidString;
}

[Fact, TestCategory("BVT"), TestCategory("Identifiers"),
 TestCategory("GrainReference")]
public void
GrainReference_Test1() {
  Guid guid = Guid.NewGuid();
  GrainId regularGrainId = GrainId.GetGrainIdForTesting(guid);
  GrainReference grainRef =
      this.environment.InternalGrainFactory.GetGrain(regularGrainId);
  ...
  grainRef = GrainReference.FromGrainId(
      systemTragetGrainId, null, null, SiloAddressUtils.NewLocalSiloAddress(1));
  this.environment.GrainFactory.BindGrainReference(grainRef);
  TestGrainReference(grainRef);

  GrainId observerGrainId = GrainId.NewClientId();
  grainRef = GrainReference.NewObserverGrainReference(
      observerGrainId, GuidId.GetNewGuidId(),
      this.environment.RuntimeClient.GrainReferenceRuntime);
  this.environment.GrainFactory.BindGrainReference(grainRef);
  TestGrainReference(grainRef);

  GrainId geoObserverGrainId = GrainId.NewClientId("clusterid");
  grainRef = GrainReference.NewObserverGrainReference(
      geoObserverGrainId, GuidId.GetNewGuidId(),
      this.environment.RuntimeClient.GrainReferenceRuntime);
  this.environment.GrainFactory.BindGrainReference(grainRef);
  TestGrainReference(grainRef);
}
...
private void TestGrainReference(GrainReference grainRef) {
  ...
  roundTripped =
      this.environment.SerializationManager.RoundTripSerializationForTesting(
          grainRef);
  Assert.Equal(grainRef, roundTripped); // GrainReference.OrleansSerializer

  roundTripped = TestingUtils.RoundTripDotNetSerializer(
      grainRef, this.environment.GrainFactory,
...
private GrainReference RoundTripGrainReferenceToKey(GrainReference input) {
  ...
  GrainReference output =
      this.environment.Services.GetRequiredService<IGrainReferenceConverter>()
          .GetGrainFromKeyString(str);
  return output;
}
}
}
