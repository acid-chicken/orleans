...
namespace UnitTests.Serialization {
  ...
  public class InternalSerializationTests {
    ...
    public void
    SerializationTests_GrainReference() {
    ...
    [TestCategory("Serialization")]
    public void
    SerializationTests_GenericGrainReference() {
    ...
    private void RunGrainReferenceSerializationTest<TGrainInterface>() {
      var counters = new List<CounterStatistic>{
          CounterStatistic.FindOrCreate(
              StatisticNames.SERIALIZATION_BODY_FALLBACK_SERIALIZATION),
          CounterStatistic.FindOrCreate(
              StatisticNames.SERIALIZATION_BODY_FALLBACK_DESERIALIZATION),
          CounterStatistic.FindOrCreate(
              StatisticNames.SERIALIZATION_BODY_FALLBACK_DEEPCOPIES)};

      // Get the (generated) grain reference implementation for a particular
      // grain interface type.
      ...
      var deserialized = this.fixture.SerializationManager.Deserialize(
          new BinaryTokenStreamReader(writer.ToByteArray()));
      var copy = (GrainReference) this.fixture.SerializationManager.DeepCopy(
          deserialized);
      ...
      Assert.IsAssignableFrom(grainReference.GetType(),
                              deserialized); // Deserialized GrainReference type
                                             // should match original type
      ...
      Assert.Equal(initialString,
                   finalString); // GrainReference serialization should not use
                                 // fallback serializer.
    }
    ...
    public GrainReference GetGrainReference<TGrainInterface>() {
      ...
      if (originalGrainType.IsGenericTypeDefinition) {
        throw new ArgumentException(
            "Cannot create grain reference for non-concrete grain type");
      }

      // The type used in attributes will not be a constructed generic type, so
      // if this is a constructed generic type then we need to get its
      // corresponding unconstructed type definition.
      var searchGrainType =
          originalGrainType.IsConstructedGenericType ? originalGrainType
              .GetGenericTypeDefinition()
          : originalGrainType;

      var type =
          originalGrainType.Assembly.DefinedTypes
              .First(_ => {
      ...
      return (GrainReference)
          type.GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic |
                              ...
                              null, new[]{typeof (GrainReference)}, null)
              .Invoke(new object[]{grainRef});
    }
  }
}
