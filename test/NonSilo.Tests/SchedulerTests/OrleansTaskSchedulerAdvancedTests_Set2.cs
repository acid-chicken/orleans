...
namespace UnitTests.SchedulerTests {
  public class OrleansTaskSchedulerAdvancedTests_Set2 : IDisposable {
    ...
    public OrleansTaskSchedulerAdvancedTests_Set2(ITestOutputHelper output) {
      ...
      this.masterScheduler = TestInternalHelper.InitializeSchedulerForTesting(
          this.context, this.loggerFactory);
    }
    ...
    public void
    ActivationSched_SimpleFifoTest() {
      // This is not a great test because there's a 50/50 shot that it will work
      // even if the scheduling is completely and thoroughly broken and both
      // closures are executed "simultaneously"
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public void
    ActivationSched_NewTask_ContinueWith_Wrapped() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;

      Task<Task>wrapped = new Task<Task>(() => {
        this.output.WriteLine(
            "#0 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
            SynchronizationContext.Current, TaskScheduler.Current);

        Task t0 = new Task(() => {
          this.output.WriteLine(
              "#1 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
              SynchronizationContext.Current, TaskScheduler.Current);
          Assert.Equal(scheduler,
                       TaskScheduler.Current); // "TaskScheduler.Current #1"
        });
        Task t1 = t0.ContinueWith(task => {
          Assert.False(task.IsFaulted, "Task #1 Faulted=" + task.Exception);

          this.output.WriteLine(
              "#2 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
              SynchronizationContext.Current, TaskScheduler.Current);
          Assert.Equal(scheduler,
                       TaskScheduler.Current); // "TaskScheduler.Current #2"
        });
        t0.Start(scheduler);
        return t1;
      });
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public void
    ActivationSched_SubTaskExecutionSequencing() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_ContinueWith_1_Test() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
      ...
      Task wrapper = new Task(() => {
        ...
        task4.ContinueWith(task => {
          ...
          Assert.False(task.IsFaulted,
                       "Faulted with Exception=" + task.Exception);
        });
      });
      ...
      try {
        ...
        Assert.True(false,
                    "Result did not arrive before timeout " + timeoutLimit);
      }
    ...
    ActivationSched_WhenAny() {
      ...
      Task wrapper = new Task(() => {
        task1 = Task<int>.Factory.StartNew(() => {
          ...
          Assert.Equal(
              scheduler,
              TaskScheduler
                  .Current); // "TaskScheduler.Current=" + TaskScheduler.Current
          pause1.WaitOne();
      ...
      pause1.Set();
      await join;
      Assert.True(join.IsCompleted && !join.IsFaulted,
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_WhenAny_Timeout() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_WhenAny_Busy_Timeout() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    ActivationSched_Task_Run() {
      ...
      Assert.True(task1.IsCompleted && !task1.IsFaulted,
                  "Task-1 Status " + task1);
      Assert.True(task2.IsCompleted && !task2.IsFaulted,
                  "Task-2 Status " + task2);
    }

    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_Task_Run_Delay() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    private async Task DoDelay(int i) {
      try {
        this.output.WriteLine(
            "Before Task.Delay #{0} TaskScheduler.Current={1}", i,
            TaskScheduler.Current);
        await Task.Delay(1);
        this.output.WriteLine("After Task.Delay #{0} TaskScheduler.Current={1}",
                              i, TaskScheduler.Current);
      } catch (ObjectDisposedException) {
        // Ignore any problems with ObjectDisposedException if console output
    ...
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_Turn_Execution_Order_Loop() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;

      const int NumChains = 100;
      const int ChainLength = 3;
      // Can we add a unit test that basicaly checks that any turn is indeed run
      // till completion before any other turn? For example, you have a long
      // running main turn and in the middle it spawns a lot of short CWs (on
      // Done promise) and StartNew. You test that no CW/StartNew runs until the
      // main turn is fully done. And run in stress.
      ...
      for (int i = 0; i < NumChains; i++) {
        ...
        taskChains[i] = new Task(() => {
          ...
          try {
            Assert.Equal(
                -1, executingGlobal); // "Detected unexpected other execution in
                                      // chain " + chainNum + " Task " + taskNum
            Assert.False(executingChain[chainNum],
                         "Detected unexpected other execution on chain " +
                             chainNum + " Task " + taskNum);
        ...
        for (int j = 1; j < ChainLength; j++) {
          ...
          task = task.ContinueWith(t => {
            if (t.IsFaulted)
              throw t.Exception;
            this.output.WriteLine("Inside Chain {0} Task {1}", chainNum,
                                  taskNum);
            try {
              Assert.Equal(
                  -1,
                  executingGlobal); // "Detected unexpected other execution in
                                    // chain " + chainNum + " Task " + taskNum
              Assert.False(executingChain[chainNum],
                           "Detected unexpected other execution on chain " +
                               chainNum + " Task " + taskNum);
              Assert.Equal(
                  taskNum - 1,
                  stageComplete[chainNum]); // "Detected unexpected execution
                                            // stage on chain " + chainNum + "
                                            // Task " + taskNum
      ...
      for (int i = 0; i < NumChains; i++) {
        TimeSpan waitCheckTime = TimeSpan.FromMilliseconds(
            150 * ChainLength * NumChains * WaitFactor);

        try {
          await resultHandles[i].Task.WithTimeout(waitCheckTime);
        } catch (TimeoutException) {
          Assert.True(false,
                      "Result did not arrive before timeout " + waitCheckTime);
        }

        bool ok = resultHandles[i].Task.Result;

        try {
          // since resultHandle being complete doesn't directly imply that the
          // final chain was completed (there's a chance for a race condition),
          // give a small chance for it to complete.
          await taskChainEnds[i].WithTimeout(TimeSpan.FromMilliseconds(10));
        } catch (TimeoutException) {
          Assert.True(
              false,
              $"Task chain end {i} should complete very shortly after after its resultHandle");
        }

        Assert.True(taskChainEnds[i].IsCompleted,
                    "Task chain " + i + " should be completed");
        Assert.False(taskChainEnds[i].IsFaulted,
                     "Task chain " + i + " should not be Faulted: " +
                         taskChainEnds[i].Exception);
        Assert.Equal(ChainLength - 1,
                     stageComplete[i]); // "Task chain " + i + " should have
                                        // completed all stages"
        Assert.True(ok,
                    "Successfully waited for ResultHandle for Task chain " + i);
      }
    }

    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    ActivationSched_Test1() {
      TaskScheduler scheduler =
          this.masterScheduler.GetWorkItemGroup(this.context).TaskScheduler;
    ...
    OrleansSched_Test1() {
      ...
      OrleansTaskScheduler orleansTaskScheduler =
          TestInternalHelper.InitializeSchedulerForTesting(context,
                                                           this.loggerFactory);
      ActivationTaskScheduler scheduler =
          orleansTaskScheduler.GetWorkItemGroup(context).TaskScheduler;

      await Run_ActivationSched_Test1(scheduler, false);
    }
    [Fact, TestCategory("Functional"), TestCategory("Scheduler")]
    public async Task
    OrleansSched_Test1_Bounce() {
      UnitTestSchedulingContext context = new UnitTestSchedulingContext();
      OrleansTaskScheduler orleansTaskScheduler =
          TestInternalHelper.InitializeSchedulerForTesting(context,
                                                           this.loggerFactory);
    ...
    internal async Task Run_ActivationSched_Test1(TaskScheduler scheduler,
      ...
      var grain = NonReentrentStressGrainWithoutState.Create(
          grainId,
          new GrainRuntime(Options.Create(new ClusterOptions()), silo, null,
                           null, null, null, null, NullLoggerFactory.Instance));

      await Task.Factory
          .StartNew(() => grain.OnActivateAsync(), CancellationToken.None,
      ...
      Task<Task>wrapper = new Task<Task>(() => {
        ...
        if (bounceToThreadPool) {
          wrapped =
              t1.ContinueWith(_ => wrappedDoneAction(), CancellationToken.None,
    ...
    private void LogContext(string what) {
      lock(Lockable) {
        this.output.WriteLine("{0}\n" + " TaskScheduler.Current={1}\n" +
                              ...
                              what,
                              (TaskScheduler.Current == null ? "null"
                               : TaskScheduler.Current.ToString()),
                              (Task.Factory.Scheduler == null ? "null"
                               : Task.Factory.Scheduler.ToString()),
                              (SynchronizationContext.Current == null ? "null"
