...
class Program {
  private static readonly Dictionary<string, Action>_benchmarks =
      new Dictionary<string, Action>{["MapReduce"] = () => {RunBenchmark(
                                         "Running MapReduce benchmark",
                                         () => {
                                           var mapReduceBenchmark =
                                               new MapReduceBenchmark();
...
, [ "Serialization" ] = () => {
  ...
  RunBenchmark("Running Transactions benchmark",
               () => {
  ...
  GC.Collect();
  {
    // All calls are cross-silo because the calling silo doesn't have any grain
    // classes.
    Console.WriteLine("## Silo to Silo ##");
    var test = new PingBenchmark(numSilos : 2, startClient
                                 : false, grainsOnSecondariesOnly
                                 : true);
    test.PingConcurrentHostedClient(blocksPerWorker : 10)
        .GetAwaiter()
        .GetResult();
    test.Shutdown().GetAwaiter().GetResult();
  }
}, [ "ConcurrentPing_OneSilo" ] = () => {
  new PingBenchmark(numSilos : 1, startClient
                    : true)
      .PingConcurrent()
      .GetAwaiter()
      .GetResult();
}, [ "ConcurrentPing_TwoSilos" ] = () => {
  new PingBenchmark(numSilos : 2, startClient
                    : true)
      .PingConcurrent()
      .GetAwaiter()
      .GetResult();
  ...
  new PingBenchmark(numSilos : 2, startClient
                    : false, grainsOnSecondariesOnly
                    : true)
      .PingConcurrentHostedClient(blocksPerWorker : 10)
      .GetAwaiter()
      .GetResult();
  ...
  new PingBenchmark(numSilos : 2, startClient
                    : false, grainsOnSecondariesOnly
                    : true)
      .PingConcurrentHostedClient(blocksPerWorker : 1000)
      .GetAwaiter()
      .GetResult();
}, [ "PingForever" ] = () => {
  ...
  RunBenchmark("Running grain storage benchmark against memory",
               () => {
  ...
  RunBenchmark("Running grain storage benchmark against Azure Table",
               () => {
                 var benchmark = new GrainStorageBenchmark();
  ...
  RunBenchmark("Running grain storage benchmark against Azure Blob",
               () => {
                 var benchmark = new GrainStorageBenchmark();
...
public static void Main(string[] args) {
  if (args.Length > 0 &&
      args[0].Equals("all", StringComparison.InvariantCultureIgnoreCase)) {
  ...
  if (args.Length == 0 || !_benchmarks.ContainsKey(args[0])) {
    ...
    _benchmarks.Select(pair => pair.Key).ToList().ForEach(Console.WriteLine);
...
private static void RunBenchmark<T>(string name, Func<T>init,
                                    Action<T>benchmarkAction,
                                    Action<T>tearDown) {
