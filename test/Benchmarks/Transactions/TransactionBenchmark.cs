...
namespace Benchmarks.Transactions {
  public class TransactionBenchmark {
    ...
    public TransactionBenchmark(int runs, int transactionsPerRun,
                                int concurrent) {
      this.runs = runs;
    ...
    public void MemoryThrottledSetup() {
      ...
      builder.AddSiloBuilderConfigurator<SiloTransactionConfigurator>();
      builder
          .AddSiloBuilderConfigurator<SiloTransactionThrottlingConfigurator>();
      this.host = builder.Build();
      this.host.Deploy();
    }
    ...
    private async Task FullRunAsync() {
      ...
      int transactionsPerRunner =
          Math.Max(1, this.transactionsPerRun / runners);
      Report[] reports = await Task.WhenAll(
          Enumerable.Range(0, runners)
              .Select(i => RunAsync(i, transactionsPerRunner, runners)));
      Report finalReport = new Report();
      foreach(Report report in reports) {
        finalReport.Succeeded += report.Succeeded;
        finalReport.Failed += report.Failed;
        finalReport.Throttled += report.Throttled;
        finalReport.Elapsed = TimeSpan.FromMilliseconds(
            Math.Max(finalReport.Elapsed.TotalMilliseconds,
                     report.Elapsed.TotalMilliseconds));
      }
      Console.WriteLine(
          $"{finalReport.Succeeded} transactions in {finalReport.Elapsed.TotalMilliseconds}ms.");
      Console.WriteLine(
          $"{(int)(finalReport.Succeeded * 1000 / finalReport.Elapsed.TotalMilliseconds)} transactions per second.");
      Console.WriteLine($"{finalReport.Failed} transactions failed.");
      Console.WriteLine(
          $"{finalReport.Throttled} transactions were throttled.");
    }

    public async Task<Report>RunAsync(int run, int transactiosPerRun,
                                      int concurrentPerRun) {
      ...
      public void Configure(ISiloBuilder hostBuilder) {
        hostBuilder.UseTransactions()
            ...
            .Configure<TelemetryOptions>(
                options => options.AddConsumer<TelemetryConsumer>())
  ...
  public class TelemetryConsumer : IMetricTelemetryConsumer {
    ...
    public void TrackMetric(string name, TimeSpan value,
                            IDictionary<string, string>properties = null) {
      TrackTrace(PrintProperties(name, value, properties));
    }
    ...
    PrintProperties<TValue>(string message, TValue value,
                            IDictionary<string, string>properties) {
    ...
    private static string
    PrintProperties(string message, IDictionary<string, string>properties) {
    ...
    AppendProperties(StringBuilder sb, IDictionary<string, string>properties) {
