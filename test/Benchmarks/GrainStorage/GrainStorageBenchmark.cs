...
namespace Benchmarks.GrainStorage {
  public class GrainStorageBenchmark {
    ...
    public async Task RunAsync() {
      ...
      Task[] tasks = {PersistLoop(isRunning),
    ...
    public async Task PersistLoop(Func<bool>running) {
      IPersistentGrain persistentGrain =
          this.host.Client.GetGrain<IPersistentGrain>(Guid.NewGuid());
      // activate grain
      ...
      while (running()) {
        ...
        maxCalltime =
            maxCalltime < calltime.Elapsed ? calltime.Elapsed : maxCalltime;
      }
      sw.Stop();
      Console.WriteLine(
          $"Performed {stored} persist operations with {failed} failures in {sw.ElapsedMilliseconds}ms.");
      Console.WriteLine(
          $"Average time in ms per call was {sw.ElapsedMilliseconds/stored}, with longest call taking {maxCalltime.TotalMilliseconds}ms.");
    }
