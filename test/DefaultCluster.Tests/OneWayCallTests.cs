...
namespace DefaultCluster.Tests.General {
  ...
  public class OneWayCallTests : HostedTestClusterEnsureDefaultStarted {
    public OneWayCallTests(DefaultClusterFixture fixture)
        : base(fixture){}
      ...
      Assert.True(task.Status == TaskStatus.RanToCompletion,
                  "Task should be synchronously completed.");
    ...
    [Fact]
    public async Task
    OneWayMethodReturnSynchronously_ViaGrain() {
      var grain = this.Client.GetGrain<IOneWayGrain>(Guid.NewGuid());
      ...
      var completedSynchronously =
          await grain.NotifyOtherGrain(otherGrain, observerReference);
    ...
    [Fact]
    public async Task
    MethodsInvokedThroughOneWayExtensionReturnSynchronously() {
      var grain = this.Client.GetGrain<ICanBeOneWayGrain>(Guid.NewGuid());

      var observer = new SimpleGrainObserver();
      var observerRef =
          await Client.CreateObjectReference<ISimpleGrainObserver>(observer);
      grain.InvokeOneWay(g => {
        Assert.False(
            object.ReferenceEquals(g, grain),
            "One way call should be executed on copy of grain reference");
    ...
    [Fact]
    public async Task
    OneWayMethodsReturnSynchronously_ViaClient_ValueTask() {
      var grain = this.Client.GetGrain<IOneWayGrain>(Guid.NewGuid());
      ...
      Assert.True(task.IsCompleted,
                  "ValueTask should be synchronously completed.");
      await observer.ReceivedValue.WithTimeout(TimeSpan.FromSeconds(10));
      var count = await grain.GetCount();
      Assert.Equal(1, count);
    ...
    [Fact]
    public async Task
    OneWayMethodReturnSynchronously_ViaGrain_ValueTask() {
      var grain = this.Client.GetGrain<IOneWayGrain>(Guid.NewGuid());
      var otherGrain = this.Client.GetGrain<IOneWayGrain>(Guid.NewGuid());
      ...
      var observerReference =
          await this.Client.CreateObjectReference<ISimpleGrainObserver>(
              observer);
      var completedSynchronously =
          await grain.NotifyOtherGrainValueTask(otherGrain, observerReference);
      Assert.True(completedSynchronously,
                  "Task should be synchronously completed.");
      await observer.ReceivedValue.WithTimeout(TimeSpan.FromSeconds(10));
      var count = await otherGrain.GetCount();
    ...
    [Fact]
    public async Task
    MethodsInvokedThroughOneWayExtensionReturnSynchronouslyValueTask() {
      var grain = this.Client.GetGrain<ICanBeOneWayGrain>(Guid.NewGuid());
