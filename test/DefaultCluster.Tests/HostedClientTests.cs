...
namespace DefaultCluster.Tests.General {
  ...
  public class HostedClientTests : IClassFixture<HostedClientTests.Fixture> {
    ...
    public class Fixture : IDisposable {
      ...
      public Fixture() {
        var (siloPort, gatewayPort) =
            TestClusterNetworkHelper.GetRandomAvailableServerPorts();
      ...
      public void Dispose() { this.Silo?.Dispose(); }
    }

    public HostedClientTests(Fixture fixture) { this.silo = fixture.Silo; }

    [Fact]
    public async Task
    HostedClient_GrainCallTest() {
    ...
    [Fact]
    public async Task
    HostedClient_ReferenceEquality_GrainCallTest() {
      var client = this.silo.Services.GetRequiredService<IClusterClient>();
    ...
    HostedClient_ObserverTest() {
      ...
      var observer = new ObserverTests.SimpleGrainObserver(
          ...
          client.ServiceProvider
              .GetRequiredService<ILogger<ISimpleGrainObserver>>());
      var reference =
    ...
    [Fact]
    public async Task
    HostedClient_StreamTest() {
      var client = this.silo.Services.GetRequiredService<IClusterClient>();

      var handle = new AsyncResultHandle();
      var vals = new List<int>();
      await client.GetStreamProvider("MemStream")
          .GetStream<int>(Guid.Empty, "hi")
          .SubscribeAsync((val, token) => {
            vals.Add(val);
            if (vals.Count >= 2)
              handle.Done = true;
            return Task.CompletedTask;
          });
      var stream = client.GetStreamProvider("MemStream")
                       .GetStream<int>(Guid.Empty, "hi");
