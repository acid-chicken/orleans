...
namespace UnitTests.MembershipTests {
  public class ClientIdPartitionDataRebuildTests : IDisposable {
    internal class Observer : ISimpleGrainObserver {
      ...
      public async Task WaitForNotification(int expectedA, int expectedB,
                                            TimeSpan timeout) {
        Assert.True(await this.semaphore.WaitAsync(timeout),
    ...
    [Fact(Skip = "Not reliable in PR build, skipping for now")]
        //[SkippableFact(typeof(SiloUnavailableException)),
        //TestCategory("Functional")]
        public async Task
        ReconstructClientIdPartitionTest_Observer() {
      // Ensure the client entry is on Silo2 partition and get a grain that live
      // on Silo3
      var grain = await SetupTestAndPickGrain<ISimpleObserverableGrain>(
          g => g.GetRuntimeInstanceId());
      var observer = new Observer();
      var reference =
          await this.hostedCluster.GrainFactory
      ...
      await this.hostedCluster.SecondarySilos[0].StopSiloAsync(stopGracefully
                                                               : false);
    ...
    [Fact(Skip = "Not reliable in PR build, skipping for now")]
        ...
        //TestCategory("Functional")]
        public async Task
        ReconstructClientIdPartitionTest_Request() {
      // Ensure the client entry is on Silo2 partition and get a grain that live
      // on Silo2
      var grain = await SetupTestAndPickGrain<ITestGrain>(
          g => g.GetRuntimeInstanceId());
    ...
    SetupTestAndPickGrain<T>(Func<T, Task<string>>getRuntimeInstanceId) where T
        : class
    , IGrainWithIntegerKey {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        var client = this.hostedCluster.ServiceProvider
                         .GetRequiredService<OutsideRuntimeClient>();
        clientId = client.CurrentActivationAddress.Grain;
        var report = await TestUtils.GetDetailedGrainReport(
            this.hostedCluster.InternalGrainFactory, clientId,
            hostedCluster.Primary);
        if (this.hostedCluster.SecondarySilos[0].SiloAddress.Equals(
                report.PrimaryForGrain)) {
          break;
        }
        clientId = null;
      ...
      for (var i = 0; i < 100; i++) {
        ...
        if (instanceId.Contains(hostedCluster.SecondarySilos[1]
                                    .SiloAddress.Endpoint.ToString())) {
          break;
        }
        grain = null;
      }
    ...
    public class ClientConfigurator : IClientBuilderConfigurator {
      public void Configure(IConfiguration configuration,
        ...
        clientBuilder.Configure<GatewayOptions>(
            options => options.PreferedGatewayIndex = 0);
      }
    }

    public void Dispose() { this.hostedCluster?.StopAllSilos(); }
  }
}
