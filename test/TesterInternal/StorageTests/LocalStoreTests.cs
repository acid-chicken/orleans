...
namespace UnitTests.StorageTests {
  ...
  public class LocalStoreTests {
    ...
    Store_Read() {
      ...
      output.WriteLine("{0} - Read time = {1}", store.GetType().FullName,
                       readTime);
    ...
    Store_WriteRead() {
      ...
      GrainReference reference =
          fixture.InternalGrainFactory.GetGrain(LegacyGrainId.NewId());
      ...
      var data = store.ReadRow(keys);
      TimeSpan readTime = sw.Elapsed;
      output.WriteLine("{0} - Write time = {1} Read time = {2}",
    ...
    Store_Delete() {
      ...
      output.WriteLine("Write returned Etag={0} after {1} {2}", eTag,
                       sw.Elapsed,
                       StorageProviderUtils.PrintOneWrite(keys, data, eTag));

      sw.Restart();
      var storedData = store.ReadRow(keys);
      output.WriteLine(
          "Read returned {0} after {1}",
          StorageProviderUtils.PrintOneWrite(keys, storedData, eTag),
          sw.Elapsed);
      ...
      Assert.True(
          ok,
          $"Row deleted OK after {sw.Elapsed}. Etag={eTag} Keys={StorageProviderUtils.PrintKeys(keys)}");

      sw.Restart();
      storedData = store.ReadRow(keys); // Try to re-read after delete
      output.WriteLine("Re-Read took {0} and returned {1}", sw.Elapsed,
                       StorageProviderUtils.PrintData(storedData));
      Assert.NotNull(data); // Should not get null data from Re-Read
      Assert.True(
          storedData.Count == 0,
          $"Should get no data from Re-Read but got: {StorageProviderUtils.PrintData(storedData)}");

      sw.Restart();
      const string oldEtag = null;
      eTag = store.WriteRow(keys, storedData, oldEtag);
      output.WriteLine(
          "Write for Keys={0} Etag={1} Data={2} returned New Etag={3} after {4}",
          StorageProviderUtils.PrintKeys(keys), oldEtag,
          StorageProviderUtils.PrintData(storedData), eTag, sw.Elapsed);
    ...
    Store_ReadMulti() {
      ...
      IList<Tuple<string, string>>keys =
          new[]{Tuple.Create("GrainType", name), Tuple.Create("GrainId", "1")}
              .ToList();
      ...
      // Write #2
      keys =
          new[]{Tuple.Create("GrainType", name), Tuple.Create("GrainId", "2")}
              .ToList();
      ...
      // Multi Read
      keys = new[]{Tuple.Create("GrainType", name)}.ToList();
    ...
    GrainState_Store_WriteRead() {
      ...
      sw.Start();
      IList<Tuple<string, string>>keys =
          new[]{Tuple.Create("GrainType", name),
                Tuple.Create("GrainId", ((LegacyGrainId) reference.GrainId)
                                            .GetPrimaryKey()
                                            .ToString("N"))}
              .ToList();
      store.WriteRow(keys, AsDictionary(state), grainState.ETag);
    ...
    private static IList<Tuple<string, string>>GetKeys(string grainTypeName,
                                                       GrainReference grain) {
      var keys = new[]{
          Tuple.Create("GrainType", grainTypeName),
          Tuple.Create(
              "GrainId",
              ((LegacyGrainId) grain.GrainId).GetPrimaryKey().ToString("N"))};
      return keys.ToList();
    }
    ...
    private static Dictionary<string, object>AsDictionary(object state) {
      return state.GetType()
          .GetProperties()
          .Select(
              v => new KeyValuePair<string, object>(v.Name, v.GetValue(state)))
