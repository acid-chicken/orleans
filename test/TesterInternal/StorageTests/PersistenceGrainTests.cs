...
public class PersistenceGrainTests_Local : OrleansTestingBase, IClassFixture<PersistenceGrainTests_Local.Fixture>, IDisposable
{
    ...
    public async Task Serialize_GrainState_DeepCopy_Stress()
    {
        ...
        for (int i = 0; i < loops; i++)
        {
            ...
            tasks.Add(Task.Run(() => {
                var copy = this.HostedCluster.SerializationManager.DeepCopy(states[idx]);
            }));
            tasks.Add(Task.Run(() => {
                var other = this.HostedCluster.SerializationManager.RoundTripSerializationForTesting(states[idx]);
            }));
        }
