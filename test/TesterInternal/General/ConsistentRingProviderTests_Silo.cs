...
namespace UnitTests.General {
  ...
  public void Configure(IConfiguration configuration,
    ...
    clientBuilder.Configure<GatewayOptions>(
        options => options.GatewayListRefreshPeriod =
...
private async Task FailureTest(Fail failCode, int numOfFailures) {
  ...
  {
    VerificationScenario(PickKey(
        fail.SiloAddress)); // fail.SiloAddress.GetConsistentHashCode());
  }

  logger.Info("FailureTest {0}, Code {1}, Stopping silos: {2}", numOfFailures,
              failCode,
              Utils.EnumerableToString(
                  failures, handle => handle.SiloAddress.ToString()));
  List<uint>keysToTest = new List<uint>();
  foreach(SiloHandle fail in failures) // verify before failure
  {
    keysToTest.Add(PickKey(
        fail.SiloAddress)); // fail.SiloAddress.GetConsistentHashCode());
...
private async Task JoinTest(int numOfJoins) {
  ...
  List<SiloHandle>silos =
      await this.HostedCluster.StartAdditionalSilosAsync(numOfJoins);
  await this.HostedCluster.WaitForLivenessToStabilizeAsync();
...
Ring_1F1J() {
  ...
  uint keyToCheck = PickKey(
      failures[0]
          .SiloAddress); // failures[0].SiloAddress.GetConsistentHashCode();
  ...
  var tasks =
      new Task[2]{Task.Factory.StartNew(
                      () => this.HostedCluster.StopSiloAsync(failures[0])),
                  this.HostedCluster.StartAdditionalSilosAsync(1).ContinueWith(
                      t => joins = t.GetAwaiter().GetResult())};
  ...
  AssertEventually(() => {
    ...
    VerificationScenario(
        PickKey(joins[0].SiloAddress)); // verify newly joined silo's key
  }, failureTimeout);
}

// failing the secondary in this scenario exposed the bug in DomainGrain ... so,
// we keep it as a separate test than Ring_1F1J
[Fact, TestCategory("Functional"), TestCategory("Ring")]
public async Task
Ring_1Fsec1J() {
  await this.HostedCluster.StartAdditionalSilosAsync(numAdditionalSilos);
  ...
  uint keyToCheck =
      PickKey(fail.SiloAddress); // fail.SiloAddress.GetConsistentHashCode();
  List<SiloHandle>joins = null;

  // kill a silo and join a new one in parallel
  logger.Info("Killing secondary silo {0} and joining a silo",
              fail.SiloAddress);
...
private uint PickKey(SiloAddress responsibleSilo) {
  ...
  for (int i = 0; i < iteration; i++) {
    ...
    SiloAddress s =
        testHooks.GetConsistentRingPrimaryTargetSilo(randomKey).Result;
    if (responsibleSilo.Equals(s))
      return randomKey;
  }
  throw new Exception(String.Format(
      "Could not pick a key that silo {0} will be responsible for. Primary.Ring = \n{1}",
      responsibleSilo,
...
private void VerifyKey(uint key, List<SiloAddress>silos) {
  var testHooks = this.Client.GetTestHooks(this.HostedCluster.Primary);
  SiloAddress truth =
      testHooks.GetConsistentRingPrimaryTargetSilo(key).Result; // expected;
  // if (truth == null) // if the truth isn't passed, we compute it here
  //{
  //    truth = silos.Find(siloAddr => (key <=
  //    siloAddr.GetConsistentHashCode())); if (truth == null)
  //    {
  //        truth = silos.First();
  //    }
  //}
  ...
  foreach(var siloHandle
              in this.HostedCluster.GetActiveSilos()) // do this for each silo
  {
    SiloAddress s =
        testHooks.GetConsistentRingPrimaryTargetSilo((uint) key).Result;
    Assert.Equal(truth, s);
  }
}

private async Task<List<SiloHandle>>getSilosToFail(Fail fail,
                                                   int numOfFailures) {
  List<SiloHandle>failures = new List<SiloHandle>();
  int count = 0, index = 0;

  // Figure out the primary directory partition and the silo hosting the
  // ReminderTableGrain.
  var tableGrain = this.GrainFactory.GetGrain<IReminderTableGrain>(
      InMemoryReminderTable.ReminderTableGrainId);
  ...
  SiloAddress reminderTableGrainPrimaryDirectoryAddress =
      (await TestUtils.GetDetailedGrainReport(
           this.HostedCluster.InternalGrainFactory, tableGrainId,
           this.HostedCluster.Primary))
          .PrimaryForGrain;
  // ask a detailed report from the directory partition owner, and get the
  // actionvation addresses
  var addresses = (await TestUtils.GetDetailedGrainReport(
                       this.HostedCluster.InternalGrainFactory, tableGrainId,
                       this.HostedCluster.GetSiloForAddress(
  ...
  foreach(var siloHandle in this.HostedCluster.GetActiveSilos()) {
    ...
    // Don't fail primary directory partition and the silo hosting the
    // ReminderTableGrain.
    if (siloAddress.Equals(reminderTableGrainPrimaryDirectoryAddress) ||
        siloAddress.Equals(reminderGrainActivation.Silo)) {
      continue;
    }
  ...
  // we can't guarantee semantics of 'Fail' if it evalutes to the primary's
  // address
  switch (fail) {
  case Fail.First:
    index = 0;
...
private void printSilos(string msg) {
  SortedList<int, SiloAddress>ids =
      new SortedList<int, SiloAddress>(numAdditionalSilos + 2);
  foreach(var siloHandle in this.HostedCluster.GetActiveSilos()) {
    ids.Add(siloHandle.SiloAddress.GetConsistentHashCode(),
            siloHandle.SiloAddress);
  }
