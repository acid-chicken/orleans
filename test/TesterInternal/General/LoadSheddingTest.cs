...
namespace UnitTests.General {
  ...
  public class LoadSheddingTest : OrleansTestingBase,
                                  IClassFixture<LoadSheddingTest.Fixture> {
    ...
    public async Task
    LoadSheddingBasic() {
      ISimpleGrain grain = this.fixture.GrainFactory.GetGrain<ISimpleGrain>(
          random.Next(), SimpleGrain.SimpleGrainNamePrefix);

      var latchPeriod = TimeSpan.FromSeconds(1);
      await this.HostedCluster.Client.GetTestHooks(this.HostedCluster.Primary)
          .LatchIsOverloaded(true, latchPeriod);
    ...
    LoadSheddingComplex() {
      ...
      await this.HostedCluster.Client.GetTestHooks(this.HostedCluster.Primary)
          .LatchIsOverloaded(false, latchPeriod);
