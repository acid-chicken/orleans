...
namespace UnitTests.Streaming {
  public class StreamProvidersTests_ProviderConfigNotLoaded
      : IClassFixture<StreamProvidersTests_ProviderConfigNotLoaded.Fixture> {
          ...
          hostBuilder.AddMemoryGrainStorage("MemoryStore")
              .ConfigureServices(services => {
                ...
                services.AddSingletonNamedService<
                    IGrainStorage, ErrorInjectionStorageProvider>(
                    "ErrorInjector");
                services.AddSingletonNamedService<
                    IControllable, ErrorInjectionStorageProvider>(
                    "ErrorInjector");
              });
        }
      }
    ...
    public static readonly string STREAM_PROVIDER_NAME =
    ...
    public StreamProvidersTests_ProviderConfigNotLoaded(
        ITestOutputHelper output, Fixture fixture) {
    ...
    ProvidersTests_ConfigNotLoaded() {
      ...
      IStreaming_ConsumerGrain consumer =
          this.HostedCluster.GrainFactory.GetGrain<IStreaming_ConsumerGrain>(
              Guid.NewGuid(), grainFullName);
      await Assert.ThrowsAsync<KeyNotFoundException>(
          () => consumer.BecomeConsumer(streamId, STREAM_PROVIDER_NAME, null));
    }

    [Fact, TestCategory("Functional"), TestCategory("Config"),
     TestCategory("ServiceId"), TestCategory("Providers")]
    public async Task
    ServiceId_ProviderRuntime() {
      var thisRunServiceId = this.fixture.ServiceId;

      SiloHandle siloHandle = this.HostedCluster.GetActiveSilos().First();
      var serviceId =
          await this.fixture.Client.GetTestHooks(siloHandle).GetServiceId();
      Assert.Equal(thisRunServiceId, serviceId); // "ServiceId active in silo"
    }

    [Fact, TestCategory("Functional"), TestCategory("Config"),
     TestCategory("ServiceId")]
    public async Task
    ServiceId_SiloRestart() {
      var configServiceId = this.fixture.GetClientServiceId();
      output.WriteLine("ServiceId={0}", this.fixture.ServiceId);

      Assert.Equal(this.fixture.ServiceId,
                   configServiceId); // "ServiceId in test config"
      ...
      foreach(var siloHandle in activeSilos) {
        await AsyncExecutorWithRetries.ExecuteWithRetries(
            async _ => {
              var serviceId = await this.fixture.Client.GetTestHooks(siloHandle)
                                  .GetServiceId();
              Assert.Equal(this.fixture.ServiceId,
                           serviceId); // "ServiceId active in silo"
            },
            30, (ex, i) => ex is OrleansException, TimeSpan.FromSeconds(60),
            new FixedBackoff(TimeSpan.FromSeconds(2)));
      }
    }
  }

  public class StreamProvidersTests_ProviderConfigLoaded
      : OrleansTestingBase,
        IClassFixture<StreamProvidersTests_ProviderConfigLoaded.Fixture> {
    public class Fixture : BaseTestClusterFixture {
      protected override void ConfigureTestCluster(TestClusterBuilder builder) {
        builder.Options.InitialSilosCount = 4;
    ...
    [Fact, TestCategory("Functional"), TestCategory("Streaming"),
     TestCategory("Providers")]
    public async Task
    ProvidersTests_ProviderWrongName() {
      Guid streamId = Guid.NewGuid();
      var grainFullName = typeof (Streaming_ConsumerGrain).FullName;
      // consumer joins first, producer later
      IStreaming_ConsumerGrain consumer =
          this.grainFactory.GetGrain<IStreaming_ConsumerGrain>(Guid.NewGuid(),
                                                               grainFullName);
      await Assert.ThrowsAsync<KeyNotFoundException>(
          () => consumer.BecomeConsumer(streamId, "WrongProviderName", null));
    }
  }
}
