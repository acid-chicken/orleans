...
namespace UnitTests.ActivationsLifeCycleTests {
  ...
  public class DeactivateOnIdleTests : OrleansTestingBase,
                                       IDisposable {
    ...
    public async Task
    DeactivateOnIdleTestInside_Basic() {
      Initialize();
      ...
      Thread.Sleep(5000);
      var age = a.GetOtherAge().WaitForResultWithThrow(
          TimeSpan.FromMilliseconds(2000));
      Assert.True(age.TotalMilliseconds < 2000,
                  "Should be newly activated grain");
    }

    [Fact, TestCategory("SlowBVT"), TestCategory("Functional")]
    public async Task
    DeactivateOnIdleTest_Stress_1() {
      Initialize();
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    DeactivateOnIdleTest_Stress_2_NonReentrant() {
      Initialize();
      var a = this.testCluster.GrainFactory.GetGrain<ICollectionTestGrain>(
          1, "UnitTests.Grains.CollectionTestGrain");
      await a.IncrCounter();
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    DeactivateOnIdleTest_Stress_3_Reentrant() {
      Initialize();
      var a = this.testCluster.GrainFactory.GetGrain<ICollectionTestGrain>(
          1, "UnitTests.Grains.ReentrantCollectionTestGrain");
      await a.IncrCounter();
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    DeactivateOnIdleTest_Stress_4_Timer() {
      Initialize();
      var a = this.testCluster.GrainFactory.GetGrain<ICollectionTestGrain>(
          1, "UnitTests.Grains.ReentrantCollectionTestGrain");
      for (int i = 0; i < 10; i++) {
        await a.StartTimer(TimeSpan.FromMilliseconds(5),
    ...
    public async Task
    DeactivateOnIdleTest_Stress_11() {
      Initialize();
      var a = this.testCluster.GrainFactory.GetGrain<ICollectionTestGrain>(1);
      List<Task>tasks = new List<Task>();
      for (int i = 0; i < 100; i++) {
    ...
    [Fact, TestCategory("Functional")]
    public async Task
    DeactivateOnIdle_NonExistentActivation_1() {
      await DeactivateOnIdle_NonExistentActivation_Runner(0);
    }

    [Fact, TestCategory("Functional")]
    public async Task
    DeactivateOnIdle_NonExistentActivation_2() {
    ...
    public class ClientConfigurator : IClientBuilderConfigurator {
      public void Configure(IConfiguration configuration,
        ...
        clientBuilder.Configure<StaticGatewayListProviderOptions>(options => {
          options.Gateways = options.Gateways.Take(1).ToList();
        });
      }
    }
    ...
    public class SiloConfigurator : ISiloConfigurator {
      public void Configure(ISiloBuilder hostBuilder) {
        ...
        hostBuilder.Configure<SiloMessagingOptions>(
            options => options.MaxForwardCount = maxForwardCount);
      }
    }

    private async Task
    DeactivateOnIdle_NonExistentActivation_Runner(int forwardCount) {
    ...
    private async Task<ICollectionTestGrain>PickGrainInNonPrimary() {
      for (int i = 0; i < 500; i++) {
        if (i % 30 == 29)
          await Task.Delay(1000); // give some extra time to stabilize if it
                                  // can't find a suitable grain

        // Create grain such that:
        // Its directory owner is not the Gateway silo. This way Gateway will
        // use its directory cache. Its activation is located on the non Gateway
        // silo as well.
        ICollectionTestGrain grain =
            this.testCluster.GrainFactory.GetGrain<ICollectionTestGrain>(i);
        GrainId grainId =
            ((GrainReference) await grain.GetGrainReference()).GrainId;
        SiloAddress primaryForGrain =
            (await TestUtils.GetDetailedGrainReport(
                 this.testCluster.InternalGrainFactory, grainId,
                 this.testCluster.Primary))
                .PrimaryForGrain;
        ...
        if (this.testCluster.Primary.SiloAddress.ToLongString().Equals(
                siloHostingActivation)) {
          continue;
        }
      ...
      Assert.True(testCluster.GetActiveSilos().Count() > 1,
                  "This logic requires at least 1 non-primary active silo");
      Assert.True(
          false,
          "Could not find a grain that activates on a non-primary silo, and has the partition be also managed by a non-primary silo");
      return null;
    }

    [Fact, TestCategory("Functional")]
    public async Task
    MissingActivation_WithoutDirectoryLazyDeregistration_MultiSilo() {
    ...
    public class NoForwardingSiloConfigurator
        : ISiloConfigurator{public void Configure(ISiloBuilder hostBuilder){
              ...
              hostBuilder.Configure<SiloMessagingOptions>(
                  options => options.MaxForwardCount = 0);
  }
}

[Fact, TestCategory("Functional")]
public async Task
MissingActivation_WithDirectoryLazyDeregistration_SingleSilo() {
...
public class LazyDeregistrationDelaySiloConfigurator
    : ISiloConfigurator{public void Configure(ISiloBuilder hostBuilder){
          hostBuilder.Configure<GrainDirectoryOptions>(
              options => options.LazyDeregistrationDelay =
...
public async Task
MissingActivation_WithoutDirectoryLazyDeregistration_MultiSilo_SecondaryFirst() {
...
private async Task
MissingActivation_Runner(int grainId, TimeSpan lazyDeregistrationDelay,
  ...
  var isMultipleSilosPresent = testCluster.SecondarySilos != null &&
                               testCluster.SecondarySilos.Count > 0;
  ...
  var secondarySiloAddress =
      isMultipleSilosPresent
                                   ? testCluster.SecondarySilos[0]
  ...
  if (isMultipleSilosPresent && forceCreationInSecondary) {
    //
    // Make sure that we proceeding with a grain which is created in the
    // secondary silo for first!
    //
  ...
  // Now we know that there's an activation; try both silos and deactivate it
  // incorrectly
  int primaryActivation =
      await this.testCluster.Client.GetTestHooks(testCluster.Primary)
          .UnregisterGrainForTesting(grainReference);
  int secondaryActivation = 0;

  if (isMultipleSilosPresent) {
    secondaryActivation = await this.testCluster.Client
                              .GetTestHooks(testCluster.SecondarySilos[0])
                              .UnregisterGrainForTesting(grainReference);
  }
  ...
  if (lazyDeregistrationDelay > TimeSpan.Zero) {
    ...
    output.WriteLine(
        $"Pausing for {0} because we are using lazy deregistration", pause);
    await Task.Delay(pause);
  }

  // Now send a message again; it should fail);
  var firstEx = await Assert.ThrowsAsync<OrleansMessageRejectionException>(
      () => grain.GetLabel());
  ...
  if (lazyDeregistrationDelay > TimeSpan.Zero || forceCreationInSecondary) {
    ...
    // Since a new instance returned, we've to check that the label is no more
    // prefixed with "hello_"
    ...
    if (forceCreationInSecondary) {
      ...
      output.WriteLine(grainSiloAddress == primarySiloAddress ?
                           "Recreated in Primary"
                       : "Recreated in Secondary");
      output.WriteLine(grainSiloAddress == primarySiloAddress ?
                           "Recreated in Primary"
                       : "Recreated in Secondary");
    }
  } else {
    var secondEx = await Assert.ThrowsAsync<OrleansMessageRejectionException>(
        () => grain.GetLabel());
    output.WriteLine("Got 2nd exception - " +
                     secondEx.GetBaseException().Message);
    Assert.True(secondEx.Message.Contains("duplicate activation") ||
