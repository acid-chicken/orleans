...
namespace UnitTests.General {
  public class DeadlockDetectionWithoutAllowCallChainReentrancyTests
      : OrleansTestingBase,
        IClassFixture<
            DeadlockDetectionWithoutAllowCallChainReentrancyTests.Fixture> {
    ...
    public DeadlockDetectionWithoutAllowCallChainReentrancyTests(
        Fixture fixture) {
      this.fixture = fixture;
    }

    // 2 silos, loop across all cases (to force all grains to be local and
    // remote):
    //      Non Reentrant A, B
    //      Reentrant C
    ...
    DeadlockDetection_1() {
      ...
      for (int i = 0; i < numIterations; i++) {
        ...
        IDeadlockNonReentrantGrain firstGrain =
            this.fixture.GrainFactory.GetGrain<IDeadlockNonReentrantGrain>(
                grainId);
        List<Tuple<long, bool>>callChain = new List<Tuple<long, bool>>();
        callChain.Add(new Tuple<long, bool>(grainId, true));
        callChain.Add(new Tuple<long, bool>(grainId, true));
        await Assert.ThrowsAsync<DeadlockException>(
            () => firstGrain.CallNext_1(callChain, 1));
      }
    }
    ...
    DeadlockDetection_3() {
      ...
      for (int i = 0; i < numIterations; i++) {
        long grainId = baseGrainId + i;
        IDeadlockReentrantGrain firstGrain =
            this.fixture.GrainFactory.GetGrain<IDeadlockReentrantGrain>(
                grainId);
        List<Tuple<long, bool>>callChain = new List<Tuple<long, bool>>();
