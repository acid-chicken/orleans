...
namespace UnitTests.GeoClusterTests {
  ...
  public class MultiClusterRegistrationTests : TestingClusterHost {
    ...
    private IEnumerable<KeyValuePair<string, ClientWrapper>>EnumerateClients() {
      for (int i = 0; i < ClusterNames.Length; i++)
        foreach(var c in Clients[i])
            yield return new KeyValuePair<string, ClientWrapper>(
                ClusterNames[i], c);
    }

    public MultiClusterRegistrationTests(ITestOutputHelper output)
        : base(output){}
      ...
      testtasks.Add(
          RunWithTimeout("ManyParallelCalls", 10000, ManyParallelCalls));
      testtasks.Add(RunWithTimeout("ObserverBasedClientNotification", 10000,
                                   ObserverBasedClientNotification));
      testtasks.Add(RunWithTimeout("StreamBasedClientNotification", 10000,
                                   StreamBasedClientNotification));

      foreach(var t in testtasks) await t;
    }
    ...
    FourClusterBattery() {
      ...
      await RunWithTimeout("Start Clusters and Clients", 180 * 1000,
                           () => StartClustersAndClients(2, 2, 1, 1));
      ...
      for (int i = 0; i < 20; i++) {
        testtasks.Add(RunWithTimeout("Deact", 20000, Deact));
        testtasks.Add(
            RunWithTimeout("SequentialCalls", 10000, SequentialCalls));
        testtasks.Add(RunWithTimeout("ParallelCalls", 10000, ParallelCalls));
        testtasks.Add(
            RunWithTimeout("ManyParallelCalls", 10000, ManyParallelCalls));
        testtasks.Add(RunWithTimeout("ObserverBasedClientNotification", 10000,
    ...
    private Task
    StartClustersAndClients(Action<TestClusterBuilder>configureTestCluster,
                            params short[] silos) {
      ...
      for (int i = 0; i < silos.Length; i++) {
        ...
        NewGeoCluster<SiloConfigurator>(globalserviceid, clustername, silos[i],
                                        configureTestCluster);
        // create one client per silo
        Parallel.For(
            0, numsilos, paralleloptions,
            (j) => c[j] = this.NewClient(
                clustername, j, ClientWrapper.Factory,
                clientBuilder => clientBuilder.AddSimpleMessageStreamProvider(
                    "SMSProvider")));
      }
      ...
      Clients [0]
      [0]
          .InjectClusterConfiguration(ClusterNames);
      WaitForMultiClusterGossipToStabilizeAsync(false).WaitWithThrow(
          TimeSpan.FromMinutes(System.Diagnostics.Debugger.IsAttached ? 60 : 1));
    ...
    public class ClientWrapper : ClientWrapperBase {
      public static readonly Func<string, int, string, Action<IClientBuilder>,
          ...
          (name, gwPort, clusterId, clientConfigurator) =>
              new ClientWrapper(name, gwPort, clusterId, clientConfigurator);
      ...
      public void Subscribe(int i, IClusterTestListener listener) {
        ...
        var obj = this.GrainFactory
                      .CreateObjectReference<IClusterTestListener>(listener)
                      .Result;
        listeners.Add(obj);
    ...
    public class ClusterTestListener : IClusterTestListener,
                                       IAsyncObserver<int> {
      public ClusterTestListener(Action<int>oncall) { this.oncall = oncall; }
    ...
    private int Next() { lock(random) return random.Next(); }
    private int[] Next(int count) {
      var result = new int[count];
      lock(random) for (int i = 0; i < count; i++) result[i] = random.Next();
      return result;
    }
    ...
    private async Task ManyParallelCalls() {
      ...
      Parallel.For(0, 20, paralleloptions,
                   i => clients[i % clients.Count].CallGrain(x));
    ...
    private async Task Deact() {
      ...
      Assert.Contains(Clusters[ClusterNames[0]].Silos,
                      silo => silo.SiloAddress.ToString() == id);
      ...
      Assert.Contains(Clusters[ClusterNames[1]].Silos,
                      silo => silo.SiloAddress.ToString() == newid);

      WriteLog("Grain {0} Check that other clusters find new activation.",
               gref);
    ...
    private async Task ObserverBasedClientNotification() {
      ...
      Parallel.For(0, Clients.Length, paralleloptions, i => {
        for (int jj = 0; jj < Clients[i].Length; jj++) {
          ...
          var listener = new ClusterTestListener((num) => {
            WriteLog("{3} observedcall {2} on Client[{0}][{1}]", i, j, num,
                     gref);
            promise.TrySetResult(num);
          });
          lock(promises) promises.Add(promise.Task);
          Clients [i]
          [j]
              .Subscribe(x, listener);
      ...
      var sortedresults =
          promises.Select(p => p.Result).OrderBy(num => num).ToArray();
    ...
    private async Task StreamBasedClientNotification() {
      ...
      foreach(var p in promises) AssertEqual(1, p.Result, gref);
    }
    ...
    BlockedDeact() {
      ...
      await RunWithTimeout("BlockedDeact", 10 * 1000, async () => {
        ...
        // deactivate grain in cluster A, but block deactivation message to
        // cluster B
        ...
        var newid = Clients [1]
                    [0]
                        .GetRuntimeId(x);
        WriteLog("{2} sees Grain {0} at {1}", gref, newid, ClusterNames[1]);
        Assert.Contains(newid, Clusters[ClusterNames[1]].Silos.Select(
                                   s => s.SiloAddress.ToString()));
    ...
    CacheCleanup() {
      ...
      await RunWithTimeout("CacheCleanup", 1000000, async () => {
        ...
        // causing entry to be installed in B's directory and/or B's directory
        // caches
        var id2 = Clients [1]
                  [0]
                      .GetRuntimeId(x);
        ...
        Clients [1]
        [0]
            .InjectClusterConfiguration(new string[]{ClusterNames[1]},
                                        "exclude A", false);
        WaitForMultiClusterGossipToStabilizeAsync(false).WaitWithThrow(
            TimeSpan.FromMinutes(
                System.Diagnostics.Debugger.IsAttached ? 60 : 1));

        // give the cache cleanup process time to remove all cached references
        // in B
        await Task.Delay(40000);

        // now try to access the grain from cluster B
        // if everything works correctly this should succeed, creating a new
        // instances on B locally (if invalid caches were not removed this times
        // out)
    ...
    CacheCleanupMultiple() {
      ...
      await RunWithTimeout("CacheCleanupMultiple", 1000000, async () => {
        ...
        for (int i = 0; i < count; i++)
          Assert.Contains(Clusters[ClusterNames[0]].Silos,
                          silo => silo.SiloAddress.ToString() == ids[i]);

        // access all the grains living in A from all the clients of B
        // causing  entries to be installed in B's directory and in B's
        // directory caches
        for (int j = 0; j < 3; j++) {
          var ids2 = grains
                         .Select((x) => Clients [1]
        ...
        // if everything works correctly this should succeed, creating new
        // instances on B locally (if invalid caches were not removed this times
        // out)
        var vals = grains
                       .Select((x) => Clients [1]
        ...
        var newids = grains
                         ...
                         .ToList();
        for (int i = 0; i < count; i++)
          Assert.Contains(newids[i], Clusters[ClusterNames[1]].Silos.Select(
                                         s => s.SiloAddress.ToString()));
