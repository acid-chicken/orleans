...
namespace Tests.GeoClusterTests {
  /// <summary>
  ///  A fixture that provides a collection of semantic tests for
  ///  log-consistency providers (concurrent reading and updating, update
  ///  propagation, conflict resolution) on a multicluster with the desired
  ///  number of clusters
  /// </summary>
  public class LogConsistencyTestFixture : IDisposable {
    ...
    public TestingClusterHost MultiCluster {
      get {
        return _hostedMultiCluster ??(_hostedMultiCluster =
                                         new TestingClusterHost());
      }
    }

    public void EnsurePreconditionsMet() { TestUtils.CheckForAzureStorage(); }

    public class ClientWrapper
        : Tests.GeoClusterTests.TestingClusterHost.ClientWrapperBase {
      ...
      public string GetGrainRef(string grainclass, int i) {
        return this.GrainFactory
            .GetGrain<UnitTests.GrainInterfaces.ILogTestGrain>(i, grainclass)
            .ToString();
      }

      public void SetALocal(string grainclass, int i, int a) {
        var grainRef =
            this.GrainFactory.GetGrain<UnitTests.GrainInterfaces.ILogTestGrain>(
                i, grainclass);
      ...
      public IEnumerable<ConnectionIssue>
      GetUnresolvedConnectionIssues(string grainclass, int i) {
        var grainRef =
            this.GrainFactory.GetGrain<UnitTests.GrainInterfaces.ILogTestGrain>(
                i, grainclass);
    ...
    public void StartClustersIfNeeded(int numclusters,
      ...
      if (MultiCluster.Clusters.Count != numclusters) {
        ...
        for (int i = 0; i < numclusters; i++) {
          ...
          MultiCluster.NewGeoCluster<LogConsistencyProviderSiloConfigurator>(
              globalserviceid, clustername, 1);
          Client[i] = this.MultiCluster.NewClient(clustername, 0,
                                                  ClientWrapper.Factory);
        }
        ...
        MultiCluster.WaitForLivenessToStabilizeAsync().WaitWithThrow(
            TimeSpan.FromMinutes(1));

        Client[0].InjectClusterConfiguration(Cluster);
        MultiCluster.WaitForMultiClusterGossipToStabilizeAsync(false)
            .WaitWithThrow(TimeSpan.FromMinutes(
                System.Diagnostics.Debugger.IsAttached ? 60 : 1));

        stopwatch.Stop();
        output.WriteLine("Multicluster is ready (elapsed = {0}).",
                         stopwatch.Elapsed);
      } else {
        output.WriteLine("Reusing existing {0} clusters and clients.",
                         numclusters);
      }
    }
    ...
    public virtual void Dispose() { _hostedMultiCluster?.Dispose(); }
    ...
    private void AssertEqual<T>(T expected, T actual, string grainIdentity) {
      if (!expected.Equals(actual)) {
        // need to write grain identity to output so we can search for it in the
        // trace
    ...
    public async Task RunChecksOnGrainClass(string grainClass,
                                            bool may_update_in_all_clusters,
                                            int phases,
                                            ITestOutputHelper output) {
      ...
      Func<Task>checker1 = () => Task.Run(() => {
        int x = GetRandom();
        var grainIdentity =
            string.Format("grainref={0}", Client[0].GetGrainRef(grainClass, x));
        ...
        // check versions
        for (int i = 0; i < numclusters; i++)
          AssertEqual(1, Client[i].GetConfirmedVersion(grainClass, x),
                      grainIdentity);
      });
      ...
      Func<int, Task>checker3 = (int numupdates) => Task.Run(() => {
        ...
        // push & get all
        AssertEqual(numupdates, Client[0].GetAGlobal(grainClass, x),
                    grainIdentity);

        for (int i = 1; i < numclusters; i++)
          AssertEqual(numupdates, Client[i].GetAGlobal(grainClass, x),
                      grainIdentity); // get all

        // check versions
        for (int i = 0; i < numclusters; i++)
          AssertEqual(numupdates, Client[i].GetConfirmedVersion(grainClass, x),
                      grainIdentity);
      });
      ...
      Func<Task>checker4 = () => Task.Run(() => {
        ...
        for (int i = 0; i < numclusters; i++) {
          int c = i;
          t.Add(Task.Run(
              () => AssertEqual(true, Client[c].GetALocal(grainClass, x) == 0,
                                grainIdentity)));
        }
        for (int i = 0; i < numclusters; i++) {
          int c = i;
          t.Add(Task.Run(
              () => AssertEqual(true, Client[c].GetAGlobal(grainClass, x) == 0,
                                grainIdentity)));
        }
      ...
      Func<int, Task>checker6 = async (int preload) => {
        ...
        await Task.WhenAny(Task.Delay(20000), Task.WhenAll(t));

        Assert.True(done.All(b => b),
                    string.Format(
                        "checker6({0}): update did not propagate within 20 sec",
                        preload));
      };
      ...
      Func<int, Task>checker7 = (int variation) => Task.Run(async () => {
        ...
        // write conditional on client 0, should always succeed
        {
          ...
          AssertEqual(1, Client[0].GetConfirmedVersion(grainClass, x),
                      grainIdentity);
        }
        ...
        // write conditional on Client[1], may or may not succeed based on
        // timing
        {
          ...
          if (result.Item1 == 0) // was stale, thus failed
          {
            ...
            AssertEqual(1, Client[1].GetConfirmedVersion(grainClass, x),
                        grainIdentity);
            // check stability
            ...
            AssertEqual(Xyz, Client[0].GetAGlobal(grainClass, x),
                        grainIdentity);
            AssertEqual(Xyz, Client[1].GetAGlobal(grainClass, x),
                        grainIdentity);
          } else // was up-to-date, thus succeeded
          {
            ...
            // version is now 2
            AssertEqual(2, Client[1].GetConfirmedVersion(grainClass, x),
                        grainIdentity);
            // check stability
            AssertEqual(444, Client[1].GetALocal(grainClass, x), grainIdentity);
            AssertEqual(444, Client[0].GetAGlobal(grainClass, x),
                        grainIdentity);
            AssertEqual(444, Client[1].GetAGlobal(grainClass, x),
                        grainIdentity);
          }
        }
      });
      ...
      if (may_update_in_all_clusters) {
        ...
        MultiCluster.SetProtocolMessageFilterForTesting(
            Cluster[0], msg => !(msg is INotificationMessage));
        await checker7(0);
        await checker7(1);
        await checker7(2);
        await checker7(3);
  ...
  internal class LogConsistencyProviderSiloConfigurator : ISiloConfigurator {
    public void Configure(ISiloBuilder hostBuilder) {
      ...
      hostBuilder.AddCustomStorageBasedLogConsistencyProvider(
          "CustomStoragePrimaryCluster", "A");
    }
  }
}
#endif
