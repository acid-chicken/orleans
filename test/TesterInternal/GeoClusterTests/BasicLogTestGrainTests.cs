...
namespace Tests.GeoClusterTests {
  ...
  public class BasicLogTestGrainTests
      : IClassFixture<BasicLogTestGrainTests.Fixture> {
    ...
    public class Fixture : BaseAzureTestClusterFixture {
      ...
      private class SiloBuilderConfigurator : ISiloConfigurator {
        public void Configure(ISiloBuilder hostBuilder) {
          hostBuilder.AddStateStorageBasedLogConsistencyProvider()
    ...
    private int GetRandom() { lock(random) return random.Next(); }

    private async Task DoBasicLogTestGrainTest(string grainClass,
                                               int phases = 100) {
    ...
    private async Task ThreeCheckers(string grainClass, int phases) {
      ...
      Func<Task>checker1 = async () => {
        int x = GetRandom();
        var grain =
            this.fixture.GrainFactory.GetGrain<ILogTestGrain>(x, grainClass);
