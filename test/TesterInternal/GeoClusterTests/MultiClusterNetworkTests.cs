...
namespace Tests.GeoClusterTests {
  ...
  public class MultiClusterNetworkTests : TestingClusterHost {
    ...
    public class ClientWrapper : ClientWrapperBase {
      ...
      public MultiClusterConfiguration
      InjectMultiClusterConf(params string[] clusters) {
        return systemManagement.InjectMultiClusterConfiguration(clusters)
            .GetResult();
      }
    ...
    TestMultiClusterConf_1_1() {
      ...
      var clientA =
          this.NewClient<ClientWrapper>(clusterA, 0, ClientWrapper.Factory);
      ...
      var clientB =
          NewClient<ClientWrapper>(clusterB, 0, ClientWrapper.Factory);
      ...
      Assert.Equal(2, gateways.Count); // "Expect 2 gateways"
      var activegateways =
          gateways.Where(g => g.Status == GatewayStatus.Active).ToList();
      Assert.Single(activegateways); // "Expect 1 active gateway"
      Assert.Equal("A", activegateways[0].ClusterId);
    }

    private void AssertSameList(List<IMultiClusterGatewayInfo>a,
                                List<IMultiClusterGatewayInfo>b) {
      Comparison<IMultiClusterGatewayInfo>comparer = (x, y) =>
          x.SiloAddress.Endpoint.ToString().CompareTo(
              y.SiloAddress.Endpoint.ToString());
      a.Sort(comparer);
      b.Sort(comparer);
      ...
      for (int i = 0; i < a.Count; i++) {
        Assert.Equal(
            a[i].SiloAddress,
            b[i].SiloAddress); // "silo address at pos " + i + " must match"
        Assert.Equal(
            a[i].ClusterId,
            b[i].ClusterId); // "cluster id at pos " + i + " must match"
        Assert.Equal(a[i].Status,
                     b[i].Status); // "status at pos " + i + " must match"
      }
    }

    public class TwoClusterSiloConfigurator
...
[SkippableFact(Skip = "https://github.com/dotnet/orleans/issues/3929"),
 TestCategory("Functional")]
public async Task
TestMultiClusterConf_3_3() {
  // use a random global service id for testing purposes
  var globalserviceid = Guid.NewGuid();
  ...
  var clientA =
      this.NewClient<ClientWrapper>(clusterA, 0, ClientWrapper.Factory);
  var portsA = Clusters[clusterA]
                   .Cluster.GetActiveSilos()
                   .OrderBy(x => x.SiloAddress)
                   .Select(x => x.SiloAddress.Endpoint.Port)
                   .ToArray();

  // create cluster B and clientB
  NewGeoCluster<TwoClusterSiloConfigurator>(globalserviceid, clusterB, 3);
  var clientB =
      this.NewClient<ClientWrapper>(clusterB, 0, ClientWrapper.Factory);
  var portsB = Clusters[clusterB]
                   .Cluster.GetActiveSilos()
                   .OrderBy(x => x.SiloAddress)
                   .Select(x => x.SiloAddress.Endpoint.Port)
                   .ToArray();
  ...
  var cur = clientA.GetMultiClusterConfiguration();
  Assert.True(cur != null && string.Join(",", cur.Clusters) ==
                                 string.Join(",", clusterA, clusterB));
  AssertSameList(clientA.GetMultiClusterGateways(),
                 clientB.GetMultiClusterGateways());

  // expect 4 active gateways, two per cluster
  var activegateways = clientA.GetMultiClusterGateways()
                           .Where(g => g.Status == GatewayStatus.Active)
                           .ToList();
  Assert.Equal(
      string.Join(",", portsA[0], portsA[1]),
      string.Join(",", activegateways.Where(g => g.ClusterId == clusterA)
                           .Select(g => g.SiloAddress.Endpoint.Port)
                           .OrderBy(x => x)));
  Assert.Equal(
      string.Join(",", portsB[0], portsB[1]),
      string.Join(",", activegateways.Where(g => g.ClusterId == clusterB)
                           .Select(g => g.SiloAddress.Endpoint.Port)
                           .OrderBy(x => x)));
  var activegatewaysB = clientB.GetMultiClusterGateways()
                            .Where(g => g.Status == GatewayStatus.Active)
                            .ToList();

  // shut down one of the gateways in cluster B gracefully
  var target = Clusters[clusterB]
                   .Cluster.GetActiveSilos()
                   .Where(h => h.SiloAddress.Endpoint.Port == portsB[1])
                   .FirstOrDefault();
  ...
  AssertSameList(clientA.GetMultiClusterGateways(),
                 clientB.GetMultiClusterGateways());
  activegateways = clientA.GetMultiClusterGateways()
                       .Where(g => g.Status == GatewayStatus.Active)
                       .ToList();
  Assert.Equal(
      string.Join(",", portsA[0], portsA[1]),
      string.Join(",", activegateways.Where(g => g.ClusterId == clusterA)
                           ...
                           .OrderBy(x => x)));
  Assert.Equal(
      string.Join(",", portsB[0], portsB[2]),
      string.Join(",", activegateways.Where(g => g.ClusterId == clusterB)
                           .Select(g => g.SiloAddress.Endpoint.Port)
                           .OrderBy(x => x)));

  // kill one of the gateways in cluster A
  target = Clusters[clusterA]
               .Cluster.GetActiveSilos()
               .Where(h => h.SiloAddress.Endpoint.Port == portsA[1])
               .FirstOrDefault();
  Assert.NotNull(target);
  ...
  while (true) {
    ...
    var killedone =
        hosts.Where(kvp => kvp.Key.Endpoint.Port == portsA[1]).FirstOrDefault();
