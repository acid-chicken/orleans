...
namespace UnitTests.StreamingTests {
  public class PubSubRendezvousGrainTests
      : OrleansTestingBase,
        IClassFixture<PubSubRendezvousGrainTests.Fixture> {
    ...
    [Fact, TestCategory("BVT"), TestCategory("Streaming"),
     TestCategory("PubSub")]
    public async Task
    RegisterConsumerFaultTest() {
      this.fixture.Logger.Info(
          "************************ RegisterConsumerFaultTest *********************************");
      var streamId = StreamId.GetStreamId(Guid.NewGuid(), "ProviderName",
                                          "StreamNamespace");
      var pubSubGrain =
          this.fixture.GrainFactory.GetGrain<IPubSubRendezvousGrain>(
              streamId.Guid, keyExtension
              : streamId.ProviderName + "_" + streamId.Namespace);
      var faultGrain = this.fixture.GrainFactory.GetGrain<IStorageFaultGrain>(
          typeof (PubSubRendezvousGrain).FullName);

      // clean call, to make sure everything is happy and pubsub has state.
      await pubSubGrain.RegisterConsumer(GuidId.GetGuidId(Guid.NewGuid()),
                                         streamId, null, null);
      int consumers = await pubSubGrain.ConsumerCount(streamId);
      Assert.Equal(1, consumers);

      // inject fault
      await faultGrain.AddFaultOnWrite(pubSubGrain as GrainReference,
                                       new ApplicationException("Write"));

      // expect exception when registering a new consumer
      await Assert.ThrowsAsync<OrleansException>(
          () => pubSubGrain.RegisterConsumer(GuidId.GetGuidId(Guid.NewGuid()),
                                             streamId, null, null));
    ...
    UnregisterConsumerFaultTest() {
      ...
      // Add two consumers so when we remove the first it does a storage write,
      // not a storage clear.
      ...
      // inject clear fault, because removing last consumer should trigger a
      // clear storage call.
      await faultGrain.AddFaultOnClear(pubSubGrain as GrainReference,
                                       new ApplicationException("Write"));
    ...
    /// <summary>
    /// This test fails because the producer must be grain reference which is
    /// not implied by the IStreamProducerExtension in the producer management
    /// calls.
    ...
    [Fact(
         Skip =
             "This test fails because the producer must be grain reference which is not implied by the IStreamProducerExtension"),
     TestCategory("BVT"), TestCategory("Streaming"), TestCategory("PubSub")]
    public async Task
    RegisterProducerFaultTest() {
      this.fixture.Logger.Info(
          "************************ RegisterProducerFaultTest *********************************");
      var streamId = StreamId.GetStreamId(Guid.NewGuid(), "ProviderName",
                                          "StreamNamespace");
    ...
    UnregisterProducerFaultTest() {
      ...
      IStreamProducerExtension firstProducer =
          new DummyStreamProducerExtension();
      IStreamProducerExtension secondProducer =
          new DummyStreamProducerExtension();
      // Add two producers so when we remove the first it does a storage write,
      // not a storage clear.
      ...
      // inject clear fault, because removing last producers should trigger a
      // clear storage call.
      await faultGrain.AddFaultOnClear(pubSubGrain as GrainReference,
                                       new ApplicationException("Write"));
    ...
    private class DummyStreamProducerExtension : IStreamProducerExtension {
      ...
      public DummyStreamProducerExtension() { id = Guid.NewGuid(); }
