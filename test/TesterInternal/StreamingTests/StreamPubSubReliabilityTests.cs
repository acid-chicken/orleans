...
namespace UnitTests.StreamingTests {
  public class StreamPubSubReliabilityTests
      : OrleansTestingBase,
        IClassFixture<StreamPubSubReliabilityTests.Fixture> {
    ...
    public class ClientConfiguretor : IClientBuilderConfigurator {
      public void Configure(IConfiguration configuration,
        ...
        clientBuilder.AddSimpleMessageStreamProvider(
            StreamTestsConstants.SMS_STREAM_PROVIDER_NAME);
      }
    }
    ...
    [Fact, TestCategory("Functional"), TestCategory("Streaming"),
     TestCategory("PubSub")]
    public async Task
    PubSub_Store_ReadError() {
      // Expected behaviour: Underlying error StorageProviderInjectedError
      // returned to caller
      //
      // Actual behaviour: Rather cryptic error OrleansException returned,
      // mentioning
      //                   root cause problem "Failed SetupActivationState" in
      //                   message text, but no more details or stack trace.

      SetErrorInjection(PubSubStoreProviderName,
                        ErrorInjectionPoint.BeforeRead);

      // TODO: expect StorageProviderInjectedError directly instead of
      // OrleansException
    ...
    [Fact, TestCategory("Functional"), TestCategory("Streaming"),
     TestCategory("PubSub")]
    public async Task
    PubSub_Store_WriteError() {
      SetErrorInjection(PubSubStoreProviderName,
                        ErrorInjectionPoint.BeforeWrite);

      var exception = await Assert.ThrowsAsync<StorageProviderInjectedError>(
          () => Test_PubSub_Stream(StreamProviderName, StreamId));
    }

    private async Task Test_PubSub_Stream(string streamProviderName,
                                          Guid streamId) {
      // Consumer
      ...
      Assert.True(
          received1 > 1,
          $"Received count for consumer {consumer} is too low = {received1}");
      ...
      Assert.Equal(0, received2); // $"Received count for consumer {consumer} is
                                  // wrong = {received2}"
    }
